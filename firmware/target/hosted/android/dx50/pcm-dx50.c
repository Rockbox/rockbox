...
{
   // if (pcm_state(handle) != SNDRV_PCM_STATE_RUNNING)
       // return;

   // while (pcm_avail_update(handle) >= period_size)
   // {
       if (fill_frames())
       {
...
static int async_rw(snd_pcm_t *handle)
{
   ...
   sample_size = pcm_frames_to_bytes(handle, pcm_get_buffer_size(handle)); //buffer_size;
...
void pcm_play_dma_init(void)
{
    ...
    config.period_count = 4;//buffer_size/period_size;
    //if (bits == 32)
    //    config.format = PCM_FORMAT_S32_LE;
    //else if (bits == 16)
    config.format = format;
    config.start_threshold = 0;//buffer_size/2;
    config.stop_threshold = 0;//period_size;
...
void pcm_play_dma_start(const void *addr, size_t size)
{
   ...
   /* if this is done at startup already, a loud click is audible on headphones. Here, some time later, 
...
//               int err = -1;//snd_pcm_recover(handle, -EPIPE, 0);
//               if (err < 0)
//                   DEBUGF("Recovery failed: %d\n", err);
               printf("No handler for STATE_XRUN!\n");
               continue;
           }
           case PCM_STATE_SETUP:
           {
               //int err = -1;//snd_pcm_prepare(handle);
               //if (err < 0)
...
#else /* DX90 */
    FILE *f = fopen("/sys/class/es9018_volume", "w");
#endif  /* DX50 */
    fprintf(f, "%d", val);
    fclose(f);
#else
