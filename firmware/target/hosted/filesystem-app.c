                                       ...
                                       const size_t bufsize)
{
    ...
    if (path_append(buf, rbhome, ".config/rockbox.org", bufsize) >= bufsize ||
        path_append(buf, NULL, pos, bufsize) >= bufsize)
        return NULL;
#endif
...
DIR * app_opendir(const char *dirname)
{
    ...
    size_t name_len = path_strip_trailing_separators(fname, NULL);
    ...
    *(char *)mempcpy(this->path, fname, name_len) = '\0';

    this->osdirp = os_opendir(this->path);
    if (!this->osdirp)
        FILE_ERROR(ERRNO, RC);

    this->fd = os_getdirfd(this->path, this->osdirp, &this->fd_is_opened);
    if (this->fd < 0)
        FILE_ERROR(ERRNO, RC);
...
int app_samedir(DIR *dirp1, DIR *dirp2)
{
    ...
    return os_fsamefile(this1->fd, this2->fd);
}
