...
static unsigned long get_lcd_pixel(int x, int y)
{
    ...
    return (*LCD_ADDR(x, y/8) & (1 << (y & 7))) ? (NUM_SHADES-1) : 0;
#else
    return (*LCD_ADDR(x, y/8) & (1 << (y & 7))) ? 0 : (NUM_SHADES-1);
#endif
#elif LCD_DEPTH == 2
#if LCD_PIXELFORMAT == HORIZONTAL_PACKING
    return colorindex[(*LCD_ADDR(x/4, y) >> (2 * (~x & 3))) & 3];
#elif LCD_PIXELFORMAT == VERTICAL_PACKING
    return colorindex[(*LCD_ADDR(x, y/4) >> (2 * (y & 3))) & 3];
#elif LCD_PIXELFORMAT == VERTICAL_INTERLEAVED
    unsigned bits = (*LCD_ADDR(x, y/8) >> (y & 7)) & 0x0101;
    return colorindex[(bits | (bits >> 7)) & 3];
#endif
#elif LCD_DEPTH == 16
#if LCD_PIXELFORMAT == RGB565SWAPPED
    unsigned bits = *LCD_ADDR(x, y);
    return (bits >> 8) | (bits << 8);
#else
#if   defined(LCD_STRIDEFORMAT) && LCD_STRIDEFORMAT == VERTICAL_STRIDE
    return *(lcd_framebuffer+LCD_HEIGHT*x+y);
#else
    return *LCD_ADDR(x, y);
#endif
#endif
