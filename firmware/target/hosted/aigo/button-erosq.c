...
static int button_map(int keycode)
{
    switch(keycode)
    {
        ...
        case KEY_LEFT:
            return BUTTON_WHEEL_CCW;

        case KEY_RIGHT:
            return BUTTON_WHEEL_CW;
...
int button_read_device(void)
{
    ...
    if (button_bitmap & (BUTTON_WHEEL_CCW|BUTTON_WHEEL_CW) &&
        current_tick - last_tick >= 2)
    {
        button_bitmap &= ~(BUTTON_WHEEL_CCW|BUTTON_WHEEL_CW);
    }
            ...
            if(poll_fds[i].revents & POLLIN)
            {
                ...
                if(size == (int)sizeof(event))
                {
                    ...
                    if(press)
                    {
                        ...
                        if (bmap & (BUTTON_WHEEL_CCW|BUTTON_WHEEL_CW))
                    ...
                    else
                    {
                        ...
                        int bmap = button_map(keycode) & ~(BUTTON_WHEEL_CCW|BUTTON_WHEEL_CW);
                        button_bitmap &= ~bmap;
                    }
                }
            }
