...
static int timer_thread(void *_data)
{
...
int sys_timer_init(void)
{
    ...
    if (!AtomicGet(&data->active)) {
        data->timermap_lock = (sysMutex *) malloc(sizeof(sysMutex));
        if (!data->timermap_lock) {
            return -1;
        }
        sys_mutex_init(data->timermap_lock);
        sys_sem_init(&data->sem, 0);

        AtomicSet(&data->active, 1);

        /* Timer threads use a callback into the app, so we can't set a limited stack size here. */
        data->thread = sys_thread_create(timer_thread,
                                         "timer",
                                         32 * 1024,                     /* 32 kB */
                                         data
                                         IF_PRIO( , PRIORITY_REALTIME ) /* Realtime priority */
                                         IF_COP( , 0 ));
        if (!data->thread) {
            sys_timer_quit();
            return -1;
...
void sys_timer_quit(void)
{
    ...
    if (AtomicCAS(&data->active, 1, 0)) { /* active? Move to inactive. */
        ...
        if (data->thread) {
            sys_sem_post(&data->sem);
            sys_wait_thread(data->thread, NULL);
            data->thread = NULL;
        }
...
int sys_add_timer(u32 interval, timer_callback_ptr callback, void *param)
{
    ...
    sys_mutex_lock(data->timermap_lock);
    entry->next = data->timermap;
    data->timermap = entry;
    sys_mutex_unlock(data->timermap_lock);
