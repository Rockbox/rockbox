...
unsigned int power_input_status(void)
{
    ...
    if (pmu_ioctl(IOCTL_PMU_IS_EXT_PWR, NULL) > 0)
...
int _battery_voltage(void)
{
    ...
    if (ioctl(max17040_dev, READ_MAX17040_REG, &ret) >= 0) {
        level = ((ret.reg1 << 4) | (ret.reg2 >> 4)) * 1.25f;
    };
    return level;
}
...
{
    int ret = ioctl(max17040_dev, IOCTL_PMU_GET_CHG_STATUS);
    if (ret != -1) {
        return !ret;
    }
    return false;
