...
int power_status = CHARGER_NOT_CONNECTED;
int charging_status = BATT_NOT_CHARGING;
...
unsigned int power_input_status(void)
{
    if (first_readout)
    {
//        ascodec_write(AFE_MUX_PMU_REG3, AFE_SUB_CHG1, 0x5C);  // CHG_I 350mA, CHG_V 4.20V
        /* 350mA, 4.20V */
        ascodec_write_pmu(AS3543_CHARGER, 0x1, 0x5C);
    }
    if (ascodec_read(AS3514_IRQ_ENRD0) & CHG_CHANGED || first_readout)
    {
