...
static const char POWER_INTERFACE[] = "/sys/power/state";
...
void lcd_enable( bool on )
{
    ...
    if( on )
    {
        FILE* f = fopen( POWER_INTERFACE, "w" );
        if( f == NULL )
        {
            DEBUGF( "ERROR %s: Can not open %s for writing.", __func__, POWER_INTERFACE );
            return;
        }

        if( fputs( "on", f ) == EOF )
        {
            DEBUGF( "ERROR %s: Write failed for %s.", __func__, POWER_INTERFACE );
        }
