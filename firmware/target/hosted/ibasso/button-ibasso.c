...
/*
    Lowest changing bit, when hold switch is toggled.
...
bool button_hold()
{
    ...
    if( f == NULL )
    {
        DEBUGF( "ERROR %s: Can not open /sys/class/axppower/holdkey for reading.", __func__ );
        _hold = false;
        return false;
    }

    /* Do not change this datatype, Rockbox will crash! */
    char hold_state;
    if( fscanf( f, "%c", &hold_state ) == EOF )
    {
        DEBUGF( "ERROR %s: Read failed for /sys/class/axppower/holdkey.", __func__ );
