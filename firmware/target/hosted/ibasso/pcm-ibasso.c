...
*/
static volatile sig_atomic_t _playback_suspended = 1;


/* Mutex for PCM thread suspend/unsuspend. */
static pthread_mutex_t _playback_suspended_mtx = PTHREAD_MUTEX_INITIALIZER;


/* Signal condition for PCM thread suspend/unsuspend. */
static pthread_cond_t _playback_suspended_cond = PTHREAD_COND_INITIALIZER;
...
static void* pcm_thread_run(void* nothing)
{
    ...
    while(true)
    {
        pthread_mutex_lock(&_playback_suspended_mtx);
        while(_playback_suspended == 1)
        {
            DEBUGF("DEBUG %s: Playback suspended.", __func__);
            pthread_cond_wait(&_playback_suspended_cond, &_playback_suspended_mtx);
        }
        pthread_mutex_unlock(&_playback_suspended_mtx);
...
void pcm_play_dma_start(const void *addr, size_t size)
{
    ...
    */
    ...
    _playback_suspended = 0;
    pthread_cond_signal(&_playback_suspended_cond);
    pthread_mutex_unlock(&_playback_suspended_mtx);
}
...
void pcm_play_dma_pause(bool pause)
{
    ...
    pthread_mutex_lock(&_playback_suspended_mtx);
    _playback_suspended = pause ? 1 : 0;
    if(_playback_suspended == 0)
    {
        pthread_cond_signal(&_playback_suspended_cond);
    }
    pthread_mutex_unlock(&_playback_suspended_mtx);
