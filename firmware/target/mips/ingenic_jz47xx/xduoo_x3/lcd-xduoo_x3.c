...
void lcd_hw_init(void)
{
    ...
    REG_GPIO_PXSLS(2)  = 0x000C73FC; /* slew -- fast rate */
...
static void lcd_write_cmd_triplet(int cmd1, int cmd2, int cmd3)
{
...
#else
    int command = 0x000030FC;
    __gpio_clear_pin(PIN_LCD_DC);

    REG_GPIO_PXDATC(2) = command;
    command = ((cmd1 & 0xC0) << 6) | ((cmd1 & 0x3F) << 2);
    ...
    REG_GPIO_PXDATC(2) = command;
    command = ((cmd2 & 0xC0) << 6) | ((cmd2 & 0x3F) << 2);
    REG_GPIO_PXDATS(2) = command;
    __gpio_clear_pin(PIN_LCD_WR);
    BITDELAY();
    __gpio_set_pin(PIN_LCD_WR);
    ...
    REG_GPIO_PXDATC(2) = command;
    command = ((cmd3 & 0xC0) << 6) | ((cmd3 & 0x3F) << 2);
    REG_GPIO_PXDATS(2) = command;
    __gpio_clear_pin(PIN_LCD_WR);
    BITDELAY();
    __gpio_set_pin(PIN_LCD_WR);
...
void lcd_write_data(const fb_data* p_bytes, int count)
{
    ...
    while (count--)
    {
        ...
        data = ((*p_bytes & 0xC0) << 6) | ((*p_bytes & 0x3F) << 2);
        p_bytes++;
