...
static void EPIN_send(unsigned int endpoint)
{
...
#ifdef USE_USB_DMA
    /* Can we use DMA? */
    if (ep->type == ep_bulk && ep->length && /*((unsigned long)ptr & 3 == 0) &&*/ !button_hold()) {
    ...
    if (ep->use_dma >= 0) {
        commit_discard_dcache_range((void*)ep->buf, ep->length);
        /* Set up DMA */
        ...
        REG_USB_ADDR(USB_INTR_DMA_BULKIN) = PHYSADDR((unsigned long)ep->buf);
        REG_USB_COUNT(USB_INTR_DMA_BULKIN) = ep->length;
...
static void tx_complete(unsigned int endpoint)
{
	struct usb_endpoint* ep = &endpoints[endpoint*2];
	uint16_t csr;
//	unsigned int length;
	...
	// XXX check underrun, complain

	if (!ep->busy) {
		logf("Entered tx_complete without work!");
		return;
	}
	...
	// XXX if dma in use, stop DMA engine and clean it up
...
static void EPDMA_handler(int number)
{
    ...
    logf("DMA_BULK%d %d %d %x %x", number, endpoint, incsr, size, (int)REG_USB_CNTL(number));
    ...
    if (number == USB_INTR_DMA_BULKIN) {
        ...
        if ((incsr & USB_INCSR_INPKTRDY) == 0)
            tx_complete(endpoint);
