...
static void EPDMA_handler(int number)
{
	...
	select_endpoint(endpoint);
        ...
        logf("DMA_BULK%d %d %d/ %x", number, size, endpoint, incsr);
...
static void usb_drv_send_internal(struct usb_endpoint* ep, void* ptr, int length, bool blocking)
{
    ...
    if (ep->type == ep_control) {
...
#ifdef USE_USB_DMA
        /* Can we use DMA? */
        if (length >= ep->fifo_size && /*((unsigned long)ptr & 3 == 0) &&*/ !button_hold()) {
	    ...
	    csr &= ~USB_INCSR_UNDERRUN;
            ...
            logf("DMA setup(%x %x %x %x - %d)", (unsigned int)PHYSADDR((unsigned long)ptr), length, dmacr, csr, ep->use_dma);
