...
static void EPDMA_handler(int number)
{
    ...
    if (number == USB_INTR_DMA_BULKIN) {
        if ((ep->use_dma == 0) || (size % ep->fifo_size)/* && (incsr & USB_INCSR_FFNOTEMPT) */) {
            ...
            logf("DMA dangling %x / %x %x", incsr, REG_USB_COUNT(USB_INTR_DMA_BULKIN), REG_USB_CNTL(USB_INTR_DMA_BULKIN));
            REG_USB_INCSR = incsr;
        }
        ep->sent += size;
        logf("DMA xfer %d (%d/%d)", size, ep->sent, ep->length);
        ep->use_dma = -1;
