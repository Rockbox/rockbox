...
static intptr_t dma_cb_args[DMA_NUM_USED_CHANNELS];

static void dma_no_cb(int event, intptr_t arg)
{
    (void)event;
    (void)arg;
...
void dma_set_callback(int chn, dma_cb_func cb, intptr_t arg)
{
...
void PDMA(void)
{
    ...
    for(int i = 0; i < DMA_NUM_USED_CHANNELS; ++i) {
        ...
        REG_DMA_CHN_CS(i) = 0;
        dma_callbacks[i](evt, dma_cb_args[i]);
    }
...
void PDMAD(void)
{
    ...
    for(int i = 0; i < DMA_NUM_USED_CHANNELS; ++i) {
        ...
        dma_callbacks[i](DMA_EVENT_INTERRUPT, dma_cb_args[i]);
    }
