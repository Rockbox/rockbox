...
int button_read_device(int* data)
{
     ...
     *
     * TODO: Maybe it's not so great after all? it seems to skip in the
     * opposite direction of turning every so often, not sure if this is a
     * software glitch due to interrupt latency or a hardware problem. */
    if(wheel_pos > 0)
        r |= BUTTON_VOL_UP;
    else if(wheel_pos < 0)
        r |= BUTTON_VOL_DOWN;
