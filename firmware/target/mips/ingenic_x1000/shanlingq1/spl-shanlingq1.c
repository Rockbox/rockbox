...
/* TODO: this is basically a duplicate of the M3K.
 *
 * It is likely all X1000/E targets will always have the same configuration.
 * X1000 (non-E) targets will have a slightly different but mostly identical
 * setup. So it's probably possible to consolidate both M3K and Q1 SPLs and
 * the bootloaders will be similar as well.
 */
...
const struct spl_boot_option spl_boot_options[] = {
    ...
    },
    {
        ...
        /* NOTE: xImage/uImage kernels have load/exec addr embedded.
         * Maybe we should do the same thing with Rockbox in the next
         * revision of the bootloader? Then we can parse it and avoid
         * having to put load/exec address in the table. */
    },
};
...
{
    const uint32_t pinmask = (1 << 21) | (1 << 22);
    ...
    gpioz_configure(GPIO_B, pinmask, GPIOF_INPUT);

    /* Poll the pins for a short duration to detect a keypress */
    do {
        lastpin = pin;
        pin = ~REG_GPIO_PIN(GPIO_B) & pinmask;
        if(pin != lastpin) {
    ...
    if(iter_count < max_iter_count && (pin & (1 << 22))) {
        if(pin & (1 << 21))
            return BOOTOPTION_RECOVERY; /* Next+Prev */
        else
            return BOOTOPTION_ORIG_FW; /* Next */
    } else {
        return BOOTOPTION_ROCKBOX; /* Prev or no buttons */
    }
}
