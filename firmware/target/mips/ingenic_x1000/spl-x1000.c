...
void spl_main(void)
{
    ...
    if(!opt->cmdline) {
         ...
         * In the case of Rockbox, binary's LMA = exec address. */
        uint32_t in_size = read_be32(&load_addr[0]);
        uint32_t out_size = read_be32(&load_addr[4]);
        uint32_t expected_out_size = out_size;
        int rc = ucl_nrv2e_decompress_8(&load_addr[8], in_size,
                                        (uint8_t*)opt->exec_addr, &out_size);
        if(rc != 0 || out_size != expected_out_size)
            spl_error();
    }
