...
#define MSC_CLKSRC_SCLK_A 0
#define MSC_CLKSRC_MPLL   1

static const msc_config msc_configs[] = {
#ifdef FIIO_M3K
#define MSC_CLOCK_SOURCE MSC_CLKSRC_SCLK_A
...
static void msc_init_one(msc_drv* d, int msc)
{
    ...
    jz_writef(CPM_MSC0CDR, CE(1), CLKDIV(0),
              CLKSRC(MSC_CLOCK_SOURCE == MSC_CLKSRC_SCLK_A ? 0 : 1));
...
void msc_set_speed(msc_drv* d, int rate)
{
    ...
    /* Handle MSCxDIV */
    unsigned src_freq;
    if(MSC_CLOCK_SOURCE == MSC_CLKSRC_SCLK_A)
        src_freq = clk_get_sclk_a() / 2;
    else
        src_freq = clk_get_mpll() / 2;

    unsigned div = 1;
    while(div < 256 && src_freq/div > freq1)
        div += 1;
    ...
    /* Handle MSC_CLKRT */
    unsigned clkrt = 0;
    unsigned out_freq = src_freq / div;
    while(clkrt < 7 && out_freq > freq2) {
        out_freq /= 2;
        clkrt += 1;
    }
    ...
    /* Clear cached clock */
    clk_notify_change(d->msc_nr == 0 ? X1000_CLK_MSC0 : X1000_CLK_MSC1);
}
...
void msc_async_start(msc_drv* d, msc_req* r)
{
    ...
    /* Timeout in case of hardware lockups */
...
static void msc_interrupt(msc_drv* d)
{
    ...
    if(iflag & d->iflag_done) {
        /* This discard is necessary only for read operations; it handles the
         * case where accessing data adjacent to the DMA buffer causes the CPU
         * to read ahead of the access and load DMA buffer memory into cache.
         * If this happens before DMA is finished, then the CPU would have a
         * stale copy of the data.
         *
         * The real solution is to map DMA buffers to uncached addresses so
         * the cache can't interfere, but I think modifying Rockbox to work
         * that way is more trouble than it's worth.
         */
