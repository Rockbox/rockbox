...
int aic_i2s_set_mclk(int clksrc, unsigned fs, unsigned mult)
{
    /* get the input clock rate */
    unsigned src_freq;
    switch(clksrc) {
    case AIC_CLKSRC_EXCLK:  src_freq = X1000_EXCLK_FREQ; break;
    case AIC_CLKSRC_SCLK_A: src_freq = clk_get_sclk_a(); break;
    case AIC_CLKSRC_MPLL:   src_freq = clk_get_mpll(); break;
    default: return -1;
    }
    ...
    if(clksrc == AIC_CLKSRC_EXCLK) {
        ...
        jz_writef(CPM_I2SCDR, PCS(clksrc == AIC_CLKSRC_MPLL ? 1 : 0),
                  CS(1), CE(1), DIV_M(m), DIV_N(n));
        jz_write(CPM_I2SCDR1, REG_CPM_I2SCDR1);
        REG_AIC_I2SDIV = (mult / 64) - 1;
    }

    clk_notify_change(X1000_CLK_I2S_MCLK);
    clk_notify_change(X1000_CLK_I2S_BCLK);
    return 0;
}
