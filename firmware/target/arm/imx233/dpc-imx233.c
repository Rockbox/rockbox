...
enum dpc_ctx_state_t
{
    ...
    CTX_STOPPED /* context is stopped but blocked caller needed to acknowledge */
};
...
static void reschedule(void)
{
    ...
    for(int i = 0; i < NR_CONTEXT; i++)
    {
        ...
        if(fire)
        {
             ...
             * back to running state after call but still no scheduled */
            dpc_ctx[i].state = CTX_STOPPED;
            dpc_ctx[i].fn(&dpc_ctx[i], dpc_ctx[i].user);
            /* context stilled marked as stopped after call: perform cleanup */
