...
{
#if IMX233_SUBTARGET < 3780
    return BR_ICOLL_PRIORITYn(src / 4, PRIORITYx(src % 4));
#else
    return BR_ICOLL_INTERRUPTn(src, PRIORITY);
#endif
}
...
#if IMX233_SUBTARGET < 3780
    info.enabled = BR_ICOLL_PRIORITYn(src / 4, ENABLEx(src % 4));
#else
    info.enabled = BR_ICOLL_INTERRUPTn(src, ENABLE);
#endif
...
#if IMX233_SUBTARGET < 3780
    if(enable)
        BM_ICOLL_PRIORITYn_SET(src / 4, SOFTIRQx(src % 4));
    else
        BM_ICOLL_PRIORITYn_CLR(src / 4, SOFTIRQx(src % 4));
#else
    if(enable)
        BM_ICOLL_INTERRUPTn_SET(src, SOFTIRQ);
    else
        BM_ICOLL_INTERRUPTn_CLR(src, SOFTIRQ);
#endif
}
...
#if IMX233_SUBTARGET < 3780
    if(enable)
        BM_ICOLL_PRIORITYn_SET(src / 4, ENABLEx(src % 4));
    else
        BM_ICOLL_PRIORITYn_CLR(src / 4, ENABLEx(src % 4));
#else
    if(enable)
        BM_ICOLL_INTERRUPTn_SET(src, ENABLE);
    else
        BM_ICOLL_INTERRUPTn_CLR(src, ENABLE);
#endif
}
...
void imx233_icoll_init(void)
{
    ...
    BM_ICOLL_CTRL_SET(ARM_RSE_MODE);
    ...
    BM_ICOLL_CTRL_SET(IRQ_FINAL_ENABLE);
}
