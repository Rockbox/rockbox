    ...
    unsigned src, unsigned prescale, bool polarity, imx233_timer_fn_t fn)
{
    ...
    BW_TIMROT_TIMCTRLn(timer_nr, SELECT_V(NEVER_TICK), UPDATE(1));
    ...
    /* start timer */
    BW_TIMROT_TIMCTRLn(timer_nr, SELECT(src), PRESCALE(prescale),
...
struct imx233_timrot_info_t imx233_timrot_get_info(unsigned timer_nr)
{
    ...
    info.src = BR_TIMROT_TIMCTRLn(timer_nr, SELECT);
    info.prescale = BR_TIMROT_TIMCTRLn(timer_nr, PRESCALE);
    info.reload = BR_TIMROT_TIMCTRLn(timer_nr, RELOAD);
    info.polarity = BR_TIMROT_TIMCTRLn(timer_nr, POLARITY);
    info.fixed_count = BR_TIMROT_TIMCOUNTn(timer_nr, FIXED_COUNT);
    info.run_count = BR_TIMROT_TIMCOUNTn(timer_nr, RUNNING_COUNT);
    return info;
}
