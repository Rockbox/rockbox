...
void imx233_audioout_preinit(void)
{
    ...
    /* Set capless mode */
#ifdef IMX233_AUDIO_COUPLING_MODE
#if IMX233_AUDIO_COUPLING_MODE == ACM_CAP
    BF_SET(AUDIOOUT_PWRDN, CAPLESS);
#elif IMX233_AUDIO_COUPLING_MODE == ACM_CAPLESS
    BF_CLR(AUDIOOUT_PWRDN, CAPLESS);
#else
    #error Invalid output coupling mode (ACM_CAP or ACM_CAPLESS)
#endif
#else
    #error Please select output coupling mode (ACM_CAP or ACM_CAPLESS)
#endif
