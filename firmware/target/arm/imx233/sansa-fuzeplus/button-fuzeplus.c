...
bool button_debug_screen(void)
{
    ...
    unsigned char power_reg = rmi_read_single(RMI_DEVICE_CONTROL);    
    union
    {
        unsigned char data;
        signed char value;
    ...
    while(1)
    {
        ...
        lcd_putsf(30, 6, "r_rate: %d", power_reg & 0xc0);
        lcd_putsf(30, 7, "p_mode: %d", power_reg & 0x07);
        ...
        if(btns & BUTTON_VOL_DOWN || btns & BUTTON_VOL_UP)
        {
            ...
            {
                if(sensibility_counter == -15)
                {
                    if((power_reg & 0xc0) == RMI_REPORT_RATE_NORMAL)
                    {
                        /* we go to low reporting mode */
                        power_reg -= 0x40;
                    }
                    else
                    {
                        /* we go to normal reporting mode */
                        power_reg += 0x40;
                    }
                }
                else if (sensibility_counter == 15)
                {
                    /* go to next sleep mode or go back to first one */
                    if((power_reg & 7) == 4)
                        power_reg &= 0xc0;
                    else
                        power_reg += 1; 
                ...
                rmi_write_single(RMI_DEVICE_CONTROL, power_reg);
            }
        }
...
{
    /* rmi_set_sleep_mode() and rmi_set_report_rate() are not doing anything if the value
       they are given is already the one setted */
    rmi_set_sleep_mode(RMI_SLEEP_MODE_LOW_POWER);
    rmi_set_report_rate(RMI_REPORT_RATE_NORMAL);
        ...
        switch(ev.id)
        {
            ...
            default:
                ...
                if(TIME_AFTER(current_tick, last_activity + ACTIVITY_TMO))
                {
                    ...
                    rmi_set_report_rate(RMI_REPORT_RATE_LOW);
                }
                break;
        }
    }
