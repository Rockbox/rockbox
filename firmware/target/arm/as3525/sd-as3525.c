...
#define SD_MAX_TIMEOUT	        ((AS3525_PCLK_FREQ) / 1000 * 60000) /* 60 s */
...
#define EXT_SD_BITS (1<<2)
    ...
    static bool disk_slow = false;
#else
    const bool disk_slow = false;
#endif
...
static inline void mci_delay(usec) { udelay(usec) ; }
...
static inline void sd_set_clock(int drive)
{
    /* Boost MCICLK to operating speed */
    if(drive == INTERNAL_AS3525)
        /* MCICLK = PCLK/2 = 31MHz(STD) or PCLK/4 = 15.5 Mhz (SLOW)*/
        MCI_CLOCK(drive) = (!disk_slow ? MCI_HALFSPEED : MCI_QUARTERSPEED);   

#if defined(HAVE_MULTIDRIVE)
    else
    {
        hs_card = (card_info[drive].speed == 50000000) && !disk_slow;
        /* MCICLK = PCLK/2 = 31MHz(HS) or PCLK/4 = 15.5 Mhz (STD)*/
        MCI_CLOCK(drive) = (hs_card ? MCI_HALFSPEED : MCI_QUARTERSPEED);
    }
#endif
    mci_delay(CLK_CHANGE_DELAY);
}
...
static int sd_init_card(const int drive)
{
    ...
    if(sd_v2 && !disk_slow)
    {
        ...
        if(sd_wait_for_tran_state(drive))
            return -6;
        /* CMD6 0xf indicates no influence, 0x1 - HS Access*/
                               ...
                               int count, void* buf, const bool write)
{
    ...
    while(count > 0)
    {
...
#ifdef HAVE_MULTIDRIVE
            if((drive == SD_SLOT_AS3525) && (!hs_card || disk_slow))
                mci_delay(COMMAND_REG_DELAY);
#endif
        }
        else
        ...
        MCI_DATA_TIMER(drive) = disk_slow ? SD_MAX_TIMEOUT : 
                              write ? SD_MAX_WRITE_TIMEOUT : SD_MAX_READ_TIMEOUT;
...
void sd_set_low_speed(bool slow)
{
    ...
    */
    mci_cmd_delay(COMMAND_REG_DELAY);
    disk_slow = slow;

    if (slow)
        CGU_IDE = (CGU_IDE & ~(0xF << 2)) | (AS3525_IDE_DIV_SLOW << 2);
    else
