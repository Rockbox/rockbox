...
static int calc_freq(int clk)
{
    ...
    switch(clk) {
        case CLK_PLLA:
            ...
            return (CLK_MAIN / 2) * DIV_FINITE(f, (r * od));
            ...
            switch(CGU_USB & 3) {     /* 0-> div=1  other->div=1/(2*n)  */
                case 0:
                    if (!((CGU_USB>>2) & 0x7))
                        ...
                        return DIV_FINITE(CLK_MAIN, (2*((CGU_USB>>2) & 0x7)));
                case 1:
                    if (!((CGU_USB>>2) & 0x7))
                        return calc_freq(CLK_PLLA);
                    else
                        return DIV_FINITE(calc_freq(CLK_PLLA), (2*((CGU_USB>>2) & 0x7)));
                case 2:
                    if (!((CGU_USB>>2) & 0x7))
                        return calc_freq(CLK_PLLB);
                    else
                        return DIV_FINITE(calc_freq(CLK_PLLB), (2*((CGU_USB>>2) & 0x7)));
                default:
                    return 0;
            }
        default:
...
#endif
#else /*AS3525v2*/
        lcd_putsf(x, line++, "SD  :%3dMHz    %3dMHz", AS3525_SDSLOT_FREQ/D_MHZ,
                                                    calc_freq(CLK_SD)/D_MHZ);
        long freq_ssp = calc_freq(CLK_SSP);
        if (freq_ssp >= 1000000)
            lcd_putsf(x, line++, "SSP :%3dMHz    %3dMHz", AS3525_SSP_FREQ/D_MHZ,
