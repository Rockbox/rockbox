...
/*#define CLK_DIV_MIN(ref, target, off, min) (((ref + target - 1) / target) + off < min)? ((ref + target - 1) / target) + off : min*/
...
#if LCD_DEPTH > 1
  #define AS3525_SSP_PRESCALER_SLOW ((CLK_DIV(AS3525_PCLK_FREQ, 2000000) + 1) & ~1)  /*Max value for divider - must be an even number */
#else
  #define AS3525_SSP_PRESCALER_SLOW 0xFE & ~1  /*Max value for divider - must be an even number */
#endif
