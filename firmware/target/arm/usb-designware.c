...
#endif
    /* Flush FIFO */
    DWC_GRSTCTL = TXFNUM(fnum) | TXFFLSH;
    while (DWC_GRSTCTL & TXFFLSH);
...
#endif
#ifdef USB_DW_SHARED_FIFO
        eptsiz |= MCCNT(IS_PERIODIC(epnum));
#endif
        nak = CNAK;
    }
    else
...
#ifdef USB_DW_SHARED_FIFO
        DWC_GINTMSK |= (IS_PERIODIC(epnum) ? PTXFE : NPTXFE);
#else
        DWC_DIEPEMPMSK |= (1 << GET_DTXFNUM(epnum));
#endif
#endif
...
static void usb_dw_abort_in_endpoint(int epnum)
{
    ...
    if (dw_ep->busy)
    {
        usb_dw_flush_endpoint(epnum, USB_DW_EPDIR_IN);
...
static void usb_dw_init(void)
{
     ...
     */
    DWC_GRSTCTL = TXFNUM(0x10) | TXFFLSH | RXFFLSH;
    while (DWC_GRSTCTL & (TXFFLSH|RXFFLSH));
    //udelay(3); /* Wait 3 PHY cycles */
