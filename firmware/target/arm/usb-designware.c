...
static void usb_dw_flush_in_endpoint(int ep)
{
    ...
    if (DESIGNWARE_DIEPCTL(ep) & EPENA)               //(usb_dw_config.core->inep_regs[ep].diepctl.b.epena)
    {
        ...
        // Flush it all the way down!
                                                   //union usb_dw_grstctl grstctl = { .b = { .txfnum = usb_dw_config.co
...
static void usb_dw_try_push(int ep)
{
    ...
    if (!(diepctl & EPENA) || !(diepctl & USBAEP) || (diepctl & STALL) || (diepctl & NAKSTS)) return; //if (!depctl.b.e

    uint32_t bytesleft = DESIGNWARE_DIEPTSIZ(ep) & 0x7ffff; //int bytesleft = usb_dw_config.core->inep_regs[ep].dieptsi
    if (!bytesleft) return;
    uint32_t maxpacket = ep ? (DESIGNWARE_DIEPCTL(ep) & 0x3ff) : 64; //int maxpacket = ep ? usb_dw_config.core->inep_re
    ...
    if (((fifospace >> 16) & 0xff) && (((fifospace & 0xffff) << 2) >= words)) //if (fifospace.b.nptxqspcavail && fifosp
        while (words--) DESIGNWARE_DFIFO(ep) = *usb_dw_ep_state[ep].in.txaddr++; //usb_dw_config.core->dfifo[ep][0] = *
...
int usb_dw_get_stall(struct usb_endpoint_number ep)
{
    ...
    return !!(DESIGNWARE_DOEPCTL(ep.number) & STALL);     // !!usb_dw_config.core->outep_regs[ep.number].doepctl.b.stal
}
...
void usb_dw_set_stall(struct usb_endpoint_number ep, int stall)
{
    if (ep.direction == USB_ENDPOINT_DIRECTION_IN)
    {
        ...
        {
            DESIGNWARE_DIEPCTL(ep.number) &= ~STALL;
            DESIGNWARE_DIEPCTL(ep.number) |= SD0PID; // if (!stall) usb_dw_config.core->inep_regs[ep.number].diepctl.b.
        }
    }
    else
    {
        ...
        {
            DESIGNWARE_DOEPCTL(ep.number) &= ~STALL;
            DESIGNWARE_DOEPCTL(ep.number) |= SD0PID; //if (!stall) usb_dw_config.core->outep_regs[ep.number].doepctl.b.
        }
    }
}
                         ...
                         enum usb_endpoint_type type, int maxpacket)
{
    ...
    int nextep = (DESIGNWARE_DIEPCTL(ep.number) >> 10) & 0x0f; //usb_dw_config.core->inep_regs[ep.number].diepctl.b.nex
    //union usb_dw_depctl depctl = { .b = { .usbactep = 1, .eptype = type, .mps = maxpacket,
    //                                      .txfnum = txfifo, .setd0pid = 1, .nextep = nextep } };
    if (ep.direction == USB_ENDPOINT_DIRECTION_IN)
    {
               DESIGNWARE_DIEPCTL(ep.number) = SD0PID|DTXFNUM(txfifo)|EPTYP(type)|USBAEP|NEXTEP(nextep)|maxpacket; //us
        DESIGNWARE_DAINTMSK |= (1<<ep.number); //usb_dw_config.core->dregs.daintmsk.ep.in |= (1 << ep.number);
    }
    else
    {
               DESIGNWARE_DOEPCTL(ep.number) = SD0PID|DTXFNUM(txfifo)|EPTYP(type)|USBAEP|NEXTEP(nextep)|maxpacket; //us
        DESIGNWARE_DAINTMSK |= (1<<ep.number)<<16; //usb_dw_config.core->dregs.daintmsk.ep.out |= (1 << ep.number);
    }
}
...
void usb_dw_ep0_start_rx(int non_setup, int size)
{
    ...
    DESIGNWARE_DOEPTSIZ(0) = STUPCNT(3)|PKTCNT(!!non_setup)|size; //usb_dw_config.core->outep_regs[0].doeptsiz.d32 = de
...
void usb_dw_ep0_start_tx(const void* buf, int len)
{
    ...
    if (len)
    {
        ...
        if (usb_dw_config.use_dma) DESIGNWARE_DIEPDMA(0) = (uint32_t)buf; //usb_dw_config.core->inep_regs[0].diepdma = 
...
static void usb_handle_xfer_complete(struct usb_endpoint_number ep, int bytesleft)
{
    ...
    if (endpoint->busy)
    {
        ...
        usb_core_transfer_complete(ep.number, ep.direction == USB_ENDPOINT_DIRECTION_OUT ? USB_DIR_OUT : USB_DIR_IN, 0,
        endpoint->done = true;
        semaphore_release(&endpoint->complete);
    }
...
void usb_dw_irq(void)
{
    ...
    if ((gintsts & NPTXFE) && (DESIGNWARE_GINTMSK & NPTXFE))//(gintsts.b.nptxfempty && usb_dw_config.core->gregs.gintms
    {
    ...
    if (gintsts & IEPINT)//(gintsts.b.inepintr)
    {
        ...
        for (ep = 0; ep < 16; ep++)
            if (daint & (1 << ep))
            {
                ...
                {
                    // Memory to device part of the "software DMA" implementation, used to transmit data if use_dma == 
                    ...
                    else
                    {
                        ...
                        while (left)
                        {
                            int words = DESIGNWARE_DTXFSTS(ep) & 0xffff; //usb_dw_config.core->inep_regs[ep].dtxfsts.b.
                            if (words > left)
                                words = left;
                            ...
                            left -= words;
                            while (words--)
                                DESIGNWARE_DFIFO(ep) = *usb_dw_ep_state[ep].in.txaddr++; //usb_dw_config.core->dfifo[ep
                        }
                    }
                }
                struct usb_endpoint_number epnum = { .direction = USB_ENDPOINT_DIRECTION_IN, .number = ep };
                uint32_t bytesleft = DESIGNWARE_DIEPTSIZ(ep) & 0x7ffff;//usb_dw_config.core->inep_regs[ep].dieptsiz.b.x
                if (epints & TOC)
                    usb_handle_timeout(epnum); //(epints.b.timeout) usb_handle_timeout(epnum, bytesleft);

                if (epints & XFRC)
                    usb_handle_xfer_complete(epnum, bytesleft); //(epints.b.xfercompl) usb_handle_xfer_complete(epnum, 
    ...
    if (gintsts & OEPINT)//(gintsts.b.outepintr)
    {
        ...
        for (ep = 0; ep < 16; ep++)
            if (daint & ((1<<ep)<<16))//(daint.ep.out & (1 << ep))
            {
                ...
                {
                    if (usb_dw_config.use_dma) discard_dcache_range((const void*)&(DESIGNWARE_DIEPDMA(ep)), 8);//usb_dw
                    usb_dw_flush_in_endpoint(ep);
                    usb_handle_setup_received(epnum);
                }
                else if (epints & XFRC)//(epints.b.xfercompl)
                {
                    int bytesleft = DESIGNWARE_DIEPTSIZ(ep) & 0x7ffff; //usb_dw_config.core->inep_regs[ep].dieptsiz.b.x
...
void usb_dw_init(void)
{
    ...
    // Configure the core
    //union usb_dw_gahbcfg gahbcfg = { .b = { .dmaenable = usb_dw_config.use_dma, .hburstlen = USB_DW_AHB_BURST_LEN, .g
    ...
    if (usb_dw_config.use_dma)
    {
        //union usb_dw_dthrctl dthrctl = { .b = { .arb_park_en = 1, .rx_thr_en = 1, .iso_thr_en = 0, .non_iso_thr_en = 
