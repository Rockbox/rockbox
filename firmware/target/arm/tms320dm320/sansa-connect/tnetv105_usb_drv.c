 ...
 * This driver was modeled strongly after the pxa usb driver.
 *
...
static int tnetv_hw_reset(void)
{
    ...
    wreg = (uint32_t *) DM320_VLYNQ_CTRL;
    *wreg |= (VLYNQ_CTL_CLKDIR_MASK | DM320_VLYNQ_CTRL_PWR_MAN);
    *wreg |= VLYNQ_CTL_RESET_MASK;
    ...
    *wreg &= ~VLYNQ_CTL_RESET_MASK;
    ...
    timeout = 0;
    wreg = (uint32_t *) DM320_VLYNQ_STAT;
    while (!(*wreg & VLYNQ_STS_LINK_MASK) && timeout++ < 50);
    {
        mdelay(40);
    }

    if (!(*wreg & VLYNQ_STS_LINK_MASK))
    {
    ...
    wreg = (uint32_t *) DM320_VLYNQ_TXMAP;
    *wreg = DM320_VLYNQ_PADDR;
    wreg = (uint32_t *) DM320_VLYNQ_RXMAPOF1;
    *wreg = CONFIG_SDRAM_START;
    wreg = (uint32_t *) DM320_VLYNQ_RXMAPSZ1;
    *wreg = SDRAM_SIZE;

    /* set up vlynq remote map for tnetv105 */
    wreg = (uint32_t *) DM320_VLYNQ_TXMAP_REM;
    *wreg = 0x00000000;
    wreg = (uint32_t *) DM320_VLYNQ_RXMAPOF1_REM;
    *wreg = 0x0C000000;
    wreg = (uint32_t *) DM320_VLYNQ_RXMAPSZ1_REM;
    *wreg = 0x00030000;
...
static void tnetv_usb_reset(void)
{
    ...
    reg = (volatile uint32_t *) DM320_VLYNQ_INTPTR;
    *reg = DM320_VLYNQ_INTPND_PHY;

    /* point VLYNQ remote interrupts at the pending register */
    reg = (volatile uint32_t *) DM320_VLYNQ_INTPTR_REM;
    *reg = 0;

    /* clear out interrupt register */
    reg = (volatile uint32_t *) DM320_VLYNQ_INTST;
    *reg |= 0xFFFFFFFF;

    /* enable interrupts on remote device */
    reg = (volatile uint32_t *) DM320_VLYNQ_CTRL_REM;
    *reg |= (DM320_VLYNQ_CTRL_INT_EN);
    reg = (volatile uint32_t *)  DM320_VLYNQ_INTVEC30_REM;
    *reg = 0x8180;

    /* enable VLYNQ interrupts & set interrupts to trigger VLYNQ int */
    reg = (volatile uint32_t *) DM320_VLYNQ_CTRL;
    *reg |= (DM320_VLYNQ_CTRL_INT_LOC | DM320_VLYNQ_CTRL_INT_CFG);
}
...
void VLYNQ(void)
{
    ...
    {
        tnetv_usb_reg_write(DM320_VLYNQ_INTST, VLYNQ_INTR_USB20);
        ...
        if (sysIntrStatus.f.setup)
        {
            ...
            /* if there is a data stage for the OUT transaction, disable the nak bit */
            if ((usbCtrl.f.dir == 0) && (setup.wLength > 0))
            {
                ep0Cnt.val = tnetv_usb_reg_read(TNETV_USB_EP0_CNT);
                ep0Cnt.f.out_xbuf_nak = 0;
                tnetv_usb_reg_write(TNETV_USB_EP0_CNT, ep0Cnt.val);
    ...
    if (vlynq_intr & VLYNQ_INTR_CPPI)
    {
        ...
        tnetv_usb_reg_write(DM320_VLYNQ_INTST, VLYNQ_INTR_CPPI);
