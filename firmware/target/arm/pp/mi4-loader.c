...
static void tea_decrypt_buf(unsigned char* src, unsigned char* dest, 
...
static inline bool tea_test_key(unsigned char magic_enc[8], uint32_t * key,
                                int unaligned)
{
...
int load_mi4(unsigned char* buf, const char* firmware, unsigned int buffer_size)
{
    ...
    rc = read(fd, &mi4header, MI4_HEADER_SIZE);
    if (rc < MI4_HEADER_SIZE)
    {
        ret = EREAD_HEADER_FAILED;
        goto end;
    }

    len = mi4header.mi4size-MI4_HEADER_SIZE;
