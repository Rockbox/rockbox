...
int load_mi4_filename(unsigned char* buf,
             const char* filename,
...
int load_mi4(unsigned char* buf, const char* firmware, unsigned int buffer_size)
{
    ...
    if (firmware[0] != '/')
    {
        ...
        printf("Searching [%d] Volumes", NUM_VOLUMES);
        for (unsigned int i = (NUM_DRIVES * 4) - 1; i > 0 && ret < 0; i--)
        {
