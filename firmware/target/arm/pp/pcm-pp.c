 ...
 */
#define MAX_DMA_CHUNK_PERIOD (pcm_curr_sampr >> 8) /* ~1/256 seconds */
...
{
    int status = disable_fiq_save();
    unsigned long addr = dma_play_data.addr;
...
static unsigned long rec_peak_addr IBSS_ATTR;
...
void fiq_record(void)
{
    ...
    if (pcm_rec_dma_complete_callback(PCM_DMAST_OK, &dma_rec_data.p_w,
                                      &dma_rec_data.frames))
    {
        rec_peak_addr = dma_rec_data.addr;
