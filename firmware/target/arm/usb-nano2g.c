...
static void reset_endpoints(int reinit)
{
    ...
    DEPCTL(0, false) = 0x8800;  /* EP0 IN ACTIVE NEXT=1 */
    DEPCTL(0, true) = 0x8000;  /* EP0 OUT ACTIVE */
    DEPTSIZ(0, true) = 0x20080040;  /* EP0 OUT Transfer Size:
                                64 Bytes, 1 Packet, 1 Setup Packet */
    DEPDMA(0, true) = &ctrlreq;
    DEPCTL(0, true) |= 0x84000000;  /* EP0 OUT ENABLE CLEARNAK */
    if (reinit)
    {
        ...
        DEPCTL(1, false) = 0x10001800;
        /* EP2 OUT INACTIVE DATA0 SIZE=0 */
        DEPCTL(2, true) = 0x10000000;
        /* EP3 IN INACTIVE DATA0 SIZE=0 NEXT=0 */
        DEPCTL(3, false) = 0x10000000;
        /* EP4 OUT INACTIVE DATA0 SIZE=0 */
        DEPCTL(4, true) = 0x10000000;
    }
    else
    {
        /* INACTIVE DATA0 */
        DEPCTL(1, false) = (DEPCTL(1, false) & ~0x00008000) | 0x10000000;
        DEPCTL(2, true) = (DEPCTL(2, true) & ~0x00008000) | 0x10000000;
        DEPCTL(3, false) = (DEPCTL(3, false) & ~0x00008000) | 0x10000000;
        DEPCTL(4, true) = (DEPCTL(4, true) & ~0x00008000) | 0x10000000;
    }
...
int usb_drv_request_endpoint(int type, int dir)
{
    ...
    while (ep < USB_NUM_ENDPOINTS)
    {
        if (!endpoints[ep].active)
        {
            ...
            if (dir) DEPCTL(ep, false) = (DEPCTL(ep, false) & ~0x000C0000) | newbits;
            else DEPCTL(ep, true) = (DEPCTL(ep, true) & ~0x000C0000) | newbits;
            break;
        }
        ep += 2;
    }
...
void INT_USB_FUNC(void)
{
    ...
    if (ints & 0x2000)  /* enumeration done, we now know the speed */
    {
        ...
        DEPCTL(1, false) = (DEPCTL(1, false) & ~0x000003FF) | maxpacket;
        DEPCTL(2, true) = (DEPCTL(2, true) & ~0x000003FF) | maxpacket;
        DEPCTL(3, false) = (DEPCTL(3, false) & ~0x000003FF) | maxpacket;
        DEPCTL(4, true) = (DEPCTL(4, true) & ~0x000003FF) | maxpacket;
    }
        ...
        for (i = 0; i < 4; i += i + 1)  // 0, 1, 3
            if ((epints = DEPINT(i, false)))
            {
                if (epints & 1)  /* Transfer completed */
                {
                    ...
                    int bytes = endpoints[i].size - (DEPTSIZ(i, false) & 0x3FFFF);
                    if (endpoints[i].busy)
                    {
                        endpoints[i].busy = false;
                        endpoints[i].rc = 0;
    ...
    if (ints & 0x80000)  /* OUT EP event */
        for (i = 0; i < USB_NUM_ENDPOINTS; i += 2)
            if ((epints = DEPINT(i, true)))
            {
                ...
                {
                    commit_discard_dcache();
                    int bytes = endpoints[i].size - (DEPTSIZ(i, true) & 0x3FFFF);
                    if (endpoints[i].busy)
                    {
                        endpoints[i].busy = false;
                        endpoints[i].rc = 0;
                ...
                if (!i)
                {
                    DEPTSIZ(0, true) = 0x20080040;
                    DEPDMA(0, true) = &ctrlreq;
                    DEPCTL(0, true) |= 0x84000000;
                }
...
static void ep_send(int ep, const void *ptr, int length)
{
    ...
    DEPCTL(ep, false) |= 0x8000;  /* EPx OUT ACTIVE */
    int blocksize = usb_drv_port_speed() ? 512 : 64;
    int packets = (length + blocksize - 1) / blocksize;
    if (!length)
    {
        DEPTSIZ(ep, false) = 1 << 19;  /* one empty packet */
        DEPDMA(ep, false) = NULL;
    }
    else
    {
        DEPTSIZ(ep, false) = length | (packets << 19);
        DEPDMA(ep, false) = ptr;
    }
    commit_dcache();
    DEPCTL(ep, false) |= 0x84000000;  /* EPx OUT ENABLE CLEARNAK */
}
...
static void ep_recv(int ep, void *ptr, int length)
{
    endpoints[ep].busy = true;
    endpoints[ep].size = length;
    DEPCTL(ep, true) &= ~0x20000;  /* EPx UNSTALL */
    DEPCTL(ep, true) |= 0x8000;  /* EPx OUT ACTIVE */
    int blocksize = usb_drv_port_speed() ? 512 : 64;
    int packets = (length + blocksize - 1) / blocksize;
    if (!length)
    {
        DEPTSIZ(ep, true) = 1 << 19;  /* one empty packet */
        DEPDMA(ep, true) = NULL;
    }
    else
    {
        DEPTSIZ(ep, true) = length | (packets << 19);
        DEPDMA(ep, true) = ptr;
    }
    commit_dcache();
    DEPCTL(ep, true) |= 0x84000000;  /* EPx OUT ENABLE CLEARNAK */
}
...
{
    if (in) return DEPCTL(endpoint, false) & 0x00200000 ? true : false;
    else return DEPCTL(endpoint, true) & 0x00200000 ? true : false;
}
...
void usb_drv_stall(int endpoint, bool stall, bool in)
{
    if (in)
    {
        if (stall) DEPCTL(endpoint, false) |= 0x00200000;
        else DEPCTL(endpoint, false) &= ~0x00200000;
    }
    else
    {
        if (stall) DEPCTL(endpoint, true) |= 0x00200000;
        else DEPCTL(endpoint, true) &= ~0x00200000;
    }
}
