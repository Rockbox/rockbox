...
static uint16_t ata_smart_data[0x100] __attribute__((aligned(16)));
#endif
static bool ceata;
...
static int ata_smart(uint16_t* buf)
{
    ...
    if (ceata)
    {
        ...
        /*uint32_t old = SDCI_CTRL;
        SDCI_CTRL |= SDCI_CTRL_L_ENDIAN;*/

        memset(ceata_taskfile, 0, 16);
        ceata_taskfile[0xc] = 0x4f;
        ceata_taskfile[0xd] = 0xc2;
        /*ceata_taskfile[0xe] = 0x40;*/   /* TODO: Device/Head Register???, bit6: 0->CHS, 1->LBAS */
        ceata_taskfile[0xf] = 0xb0;
        PASS_RC(ceata_wait_idle(), 2, 0);
        if (ata_identify_data[54] == 0x41)
        {
            ceata_taskfile[0x9] = 0xd8;
            PASS_RC(ceata_write_multiple_register(0, ceata_taskfile, 16), 2, 1);
            PASS_RC(ceata_check_error(), 2, 2);
        }
        ceata_taskfile[0x9] = 0xd0;
        PASS_RC(ceata_write_multiple_register(0, ceata_taskfile, 16), 2, 3);
        PASS_RC(ceata_rw_multiple_block(false, buf, 1, CEATA_COMMAND_TIMEOUT * HZ / 1000000), 2, 4);
