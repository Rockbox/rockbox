...
bool dbg_hw_info(void)
{
    ...
    while(1)
    {
        ...
        if(state == 0)
        {
            ...
            _DEBUG_PRINTF(" FClk: %d.%d",  f_clk / MHZ, (f_clk % MHZ) / KHZ);
            _DEBUG_PRINTF("  CPU: %d.%d",  c_clk / MHZ, (c_clk % MHZ) / KHZ);
            _DEBUG_PRINTF("  AHB: %d.%d",  h_clk / MHZ, (h_clk % MHZ) / KHZ);
            _DEBUG_PRINTF("   SM1: %d.%d", s_clk / MHZ, (s_clk % MHZ) / KHZ);
            _DEBUG_PRINTF("   LCD: %d.%d", l_clk / MHZ, (l_clk % MHZ) / KHZ);
            _DEBUG_PRINTF("  APB: %d.%d",  p_clk / MHZ, (p_clk % MHZ) / KHZ);
            line++;
