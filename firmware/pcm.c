...
{
    [PCM_FORMAT_S16_2] = { .size = 2, .bits = 16 },
    [PCM_FORMAT_S24_4] = { .size = 4, .bits = 24 },
    [PCM_FORMAT_S32_4] = { .size = 4, .bits = 32 },
    [PCM_FORMAT_S24_3] = { .size = 3, .bits = 24 },
};
...
static void pcm_peak_peeker(struct pcm_stream_desc *desc, struct pcm_peaks *peaks)
{
    ...
    while (p < pend) {
        ...
        size_t pincr = 4*pcm_desc_frame_size(desc);
        ...
        for (unsigned int ch = 0; ch < chnum; ch++) {
            ...
            case 4:  s = read_int32_preidx(&p, ch, false); break;
#endif
#if (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_3_BYTE_CAPS)
            case 3:  s = read_int24_preidx(&p, ch, false); break;
#endif
            default: s = read_int16_preidx(&p, ch, false); break;
            }
...
int pcm_get_format_dims(unsigned int code, struct pcm_format_dims *dims)
{
    if (!dims || code >= ARRAYLEN(pcm_sample_dims)) {
        return -1;
    }

    typeof (*pcm_sample_dims) *sd = &pcm_sample_dims[code];
    dims->bits = sd->bits;
    dims->size = sd->size;
                                 ...
                                 const struct pcm_format *format)
{
    ...
    if (rc == 0) {
        ...
        desc->sample_bits = pcm_sample_dims[format->code].bits;
#endif /* CONFIG_PCM_MULTISIZE */
        desc->frame_size  = pcm_desc_sample_size(desc) * format->chnum;
    }
    else {
                                         ...
                                         const struct pcm_format *format)
{
    ...
    if (format->code != PCM_FORMAT_S16_2 || format->chnum != 2) {
        ...
        desc->sample_bits = pcm_sample_dims[PCM_FORMAT_S16_2].bits;
#endif /* CONFIG_PCM_MULTISIZE */
        desc->frame_size  = pcm_desc_sample_size(desc) * format->chnum;
        return 0;
    }
                                    ...
                                    struct pcm_peaks *peaks)
{
    if (desc->state != PCM_STATE_STOPPED) {
        ...
        if (peaks->addr && peaks->frames) {
            ...
            const void *end = peaks->addr + peaks->frames*pcm_desc_frame_size(desc);
