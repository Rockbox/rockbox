...
unsigned long __pcm_curr_hw_sampr SHAREDBSS_ATTR = 0;
/* samplerate waiting to be set */
unsigned long __pcm_hw_sampr SHAREDBSS_ATTR = 0;
/* samplerate frequency selection index */
int __pcm_hw_fsel SHAREDBSS_ATTR = 0;
...
static void pcm_apply_device_settings(struct pcm_stream_desc *desc)
{
    if (__pcm_hw_sampr == __pcm_curr_hw_sampr)
        return;

    pcm_play_lock_device_callback();

    if (!pcm_mixer_apply_settings(desc)) {
    ...
    pcm_dma_apply_settings();
    __pcm_curr_hw_sampr = __pcm_hw_sampr;
}
...
void pcm_init(void)
{
    ...
    __pcm_hw_sampr = HW_SAMPR_DEFAULT;
    __pcm_hw_fsel = HW_FREQ_DEFAULT;
...
unsigned long pcm_set_frequency(pcm_handle_t pcm, unsigned long samplerate)
{
    ...
    enum pcm_sampr_type type = IS_CAPTURE_STREAM(desc->flags) ?
                            SAMPR_TYPE_PLAY : SAMPR_TYPE_REC;
    ...
    __pcm_hw_sampr = hw_freq_sampr[index];
    __pcm_hw_fsel = index;

    /* return actual rate obtained */
#ifdef CONFIG_SAMPR_TYPES
    return pcm_sampr_convert(__pcm_hw_sampr, SAMPR_TYPE_HW, type);
#else /* ndef CONFIG_SAMPR_TYPES */
    return __pcm_hw_sampr;
#endif /* CONFIG_SAMPR_TYPES */
}
...
int pcm_apply_settings(pcm_handle_t pcm)
{
    ...
    /* do the thing */
    pcm_apply_device_settings(desc);
...
static void pcm_playback_start_dma(struct pcm_stream_desc *desc)
{
    ...
    memset(pcm_play_dbl_buf, 0, sizeof (pcm_play_dbl_buf));
                               ...
                               struct pcm_peaks *peaks)
{
    ...
    if (desc->state == PCM_STATE_RUNNING) {
        unsigned long lookahead = peaks->period*__pcm_curr_hw_sampr / HZ;
...
{
    int index0 = pcm_play_dbl_buf_index;
    int index1 = 1 - index0;
    ...
    if (status == 0 && frames) {
        ...
        pcm_play_dbl_buf_frames[index1] =
            pcm_mixer_fill_buffer(status, pcm_play_dbl_buf[index1],
                                  PCM_DBL_BUF_FRAMES);
        return;
    }
