...
static void pcm_playback_dma_start(const void *addr, unsigned long frames)
{
    logf(" pcm_playback_dma_start");
...
static void pcm_playback_start_dma(struct pcm_stream_desc *desc)
{
    ...
    pcm_playback_dma_start(pcm_dbl_buf[0], pcm_dbl_buf_frames[0]);
}
                               ...
                               bool pause)
{
    ...
    if (pause == (desc->state == PCM_STATE_PAUSED)) {
        return;
    }
...
static inline void pcm_playback_close(struct pcm_stream_desc *desc)
{
    pcm_playback_stop(pcm);
}
                   ...
                   const struct sample_format *format)
{
    ...
    {
         rc = setup_playback_format(desc, format);
    }
