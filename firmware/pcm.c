                                    ...
                                    const void **addr, size_t *size)
{
    if (status < PCM_DMAST_OK)
        status = pcm_call_status_cb(pcm_play_status_callback, status);
...
static void update_frame_params(size_t size)
{
    ...
    frame_size  = count / frame_count * PCM_SAMPLE_SIZE;
    frame_frac  = count % frame_count;
    frame_err   = 0;
}
...
pcm_play_dma_status_callback_int(enum pcm_dma_status status)
{
    ...
    if (size == 0)
    {
        ...
        if (get_more_int(&addr, &size))
        {
            update_frame_params(size);
            pcm_call_status_cb(pcm_play_status_callback, PCM_DMAST_STARTED);
        }
    }
