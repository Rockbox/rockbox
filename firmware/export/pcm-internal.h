...
#ifdef HAVE_RECORING
#define PCM_REC_HANDLE 1
#else
#define PCM_REC_HANDLE 0
#endif

#ifdef HAVE_HARDWARE_BEEP
#define PCM_BEEP_HANDLE 0
#else
#define PCM_BEEP_HANDLE 1
#endif

/* Audio, voice, beep, plugin, rec */
#define PCM_MAX_HANDLES (3 + PCM_REC_HANDLE + PCM_BEEP_HANDLE)
...
struct pcm_stream_desc
{
    unsigned int  flags;                  /* stream attributes/setting bits */
    pcm_handle_t  id;                     /* current/next descriptor handle */
    int           state;                  /* playback/recording state */
    union {  /* rec/play mutually exclusive */
    struct {
    void          *addr;
    void (*callback)(void);
    };
    struct { /* play */
    const void    *addr_tx;               /* buffer pointer */
    pcm_play_callback_type callback_tx;   /* registered callback */
    int32_t       amplitude_tx;           /* stream's amplitude factor */
    };
#ifdef HAVE_RECORDING
    struct { /* rec */
    void          *addr_rx;               /* buffer pointer */
    pcm_record_callback_type callback_rx; /* registered callback */
    unsigned long seqnum_rx;              /* buffer sequence number */
    };
#endif /* HAVE_RECORDING */
    };
    unsigned long frames;                 /* frames remaining to rx/tx */
    unsigned long prev_frames;            /* frames rx/tx'ed in last cycle */
#ifdef CONFIG_PCM_MULTISIZE
    size_t        sample_size;            /* size of single sample */
    unsigned int  sample_bits;            /* # of value bits + sign bit */
    int           sample_shift_unity;     /* shift amount for output */
    int           sample_shift_gain;      /* shift when applying gain */
...
#define pcm_desc_sample_shift_unity(desc)   ((desc)->sample_shift_unity)
#define pcm_desc_sample_shift_gain(desc)    ((desc)->sample_shift_gain)
#else /* ndef CONFIG_PCM_MULTISIZE */
#define pcm_desc_sample_size(desc)          2
#define pcm_desc_sample_bits(desc)          16
#define pcm_desc_sample_shift_unity(desc)   0
#define pcm_desc_sample_shift_gain(desc)    (-16)
#endif /* CONFIG_PCM_MULTISIZE */
