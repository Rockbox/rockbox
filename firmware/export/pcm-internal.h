...
#ifdef HAVE_RECORDING
    pcm_record_callback_type callback_rec;
#endif
    };
...
static FORCE_INLINE void pcm_sw_volume_apply(void *buffer,
                                             unsigned long frames)
{
    extern void (* pcm_sw_volume_scaling_fn)(void *, const void *,
                                             unsigned long);
...
#define pcm_curr_sampr \
    ({ extern unsigned long __pcm_curr_hw_sampr; \
       *(const unsigned long *)&__pcm_curr_hw_sampr; })

#define pcm_sampr \
    ({ extern unsigned long __pcm_hw_sampr; \
       *(const unsigned long *)&__pcm_hw_sampr; })

#define pcm_fsel \
    ({ extern int __pcm_hw_fsel; \
       *(const int *)&__pcm_hw_fsel; })
