...
#define __struct_bpb_DRV_FAT \
    ...
    unsigned long bpb_totsec16;   /* Number of sectors (old 16-bit) */      \
    uint8_t       bpb_numfats;    /* Number of FAT structures, typ. 2 */    \
    uint8_t       bpb_media;      /* Media type (typ. 0xf0 or 0xf8) */      \
    uint16_t      bpb_fatsz16;    /* Number of used sectors per FAT */      \
    unsigned long bpb_totsec32;   /* Number of sectors on the volume        \
                                     (new 32-bit) */                        \
    uint16_t      last_word;      /* 0xAA55 */                              \
    IF_FAT16(                                                               \
    uint8_t       is_fat16; /* 1 if a FAT16 partition, 0 if FAT32 */        \
    ) /* IF_FAT16 */                                                        \
    long          bpb_rootclus;                                             \
                                                                            \
    /**** FAT32 specific *****/                                             \
    ...
    unsigned long rootdirsectornum; /* sector offset of root dir relative   \
...
#define __struct_fileinfo_DRV_FAT \
    ...
    long         dircluster;    /* first cluster of parent */         \
    __struct_idx_info_DRV_FAT;  /* directory index info */            \
    }
