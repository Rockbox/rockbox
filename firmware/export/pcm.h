...
#define CONFIG_PCM_MULTISIZE
#endif

#if !(CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_S16_2)
#error PCM_FORMAT_S16_2 support is compulsory; \
        please check CONFIG_PCM_FORMAT_CAPS
#endif
...
#elif PCM_DMA_T_FORMAT_CODE == PCM_FORMAT_S24_3
/* 3 bytes */
#define PCM_DMA_T_MIN    (INT32_MIN/0x100)
#define PCM_DMA_T_MAX    (INT32_MAX/0x100)
#define PCM_DMA_T_SIZE   ((size_t)3)
...
struct pcm_format_dims
{
    unsigned int bits; /* range bits in each sample */
    size_t       size; /* size of one sample in bytes */
};

/* return information about the format */
int pcm_get_format_dims(unsigned int code,
