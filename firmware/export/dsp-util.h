                                           ...
                                           unsigned int fracbits)
{
    if (depth + fracbits <= 32) {
        return (factor * sample + ((1 << fracbits) - 1)) >> fracbits;
    }
    else {
...
#define pcm_dma_t_mix_clip(sample1, sample2, downscale) \
    mix_clip_sample((sample1), (sample2), PCM_DMA_T_DEPTH, (downscale))
