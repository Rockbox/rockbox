...
static const char* bluetooth_dgb_entry(int selected_item, void* data,
    ...
    switch(selected_item) {
        case 0:
            snprintf(buffer, buffer_len, "HW Power: %s",
                     bluetooth_is_powered() ? "ON" : "OFF");
            break;
        case 1:
            snprintf(buffer, buffer_len, "HW Reset: %s",
                     bluetooth_is_reset() ? "ON" : "OFF");
            break;
        case 2:
            snprintf(buffer, buffer_len, "HW Awake: %s",
                     bluetooth_is_awake() ? "ON" : "OFF");
            break;
        default:
            break;
    }
...
static void bluetooth_dbg_action(int item)
{
    switch(selected_item) {
        case 0:
            bluetooth_power(!bluetooth_is_powered());
            break;
        case 1:
            bluetooth_reset(!bluetooth_is_reset());
            break;
        case 2:
            bluetooth_awake(!bluetooth_is_awake());
            break;
         default:
            break;
    }
...
bool bluetooth_dbg_screen(void)
{
    ...
    int action;
    gui_synclist_init(&lists, bluetooth_dgb_entry, NULL, false, 1, NULL);
