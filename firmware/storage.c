        ...
        case SYS_TIMEOUT:;
            ...
            if (trig) {
                ...
                /* anything could get dirt here since idle notifys aren't
                   local to the storage driver going to sleep -- won't flush
                   to unmounted volumes in USB mode but we may have only
                   gotten the message but not yet actually connected in
                   bulk storage mode -- the only safe move is to flush
                   everything possible; call will access storage only if
                   there actually is dirt */
