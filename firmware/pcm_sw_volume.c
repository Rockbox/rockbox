...
sw_volume_sample_amp(int32_t f, int32_t s, bool clipping)
{
    if (clipping) {
        return pcm_dma_t_amplify_sat(s, f, PCM_SW_VOLUME_FRACBITS);
    }
    else  {
...
static void sw_volume_buffer_cut(void *buffer, unsigned long count)
{
    ...
    while (count--)
    {
        int32_t ch0 = pcm_dma_t_read_preidx(buffer, 0, false);
        int32_t ch1 = pcm_dma_t_read_preidx(buffer, 1, false);

        ch0 = sw_volume_sample_amp(factor_l, ch0, false);
        ch1 = sw_volume_sample_amp(factor_r, ch1, false)
...
static void sw_volume_buffer_boost(void *buffer, unsigned long count)
{
    ...
    while (count--)
    {
        ...
        ch0 = swvolume_sample_amp(factor_l, ch0, true);
        ch1 = swvolume_sample_amp(factor_r, ch1, true)
...
static void sw_volume_buffer_trans(void *buffer, unsigned long count)
{
    ...
    for (unsigned long done = 0; done < count; done++)
    {
        ...
        ch0 = swvolume_sample_amp(f_l, ch0, true);
        ch1 = swvolume_sample_amp(f_r, ch1, true);
