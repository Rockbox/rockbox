...
static struct dirstr_desc * get_dirstr(DIR *dirp)
{
    ...
    errno = (dir && dir->stream.flags == FV_NONEXIST) ? ENXIO : EBADF;
    return NULL;
}
...
static int readdir_volume_inner(struct dirstr_desc *dir, struct dirent *entry)
{
    ...
    while (++dir->volumecounter < NUM_VOLUMES)
    {
        ...
        entry->info.attribute = ATTR_DIRECTORY | ATTR_VOLUME;
        entry->info.size      = 0;
...
int closedir(DIR *dirp)
{
    ...
    if (!PTR_IN_ARRAY(open_streams, dir, MAX_OPEN_DIRS) || !dir->stream.flags)
    {
        DEBUGF("dir #%d: dir not open\n", (int)(dir - open_streams));
        FILE_ERROR(EBADF, -1);
    }
...
{
    if (!result)
        FILE_ERROR_RETURN(EINVAL, -2);

    *result = NULL;

    if (!entry)
        FILE_ERROR_RETURN(EINVAL, -3);
...
/* return if two directory descriptors refer to the same directory */
int samedir(DIR *dirp1, DIR *dirp2)
{
    struct dirstr_desc * const dir1 = GET_DIRSTR(WRITER, dirp1);
    struct dirstr_desc * const dir2 = GET_DIRSTR(WRITER, dirp2);
...
int mkdir(const char *path)
{
    ...
    rc = open_stream_internal(path, FF_DIR | FF_ANYTYPE, &stream, &compinfo);
    if (rc < 0)
    {
...
int rmdir(const char *name)
{
    ...
    if (name)
    {
        ...
        const char *basename;
        int length = path_basename(name, &basename);
        if (basename[0] == PATH_DOTCH && length == 1)
        {
