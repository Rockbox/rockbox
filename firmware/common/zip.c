...
} __attribute__((packed)) zip_ed_disk;
...
typedef struct {
    ...
    // comment block (variable length)
} __attribute__((packed)) zip_cd_disk;
...
typedef struct {
    ...
    // name block (variable length)
    // extra block (variable length)
} __attribute__((packed)) zip_lf_disk;
...
static int zip_read_ed(zip* z) {
    const off_t file_size = z->size(z);
    const off_t max_size = sizeof(zip_ed_disk) + ZIP_MAX_LENGTH;
    const off_t read_size = MIN(file_size, max_size);
    const uint32_t sig = htole32(ZIP_SIG_ED);
    int mem_handle = -1;
    uint8_t* mem;
    off_t i = read_size - sizeof(zip_ed_disk);
    const zip_ed_disk* edd;
    uint16_t disk_number;
    uint16_t disk_number_cd;
    uint16_t disk_entries_cd;
    uint16_t cd_entries;
    ...
    if (file_size < (off_t) sizeof(zip_ed_disk)) {
        rv = -2;
        goto bail;
    }
...
static int zip_read_cd(zip* z, bool use_cb) {
    const zip_ed* ed = &z->ed;
    ...
    if ((cds_handle = zip_core_alloc(sizeof(zip_cd) * cd_entries)) < 0) {
        rv = -7;
        goto bail;
    }
...
static int zip_read_lf(zip* z, uint16_t i) {
    const uint32_t sig = htole32(ZIP_SIG_LF);
    const zip_cd* cd = &z->cds[i];
    zip_lf* lf = &z->lf;
    zip_lf_disk lfd;
    uint16_t name_length;
    ...
    if (z->read(z, &lfd, sizeof(zip_lf_disk)) != sizeof(zip_lf_disk))
        return -15;
...
static int zip_read_store(zip* z, void* mem, uint32_t mem_size) {
    const zip_lf* lf = &z->lf;
    zip_args* args = &z->args;
...
static int zip_read_entry(zip* z, uint16_t i, void* mem, uint32_t mem_size) {
    const zip_lf* lf = &z->lf;
    zip_args* args = &z->args;
    int rv;
...
static int zip_read_entries(zip* z) {
    const zip_ed* ed = &z->ed;
    const uint16_t cd_entries = ed->cd_entries;
    zip_lf* lf = &z->lf;
    zip_args* args = &z->args;
...
static void zip_init(zip* z, int zip_handle) {
    z->zip_handle = zip_handle;
    z->state = ZIP_STATE_INITIAL;
    z->cb = NULL;
    memset(&z->args, 0, sizeof(zip_args));
    z->ctx = NULL;
    memset(&z->ed, 0, sizeof(zip_ed));
    z->cds_handle = -1;
    z->cds = NULL;
    memset(&z->lf, 0, sizeof(zip_lf));
}

static ssize_t zip_file_read(zip* zh, void* mem, size_t mem_size) {
    zip_file* z = (zip_file*) zh;

    return read(z->file, mem, mem_size);
}

static off_t zip_file_seek(zip* zh, off_t offset, int whence) {
    zip_file* z = (zip_file*) zh;

    return lseek(z->file, offset, whence);
}

static off_t zip_file_size(zip* zh) {
    zip_file* z = (zip_file*) zh;

    return filesize(z->file);
}

static void zip_file_close(zip* zh) {
    zip_file* z = (zip_file*) zh;

    close(z->file);
}

static void zip_file_init(zip_file* z, int zip_handle, int file) {
    zip* zh = &z->base;
...
static ssize_t zip_mem_read(zip* zh, void* mem, size_t mem_size) {
    zip_mem* z = (zip_mem*) zh;
...
static off_t zip_mem_seek(zip* zh, off_t offset, int whence) {
    zip_mem* z = (zip_mem*) zh;
    off_t new_offset;
...
static off_t zip_mem_size(zip* zh) {
    zip_mem* z = (zip_mem*) zh;

    return z->mem_size;
}

static void zip_mem_close(zip* zh) {
    zip_mem* z = (zip_mem*) zh;

    core_free(z->mem_handle);
}

static void zip_mem_init(zip_mem* z, int zip_handle, int mem_handle, const void* mem, off_t mem_size) {
    zip* zh = &z->base;
...
zip* zip_open(const char* name, bool try_mem) {
    ...
    if (try_mem && (mem_handle = zip_core_alloc(mem_size = filesize(file))) >= 0) {
        if ((zip_handle = zip_core_alloc(sizeof(zip_mem))) < 0)
            goto bail;
        ...
        if ((zip_handle = zip_core_alloc(sizeof(zip_file))) < 0)
            goto bail;
