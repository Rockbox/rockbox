...
static void fileobj_sync_parent(const struct file_base_info *infop[],
                                int count)
{
#ifdef FILEOBJ_MGR_NATIVE
    FOR_EACH_BINDING(BASEINFO_VOL(infop[0]), fobp)
    {
        ...
        {
            if (!fctrl_parentdir(&infop[0]->fctrl, &fobp->bind.info.fctrl))
                continue;
...
#endif /* FILEOBJ_MGR_NATIVE */

    (void)infop; (void)count;
}
...
{
    FOR_EACH_BINDING(BASEINFO_VOL(srcinfop), fobp)
    {
        if (fobp->flags & FO_REMOVED)
            continue;

        if (fctrl_samefile(&srcinfop->fctrl, &fobp->bind.info.fctrl))
        {
                         ...
                         unsigned int callflags)
{
    ...
    fctrl_io_assign_file(&stream->ioctrl, &fobp->bind.info.fctrl);
