...
static inline uint32_t get_random_priority(struct searchtree *tree)
{
...
void searchtree_init(struct searchtree *tree)
{
    ...
    tree->root.key      = ULONG_MAX;   /* ensure all nodes are left children */
    tree->root.random   = seed;
    tree->root.child[0] = NULL;        /* first real root node goes here */
    tree->root.child[1] = NULL;        /* never used */
