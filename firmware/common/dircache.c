...
#define TINYNAME_MAX   MAX(sizeof (int), 4)
...
struct dircache_entry
{
    ...
    union {
    int         name;                  /* indirect storage (.tinyname == 0) */
    char        namebuf[TINYNAME_MAX]; /* direct storage (.tinyname == 1) */
    };
    uint32_t    firstcluster;          /* first file cluster - max 0x0ffffff4 */
    ...
    uint16_t    wrtdate;               /* entry last-modified date */
    uint16_t    wrttime;               /* entry last-modified time */
...
static struct dircache
{
    ...
    struct dircache_volume            /* per volume cache data */
    {
        ...
        uint32_t       frontier : 2;  /* data is incomplete */
...
#define FOR_EACH_SAB_COMP(sabp, p) \
    for (struct sab_component *p = sabp->top; p; p = p->parent)
        ...
        if (p == sabp->top)
        {
            ...
            else
            {
                struct dircache_entry *ceprev =
                    TYPE_FROM_MEMBER(struct dircache_entry, prevp, next);
            #ifdef DIRCACHE_NATIVE
...
/**
 * get the entry's name pointer - buffer or allocated
 */
static unsigned char * entry_name(const struct dircache_entry *ce)
{
    return ce->tinyname ? (unsigned char *)ce->namebuf : get_name(ce->name);
}
    ...
    if (dircache.namesfree >= size)
    {
        ...
        if (gapsize >= size)
        {
            ...
            if (*p == 0xff)
            {
                ...
                size_t tinysize = 1;
                while (*++p == 0xff && tinysize <= TINYNAME_MAX)
                    tinysize++;

                if (tinysize <= TINYNAME_MAX)
                {
 ...
 */
static void free_name(int nameidx)
{
    unsigned char *namep = get_name(nameidx);
    unsigned char *p = namep;

    /* mark it free with 0xff sentinel (UTF-8 invalid); anything that was
       allocated here is NOT tiny */
    while (*p)
        *p++ = 0xff;
    *p++ = 0xff;

    /* merge with any preceding adjacent tiny block */
    while (namep[-1] == 0xfe)
        *--namep = 0xff;

    /* merge with any following adjacent tiny block */
    while (*p == 0xfe)
        *p++ = 0xff;
                              ...
                              const unsigned char *name)
{
    ...
    memcpy(entry_name(ce), name, size);
    return true;
}
 ...
 */
static int create_entry(const unsigned char *basename,
                        struct dircache_entry **res)
{
    ...
    if (!entry_assign_name(*res, basename))
    {
...
/**
 * scan and build the contents of the subdirectory
 */
static int sab_process_sub(struct sab *sabp)
{
    /* first pass: read directory */
    struct sab_component *dircompp = sabp->top;
    const long dircluster = sabp->stream.infop->fatfile.firstcluster;
    ...
    while (1)
    {
        if (sabp->issab)
        {
            ...
            if (sabp->quit || !dircompp->idx)
                return 0; /* abort! */
        }
        /* else an immediate-contents directory scan */

        struct fat_direntry *fatentp = get_dir_fatent();
        int rc = uncached_readdir_internal(&sabp->stream, &sabp->info,
                                           fatentp, 0);
        if (rc < 0)
            return -2;
        else if (rc == 0)
        ...
        int idx = 0;
        struct dircache_entry *ce;
        int prev = *dircompp->prevp;

        if (prev)
        {
            /* there are entries ahead of us; they will be what was just read
               or something to be subsequently read; if it belongs ahead of
               this one, insert a new entry before it; if it's the entry just
               scanned, do nothing further and continue with the next */
            ce = get_entry(prev);
            if (ce->direntry == sabp->info.fatfile.e.entry)
            {
                dircompp->prevp = &ce->next;
                continue; /* already there */
            }
        }

        idx = create_entry(fatentp->name, &ce);
        if (!idx)
            return -3;
        ...
        *dircompp->prevp = idx;
        ...
        ce->direntry     = sabp->info.fatfile.e.entry;
        ce->direntries   = sabp->info.fatfile.e.entries;
        ce->attr         = fatentp->attr;
        ce->wrtdate      = fatentp->wrtdate;
        ce->wrttime      = fatentp->wrttime;

        /* resolve queued user bindings */
        sabp->info.fatfile.firstcluster = fatentp->firstcluster;
        sabp->info.fatfile.dircluster   = dircluster;
        sabp->info.dcfile.idx           = idx;
        sabp->info.dcfile.serialnum     = ce->serialnum;
        binding_resolve(&sabp->info);
    } /* end while */

    establish_frontier(dircompp->idx, FRONTIER_SETTLED);

    if (!sabp->issab || sabp->depth >= DIRCACHE_MAX_DEPTH)
        return 0; /* just this level/no deeper */

    /* second pass: recurse! */
    struct sab_component comp;
    comp.parent = sabp->top; /* push us on the stack */
    sabp->top = &comp;
    sabp->depth++;

    dircompp->prevp = dircompp->downp; /* rewind list */

    int idx;
    while ((idx = *dircompp->prevp))
    {
        struct dircache_entry *ce = get_entry(idx);
        dircompp->prevp = &ce->next;
        ...
        /* even if it got zoned from outside it is about to be scanned in its
           entirety and may be considered new again */
        ce->frontier = FRONTIER_NEW;

        close_stream_internal(&sabp->stream);
        ...
        sabp->info.fatfile.firstcluster = ce->firstcluster;
        sabp->info.fatfile.dircluster   = dircluster;
        sabp->info.fatfile.e.entry      = ce->direntry;
        sabp->info.fatfile.e.entries    = ce->direntries;

        filestr_base_init(&sabp->stream);
        fileobj_fileop_open(&sabp->stream, &sabp->info, FO_DIRECTORY);
        fat_rewind(&sabp->stream.fatstr);
        uncached_rewinddir_internal(&sabp->info);

        int rc = sab_process_sub(sabp);
        if (rc < 0 || sabp->quit || !dircompp->idx)
            break; /* failed, quit or parent removed */
    } /* end while */

    sabp->depth--;
    sabp->top = dircompp; /* pop us off the stack */
    return 0;
}
...
static void sab_process_dir(struct file_base_info *infop, bool issab)
{
    ...
    int *downp = get_downidxp(infop->dcfile.idx);
    if (!downp)
        return;

    struct sab_component sub;
    sub.parent       = NULL;
    sub.idx          = infop->dcfile.idx;
    sub.downp        = downp;
    sub.prevp        = downp;
    ...
    sab.info.volume = BASEINFO_VOL(infop);
#endif

    /* open initial stream and rewind it */
    filestr_base_init(&sab.stream);
    fileobj_fileop_open(&sab.stream, infop, FO_DIRECTORY);
    fat_rewind(&sab.stream.fatstr);
    uncached_rewinddir_internal(&sab.info);

    if (issab)
        DCRIVOL(infop)->sabp = &sab;

    establish_frontier(infop->dcfile.idx, FRONTIER_NEW | FRONTIER_RENEW);

    sab_process_sub(&sab);
    close_stream_internal(&sab.stream);
                            ...
                            struct dirent *entry)
{
    ...
    strcpy(entry->d_name, entry_name(ce)); /* our d_name is adequate */
    entry->info.attribute   = ce->attr;
                              ...
                              unsigned int callflags)
{
    ...
    if (!dirinfop->dcfile.serialnum)
        return uncached_readdir_internal(stream, infop, fatent, callflags);
    ...
    if (frontier != FRONTIER_SETTLED)
    {
        ...
        int rc = uncached_readdir_internal(stream, infop, fatent, callflags);
    ...
    strcpy(fatent->name, entry_name(ce));
    fatent->shortname[0]     = '\0';
    fatent->attr             = ce->attr;
    fatent->crttimetenth     = 0;
    fatent->crttime          = 0;
    fatent->crtdate          = 0;
    fatent->lstaccdate       = 0;
    fatent->wrtdate          = ce->wrtdate;
    fatent->wrttime          = ce->wrttime;
...
static void build_volumes(void)
{
    ...
    if (reserve_used > 3*DIRCACHE_RESERVE / 4)
 ...
 * place; if 'really' is false, it's just a query about what will happen
 */
static int prepare_build(bool really, bool *realloced)
{
    ...
    if (dircache_runinfo.bufsize > size &&
        dircache.reserve_used <= 3*DIRCACHE_RESERVE / 4)
    {
        if (really)
            *realloced = false;
    ...
    if (size > DIRCACHE_RESERVE && size < DIRCACHE_LIMIT)
    {
        /* start a transparent rebuild */
        transparent = true;

        if (really)
            size += DIRCACHE_RESERVE*2;
    }
    else
    {
...
static void compact_cache(void)
{
    ...
    {
        if (ce->serialnum && !ce->tinyname)
             ce->name += offset;
    }

    dircache.names += offset;

    /* assumes buflib does no damn stupid things like calling callbacks or
       changing the pointer as a result of the shrink operation; it doesn't
       as of now but if it ever starts doing that shit, this is fucked
       since we're holding filesystem locks and your head is going to hurt
       trying to fix it properly; it might better be christened "beelzelib" */
...
static void dircache_thread(void)
{
    ...
    while (1)
    {
        ...
        if (!rcp && prepare_build(false, NULL) > 0)
            continue;

        bool realloced;
        int rc = prepare_build(true, &realloced);
        if (rcp)
            *rcp = rc;
 ...
 */
static bool dircache_thread_post(int *rcp)
{
                            ...
                            const struct dirinfo *dip)
{
    ...
    ce->attr         = dip->attribute;
    ce->wrtdate      = dip->wrtdate;
    ce->wrttime      = dip->wrttime;
    if (!(dip->attribute & ATTR_DIRECTORY))
        ce->filesize = dip->size;
    ...
    if ((dip->attribute & ATTR_DIRECTORY) && !is_dotdir_name(basename))
    {
                            ...
                            const char *basename)
{
    /* requires write exclusion */
    logf("dc rename: %u \"%s\"",
         (unsigned int)bindp->info.dcfile.serialnum, newname);
    ...
    unsigned char *name = entry_name(ce);
    if (strlen(name) == strlen(basename))
    {
        /* existing name allocation is the correct size */
        strcpy(name, basename);
        return;
    }

    /* needs a new name allocation */
    entry_unassign_name(ce);

    if (!entry_assign_name(ce, basename))
    {
                          ...
                          const struct dirinfo *dirinfo)
{
    ...
    ce->firstcluster = infop->fatfile.firstcluster;
#endif
    ce->attr         = dirinfo->attribute;
    ce->wrtdate      = dirinfo->wrtdate;
    ce->wrttime      = dirinfo->wrttime;
    if (!(dirinfo->attribute & ATTR_DIRECTORY))
        ce->filesize = dirinfo->size;
}
...
static ssize_t get_path_sub(int idx, char *buf, size_t size)
{
    ...
    if (idx > 0)
    {
        ...
        struct dircache_entry *ce = get_entry(idx);
        offset = get_path_sub(ce->up, buf, size);
        if (offset < 0)
            return -3;

        cename = entry_name(ce);
    }
    else /* idx < 0 */
    {
        offset = 0;
    ...
    return offset + append_to_path(buf + offset, NULL, cename,
...
get_file_sub_scan(int idx, const char *name, size_t length, int *idxp)
{
    ...
    if (ce)
    {
        name = STRDUP_ALLOCA(name, length);

        do
        {
            if (!strcasecmp(entry_name(ce), name))
            {
                *idxp = idx;
                break;
            }
...
static int get_file_sub(const char **pathp, int *downp, int *idxp)
{
    int rc;
    const char *name;
    rc = parse_path_component(pathp, &name);
    if (rc <= 0)
        return rc;
    else if (rc >= MAX_PATH)
...
int dircache_get_file(const char *path, struct dircache_file *dcfilep)
{
    ...
    while (rc == RC_CONTINUE || rc == RC_GO_UP)
    {
        ...
        if (rc == RC_GO_UP)
        {
            volume = strip_volume(path, &path, false);
            if (!CHECK_VOL(volume))
            {
                rc = ENXIO;
                break;
...
#ifdef DIRCACHE_DUMPSTER
/**
 * dump the raw dircache buffer to disk
 */
void dircache_dump(void)
{
    int fd = open(DIRCACHE_DUMPSTERFILE, O_WRONLY|O_CREAT|O_TRUNC, 0666);
    if (fd < 0)
        return;
    ...
    {
        buffer_lock();
        write(fd, dircache_runinfo.p + ENTRYSIZE,
...
#define DIRCACHE_MAGIC1  0x55d0c0a1
#define DIRCACHE_MAGIC2  0xaad0c0a1

/* dircache persistence file header */
struct dircache_maindata
{
    uint32_t        magic1;     /* DIRCACHE_MAGIC1 */
    struct dircache dircache;   /* metadata of the cache! */
    uint32_t        magic2;     /* DIRCACHE_MAGIC2 */
} __attribute__((packed, aligned (4)));

/* two magics guard against a changing sizeof (struct dircache) between
...
int dircache_load(void)
{
    ...
    int fd = open_dircache_file(O_RDONLY, 0);
    if (fd < 0)
        return -1;
    ...
    if (maindata.magic1 != DIRCACHE_MAGIC1 ||
        maindata.magic2 != DIRCACHE_MAGIC2)
    {
        logf("Dircache: invalid header magic");
        goto error_nolock;
    }

    #warning Do some more sanity checks!
    if (maindata.size != maindata.sizeentries + maindata.names_size ||
        ALIGN_DOWN(maindata.size, ENTRYSIZE) != maindata.size)
    {
    ...
    size_t bufsize = maindata.size + DIRCACHE_RESERVE + 1;
    handle = alloc_cache(bufsize);
    if (handle <= 0)
    {
        logf("Dircache failed allocation");
        goto error_nolock;
    }
    ...
    dircache.numentries = maindata.sizeentries / ENTRYSIZE;

    /* read the dircache file into memory; start with the entries */
    size = maindata.sizeentries;
    ...
    dircache.names -= maindata.sizenames;
    *get_name(dircache.names - 1) = 0;

    size = maindata.sizenames;
    ...
    /* only names will be changed in relative position so fix up those
       references */
    ssize_t offset = dircache.names - maindata.names;
    if (offset != 0)
    {
...
int dircache_save(void)
{
    ...
    struct dircache_maindata maindata =
    {
        .magic1   = DIRCACHE_MAGIC1,
        .dircache = dircache,
        .magic2   = DIRCACHE_MAGIC2
    }; 

    /* store the size since it better detects an invalid header */
    maindata.sizeentries = maindata.numentries * ENTRYSIZE;
    ...
    if (write(fd, get_name(maindata.names), size) != size)
    {
