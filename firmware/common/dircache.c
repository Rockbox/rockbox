                                ...
                                const unsigned char *newname)
{
    ...
    {
        char *p = mempcpy(ce->namebuf, newname, newlen);
...
static void sab_process_sub(struct sab *sabp)
{
    ...
    int maxlevel = sabp->stackend - sabp->stack;
    ...
    while (1)
    {
        ...
        while (!compp->idx || compp <= sabp->stack)
        {
            /* was removed/deepest level */
            compp = ++sabp->top;
            if (compp >= sabp->stackend)
                return; /* initial directory removed */
        }
