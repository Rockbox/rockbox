...
void volume_size(IF_MV(int volume,) unsigned long *size, unsigned long *free)
{
    ...
    if (!CHECK_VOL(volume) || !fat_size(IF_MV(volume,) size, free))
    {
        if (size) *size = 0;
        if (free) *free = 0;
    }
...
{
    disk_reader_lock();
    bool mounted = fat_ismounted(IF_MV(volume));
    disk_reader_unlock();
    return mounted;
}
#endif /* HAVE_DIRCACHE */
...
#ifdef HAVE_HOTSWAP
static bool volume_properties(int volume, bool present)
{
    bool res = false;
    ...
    if (CHECK_VOL(volume))
    {
        ...
        if (vd != -1)
            res = present ? storage_present(vd) : storage_removable(vd);
    }
...
{
    return volume_properties(volume, false);
}

bool volume_present(int volume)
{
    return volume_properties(volume, true);
}
#endif /* HAVE_HOTSWAP */
