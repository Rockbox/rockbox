 ...
 */
size_t path_strip_basename(const char *name, const char **nameptr)
{
 ...
 * detected.
 */
size_t path_append(char *buffer, const char *basepath,
                   const char *component, size_t bufsize)
{
    ...
    if (basepath)
    {
        /* this concats 'basepath' and 'component', overwriting 'buffer' */
        baselen = strlcpy(buffer, basepath, bufsize);
    }
    else
    {
        /* base path is in 'buffer' (append 'component' to 'buffer') */
        basepath = buffer;
    ...
    /* save a bit of space and gobble up the leading separators of what's
       being appended if any exist */
    component = GOBBLE_PATH_SEPCH(component);

    if (slashlen && bufsize > 0 && --bufsize)
        *buffer++ = PATH_SEPCH;

    return baselen + slashlen + strlcpy(buffer, component, bufsize);
}

/* Returns the location and length of the next path component, consuming the
 * input in the process. If 'want_dots' is true, then '.' components are
 * returned, otherwise they are silently consumed.
 *
 * Returns: 0 if the input has been consumed
 *          -1 if ".." was found (assume length is 2 if you wish to process)
