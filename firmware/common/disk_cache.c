...
{
    struct lld_node node;   /* LRU list links */
...
static struct lld_head cache_lru; /* LRU cache list (head = LRU item) */
...
{
    struct lld_node *lru = cache_lru.head;
    struct lld_node *node = &which->node;
