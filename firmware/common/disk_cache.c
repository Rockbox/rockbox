...
#define IOC_NUM_ENTRIES     (IOCACHE_SIZE*1024 / IOCACHE_BUFSIZE)
...
static PROBEFUNC_ATTR void buffer_write_back(struct iocache_buf_desc *bd)
{
    ...
    int rc = ((flags & IOBUF_CB) && bpb->ioc.wbcb) ?
                                       ...
                                       struct iocache_wb *wb)
{
    ...
    if (LIKELY(bd->flags & IOBUF_VALID))
    {
        ...
        /* parent is safe from adjustment if the tree is for a different
           volume since a node can only be in one tree at a time */
        index_remove_ent_adj(BD_BPB(bd), ent, num, &parent);
    }
