 ...
 * really speeds it up */
#define PROBEFUNC_ATTR      __attribute__((hot))
...
struct iocache_buf_desc
{
    ...
    unsigned long num;      /* device buffer number */
...
struct iocache_entry
{
    ...
    struct searchtree_node  idxnode; /* indexing node */
};
...
struct iocache_wb
{
    ...
    struct iocache_buf_desc bdwb;    /* buffer doing writeback */
};
...
#define IDXNODE_ENT(trn)    container_of(trn, struct iocache_entry, idxnode)
#define IDXNODE_BD(trn)     (&IDXNODE_ENT(trn)->bd)
...
/** writeback queue **/
static inline void wbq_init(struct bpb *bpb)
{
    ll_init(&bpb->ioc.wbq);
}
...
static PROBEFUNC_ATTR void wbq_addbuf(struct iocache_buf_desc *bd,
                                      struct iocache_wb *wb)
{
...
static PROBEFUNC_ATTR void wbq_commit(struct bpb *bpb)
{
    ...
    while ((wbp = wbq_get_next(wbq)))
    {
        /* TODO: Assess whether it's worthwhile to grab the buffer back
           ...
           in the queue. */
        struct iocache_buf_desc *bd = &wbp->bdwb;
...
static FORCE_INLINE
    struct searchtree_node * index_next(struct searchtree_node *node)
{
    return searchtree_successor_of(node);
}

static FORCE_INLINE unsigned long index_num(struct searchtree_node *node)
{
...
static FORCE_INLINE void index_insert(struct bpb *bpb,
                                      struct searchtree_node *node,
                                      unsigned long num,
                                      struct searchtree_node *parent)
{
    searchtree_insert(&bpb->ioc.index, node, num, parent);
}

static FORCE_INLINE void index_remove(struct bpb *bpb,
                                      struct searchtree_node *node,
                                      unsigned long num,
                                      struct searchtree_node **parentp)
{
    searchtree_remove(&bpb->ioc.index, node, num, parentp);
}

static FORCE_INLINE void index_remove_node(struct bpb *bpb,
                                           struct searchtree_node *node)
{
    searchtree_remove(&bpb->ioc.index, node, 0, NULL);
}
...
static PROBEFUNC_ATTR int buffer_fill(struct iocache_buf_desc *bd)
{
    ...
    int rc = storage_read_sectors(IF_MD(bd->bpb->drive,) bd->num, 1, bd->buf);
    if (rc < 0)
    {
                                       ...
                                       struct iocache_wb *wb)
{
    ...
    struct searchtree_node *parent;
    struct searchtree_node *node = index_search(bpb, num, &parent);
    if (node)
        return IDXNODE_ENT(node); /* ent */

    /* if cache is full, something has to be evicted; if cache is filling,
       this will return an unused entry */
    struct iocache_entry *ent = lru_select_victim_ent(priority);
    if (!ent)
        return NULL;
    ...
    if (LIKELY(bd->flags & IOBUF_VALID))
    {
        ...
        index_remove(bdbpb, &ent->idxnode, num,
                     IF_MV( bdbpb != bpb ? NULL : ) &parent);
    }

    index_insert(bpb, &ent->idxnode, num, parent);
                                  ...
                                  bool fill)
{
    ...
    if (flags == 0)
    {
        ...
        {
            index_remove_node(bpb, &ent->idxnode);
            bd->priority = 0;
            flags = IOBUF_ERR;
        }
                       ...
                       bool write)
{
    ...
    while (1)
    {
        ...
        int cnt = bd ? bd->num - start : (unsigned long)count;
...
void iocache_flush_volume(struct bpb *bpb)
{
         ...
         node; node = index_next(node))
    {
        ...
        struct iocache_buf_desc *bd = IDXNODE_BD(node);
