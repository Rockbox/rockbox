...
#define FMT_LENMOD_h    0x01   /* signed/unsigned short (%h<radix>) */
#define FMT_LENMOD_hh   0x02   /* signed/unsigned char (%hh<radix>) */
#define FMT_LENMOD_j    0x04   /* intmax_t/uintmax_t (%j<radix>) */
#define FMT_LENMOD_l    0x08   /* signed/unsigned long (%l<radix>) */
#define FMT_LENMOD_ll   0x10   /* signed/unsigned long long (%ll<radix>) */
#define FMT_LENMOD_t    0x20   /* signed/unsigned ptrdiff_t (%t<radix>) */
#define FMT_LENMOD_z    0x40   /* size_t/ssize_t (%z<radix>) */

/* compulsory radixes: c, d, i, u, s */
#define FMT_RADIX_c     0x01   /* single character (%c) */
#define FMT_RADIX_d     0x02   /* signed integer type, decimal (%d %i) */
#define FMT_RADIX_n     0x04   /* bytes output so far (%n) */
#define FMT_RADIX_o     0x08   /* unsigned integer type, octal (%o) */
#define FMT_RADIX_p     0x10   /* pointer (%p %P) */
#define FMT_RADIX_s     0x20   /* string (%s) */
#define FMT_RADIX_u     0x40   /* unsigned integer type, decimal (%u) */
#define FMT_RADIX_x     0x80   /* unsigned integer type, hex (%x %X) */
...
#define CHAR_INT_PARAM  int
#define UCHAR_INT_PARAM int
#define SHRT_INT_PARAM  int
#if USHRT_MAX == UINT_MAX
#define USHRT_INT_PARAM unsigned int
#else
#define USHRT_INT_PARAM int
#endif
                                    ...
                                    int lenmod)
{
    ...
    const char *nil = memchr(s, 0x00, len);
    if (nil) {
        len = nil - s;
    }
             ...
             va_list ap)
{
   ...
   flags_done:
        ...
        case 'i':
            ...
            #if (FMT_LENMOD & FMT_LENMOD_h)
            case LENMOD_h:
                val_d = (signed short)va_arg(ap, SHRT_INT_PARAM);
                goto branch_fmt_d;
            #endif
            #if (FMT_LENMOD & FMT_LENMOD_hh)
            case LENMOD_hh:
                val_d = (signed char)va_arg(ap, CHAR_INT_PARAM);
                goto branch_fmt_d;
            #endif
            #if (FMT_LENMOD & FMT_LENMOD_j)
            case LENMOD_j:
        ...
        case 'X':
            ...
            #if (FMT_LENMOD & FMT_LENMOD_h)
            case LENMOD_h:
                val_u = (unsigned short)va_arg(ap, USHRT_INT_PARAM);
                goto branch_fmt_u;
            #endif
            #if (FMT_LENMOD & FMT_LENMOD_hh)
            case LENMOD_hh:
                val_u = (unsigned char)va_arg(ap, UCHAR_INT_PARAM);
                goto branch_fmt_u;
            #endif
            #if (FMT_LENMOD & FMT_LENMOD_j)
            case LENMOD_j:
