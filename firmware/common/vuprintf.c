...
#define format_double_int10     format_arbitrary_int10
#define format_double_frac10    format_arbitrary_frac10
#endif
...
static long parse_ieee754_double(double f,
                                 struct arbitrary_int *ia,
                                 struct arbitrary_int *fa,
                                 struct fmt_buf *fmt_buf)
{
    long rc = 0;

    uint64_t f64 = *(uint64_t *)&f;
    int e = ((int)(f64 >> 52) & 0x7ff) - 1023; /* -1023..1024 */
    uint64_t mantissa = f64 & 0x000fffffffffffffull;
...
static void init_ieee754_double_chunks(struct arbitrary_int *a,
                                       uint32_t *a_chunks)
{
                               ...
                               void *userp)
{
    ...
    char buf[6 + ia.len + f_prec + 1];
    char *p_last  = &buf[6 + ia.len];
    char *p_dec   = p_last;
    char *p_first = format_double_int10(&ia, p_last);

    if (explen) {
        if (!ia.val && fa.len) {
            p_first = p_last = &buf[6];
