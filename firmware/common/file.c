...
/* main function to hide/unhide selected file/directory */
static int hide_file_dir_part(const char* name, bool hide, bool is_file)
{
    int rc;
    struct filedesc* file;

    /* Can't use dircache now, because we need to access the fat structures. */
    int fd = open_internal_part(name, O_RDONLY, false, is_file);
    if (fd < 0) {
        if (is_file && (errno == EISDIR))
            return file_hidedir(name, hide);
        else
            return fd * 10 - 1;
    }
...
{
    return hide_file_dir_part(name, hide, false);
}

/* hide/unhide selected file/directory */
int file_hide(const char* name, bool hide)
{
    return hide_file_dir_part(name, hide, true);
}
