...
static int flush_cache(struct filestr_desc *file)
{
    ...
    if (rc < 0)
        FILE_ERROR(rc == FAT_RC_ENOSPC ? ENOSPC : EIO, rc * 10 - 2);
                              ...
                              bool write_now)
{
    ...
    if (write_now)
    {
        unsigned long sector = filesize_sectors(MIN(size, cursize));
        ...
        rc = fat_truncate(&file->stream.fatstr);
        if (rc < 0)
...
static int fsync_internal(struct filestr_desc *file)
{
    ...
    /* flush sector cache? */
    if (file->stream.cachep->flags & FSC_DIRTY)
    {
        rc = flush_cache(file);
        if (rc < 0)
            FILE_ERROR(ERRNO, rc * 10 - 1);
    ...
    if (foflags & FO_TRUNC)
    {
        rc = ftruncate_internal(file, size, true);
        if (rc < 0)
...
static int open_internal_inner(const char *path,
...
/* allocate a file descriptor, if needed, and open a new stream */
static int open_internal(const char *path, int oflag, unsigned int callflags,
                         struct filestr_desc **fileres)
{
    ...
    int fildes = 0;
    struct filestr_desc *file = *fileres;

    if (!file)
    {
        /* no file passed as argument to use so allocate one */
        fildes = alloc_filestr(&file);
        if (fildes < 0)
            FILE_ERROR(EMFILE, -1);
    }
    ...
    if (oflag & O_NOISODECODE)
        callflags |= FF_NOISO;

    rc = open_internal_inner(path, file, callflags);
    if (rc < 0)
                         ...
                         bool write)
{
    ...
    if (write)
    {
        /* if opened in append mode and writing, move pointer to end */
        if (file->stream.flags & FD_APPEND)
            file->offset = MIN(size, FILE_SIZE_MAX);

        /* would file get too large? */
        file_size_t filerem = FILE_SIZE_MAX - file->offset;
        if (filerem < nbyte)
            FILE_ERROR_RETURN(EFBIG, -1);
    }
    else
    {
        /* attempt to read past EOF or beyond addressable range? */
        file_size_t filerem = MIN(size, FILE_SIZE_MAX) - file->offset;
        if (nbyte > filerem)
            nbyte = filerem;
    }
...
/* return the filestr_desc associate with the file descriptor */
struct filestr_base * file_get_stream_internal(int fildes, off_t *offsetp)
{
    struct filestr_desc *file = &open_streams[fildes];

    if (fildes < 0 || fildes >= MAX_OPEN_FILES || !file->stream.flags)
        return NULL;
...
/* ONE thread may use this at a time!
 * Wrap in calls to file_internal_aux_(un)lock
 * O_ACCMODE flags are ignored.
 */
...
{
    if (!path || !buf)
        FILE_ERROR_RETURN(EINVAL, -1);

    oflag &= ~O_ACCMODE; /* read only! */

    struct filestr_desc auxfile, *file = &auxfile;
    ssize_t rc = open_internal(path, oflag, FF_FILE, &file);
    if (rc < 0)
        FILE_ERROR(ERRNO, rc * 10 - 2);

    FILESTR_LOCK(READER, &file->stream);

    rc = lseek_internal(file, offset, SEEK_SET);
    if (rc < 0)
        FILE_ERROR(ERRNO, rc * 10 - 3);
...
file_error:
    ...
    {
        close_internal(file);
        FILESTR_UNLOCK(READER, &file->stream);
    }
    return rc;
}
...
int open(const char *path, int oflag)
{
    ...
    int rc = open_internal(path, oflag, FF_ANYTYPE, &file);
    if (rc < 0)
        FILE_ERROR(ERRNO, rc * 10 - 1);
...
int rename(const char *old, const char *new)
{
    ...
    bool old_isopen = false, new_isopen = false;

    file_internal_lock_WRITER();

    /* open 'old'; it must exist */
    rc = open_stream_internal(old, FF_ANYTYPE, &oldstr, &oldinfo);
    if (rc < 0)
    {
    ...
    rc = open_stream_internal(new, callflags, &newstr, &newinfo);
    if (rc < 0)
    {
    ...
    if (oldinfo.parentinfo.fatfile.volume !=
        newinfo.parentinfo.fatfile.volume)
    {
    ...
    const char * const oldname = STRDUP_ALLOCA(oldinfo.name, oldinfo.length);
    const char * const newname = STRDUP_ALLOCA(newinfo.name, newinfo.length);
    ...
    if (rc > 0)
    {
        ...
        is_overwrite = !fat_file_is_same(&oldstr.infop->fatfile,
                                         &newstr.infop->fatfile);
        if (is_overwrite)
        {
            if (are_dirs)
            {
    ...
    if (rc < 0)
    {
        ...
        FILE_ERROR(rc == FAT_RC_ENOSPC ? ENOSPC : EIO, rc * 10 - 8);
    }
...
int fsamefile(int fildes1, int fildes2)
{
    ...
    struct filestr_desc * const file2 = GET_FILESTR(WRITER, fildes2);
    ...
    /* works even if removed */
    rc = file1->stream.bindp == file2->stream.bindp ? 1 : 0;

file_error:
    if (file2)
        RELEASE_FILESTR(WRITER, file2);
    if (file1)
...
#if 0
/* test if two paths refer to the same file */
int samefile(const char *path1, const char *path2)
{
    ...
    struct filestr_base file1, file2;
    int rc1 = open_stream_internal(path1, FF_ANYTYPE, &file1, NULL);
    int rc2 = open_stream_internal(path2, FF_ANYTYPE, &file2, NULL);

    if (rc1 > 0 && rc2 > 0)
        rc = file1.stream.bindp == file2.stream.bindp ? 1 : 0;

    close_stream_internal(&file1);
    close_stream_internal(&file2);
