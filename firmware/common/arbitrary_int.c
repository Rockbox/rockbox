...
char * format_arbitrary_int10(struct arbitrary_int *a,
                              char *p_end)
{
    /*
...
char * format_arbitrary_frac10(struct arbitrary_int *a,
                               char *p_start,
                               long precision)
{
    /*
    ...
    if (trimlz) {
        /* trim leading zeros and provide <precision> digits; a->len
           will end up greater than the specified precision */
        precision = -precision;
    }
    ...
    while (precision > 0) {
        ...
        for (long i = bottomchunk; i < numchunks; i++) {
            uint64_t p_part = (uint64_t)chunks[i] * 1000000000 + c_part;
            c_part = p_part >> 32;
            chunks[i] = p_part;
        }
