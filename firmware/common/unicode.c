...
static int move_callback(int handle, void *current, void *new)
{
    /* we don't keep a pointer but we have to stop it if this applies to a
       ...
       call */
    return (handle != default_cp_handle || cp_table_ref) ?
    ...
    while (1) {
        if (tid == default_cp_tid) {
            ...
            if (default_cp_handle > 0)
                cp_table = core_get_data(default_cp_handle);
...
void set_codepage(int cp)
{
    ...
    while (1) {
        ...
        if (handle >= 0 && (cp_table_ref & ~CP_LOADING) == 0) {
