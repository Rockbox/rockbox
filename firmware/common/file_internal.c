...
/* Functionality to get moved to global storage thread; this can't properly
   coordinate with storage notifys and forced sleep (after which a flush is
   needed before sleeping the drive) */
unsigned int __vol_active IBSS_ATTR;
static long flusher_stack[DEFAULT_STACK_SIZE / sizeof (long) / 2];
static NORETURN_ATTR void flusher_thread(void)
{
...
void filesystem_init(void)
{
    ...
    iocache_init();
    fileobj_mgr_init();
    create_thread(flusher_thread, flusher_stack, sizeof (flusher_stack), 0,
                  "flusher tmp" IF_PRIO(, PRIORITY_BACKGROUND+1) IF_COP(,CPU));
}
