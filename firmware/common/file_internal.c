...
struct mutex file_internal_aux_mtx SHAREDBSS_ATTR;
                              ...
                              unsigned int callflags)
{
    return fat_readdir(&stream->fatstr, &infop->fatfile.e,
                       filestr_get_cache(stream), fatent, callflags);
}
...
/* check if the directory is empty (ie. only "." and/or ".." entries
   exist at most); doesn't rely on directory cache for this information */
int test_dir_empty_internal(struct filestr_base *stream)
{
    ...
    uncached_rewinddir_internal(&info);

    /* there is no need to potentially load codepages for this test, thus
       "FF_NOISO" */
    while ((rc = uncached_readdir_internal(stream, &info, &dir_fatent,
                                           FF_NOISO)) > 0)
    {
...
/* fill all the fields of the dirinfo structure from a fat_direntry */
void fat_direntry_dirinfo(struct dirinfo *dirinfo,
                          const struct fat_direntry *fatent)
{
    dirinfo->attribute = fatent->attr;
    dirinfo->size      = fatent->filesize;
    dirinfo->wrtdate   = fatent->wrtdate;
    dirinfo->wrttime   = fatent->wrttime;
}

void dir_fatent_dirinfo(struct dirinfo *dirinfo)
{
    fat_direntry_dirinfo(dirinfo, &dir_fatent);
}
                                         ...
                                         struct pathwalk_component *compp)
{
    ...
    char * compname = STRDUP_ALLOCA(compp->name, compp->length);
    ...
    while ((rc = readdir_internal(stream, &compp->info,
                                  &dir_fatent, callflags)) > 0)
    {
                       ...
                       struct pathwalk_component *parentp)
{
    ...
    rc = parse_path_component(&walkp->path, &compp->name);
    if (rc <= 0)
        return rc;
    else if (rc >= MAX_PATH)
                         ...
                         struct path_component_info *compinfo)
{
    ...
    do
    {
        if (rc == RC_GO_UP)
        {
            ...
            const char *p;
            volume = strip_volume(pathptr, &p, false);
            if (!CHECK_VOL(volume))
            {
...
int create_stream_internal(struct file_base_info *parentinfop,
                           ...
                           struct filestr_base *stream)
{
    /* assumes a attempt was made beforehand to open *stream with
       open_stream_internal() which returned zero (successfully not found),
       so does not initialize it here */
    const char * const name = STRDUP_ALLOCA(basename, length);
    DEBUGF("Creating \"%s\"\n", name);
...
void file_internal_init(void)
{
    mrsw_init(&file_internal_mrsw);
    mutex_init(&file_internal_aux_mtx);
    dc_init();
    fileobj_mgr_init();
}
