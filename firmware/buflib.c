...
buffer_alloc:
    ...
    BDEBUGF(stderr,
            "buflib_alloc_ex: size=%d handle=%p clb=%p crc=0x%0x name=\"%s\"\n",
            size, (void *)handle, (void *)ops, crc_slot->crc, block[3].name);
...
void buflib_check_valid(struct buflib_context *ctx)
{
                           ...
                           this += abs(this->val))
    {
        ...
        crc_slot = (union buflib_data*)((union buflib_data*)this[1].handle)->alloc - 1;
