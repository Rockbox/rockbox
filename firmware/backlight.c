...
void buttonlight_on_ignore_next(bool value)
{
...
void backlight_thread(void)
{
    ...
    || (CONFIG_BACKLIGHT_FADING == BACKLIGHT_FADING_SW_HW_REG)
        ...
        {   /* These events are only processed if backlight isn't locked */
            ...
            case EVENT_IGNORED:/* Allow timer to be reset if event is ignored */                
                break;
...
void backlight_on(void)
{
    ...
    else /* allow calls to backlight to still reset timeout */
    {
        queue_remove_from_head(&backlight_queue, EVENT_IGNORED);
        queue_post(&backlight_queue, EVENT_IGNORED, 0);
    }
}

void backlight_on_ignore_next(bool value)
{
