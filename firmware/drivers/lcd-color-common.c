...
struct viewport* lcd_current_viewport IDATA_ATTR = &default_vp;
...
static fb_data *lcd_frameaddress_default(int x, int y)
{
    ...
    element = ((x) * lcd_current_viewport->stride) + (y);
#else
    element = ((y) * lcd_current_viewport->stride) + (x);
#endif
    return lcd_current_viewport->buffer->data + (element % lcd_current_viewport->buffer->elems);
}
...
void lcd_set_backdrop(fb_data* backdrop)
{
    ...
    if (backdrop)
    {
        lcd_backdrop_offset = (intptr_t)backdrop - (intptr_t)lcd_current_viewport->buffer->data;
