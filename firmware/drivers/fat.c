                                 ...
                                 long cluster)
{
    ...
    {
        unsigned long c = (unsigned long)cluster - 2;

        if (c >= bpb->dataclusters)
        {
...
static void fat_recalc_free(struct bpb *bpb)
{
    ...
    for (unsigned long c = 2; c < bpb->dataclusters + 2; c++)
    {
        unsigned long offset = isfat16 ? (c % CLUSTERS_PER_FAT16_SECTOR) :
                                         (c % CLUSTERS_PER_FAT32_SECTOR);
        if (!offset && sec)
        {
            iobuf_release(&iob);
            sec = NULL;
...
static long alloc_cluster16(struct bpb *bpb,
                            ...
                            struct iobuf_handle *iobp)
{
    unsigned long cstart = startcluster;

    if (cstart - 2 >= bpb->dataclusters)
        cstart = 2;

    unsigned long climit = bpb->dataclusters + 2;
    ...
    while (1)
    {
        ...
        for (unsigned long c = cstart; c < climit; c++)
        {
            unsigned long offset = c % CLUSTERS_PER_FAT16_SECTOR;
            if (!offset && sec)
            {
                iobuf_release(iobp);
                sec = NULL;
                              ...
                              struct iobuf_handle *iobp)
{
    ...
    if (entry - 2 >= bpb->dataclusters)
        panicf("FAT16 range:%lu", entry);
    ...
    unsigned long newval_le = htole16((uint16_t)val);

    if (!curval_le)
    {
        /* only alloc_cluster may write a free entry */
                            ...
                            struct iobuf_handle *iobp)
{
    ...
    while (1)
    {
        ...
        for (unsigned long c = cstart; c < climit; c++)
        {
            unsigned long offset = c % CLUSTERS_PER_FAT32_SECTOR;
            if (!offset && sec)
            {
                iobuf_release(iobp);
                sec = NULL;
            ...
            unsigned long top4_le = htole32(0xf0000000);
            unsigned long curval_le = sec[offset];

            if (curval_le & ~top4_le)
                continue;

            top4_le &= curval_le;

            unsigned long newval_le = top4_le | htole32(FAT32_EOF_MARK);
                              ...
                              struct iobuf_handle *iobp)
{
    ...
    unsigned long top4_le = htole32(0xf0000000);
    unsigned long curval_le = sec[offset];
    unsigned long newval_le = htole32((uint32_t)val) & ~top4_le;
                     ...
                     bool write)
{
    ...
    if (write)
    {
        if (start < bpb->firstallowed)
            panicf("Write %lu %c data", bpb->firstallowed - start, '<');

        if (start + count > bpb->totsec)
            panicf("Write %lu %c data", start + count - bpb->totsec, '>');
    }
                          ...
                          unsigned int flags)
{
    ...
    for (unsigned int i = 0; i < longentries; i++)
    {
        ...
        {
            panicf("Dir entry %d in sector %lu is not free! "
                   "%02x%02x%02x%02x",
                   i + firstentry, dirstr->lastsector,
