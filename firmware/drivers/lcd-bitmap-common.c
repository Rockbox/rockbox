...
{
#if LCDM(DEPTH) == 2 && LCDM(PIXELFORMAT) != HORIZONTAL_PACKING /*HACK*/
    ...
    if (bm->format != FORMAT_MONO)
        LCDFN(bitmap_part)((FBFN(data)*)(bm->data),
            src_x, src_y, LCDM(FBSTRIDE)(bm->width, bm->height), x, y, width, height);
    else
#endif
        LCDFN(mono_bitmap_part)(bm->data,
            src_x, src_y, bm->width, x, y, width, height);
}
