...
struct viewport* LCDFN(init_viewport)(struct viewport* vp)
{
    ...
    {
        if (!vp->buffer->stride)
            vp->buffer->stride = THIS_STRIDE(LCDM(WIDTH), LCDM(HEIGHT));
...
void LCDFN(set_viewport)(struct viewport* vp)
{
...
#if LCDM(DEPTH) > 1
    LCDFN(set_foreground)(LCDFN(current_viewport)->fg_pattern);
    LCDFN(set_background)(LCDFN(current_viewport)->bg_pattern);
#endif
        ...
        || vp->y + vp->height > LCDM(HEIGHT))
    {
        ...
        DEBUGF("NOTE: "
            ...
            LCDFN(current_viewport)->x, LCDFN(current_viewport)->y,
            LCDFN(current_viewport)->width, LCDFN(current_viewport)->height);
    }
#endif
...
{
    LCDFN(update_rect)(LCDFN(current_viewport)->x, LCDFN(current_viewport)->y,
                    LCDFN(current_viewport)->width, LCDFN(current_viewport)->height);
}

void LCDFN(update_viewport_rect)(int x, int y, int width, int height)
{
    LCDFN(update_rect)(LCDFN(current_viewport)->x + x, LCDFN(current_viewport)->y + y, width, height);
}
