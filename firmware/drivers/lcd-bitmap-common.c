...
{
    LCDFN(drawrect)(0, 0, lcd_current_viewport->width, lcd_current_viewport->height);
}

/*
 * fills the rectangle formed by lcd_current_viewport
 **/
void LCDFN(fill_viewport)(void)
{
    LCDFN(fillrect)(0, 0, lcd_current_viewport->width, lcd_current_viewport->height);
}
...
void LCDFN(set_viewport)(struct viewport* vp)
{
    ...
    {
        if (!vp->stride) vp->stride = STRIDE_MAIN(LCDM(WIDTH), LCDM(HEIGHT));
        ...
        lcd_current_viewport = vp;
    }


#if LCDM(DEPTH) > 1
    LCDFN(set_foreground)(lcd_current_viewport->fg_pattern);
    LCDFN(set_background)(lcd_current_viewport->bg_pattern);
#endif
     ...
     *  expected.
     */
    if((unsigned) lcd_current_viewport->x > (unsigned) LCDM(WIDTH)
        || (unsigned) lcd_current_viewport->y > (unsigned) LCDM(HEIGHT)
        || lcd_current_viewport->x + lcd_current_viewport->width > LCDM(WIDTH)
        || lcd_current_viewport->y + lcd_current_viewport->height > LCDM(HEIGHT))
    {
        ...
        DEBUGF("NOTE: "
            ...
            lcd_current_viewport->x, lcd_current_viewport->y,
            lcd_current_viewport->width, lcd_current_viewport->height);
    }
#endif
}
...
{
    *is_default = (lcd_current_viewport == &default_vp);
    return lcd_current_viewport;
}

void LCDFN(update_viewport)(void)
{
    LCDFN(update_rect)(lcd_current_viewport->x, lcd_current_viewport->y,
                    lcd_current_viewport->width, lcd_current_viewport->height);
}

void LCDFN(update_viewport_rect)(int x, int y, int width, int height)
{
    LCDFN(update_rect)(lcd_current_viewport->x + x, lcd_current_viewport->y + y, width, height);
}
...
{
    unsigned short *ucs;
    font_lock(lcd_current_viewport->font, true);
    struct font* pf = font_get(lcd_current_viewport->font);
    int vp_flags = lcd_current_viewport->flags;
    ...
    if ((vp_flags & VP_FLAG_ALIGNMENT_MASK) != 0)
    {
        ...
        {
            x = ((lcd_current_viewport->width - w)/ 2) + x;
            if (x < 0)
                x = 0;
        }
        else
        {
            x = lcd_current_viewport->width - w - x;
            x += ofs;
            ofs = 0;
        }
    }
    ...
    for (ucs = bidi_l2v(str, 1); *ucs; ucs++)
    {
        ...
        if (x >= lcd_current_viewport->width)
            break;
        ...
        if (is_diac)
        {
             ...
             **/
            drawmode = lcd_current_viewport->drawmode;
            lcd_current_viewport->drawmode = DRMODE_FG;
        ...
        if (is_diac)
        {
            lcd_current_viewport->drawmode = drawmode;
        }
    ...
    font_lock(lcd_current_viewport->font, false);
}
...
static struct scrollinfo* find_scrolling_line(int x, int y)
{
    ...
    for(i=0; i<LCDFN(scroll_info).lines; i++)
    {
        ...
        if (s->x == x && s->y == y && s->vp == lcd_current_viewport)
            return s;
    }
    return NULL;
}
                                     ...
                                     void *data)
{
    ...
    cwidth = font_get(lcd_current_viewport->font)->maxwidth;
    height = font_get(lcd_current_viewport->font)->height;
    y = y * (linebased ? height : 1);
    x = x * (linebased ? cwidth : 1);
    width = lcd_current_viewport->width - x;

    if (y >= lcd_current_viewport->height)
        return false;
     ...
     * cleared as well */
    if (w < width || restart) {
        LCDFN(scroll_stop_viewport_rect)(lcd_current_viewport, x, y, width, height);
    ...
    if ( LCDFN(scroll_info).bidir_limit ) {
        s->bidir = w < (lcd_current_viewport->width) *
