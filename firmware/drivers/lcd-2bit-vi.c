...
struct frame_buffer_t LCDFN(framebuffer_default) =
{
    ...
    .elems          = (sizeof(FBFN(data))*LCDM(FBWIDTH)*LCDM(FBHEIGHT)),
};
...
struct viewport * LCDFN(current_viewport) IBSS_ATTR;
...
#endif
    return fb->data + (element % fb->elems);
}
...
void LCDFN(fillrect)(int x, int y, int width, int height)
{
    ...
    {
        if ((lcd_current_viewport->drawmode & DRMODE_BG) && !backdrop)
        {
            fillopt = true;
            bits = bg_pattern;
        }
