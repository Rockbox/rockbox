...
void LCDFN(clear_viewport)(void)
{
    ...
    {
        lastmode = LCDFN(current_viewport)->drawmode;

        /* Invert the INVERSEVID bit and set basic mode to SOLID */
        LCDFN(current_viewport)->drawmode = (~lastmode & DRMODE_INVERSEVID) | 
                               DRMODE_SOLID;

        LCDFN(fillrect)(0, 0, LCDFN(current_viewport)->width, LCDFN(current_viewport)->height);

        LCDFN(current_viewport)->drawmode = lastmode;

        LCDFN(scroll_stop_viewport)(LCDFN(current_viewport));
    }
    LCDFN(current_viewport)->flags &= ~(VP_FLAG_VP_SET_CLEAN);
}
...
void LCDFN(vline)(int x, int y1, int y2)
{
    ...
    bfunc = LCDFN(blockfuncs)[LCDFN(current_viewport)->drawmode];
    dst   = LCDFB(x,y1>>3);
    stride_dst = LCDFN(current_viewport)->buffer->stride;
    ny    = y2 - (y1 & ~7);
...
void LCDFN(fillrect)(int x, int y, int width, int height)
{
    ...
    {
        if ((LCDFN(current_viewport)->drawmode & DRMODE_BG) && !backdrop)
        {
            fillopt = true;
            bits = bg_pattern;
        }
