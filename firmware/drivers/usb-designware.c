 ...
 */
#if CONFIG_CPU == X1000
# define USB_DW_DMA_ADDR2PHYS(x) PHYSADDR(x)
#else
# define USB_DW_DMA_ADDR2PHYS(x) x
#endif
...
#else
            (void) bytesinfifo;
            DWC_DIEPDMA(ep) = USB_DW_DMA_ADDR2PHYS((uint32_t)(dw_ep->addr) + sentbytes);
#endif
...
#ifndef USB_DW_ARCH_SLAVE
    DWC_EPDMA(epnum, epdir) = USB_DW_DMA_ADDR2PHYS((uint32_t)buf);
#endif
