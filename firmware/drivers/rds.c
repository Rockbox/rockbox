...
static long ps_text_timeout;    /* timeout to discard message */
...
void rds_sync(void)
{
    ...
    if (rds_active) {
        if (TIMED_OUT(ps_text_timeout)) {
            ps_copy[0] = '\0';
        }
        if (TIMED_OUT(rt_text_timeout)) {
            rt_copy[0] = '\0';
        }
...
static void handle_group0(const uint16_t data[4])
{
    ...
    if (memcmp(ps, PS_DATA_INC(1), 8) == 0) {
        memcpy(ps_copy, ps, 9);
        SET_TIMEOUT(ps_text_timeout, TEXT_TIMEOUT);
    }
}
...
static void handle_group2(const uint16_t data[4])
{
    ...
    if (memcmp(rt, RT_DATA_INC(1), pos) == 0) {
        ...
        SET_TIMEOUT(rt_text_timeout, TEXT_TIMEOUT);
    }
}
...
void rds_process(const uint16_t data[4])
{
    ...
    rds_timeout = current_tick + RDS_TIMEOUT;
    rds_active  = true;
}


/* call rds_sync() before calling these directly */
...
{
    int oldlevel = rds_disable_irq_save();
    size_t size = copy_rds_string(dst, ps_copy, dstsize);
    rds_restore_irq(oldlevel);
    return size;
...
{
    int oldlevel = rds_disable_irq_save();
    size_t size = copy_rds_string(dst, rt_copy, dstsize);
    rds_restore_irq(oldlevel);
    return size;
}
