...
static void alsa_init_controls(void)
{
    ...
    /* allocate list and info on stack so there is no need to free them */
...
void audiohw_preinit(void)
{
    ...
    alsa_set_control_bool("CODEC Acoustic Switch", true);
...
{
    long vol[2];
    /* the driver expects percent, convert from centibel in range 0..100 */
    vol[0] = vol_l / 10;
    vol[1] = vol_r / 10;
    printf("set vol %d,%d\n", vol[0], vol[1]);
    alsa_set_control_ints("Playback Volume", 2, vol);
}
