...
static void audio_get_alc_data(struct nwz_audio_alc_data_ex_t *data)
{
    if(ioctl(fd_hw, NWZ_AUDIO_GET_ALC_DATA_EX, data) < 0)
        panicf("ioctl(NWZ_AUDIO_GET_ALC_DATA_EX) failed");
}
...
void audiohw_preinit(void)
{
    ...
    /* init hw */
    hw_open();
    /* get sound curve for HP */
    struct nwz_audio_alc_data_ex_t alc_data;
    alc_data.type = NWZ_ALC_TYPE_HP_ARM;
    audio_get_alc_data(&alc_data);
    printf("ALC data (HP ARM):\n ");
    for(int i = 0; i < NEW_AUDIO_ALC_DATA_NUM; i++)
        printf(" %d", alc_data.table[i]);
    printf("\n");
}
...
void audiohw_set_volume(int vol_l, int vol_r)
{
    ...
    vol[0] = vol_l / 10;
    vol[1] = vol_r / 10;
    /* value is in range -30..0 so fix it */
    vol[0] += 30;
    vol[1] += 30;
