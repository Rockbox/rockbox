...
static void setpixel(int x, int y)
{
    ...
    fb_data *address = LCD_ADDR(x,y>>2);
...
void lcd_clear_display(void)
{
    ...
    {
        memset(lcd_framebuffer, fg_pattern, sizeof lcd_static_framebuffer);
    }
    else
    {
        if (lcd_backdrop)
            memcpy(lcd_framebuffer, lcd_backdrop, sizeof lcd_static_framebuffer);
        else
            memset(lcd_framebuffer, bg_pattern, sizeof lcd_static_framebuffer);
    }
