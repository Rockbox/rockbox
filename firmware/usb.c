...
{
    logf("usb_signal_transfer_completion %ld, req=0x%x",
         current_tick,
         ((struct usb_ctrlrequest *)(event_data->data))->bRequest);
...
static void NORETURN_ATTR usb_thread(void)
{
    ...
    while(1)
    {
        ...
        case USB_TRANSFER_COMPLETION:
            ...
            logf("usb_thread() %ld, req=0x%x",
                 current_tick,
                 ((struct usb_ctrlrequest *)(((struct usb_transfer_completion_event_data*)ev.data)->data))->bRequest);
