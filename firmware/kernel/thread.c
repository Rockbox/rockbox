...
static struct thread_entry * lock_blocker_thread(struct blocker *bl)
{
    ...
    while (1)
    {
        ...
        if (TRY_LOCK_THREAD(t)) /* TRY, or else deadlocks are possible */
        {
            if (bl->thread == t)
                return t;
...
static void wakeup_thread_queue_multi_transfer(struct thread_entry *thread)
{
    ...
    struct thread_entry *t = *bqp, *tfirst = t;
    int priority = PRIORITY_IDLE;

    if (blt->retval == 1)
    {
        /* Find highest priority of threads that will remain; each woken thread
           must inherit it */
        do
        {
            if (t->retval == 0)
    ...
    /* Get rid of what we had and finalize the ownership transfer */
    priority_disinherit_internal(blt, blsplay->blocker.priority);
    blsplay->blocker.thread = count == 1 ? thread : NULL;
    blsplay->blocker.priority = priority;
    UNLOCK_THREAD(blt);
...
{
#ifdef HAVE_PRIORITY_SCHEDULING
    static void (*funcs[])(struct thread_entry *thread) IDATA_ATTR =
    {
        [WAKEUP_DEFAULT]        = NULL,
