...
/* If a thread has a blocker but the blocker thread is NULL, then it
   references this and the struct blocker may be reinterpreted as such.
   Use a small type for .tcounts to save RAM, allowing 255 claims for a
   thread. If it needs more then you're doing it wrong. */
struct blocker_splay
{
    struct blocker  blocker;             /* blocker info (first!) */
#ifdef HAVE_PRIORITY_SCHEDULING
    uint8_t         tcounts[MAXTHREADS]; /* per-thread lock counts */
...
#ifdef HAVE_PRIORITY_SCHEDULING
#define wakeup_thread(list, proto)  wakeup_thread_((list), (proto))
int thread_set_priority(unsigned int thread_id, int priority);
int thread_get_priority(unsigned int thread_id);
#else /* !HAVE_PRIORITY_SCHEDULING */
#define wakeup_thread(list, proto)  wakeup_thread_((list))
#endif /* HAVE_PRIORITY_SCHEDULING */
