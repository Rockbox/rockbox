...
{
  switch (setting) {
  case SLEEPTIMER_STOP:     set_sleep_timer(0); break;                    /* off */
  case SLEEPTIMER_START:    set_sleep_timer(sleeptimer_duration);break;   /* on (if sleeptimer_duration > 0) */
  case SLEEPTIMER_CONTINUE: set_sleep_timer(get_sleep_timer());break;     /* continue (if runnint sleeptimer > 0) */
  }
}
