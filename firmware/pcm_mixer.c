...
#define PCM_STR_DESC_ADDR_TX_OFFS (offsetof (struct pcm_stream_desc, addr_tx))
#define PCM_STR_DESC_AMP_TX_OFFS  (offsetof (struct pcm_stream_desc, amplitude_tx))

/* Packed pointer array of all not-stopped streams (+ NULL) */
...
#define CONV_DBE(_size, _bits, _chnum, _gain, _write, _mix) \
    { .size = (_size), .bits = (_bits), .chnum = (_chnum), .gain = (_gain), \
...
int pcm_mixer_format_config(struct pcm_stream_desc *desc)
{
    static const struct
    {
        ...
        unsigned long bits  : 8; /* 0=match any */
        unsigned long chnum : 2;
        unsigned long gain  : 1; /* 0=unity, 1=gain */
        struct pcm_dma_t_convert conv;
    } conv_db[] =
    {
#ifdef CONFIG_PCM_MULTISIZE
#if (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_2_BYTE_INT)
        CONV_DBE(2, 0, 1, 0, write_samples_unity_1ch_s2i_sdi,
                             mix_samples_unity_1ch_s2i_sdi),
        CONV_DBE(2, 0, 1, 1, write_samples_gain_1ch_s2i_sdi,
                             mix_samples_gain_1ch_s2i_sdi),
        CONV_DBE(2, 0, 2, 0, write_samples_unity_2ch_s2i_sdi,
                             mix_samples_unity_2ch_s2i_sdi),
        CONV_DBE(2, 0, 2, 1, write_samples_gain_2ch_s2i_sdi,
                             mix_samples_gain_2ch_s2i_sdi),
#endif /* (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_2_BYTE_INT) */
#if (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_3_BYTE_INT)
        CONV_DBE(3, 0, 1, 0, write_samples_unity_1ch_s3i_sdi,
                             mix_samples_unity_1ch_s3i_sdi),
        CONV_DBE(3, 0, 1, 1, write_samples_gain_1ch_s3i_sdi,
                             mix_samples_gain_1ch_s3i_sdi),
        CONV_DBE(3, 0, 2, 0, write_samples_unity_2ch_s3i_sdi,
                             mix_samples_unity_2ch_s3i_sdi),
        CONV_DBE(3, 0, 2, 1, write_samples_gain_2ch_s3i_sdi,
                             mix_samples_gain_2ch_s3i_di),
#endif /* (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_3_BYTE_INT) */
#if (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_4_BYTE_INT)
        CONV_DBE(4, 0, 1, 0, write_samples_unity_1ch_s4i_sdi,
                             mix_samples_unity_1ch_s4i_sdi),
        CONV_DBE(4, 0, 1, 1, write_samples_gain_1ch_s4i_sdi,
                             mix_samples_gain_1ch_s4i_sdi),
        CONV_DBE(4, 0, 2, 0, write_samples_unity_2ch_s4i_sdi,
                             mix_samples_unity_2ch_s4i_sdi),
        CONV_DBE(4, 0, 2, 1, write_samples_gain_2ch_s4i_sdi,
                             mix_samples_gain_2ch_s4i_sdi),
#endif /*  (CONFIG_PCM_FORMAT_CAPS & PCM_FORMAT_CAP_4_BYTE_INT) */
#else /* ndef CONFIG_PCM_MULTISIZE */
        CONV_DBE(2, 16, 1, 0, write_samples_unity_1ch_s16i_s16i,
                              mix_samples_unity_1ch_s16i_s16i),
        CONV_DBE(2, 16, 1, 1, write_samples_gain_1ch_s16i_s16i,
                              mix_samples_gain_1ch_s16i_s16i),
        CONV_DBE(2, 16, 2, 0, write_samples_unity_2ch_s16i_s16i,
                              mix_samples_unity_2ch_s16i_s16i),
        CONV_DBE(2, 16, 2, 1, write_samples_gain_2ch_s16i_s16i,
                              mix_samples_gain_2ch_s16i_s16i),
#endif /* CONFIG_PCM_MULTISIZE */
    };

    size_t size = pcm_desc_sample_size(desc);
    unsigned int bits = pcm_desc_sample_bits(desc);
    unsigned int chnum = desc->chnum;
    unsigned int gain  = desc->amplitude_tx != PCM_AMP_UNITY;

    if (size * 8 < bits) {
        return -1; /* nonsense format */
    }
...
static void stop_streams(int status, struct pcm_stream_desc *desc_except)
{
    ...
    while ((desc = *active)) {
        ...
        if (status != 0 && desc->callback) {
            desc->addr_tx = NULL;
            desc->frames = 0;
            desc->callback_tx(status, &desc->addr_tx, &desc->frames);
        }
...
fill_buffer:
    ...
    while (desc) {
        ...
        if (desc->frames) {
            ...
            if (desc->callback) {
                desc->addr_tx = NULL;
                st = desc->callback_tx(status, &desc->addr_tx, &desc->frames);
            }

            if (st != 0 || !(desc->addr_tx && desc->frames)) {
                stream_stopped(desc);
                desc = *desc_p;
                continue;
            }
