...
static uint32_t downmix_buf[NUM_BUFS][MAX_MIX_FRAME_SAMPLES] DOWNMIX_BUF_IBSS MEM_ALIGN_ATTR;
...
static void mixer_pcm_callback(const void **addr, size_t *size)
{
    ...
    *size = next_size;
    ...
    logf("m cb %d/%d %x %d", downmix_index_r, downmix_index_w, (int) *addr, *size);
...
static void mixer_start_pcm(void)
{
    ...
    logf("start %x, %d", start, mix_frame_size);
