...
#include "crc32.h"
#endif
>>>>>>> Multiboot Firmware Root Redirect - Do Not Commit
...
=======
#if defined(HAVE_MULTIBOOT) && !defined(SIMULATOR)
    static char rootpath[VOL_MAX_LEN+2] = RB_ROOT_CONTENTS_DIR;
    static bool redirected = false;
    int boot_volume = 0;
    unsigned int crc = 0;

        root_mount_path(path, NSITEM_MOUNTED);
        ...
        if (crc == boot_data.crc)
        {
            ...
            root_mount_path(path, volume == boot_volume ? NSITEM_HIDDEN :
                                                          NSITEM_MOUNTED);
            if (volume == boot_volume && !redirected)
            {
                if (get_redirect_dir(rootpath, sizeof(rootpath),
                                                    boot_volume, "", "") > 0)
                {
                    rootpath[path_strip_trailing_separators(rootpath,NULL)] = '\0';
                }
                else /* Error occurred, card removed? Set root to default */
                {
                    root_mount_path(RB_ROOT_CONTENTS_DIR, NSITEM_CONTENTS);
                    boot_volume = path_strip_volume(RB_ROOT_CONTENTS_DIR,
                                                                    NULL, false);
                }
            }
        } /*CRC OK*/
        ...
        {
            redirected = true;
            root_mount_path(rootpath, NSITEM_CONTENTS);
        }
