...
#ifdef HAVE_DIRCACHE
    struct dirinfo di;
    dir_fatent_dirinfo(&di);
    dircache_fileop_create(dirinfop, stream->bindp, basename, &di);
#endif
...
static inline void fileop_onsync_internal(struct filestr_base *stream)
{
    ...
    struct dirinfo di;
    dir_fatent_dirinfo(&di);
    dircache_fileop_sync(stream->bindp, &di);
#endif
}
...
#else
    uncached_rewinddir_dirent(scnap);
#endif
}
...
{
#ifdef HAVE_DIRCACHE
    return dircache_readdir_internal(stream, infop, fatent, callflags);
#else
