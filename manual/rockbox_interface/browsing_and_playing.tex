...
\opt{kbd_point_mode}{
  On this player the keyboard is laid out for direct touch where the keys are sized
  for a fingertip and are tapped individually, rather than being selected
  with the 3\,\texttimes\,3 grid used on other touchscreen targets. By default the
  characters are spread over three pages with lower case and accented
  letters, their upper-case forms in the same positions, and a third page
  of punctuation and additional European characters. Tap
  \textbf{\textgreater} below the keys to change page.
}
