...
\section{Sample Delay}
  Sample delay enables an offset delay to one of the channels. This setting helps to move
  the stereo image towards to the front of the driver in a car audio
  envirement where driver's seat is mostlly not in the center.
  All values below 0 will delay the left channel, thus make left speaker farther away
  from the actual position. Follow the same principle that positive
  values will move the right speaker away from driver, therefore benefits those
  where driver's seat is on the right side.
