...
practice. It knows two modes of operation, depending on it being
started from the plugin menu or as viewer for tempomap (\verb:.tempo:)
files.

The sound is a nicely annoying square wave that shall pierce through
the other noise around you. In addition, the display also indicates
the beats while playing so that you can discretely place the device
in your sight and don't need headphones or have to annoy band /
audience members.

\subsubsection{Simple Interactive Mode}

This is the classic mode of operation that is active when starting the
plugin directly from the menu. It offers an uniform metronme sound at
constant tempo.
