...
\note{You can hide the options to shuffle tracks or to queue tracks, if you wish.
The latter can also be put into its own submenu.  Simply go to
\setting{Settings} $\rightarrow$ \setting{General Settings} $\rightarrow$ \setting{Playlist}
$\rightarrow$ \setting{Current Playlist} and select either \setting{Shuffled Adding} or
\setting{Queuing} to customize the displayed set of options.}
