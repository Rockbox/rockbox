...
\opt{disk_storage}{
Rockbox supports very large drive capacities when used with GPT partitioning on devices that utilize ATA storage, but due to the limitations of the FAT32 filesystem, individual paritions cannot exceed 2\,TiB. Additionally, there may be underlying platform limitations that make it difficult, if not impossible, utilize a drive larger than some arbitrary size or boot from a drive using GPT partitioning.
...
\note{In practice, no Rockbox-capable device can currently handle total drive capacities exceeding 2\,TiB.}
}
...
\item[Single Secure Digital (SD).] While these adapters come in different form factors from multiple vendors,
  ...
  do not support 2\,TiB or larger SDUC cards.
