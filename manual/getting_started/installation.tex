...
\opt{ipodnano,ipodnano2g,ipodnano3g,ipodvideo,ipod6g,e200,c200,c200v2,e200v2,clipv1,clipv2,cowond2,fuze,fuzev2}{
\begin{description}
\item[Supported hardware versions.]
    \opt{ipodnano,ipodnano2g,ipodnano3g}{
    The \playertype{} is available in multiple versions, not
    all of which run Rockbox.  Rockbox presently runs on the first,
    second and third generation iPod Nano. Rockbox does
    \emph{not} run on the fourth, fifth, sixth or seventh generation
    iPod Nano.
  ...
  \opt{ipodnano3g}{
    The \playertype{} referred to by this manual is the third generation
    model, the short and wide one with a \dapdisplaywidth{}x%
    \dapdisplayheight{} screen. It came with 4GB and 8GB of flash storage.
    Apple fitted several different NAND chips, and Rockbox so far supports
    the models with Hynix NAND, 4GB and 8GB: on any other chip the
    bootloader says so and starts Apple's firmware instead. The
    installation section below says how to have your player's chip added.
  }
  \opt{ipodvideo}{
...
\opt{disk_storage}{
Rockbox supports very large drive (over 2\,TiB) capacities when used with GPT partitioning on \dap{}s that utilize ATA storage, but due to the limitations of the FAT32 filesystem, individual paritions cannot exceed 2\,TiB. Additionally, there may be underlying platform limitations that make it difficult, if not impossible, utilize a drive larger than some arbitrary size or boot from a drive using GPT partitioning.

\opt{ipod6g}{
For example, the stock Apple firmware on earlier 6th generation iPod Classic models\footnote{MB029, MB145, MB147, MB150} is limited to drives no larger than 128\,GiB, and booting into the Apple firmware firmware with larger drive sizes will result in massive data corruption.  Later 6th generation iPod Classic models\footnote{MB562, MB565, MC293, MC297} (sometimes referred to as ``7th gen'') do not have this issue.  Rockbox itself will function correctly on all 6th/7th generation iPod Classic models.
}

\note{In practice, no Rockbox-capable \dap{} can currently handle total drive capacities exceeding 2\,TiB.}
}

\opt{sd_storage}{
  Rockbox does not currently support SDUC cards, i.e. those with capacities
  exceeding 2\,TiB.  However, this is purely a software limitation with
  Rockbox's SD controller drivers, and will be addressed in a future release.

  \note{SD cards exceeding 32\,GiB are pre-formated using the exFAT filesystem with GPT partitioning.
    Before they can be used with Rockbox, they must be reformatted with FAT32.}
}

\opt{disk_storage,sd_storage}{
\note{The disk format utility included with all versions Microsoft Windows released to date will refuse to create/format FAT32 partitons exceeding 32\,GiB, so users of larger storage devices will need an alternative tool. Options include GParted Live\footnote{\url{https://gparted.org/}}\opt{ipod}{ or performing a restore using iTunes on Windows}. \opt{sd_storage}{Additionally, devices that support removable storage typically have a built-in format tool that can be used.}}
}
...
It is also worth noting that data transfer rates are not likely to be improved when using solid-state storage; the underlying \dap{} hardware is simply too slow to materially benefit. However, the improved access times should help the \dap{} to feel much more responsive.
}
...
\opt{gigabeats,fiiom3k,shanlingq1,ipodnano3g}{
...
\optv{ipod}{
  ...
  \opt{ipodnano3g}{
    Follow the DFU-mode steps above, then run
    \fname{mks5lboot --bl-uninst ipodnano3g} to remove the bootloader.
  }
}
