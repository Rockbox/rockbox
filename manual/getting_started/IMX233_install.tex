...
\begin{itemize}
  \item An original firmware (OF) from \playerman's website:
    ...
    \opt{creativezenxfi3}{
      \url{http://support.creative.com/Products/ProductDetails.aspx%
      ?catID=948&CatName=Entertainment+Devices+\%2F+MP3+Players&prodID=20866%
      &prodName=ZEN+X-Fi3}}

  \item The bootloader:
    \opt{fuzeplus}{\download{bootloader/sandisk-sansa/fuzeplus/bootloader-fuzeplus.sansa}}
    \opt{creativezenxfi3}{\download{bootloader/creative/zenxfi3/bootloader-zenxfi3.creative}}
...
\item Run mkimxboot.exe, passing according to the following syntax the name
  ...
  \begin{code}[firstline=\opt{fuzeplus}{1}\opt{creativezenxfi3}{2},%
               lastline=\opt{fuzeplus}{1}\opt{creativezenxfi3}{2}]
    mkimxboot.exe -i firmware.sb -b bootloader-fuzeplus.sansa -o patched.sb -t dualboot
    mkimxboot.exe -i firmware.sb -b bootloader-zenxfi3.creative -o patched.sb -t dualboot
  \end{code}
  ...
  \item Copy the output file (``patched.sb'' in the example given above) to the
    ...
    \opt{fuzeplus,creativezenxfi3}{(``firmware.sb''}  above).
...
% See above for the code environment arguments explanation.
  \begin{code}[firstline=\opt{fuzeplus}{1}\opt{creativezenxfi3}{2},%
    ...
    ./mkimxboot -i firmware.sb -b bootloader-zenxfi3.creative -o patched.sb -t dualboot
  \end{code}
