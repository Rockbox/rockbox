...
static FILE * openinfile( const TCHAR * filename )
{
...
static FILE * openoutfile( const TCHAR * filename )
{
...
int iriver_decode(TCHAR *infile_name, TCHAR *outfile_name, unsigned int modify,
                  enum striptype stripmode )
{
    ...
    unsigned int minsize, maxsize, size1, size2;
           ...
           ( lenread = fread( blockdata+16, 1, 16, infile ) ) == 16 )
    {
        ...
        if( fp == 16 )
            size1 = (blockdata[31] <<  0) |
                    (blockdata[30] <<  8) |
                    (blockdata[29] << 16) |
                    (blockdata[28] << 24);
        else if( fp == 32 )
            size2 = (blockdata[31] <<  0) |
                    (blockdata[30] <<  8) |
                    (blockdata[29] << 16) |
                    (blockdata[28] << 24);
