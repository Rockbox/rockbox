...
do
    ...
    case $arch in
        ...
        [Aa])
            ...
            ;;
        [Rr])
            # gcc config, specifies the arm subtarget
            gccopts="--with-arch=armv6 --with-tune=arm1176jzf-s --with-fpu=vfp --with-float=softfp"
            # glibc config, see below
            glibcopts="--enable-kernel=2.6.24 --enable-oldest-abi=2.4"
            ...
            build_linux_toolchain "arm-ypr0-linux-gnueabi" "2.26.1" "" "4.9.4" \
                "$gccopts" "2.6.27.59" "2.19" "$glibcopts"
            ;;
        [Zz])
            # gcc config, specifies the arm subtarget
            gccopts="--with-arch=armv6 --with-tune=arm1176jzf-s --with-fpu=vfp --with-float=softfp"
            # glibc config, see below
            glibcopts="--enable-kernel=2.6.23 --enable-oldest-abi=2.7"
            # some notes about the tools
            # gcc: 4.9.4 is the latest 4.9.x stable branch, also the only one that
            #      compiles with GCC >6
            # kernel: 2.6.32.68 is the latest 2.6.x stable kernel
            ...
            build_linux_toolchain "arm-sony-linux-gnueabi" "2.26.1" "" "4.9.4" \
