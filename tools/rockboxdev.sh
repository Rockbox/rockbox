...
build() {
	...
	configure_params="--enable-languages=c --disable-libssp $configure_params"
        ...
        if [ "$system" == "Darwin" ] ; then
            # gcc-libcpp17: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=111632
            patch="$patch apple_silicon.patch apple_silicon-zlib.patch gcc-libcpp17.patch"
    ...
    if test -n "$needs_libs"; then
        ...
        if (echo $needs_libs | grep -q mpc && test ! -d mpc); then
            ...
            getfile "mpc-1.3.1.tar.gz" "http://www.multiprecision.org/downloads"
            tar xzf $dlwhere/mpc-1.3.1.tar.gz
            ln -s mpc-1.3.1 mpc
        fi

        if (echo $needs_libs | grep -q isl && test ! -d isl); then
            echo "ROCKBOXDEV: Getting ISL"
            getfile "isl-0.24.tar.bz2" "https://gcc.gnu.org/pub/gcc/infrastructure"
            tar xjf $dlwhere/isl-0.24.tar.bz2
            ln -s isl-0.24 isl
        fi
        cd $builddir
    fi
...
do
    ...
    case $arch in
        [Ii])
            build "binutils" "mipsel-elf" "2.40" "binutils-c23.patch" "--disable-werror" "gmp isl"
            build "gcc" "mipsel-elf" "10.5.0" "" "" "gmp mpfr mpc isl"
            ;;

        [Mm])
            build "binutils" "m68k-elf" "2.40" "" "--disable-werror" "gmp isl"
            build "gcc" "m68k-elf" "10.5.0" "" "--with-arch=cf MAKEINFO=missing" "gmp mpfr mpc isl"
            ;;
        ...
        [Aa])
            ...
            build "binutils" "arm-elf-eabi" "2.40" "" "$binopts --disable-werror" "gmp isl"
            build "gcc" "arm-elf-eabi" "10.5.0" "rockbox-multilibs-noexceptions-arm-elf-eabi-gcc-9.5.0.diff" "$gccopts MAKEINFO=missing" "gmp mpfr mpc isl"
            ;;
        [Xx])
            # IMPORTANT NOTE
	    ...
	    #
            ...
            build_linux_toolchain "arm-rockbox-linux-gnueabi" "2.40" "" "" "10.5.0" \
	    ...
	    #
            ...
            build_linux_toolchain "mipsel-rockbox-linux-gnu" "2.40" "" "binutils-c23.patch" "10.5.0" \
