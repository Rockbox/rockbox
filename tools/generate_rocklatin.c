...
#define BITMAP_WIDTH ((CHAR_WIDTH)*17)
#define BITMAP_HEIGHT ((CHAR_HEIGHT)*17)

#define SIZE 2

unsigned int bitmap[BITMAP_WIDTH][BITMAP_HEIGHT];

extern unsigned char extended_font_player[256][8];

extern unsigned char font_new_player[256][5];
extern unsigned char font_old_player[256][5];
...
unsigned short *rocklatin;

char show_subst=0;
char show_hardware=0;
char shadow_show_hardware=0;
...
void draw_extended_char(unsigned char chr, int x, int y)
{
  int x2, y2;
  unsigned char *data;
  short mapped_char;
  ...
  data=extended_font_player[chr];
  col=0xf00000;

  for (x2=0; x2<5; x2++) {
    for (y2=0; y2<7; y2++) {
      if (data[y2]&((1<<4)>>x2))
        set_pixel(x+x2, y+y2, col);
      else
        set_pixel(x+x2, y+y2, 0xc0c0c0);
    }
  }
}


void draw_char(unsigned char chr, int x, int y)
{
  int x2, y2;
  unsigned char *data;
  short mapped_char;
  ...
  if (show_hardware)
    ...
    mapped_char=rocklatin[chr];
    if (chr<=0x20 && mapped_char==0xff)
      mapped_char=rocklatin[' '];
  }
  if (mapped_char>=512) {
    if (show_subst) {
      chr=lcd_player_extended_lcd_to_rocklatin1[mapped_char-512];
      mapped_char=rocklatin[chr];
      data=(*font)[mapped_char];
    } else {
      draw_extended_char(mapped_char-512, x, y);
      return;
    }
    col=0xf00000;
  } else if (mapped_char<256) {
    data=(*font)[mapped_char];
  } else if (mapped_char>=256) {
    mapped_char=mapped_char-256;
  }
...
int main (int argc, char** argv)
{
  ...
  for (loop=0; loop<6; loop++) {
    switch (loop) {
    case 0:
      font=&font_old_player;
      rocklatin=old_lcd_rocklatin1_to_xlcd;
      filename="lcd_old.gif";
      show_subst=0;
      show_hardware=0;
      break;
    case 1:
      font=&font_new_player;
      rocklatin=new_lcd_rocklatin1_to_xlcd;
    ...
    fprintf(fp, "P3\n#PNM showing a picture of the Rocklatin charset\n");

    fprintf(fp, "%d %d\n255\n", BITMAP_HEIGHT*SIZE, BITMAP_WIDTH*SIZE);

    shadow_show_hardware=show_hardware;
    show_hardware=0;
    for (x=0; x<16; x++) {
      draw_char(x+(x<10?'0':'A'-10), CHAR_WIDTH+CHAR_WIDTH*x+1, 0);
      draw_char(x+(x<10?'0':'A'-10), 1, CHAR_WIDTH+CHAR_WIDTH*x);
    }
    show_hardware=shadow_show_hardware;

    for (x=0; x<16; x++) {
      for (y=0; y<16; y++) {
        draw_char(x+y*16, CHAR_WIDTH+CHAR_WIDTH*x+1, y*CHAR_HEIGHT+CHAR_HEIGHT);
      }
    }
    ...
    for (y=0; y<BITMAP_HEIGHT; y++) {
      ...
      for (i=0; i<SIZE; i++) {
        for (x=0; x<BITMAP_WIDTH; x++) {
          ...
          if (y<CHAR_HEIGHT || x<CHAR_WIDTH) {
            col=0xffffffff-col;
          }
          for (z=0; z<SIZE; z++) {
            fprintf(fp, "%d ", (col>>16)&255);
            fprintf(fp, "%d ", (col>>8)&255);
            fprintf(fp, "%d  ", (col)&255);
          }
        }
        fprintf(fp, "\n");
      }
