...
#define err_printf(col, ...) printf(u, true, col, __VA_ARGS__)
    ...
    MD5_CTX md5_ctx;
    MD5_Init(&md5_ctx);
    MD5_Update(&md5_ctx, md5 + 1, size - sizeof(struct upg_header_t));
    MD5_Final(actual_md5, &md5_ctx);
    if(memcmp(actual_md5, md5->md5, 16) != 0)
    {
    ...
    int ret = fwp_read(hdr, size - sizeof(*md5), hdr, (void *)key);
    if(ret)
    {
        err_printf(GREY, "Crypto error: %d\n", ret);
        return NULL;
    }
    ...
    char sig[NWZ_SIG_SIZE], size_t *out_size, void *u, generic_printf_t printf)
{
    ...
    int ret = fwp_write(hdr, tot_size - sizeof(*md5), hdr, (void *)key);
    if(ret)
    {
        err_printf(GREY, "Crypto error: %d\n", ret);
        free(buf);
    ...
    MD5_Update(&md5_ctx, hdr, tot_size - sizeof(*md5));
    /* write final MD5 */
    MD5_Final(md5->md5, &md5_ctx);
    *out_size = tot_size;
    return buf;
}
