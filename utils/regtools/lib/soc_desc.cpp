...
instance_t *get_inst(std::vector< instance_t > *insts, soc_id_t id)
{
    if(insts == 0)
        return 0;
...
instance_t *find_inst(soc_ref_t soc, const std::vector< soc_id_t >& inst_path,
    const std::vector< size_t >& index_path, soc_addr_t& addr,
    std::vector< soc_id_t >& node_path)
{
    addr = 0;
    instance_t *inst = 0;
    std::vector< node_t > *nodes = &soc.get()->node;
    node_path.clear();
    if(inst_path.size() != index_path.size())
        return 0;
    for(size_t i = 0; i < inst_path.size(); i++)
    {
        for(size_t j = 0; j < (*nodes).size(); j++)
        {
            node_t *n = &(*nodes)[j];
            inst = get_inst(&n->instance, inst_path[i]);
            if(inst != 0)
            {
                node_path.push_back(n->id);
                nodes = &n->node;
                break;
            }
        }
