...
std::vector< field_ref_t > register_ref_t::fields() const
{
    ...
    for(size_t i = 0; i < r->field.size(); i++)
        fields.push_back(field_ref_t(*this, r->field[i].name));
    return fields;
}
...
instance_t *find_inst(soc_ref_t soc, const std::vector< std::string >& name_path,
    ...
    std::vector< std::string >& node_path)
{
    ...
    std::vector< node_t > *nodes = &soc.get()->node;
    node_path.clear();
    if(name_path.size() != index_path.size())
        return 0;
...
node_inst_t::node_inst_t(soc_ref_t soc, const std::vector< std::string >& names,
        const std::vector< size_t >& indexes)
    :m_soc(soc), m_name_path(names), m_index_path(indexes)
{
}

node_inst_t node_inst_t::make_invalid(soc_ref_t soc)
{
    return node_inst_t(soc, std::vector< std::string >(1, "<invalid>"),
...
node_ref_t node_inst_t::node() const
{
    soc_addr_t addr;
    std::vector< std::string > node_path;
    if(is_root() || find_inst(soc(), m_name_path, m_index_path, addr, node_path))
        return node_ref_t(m_soc, node_path);
    else
        return node_ref_t::make_invalid(m_soc);
}
...
node_inst_t node_inst_t::parent() const
{
    ...
    if(!indexes.empty())
        indexes.pop_back();
    return node_inst_t(m_soc, names, indexes);
}
...
instance_t *node_inst_t::get() const
{
    soc_addr_t addr;
    std::vector< std::string > node_path;
    return find_inst(soc(), m_name_path, m_index_path, addr, node_path);
}
        ...
        for(size_t j = 0; j < node.instance.size(); j++)
        {
            ...
            switch(inst.type)
            {
                case instance_t::SINGLE:
                    ...
                    list.push_back(node_inst_t(m_soc, n_path, i_path));
                    i_path.pop_back();
                    break;
                case instance_t::RANGE:
