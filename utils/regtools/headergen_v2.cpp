 ...
 * - PREFIX: this macro will contain the prefix for fields
 * - for each field F:
 ...
 * - PREFIX: same as PREFIX (ie same fields)
 *
...
bool common_generator::generate_register(std::ostream& os, const pseudo_node_inst_t& reg)
{
    ...
    for(size_t i = 0; i < fields.size(); i++)
    {
        ...
        ctx.add(bfm_prefix + fname + type_xfix(MT_FIELD_BFM, false) + "(_v)",
            bm_prefix + fname + type_xfix(MT_FIELD_BM, false));
        /* print BFV macro: ((enum_v) << pos) & mask) */
        ctx.add(bfv_prefix + fname + type_xfix(MT_FIELD_BFV, false) + "(_e)",
            bf_prefix + fname + type_xfix(MT_FIELD_BF, false) + "(" +
            bv_prefix + "##_e)");
        /* print BFMV macro: masl */
        ctx.add(bfmv_prefix + fname + type_xfix(MT_FIELD_BFMV, false) + "(_v)",
            bm_prefix + fname + type_xfix(MT_FIELD_BM, false));
    }
    ...
    for(int i = 0; i < 3; i++)
    {
        for(int j = 0; j < 3; j++)
        {
            ...
            if(at[i] == AT_RW)
            {
                ...
                oss << "(vor) | ((vand) & " << io_type << register_op_prefix() << register_op_name(RO_READ) + "(name)";
                oss << "))";
            }
            else if(at[i] == AT_WO)
...
class imx_generator : public common_generator
{
    ...
    std::string type_xfix(macro_type_t type, bool prefix) const
    {
        switch(type)
        {
            case MT_REG_ADDR: return prefix ? "RA_" : "";
            case MT_REG_TYPE: return prefix ? "RT_" : "";
            case MT_REG_NAME: return prefix ? "RN_" : "";
            case MT_REG_INDEX: return prefix ? "RI_" : "";
            case MT_FIELD_BP: return prefix ? "BP_" : "";
            case MT_FIELD_BM: return prefix ? "BM_" : "";
            case MT_FIELD_BV: return prefix ? "BV_" : "";
            case MT_FIELD_BF: return prefix ? "BF_" : "";
