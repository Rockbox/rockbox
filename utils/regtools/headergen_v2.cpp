 ...
 *   #endif
 *
 * NOTE: the tool will also generator a copyright header which contains the Rockbox
 * logo, a list of soc names and version, and authors in the description file.
 * This part cannot be customised at the moment.
 *
 * In order to list all register, the generator will recursively enumerate all
 ...
 *
 * This process will give a list of pseudo-instance: it is register instance
 ...
 *
 * In order to generate the macro name, the generate on you providing detailled
...
protected:
     ...
     * to be ignored */
    enum register_flag_t /// flag to consider
    {
    ...
    enum macro_type_t /// prefix/suffix type
    {
    ...
    /// return the complete macro name, default uses all the other functions
...
std::string common_generator::macro_basename(const pseudo_node_inst_t& inst_, macro_type_t type) const
{
    ...
    return type_xfix(type, true) + str;
}
...
bool common_generator::generate_register(std::ostream& os, const pseudo_node_inst_t& reg)
{
    ...
    std::string name = macro_basename(reg, MT_REG_ADDR) + type_xfix(MT_REG_ADDR, false);
    ...
    ctx.add(macro_basename(reg, MT_REG_ADDR) + type_xfix(MT_REG_ADDR, false) +
        param_str, addr);
    /* print TYPED macro: (*(volatile uintN_t *)(addr)) */
    unsigned width = reg.inst.node().reg().get()->width;
    ctx.add(macro_basename(reg, MT_REG_TYPED) + type_xfix(MT_REG_TYPED, false) +
        param_str, "(*(volatile uint" + to_str(width) + "_t *)" + addr + ")");
    /* print fields */
    ...
    for(size_t i = 0; i < fields.size(); i++)
    {
        ...
        ctx.add(bf_prefix + fname + type_xfix(MT_FIELD_BF, false) + "(_v)",
            "(((_v) << " + to_str(fields[i].get()->pos) + ") & " +
            to_hex(fields[i].get()->bitmask()) + ")");
        /* print BFV macro: ((enum_v) << pos) & mask) */
        ctx.add(bf_prefix + fname + type_xfix(MT_FIELD_BFV, false) + "(_v)",
            "((" + bv_prefix + "##_v << " + to_str(fields[i].get()->pos) + ") & " +
            to_hex(fields[i].get()->bitmask()) + ")");
    }
...
bool common_generator::generate(error_context_t& ctx)
{
...
class imx233_generator : public common_generator
{
    ...
    bool register_flag(const node_inst_t& inst, register_flag_t flag) const
    {
        switch(flag)
        {
            case RF_GENERATE_ALL_INST: return true;
