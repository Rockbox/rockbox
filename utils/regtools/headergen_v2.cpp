 ...
 * - BW: write fields OR, shorthand for HW_<reg> = BF_OR(<reg>, ...)
 * - BM: write masks OR, shorthand for HW_<reg> = BM_OR(<reg>, ...)
 * - BR: read field
 * - for each field F:
 ...
 * - TYPED: typed macro for ADDR
 * - BW: same as BW but writes to register with offset
 * - BR: same as BR but reads register with offset
 *
...
protected:
    ...
    enum macro_type_t
    {
        ...
        MT_REG_BW, /// write with BF_OR
        MT_REG_BM, /// write with BM_OR
...
bool common_generator::generate_register(std::ostream& os, const pseudo_node_inst_t& reg)
{
    ...
    std::vector< variant_ref_t > variants = reg.inst.node().reg().variants();
    ...
    for(size_t i = 0; i < var_prefix.size(); i++)
    {
        ...
        std::string macro_typed = type_xfix(MT_REG_TYPED, true) + var_basename +
            type_xfix(MT_REG_TYPED, false);
        ...
        ctx.add(type_xfix(MT_REG_BW, true) + var_basename + type_xfix(MT_REG_BW, false) + "(" +
            param_str_no_paren_comma + "...)",
            macro_typed + param_str + " = " + macro_name(MN_BF_OR) +
            "(" + basename + ", __VA_ARGS__)");
        /* print BM macro: TYPED(reg) = BM_OR(reg, ...) */
        ctx.add(type_xfix(MT_REG_BM, true) + var_basename + type_xfix(MT_REG_BM, false) + "(" +
            param_str_no_paren_comma + "...)",
            macro_typed + param_str + " = " + macro_name(MN_BM_OR) +
            "(" + basename + ", __VA_ARGS__)");
        /* print BR macro: ((TYPED(reg) & BM(reg, field)) >> BP(reg, field)) */
        ctx.add(type_xfix(MT_REG_BR, true) + var_basename + type_xfix(MT_REG_BR, false) + "(" +
