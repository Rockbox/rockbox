...
void print_inst(std::ostream& fout, node_inst_t inst, bool end = true)
{
    if(!inst.is_root())
    {
        print_inst(fout, inst.parent(), false);
        fout << "." << inst.name();
        if(inst.is_indexed())
            fout << "[" << inst.index() << "]";
    }
...
private:
    ...
    void print_inst(std::ostream& fout, const pseudo_node_inst_t& inst, bool end = true);
...
void common_generator::print_inst(std::ostream& fout, const pseudo_node_inst_t& inst, bool end)
{
    if(!inst.inst.is_root())
    {
        ...
        if(inst.inst.is_indexed())
        {
            if(inst.parametric.back())
                ...
                fout << "[" << inst.inst.index() << "]";
        }
    }
    else
        fout << inst.inst.soc().get()->name;
    if(end)
        fout << "\n";
}
...
bool common_generator::generate_macro_header(error_context_t& ectx)
{
    ...
    if(!fout)
    {
        ectx.add(soc_desc::error_t(soc_desc::error_t::FATAL, "",
            "Cannot create '" + m_outdir + "/" + macro_header() + "'"));
        return false;
    }
...
bool common_generator::generate(error_context_t& ectx)
{
    ...
    for(; it != regmap.end(); ++it)
    {
        ...
        {
            ectx.add(soc_desc::error_t(soc_desc::error_t::FATAL, "",
                "Cannot create '" + m_outdir + "/" + it->first));
            return false;
        }
        ...
        for(size_t i = 0; i < it->second.size(); i++)
        {
            ...
            {
                std::ostringstream oss;
                oss << "Cannot generate register ";
                print_inst(oss, it->second[i], false);
                ectx.add(soc_desc::error_t(soc_desc::error_t::FATAL, "", oss.str()));
                return false;
            }
        }
    ...
    if(has_selectors())
    {
        ...
        for(jt = headers.begin(); jt != headers.end(); ++jt)
        {
            ...
            {
                ectx.add(soc_desc::error_t(soc_desc::error_t::FATAL, "",
                    "Cannot create selector '" + m_outdir + "/" + jt->first + "'"));
                return false;
            }
            print_copyright(fout, std::vector< soc_ref_t >());
