    ...
    if(in.formula.type == soc_desc_v1::REG_FORMULA_NONE)
    {
        ...
        for(size_t i = 0; i < in.addr.size(); i++)
            if(!convert_v1_to_v2(in.addr[i], out.instance[i], ctx))
                return false;
        ctx.add(error_t(error_t::INFO, loc, "promoted name to title, using address names"));
    }
    else
    {
...
{
    std::string loc = _loc + "." + in.name;
    ctx.add(error_t(error_t::INFO, loc, "dropped name, using address names"));
    if(!in.version.empty())
...
void check_node(const std::string& _path, const node_t& node, error_context_t& ctx)
{
    ...
    if(node.instance.empty())
    {
        ...
        path = _path + "." + node.instance[0].name;
    ...
    error_context_t& ctx)
{
    ...
    for(size_t i = 0; i < nodes.size(); i++)
        for(size_t j = 0; j < nodes[i].instance.size(); j++)
        {
            ...
            if(names.find(n) != names.end())
                ctx.add(error_t(error_t::FATAL, path, "duplicate name '" + n + "' in subnodes"));
            names.insert(n);
        }
}
