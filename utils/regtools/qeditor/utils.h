...
class MyTabBarEdit: public QLineEdit
{
    Q_OBJECT
public:
    MyTabBarEdit(QWidget* parent = 0);
    void setText(const QString &text);

signals:
    void editAccepted(const QString &text);
    void editCancelled();
...
{
    Q_OBJECT
    Q_PROPERTY(bool tabsEditable READ tabsEditable WRITE setTabsEditable)
public:
    MyTabBar(QWidget *parent = 0);
    inline bool tabsEditable() const { return m_tabs_editable; }
    void setTabsEditable(bool editable);

signals:
    void tabEditRequested(int index, const QString& text);
...
protected:
    ...
    virtual void triggerEdit(int index);
    virtual void finishEdit(bool cancel);

    bool m_tabs_editable;
    int m_edit_index;
    MyTabBarEdit *m_edit_line;
};
#endif
...
class YPathButton : public QToolButton
{
    Q_OBJECT
public:
    YPathButton(QWidget *parent = 0);

    virtual QSize sizeHint() const;

    enum Mode { Normal, Bold, Arrow };
...
class YPathBar : public QWidget
{
    Q_OBJECT
public:
    YPathBar(QWidget *parent);
...
protected slots:
    void onSelect(QAction *act);

protected:
    void updateLayout();
    QVector< QModelIndex > computePath(QModelIndex index);
    YPathButton *createButton(bool link);
    void setupButton(YPathButton *button, const QModelIndex& index, bool link);

    QAbstractItemModel *m_model;
    QModelIndex m_current;
    QHBoxLayout *m_layout;
    QVector< YPathButton * > m_button;
};
