...
bool RamIoBackend::WriteRegister(soc_addr_t addr, soc_word_t value, WriteMode mode)
{
    switch(mode)
    {
...
bool FileIoBackend::Reload()
{
    ...
    while(!in.atEnd())
    {
        ...
        bool key_ok,val_ok;
        soc_word_t val = line.mid(idx + 1).trimmed().toULong(&val_ok, 0);
...
bool FileIoBackend::WriteRegister(soc_addr_t addr, soc_word_t value, WriteMode mode)
{
    m_dirty = true;
    return RamIoBackend::WriteRegister(addr, value, mode);
}
...
soc_desc::node_inst_t BackendHelper::ParsePath(const QString& path)
{
    ...
    while(pos < path.size())
    {
        ...
        if(lidx < next)
        {
            ...
            /* invalid number ? */
            bool ok = false;
            size_t idx = path.mid(lidx + 1, ridx - lidx).toUInt(&ok);
...
bool BackendHelper::ReadRegister(const soc_desc::node_inst_t& inst, soc_word_t& v)
{
    soc_addr_t addr;
...
bool BackendHelper::DumpAllRegisters(BackendHelper *bh, const soc_desc::node_inst_t& inst,
    bool ignore_errors)
{
    bool ret = true;
