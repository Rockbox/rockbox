...
void PinAnalyser::FillList()
{
    ...
    for(int bank = 0; bank < 4; bank++)
    {
        ...
        bool error = false;
        for(int i = 0; i < 2; i++)
            if(!ReadRegisterOld("PINCTRL", QString("MUXSEL%1").arg(bank * 2 + i), muxsel[i]))
                error = true;
        /* don't make an error for those since some do not exist */
        for(int i = 0; i < 4; i++)
            if(!ReadRegisterOld("PINCTRL", QString("DRIVE%1").arg(bank * 4 + i), drive[i]))
                drive[i] = 0;
        if(error)
            continue;
        if(!ReadRegisterOld("PINCTRL", QString("PULL%1").arg(bank), pull))
            pull = 0;
        if(!ReadRegisterOld("PINCTRL", QString("DIN%1").arg(bank), in))
            in = 0;
        if(!ReadRegisterOld("PINCTRL", QString("DOUT%1").arg(bank), out))
            out = 0;
        if(!ReadRegisterOld("PINCTRL", QString("DOE%1").arg(bank), oe))
            oe = 0;
