...
void PinAnalyser::FillListJZ4760B()
{
    ...
    for(int bank = 0; bank < soc->nr_banks; bank++)
    {
        ...
        table->setHorizontalHeaderItem(6, new QTableWidgetItem("Output"));
        table->setHorizontalHeaderItem(7, new QTableWidgetItem("Input"));
        table->verticalHeader()->setVisible(false);
        table->horizontalHeader()->setStretchLastSection(true);
        ...
        for(int pin = 0; pin < b->nr_pins; pin++)
        {
            ...
            else
            {
                ...
                out_str = (dir & mask) ? QString("%1").arg((out >> pin) & 1) : "";
            }
            ...
            /* output */
            table->setItem(row, 6, new QTableWidgetItem(out_str));
            /* input */
            table->setItem(row, 7, new QTableWidgetItem(QString("%1").arg((in >> pin) & 1)));
            /* center all cells, make them read-only */
            for(int i = 0; i <= 7; i++)
            {
