...
class node_ref_t
{
    ...
    std::vector< std::string > m_path; /* path from the root */

    node_ref_t(soc_ref_t soc);
    node_ref_t(soc_ref_t soc, const std::vector< std::string >& path);
public:
    /** Builds an invalid reference */
    static node_ref_t make_invalid(soc_ref_t soc);
    ...
    node_ref_t child(const std::string& name);
...
public:
    /** Builds an invalid reference */
    static register_ref_t make_invalid(soc_ref_t soc);
...
class field_ref_t
{
    ...
    field_ref_t(register_ref_t reg, const std::string& name);
public:
    /** Builds an invalid reference */
    static field_ref_t make_invalid(soc_ref_t soc);
    /** Check whether this reference is valid/exists */
    bool valid();
...
class node_inst_t
{
    ...
    std::vector< std::string > m_name_path; /* list of instance names */
    std::vector< size_t > m_index_path; /* list of instance indexes */

    node_inst_t(soc_ref_t soc);
public:
    /* FIXME: makes this private */
    node_inst_t(node_ref_t node, const std::vector< std::string >& names,
        const std::vector< size_t >& indexes);

    /** Builds an invalid reference */
    static node_inst_t make_invalid(soc_ref_t soc);
    ...
    node_inst_t child(const std::string& name);
    ...
    node_inst_t child(const std::string& name, size_t index);
    /** Returns a list of all instances of subnodes of this node's instance */
    std::vector< node_inst_t > children();
