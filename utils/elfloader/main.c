...
static void help(void)
{
    printf("This utility will process supplied ELF file\n"
           ...
           "-uncache_base - uncached mem alias address (default 0x00000000)\n"
           "-elf          - elf file to process\n"
           "-core_elf     - rockbox.elf to resolve symbols agains\n");
}
...
int main(int argv, char **argc)
{
    ...
    while (i<argv)
    {
        ...
        else if (strcmp(argc[i], "-core_elf") == 0)
        {
            i++;
            if (i != argv)
                core_elf = argc[i++];
    ...
    if (core_elf)
        read_symbols_hashtable(core_elf);
