...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        printf("Missing arguments: output filename");
        return 1;
    }
    ...
    if (rev_info_file != NULL)
    {
        for (int i = 0; i < GENERIC_HEADER_LINES; i++)
        {
            ...
            error += fprintf(output_file, "%s", header) != strlen(header);
        }
    ...
    for (int i = 0; i < YPR0_COMPONENTS_COUNT; i++)
    {
        ...
        // write data
        fwrite(fw.component_data[i], sizeof(char), fw.component_size[i], output_file);
        // padding
