...
#define DTXFSTS(n)  REG(USB_DEV_OEP + 0x18 + (n) * 0x20)
/* Power */
...
int usb_drv_recv(int endpoint, void* ptr, int length)
{
    if(endpoint != 0)
        return -1;
    int mps = FIELD_GET(DOEPCTL0, MPSIZ) == DOEPCTL0_MPSIZ_64B ? 64 : 8;
    unsigned pktcnt = (length + mps - 1) / mps;
    if(pktcnt == 0)
        pktcnt = 1;
    uint32_t *p = ptr;
    ...
    DOEPCTL(0) |= DIEPCTL_EPENA | DIEPCTL_CNAK;
    return -1;
}
...
void usb_drv_stall(int endpoint, bool stall, bool in)
{
    ...
    {
        if(stall)
            DOEPCTL(endpoint) |= DIEPCTL_STALL;
        else
            DOEPCTL(endpoint) &= ~DIEPCTL_STALL;
    }
}
...
void usb_drv_irq(void)
{
    ...
    if(gintsts & GINTSTS_ENUMDNE)
    {
        ...
        unsigned sz = usb_drv_port_speed() ? DOEPCTL0_MPSIZ_8B : DOEPCTL0_MPSIZ_64B;
        FIELD_SET(DIEPCTL0, MPSIZ, sz); // apparently OUT max packet size is the one in the IN register
        DOEPSIZ(0) = FIELD_MAKE(DOEPSIZ0, STUPCNT, 1) |
...
void usb_drv_init(void)
{
    ...
    DOEPCTL(0) = DOEPCTL_EPDIS | DOEPCTL_SNAK; // disable EP0
    DOEPSIZ(0) = 0;
