...
#define DTHRCTL     REG(USB_DEV + 0x30)
#define DTHRCTL_NONISOTHREN (1 << 0)
#define DTHRCTL_TXTHRLEN_BP 2
#define DTHRCTL_TXTHRLEN_BM (0xff << 2)
...
#define DFIFO(n)    REG(USB_BASE + ((n) + 1) * 0x1000)
...
{
    /* done during the SETUP phase because it must done before the ACK */
...
void usb_drv_init(void)
{
    ...
    /* at high-speed, the CPU might not have enough power to push data fast
     * enough, so make sure the transmit threshold guarantees the whole packet
     * is written before sending */
    FIELD_SET(DTHRCTL, TXTHRLEN, 16); // 16 DWORDS is 64 bytes
    DTHRCTL |= DTHRCTL_NONISOTHREN;
    /* setup endpoints */
