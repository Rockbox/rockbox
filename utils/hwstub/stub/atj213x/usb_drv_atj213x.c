...
#if 0
    /* don't use interrupt, clear flags manually in usb_drv_recv */
    if (epoutirq)
    {
...
int usb_drv_recv_setup(struct usb_ctrlrequest *req)
{
    ...
    /* FIXME I think the core handles SET_ADDRESS completely in hardware
     * (including sending the NAK), so maybe we should not pass-through this
     * request here, or have a define so that the usb core doesn't send the
     * NAK on SET_ADDRESS, since that could cause some confusion */
