...
std::shared_ptr<hwstub_context> g_ctx;
...
std::string get_error_string(hwstub_error err)
{
    switch(err)
    {
        case HWSTUB_SUCCESS: return "success";
        case HWSTUB_ERROR: return "error";
        case HWSTUB_DISCONNECTED: return "device disconnected";
        case HWSTUB_NO_CONTEXT: return "context destroyed";
        case HWSTUB_USB_ERROR: return "usb error";
...
void print_dev_details(std::shared_ptr<hwstub_device> dev)
{
    std::shared_ptr<hwstub_handle> h;
    hwstub_error err = dev->open(h);
    if(err != HWSTUB_SUCCESS)
    {
        printf("  [cannot open dev: %s]\n", get_error_string(err).c_str());
        return;
    }
    /* version */
...
void print_device(std::shared_ptr<hwstub_device> dev, bool arrived = true)
{
...
void dev_changed(std::shared_ptr<hwstub_context> ctx, bool arrived, std::shared_ptr<hwstub_device> dev)
{
...
int main(int argc, char **argv)
{
    ...
    g_ctx = hwstub_usb_context::create(g_usb_ctx, true); /* cleanup on delete */
    print_list();
    g_ctx->register_callback(dev_changed);
    g_ctx->start_polling(std::chrono::seconds(1));
    while(!g_exit_loop)
