...
int hwstub_tcp_send(int connfd, int32_t id, uint32_t cmd, void *buf, uint32_t len)
{
    int n;
 ...
 * NOTE: will lock the global list */
                ...
                else
                {
                    ...
                    for (size_t i=0; i<g_devlist.size(); i++)
                    {
                        if (g_devlist[i]->is_connected())
                        {
                            ...
                            {
                                printf("HWSERVER_GET_DEV_LIST: %d:%s\n", g_devlist[i]->devinfo.id, g_devlist[i]->devinfo.hwdesc.bName);
