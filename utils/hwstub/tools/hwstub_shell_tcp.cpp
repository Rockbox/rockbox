...
int hwstub_tcp_rw_mem(int connfd, int read, uint32_t addr, void *buf, size_t sz)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = read ? HWSTUB_READ2 : HWSTUB_WRITE,
        .addr = addr,
        .len = sz
    };
...
int hwstub_tcp_rw_mem_atomic(int connfd, int read, uint32_t addr, void *buf, size_t sz)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = read ? HWSTUB_READ2_ATOMIC : HWSTUB_WRITE_ATOMIC,
        .addr = addr,
        .len = sz
    };
...
int hwstub_tcp_call(int connfd, uint32_t addr)
{
    return hwstub_tcp_exec(dev, addr, HWSTUB_EXEC_CALL);
}

int hwstub_tcp_jump(int connfd, uint32_t addr)
{
