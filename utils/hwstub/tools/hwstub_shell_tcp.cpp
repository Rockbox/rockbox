...
int hwstub_tcp_get_log(int connfd, void *buf, size_t sz)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = HWSTUB_GET_LOG,
...
/* Returns number of bytes filled */
int hwstub_tcp_get_desc(int connfd, uint16_t desc, void *info, size_t sz)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = HWSTUB_GET_DESC,
...
int hwstub_tcp_rw_mem(int connfd, int rd, uint32_t addr, void *buf, size_t sz)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = rd ? (uint32_t)HWSTUB_READ2 : (uint32_t)HWSTUB_WRITE,
        .addr = addr,
        .len = (uint32_t)sz
    };
    ...
    if (p.cmd == HWSTUB_READ2_ACK && p.len > 0)
        n = read(connfd, buf, p.len);
    else if (p.cmd == HWSTUB_WRITE_NACK || p.cmd == HWSTUB_READ2_NACK)
        return -1;

    return p.len;
}

int hwstub_tcp_rw_mem_atomic(int connfd, int rd, uint32_t addr, void *buf, size_t sz)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = rd ? (uint32_t)HWSTUB_READ2_ATOMIC : (uint32_t)HWSTUB_WRITE_ATOMIC,
        .addr = addr,
        .len = (uint32_t)sz
    };
    ...
    if (p.cmd == HWSTUB_READ2_ATOMIC_ACK && p.len > 0)
        n = read(connfd, buf, p.len);
    else if (p.cmd == HWSTUB_WRITE_ATOMIC_NACK ||
...
/* Returns <0 on error */
int hwstub_tcp_exec(int connfd, uint32_t addr, uint16_t flags)
{
    int n;
    hwstub_tcp_cmd_t p = {
        .cmd = HWSTUB_EXEC,
...
void hwstub_tcp_call(int connfd, uint32_t addr)
{
    hwstub_tcp_exec(connfd, addr, HWSTUB_EXEC_CALL);
}

void hwstub_tcp_jump(int connfd, uint32_t addr)
{
    hwstub_tcp_exec(connfd, addr, HWSTUB_EXEC_JUMP);
}

/* Returns -1 on error */
int hwstub_tcp_open(void)
{
    int sockfd = -1, s;
...
void hwstub_tcp_release(int connfd)
{
    close(connfd);
}

void print_log(int connfd)
{
    do
    {
        char buffer[128];
        int length = hwstub_tcp_get_log(connfd, buffer, sizeof(buffer) - 1);
        if(length <= 0)
            break;
        buffer[length] = 0;
        printf("%s", buffer);
...
{
    uint8_t u;
    if(hwstub_tcp_rw_mem_atomic(connfd, 1, addr, &u, sizeof(u)) != sizeof(u))
...
{
    uint8_t u = val;
    if(hwstub_tcp_rw_mem_atomic(connfd, 0, addr, &u, sizeof(u)) != sizeof(u))
...
int my_lua_call(lua_State *state)
{
    ...
    hwstub_tcp_call(connfd, luaL_checkunsigned(state, 1));
    return 0;
}
...
int my_lua_jump(lua_State *state)
{
    ...
    hwstub_tcp_jump(connfd, luaL_checkunsigned(state, 1));
    return 0;
}
...
int main(int argc, char **argv)
{
...
#endif
    connfd = hwstub_tcp_open();

    printf("connfd: %d\n", connfd);

    // get hwstub information
    int ret = hwstub_tcp_get_desc(connfd, HWSTUB_DT_VERSION, &g_hwdev_ver, sizeof(g_hwdev_ver));
    ...
    ret = hwstub_tcp_get_desc(connfd, HWSTUB_DT_LAYOUT, &g_hwdev_layout, sizeof(g_hwdev_layout));
    ...
    // get target
    ret = hwstub_tcp_get_desc(connfd, HWSTUB_DT_TARGET, &g_hwdev_target, sizeof(g_hwdev_target));
    ...
    if(g_hwdev_target.dID == HWSTUB_TARGET_STMP)
    {
        ret = hwstub_tcp_get_desc(connfd, HWSTUB_DT_STMP, &g_hwdev_stmp, sizeof(g_hwdev_stmp));
    ...
    {
        ret = hwstub_tcp_get_desc(connfd, HWSTUB_DT_PP, &g_hwdev_pp, sizeof(g_hwdev_pp));
