...
uint8_t hwstub_usb_device::get_bus_number()
{
    return libusb_get_bus_number(native_device());
}

uint8_t hwstub_usb_device::get_address()
{
...
ssize_t hwstub_usb_rb_handle::read_dev(uint32_t addr, void *buf, size_t sz, bool atomic)
{
    ...
    return interpret_libusb_error(libusb_control_transfer(m_handle,
        ...
        (unsigned char *)buf, sz, 1000));
}

ssize_t hwstub_usb_rb_handle::write_dev(uint32_t addr, const void *buf, size_t sz, bool atomic)
{
    ...
    ssize_t ret = interpret_libusb_error(libusb_control_transfer(m_handle,
        ...
        (unsigned char *)req, sz + hdr_sz, 1000), sz + hdr_sz);
    delete tmp_buf;
    return ret == HWSTUB_SUCCESS ? sz : ret;
}
...
hwstub_error hwstub_usb_jz_handle::exec_dev(uint32_t addr, uint16_t flags)
{
    ...
    hwstub_error ret = jz_flush_caches();
...
ssize_t hwstub_usb_jz_handle::read_dev(uint32_t addr, void *buf, size_t sz, bool atomic)
{
    (void) atomic;
    /* FIXME we cannot be sure how about the ROM handles reads, atomic transfers
     * may not be atomic */
    hwstub_error ret = jz_set_addr(addr);
    if(ret == HWSTUB_SUCCESS)
     ret = jz_set_length(sz);
    if(ret == HWSTUB_SUCCESS)
        return jz_upload(buf, sz);
    else
        return (ssize_t)ret;
}

ssize_t hwstub_usb_jz_handle::write_dev(uint32_t addr, const void *buf, size_t sz, bool atomic)
{
    (void) atomic;
    /* FIXME we cannot be sure how about the ROM handles writes, atomic transfers
     * may not be atomic */
    hwstub_error ret = jz_set_addr(addr);
    if(ret == HWSTUB_SUCCESS)
     ret = jz_set_length(sz);
    if(ret == HWSTUB_SUCCESS)
        return jz_download(buf, sz);
    else
        return (ssize_t)ret;
}
...
ssize_t hwstub_usb_jz_handle::jz_download(const void *data, size_t length)
{
    int xfered;
    ssize_t ret = interpret_libusb_error(libusb_bulk_transfer(m_handle,
