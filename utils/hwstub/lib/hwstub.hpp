...
enum hwstub_error
{
    ...
    HWSTUB_DUMMY = -6, /** Call on dummy device/handle */
};
...
public:
    ...
    hwstub_error get_dummy_device(std::shared_ptr<hwstub_device>& dev);
...
public:
    ...
    ssize_t read(uint32_t addr, void *buf, size_t sz, bool atomic);
    ssize_t write(uint32_t addr, const void *buf, size_t sz, bool atomic);
...
protected:
    ...
    virtual ssize_t read_dev(uint32_t addr, void *buf, size_t sz, bool atomic) = 0;
    virtual ssize_t write_dev(uint32_t addr, const void *buf, size_t sz, bool atomic) = 0;
...
/** Dummy device */
class hwstub_dummy_device : public hwstub_device
{
    friend class hwstub_context; /* for ctor */
protected:
    hwstub_dummy_device(std::shared_ptr<hwstub_context> ctx);
public:
    virtual ~hwstub_dummy_device();
...
/** Dummy handle */
class hwstub_dummy_handle : public hwstub_handle
{
    friend class hwstub_dummy_device;
protected:
    hwstub_dummy_handle(std::shared_ptr<hwstub_device> dev);
public:
    virtual ~hwstub_dummy_handle();

protected:
    virtual ssize_t read_dev(uint32_t addr, void *buf, size_t sz, bool atomic);
    virtual ssize_t write_dev(uint32_t addr, const void *buf, size_t sz, bool atomic);
