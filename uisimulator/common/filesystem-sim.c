...
struct dirstr_desc
{
    ...
    char              *osdirname;    /* Host OS path to directory stream */
...
int sim_get_os_path(char *buffer, const char *path, size_t bufsize)
{
    ...
    {
        const char *p;
        ssize_t len = parse_path_component(&path, &p, true);
        ...
        char compname[len + 1];
        *(char *)mempcpy(compname, p, len) = 0;
        size = path_append(buffer, NULL, compname, bufsize);
        if (size >= bufsize)
        {
            errno = ENAMETOOLONG;
            return -1;
...
DIR * sim_opendir(const char *dirname)
{
    ...
    dirstr->osdirp = os_opendir(osdirname);
    if (!dirstr->osdirp)
        return NULL;

    dirstr->osfd = os_getdirfd(osdirname, dirstr->osdirp,
                               &dirstr->osfd_opened);
    if (dirstr->osfd >= 0)
    {
...
#ifdef __MINGW32__
    /* no fstatat: build file name for stat() */
...
int os_volume_path(IF_MV(int volume, ) char *buffer, size_t bufsize)
{
    ...
    if (path_append(tmpbuf, NULL, volname, sizeof (volname)) >= sizeof (volname))
        return -1;
#endif /* HAVE_MULTIVOLUME */

    if (path_append(tmpbuf, NULL, ".", sizeof (tmpbuf)) >= sizeof (tmpbuf))
        return -1;
