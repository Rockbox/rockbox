...
int get_inactivity_tmo(void)
{
    if(button_hold())
        ...
        return 30 * HZ; /* Inactivity timeout when not on hold */
}

enum boot_mode get_boot_mode(enum boot_mode init_mode)
{
    ...
    while(true)
    {
        ...
        if(TIME_AFTER(current_tick, last_activity + get_inactivity_tmo()))
            return BOOT_STOP;
        /* redraw */
        lcd_clear_display();
        /* display top text */
...
{
    const char *choices[] = {"Service menu", "Run script"};
    int choice = choice_screen("TOOLS MENU", true, 2, choices);
    if(choice == 0)
    {
        /* run service menu */
        fflush(stdout);
...
int main(int argc, char **argv)
{
     ...
     * user partition */
    int fd = open("/contents/rockbox.log", O_RDWR | O_CREAT | O_TRUNC);
    if(fd >= 0)
    {
    ...
    while(true)
    {
        enum boot_mode mode = load_boot_mode(BOOT_OF);
        mode = get_boot_mode(mode);
        /* don't save special boot modes */
        if(mode < BOOT_COUNT)
            save_boot_mode(mode);
