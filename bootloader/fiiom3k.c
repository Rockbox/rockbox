...
int GLB_LINE = 0;
extern volatile int LCD_INITED;
...
void main(void)
{
    ...
    uint32_t* addr = (uint32_t*)(loadbuffer + (0x8007f3d0 - 0x80004000));
    while(1) {
        lcd_fillrect(0, LCD_HEIGHT - 48, LCD_WIDTH, 12);
        uint32_t realaddr = (uint32_t)addr;
        realaddr -= (uint32_t)&loadbuffer[0];
        realaddr += 0x80004000;
        lcd_putsxyf(0, LCD_HEIGHT-48, "addr:%08lx data:%08lx", realaddr, *addr);
        lcd_update();
