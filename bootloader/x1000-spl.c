...
# error "unknown EXCLK frequency"
    ...
    /* set DDR clock to MPLL/(2+1) = 200 MHz */
    jz_writef(CPM_CLKGR, DDR(0));
    jz_writef(CPM_DDRCDR, CLKDIV(2), CLKSRC_V(MPLL), CE(1));
    ...
    /* Allow USB tool to confirm init */
    char* in  = (char*)"Success!";
    char* out = (char*)0xa0000000;
    while(*in)
        *out++ = *in++;
}
