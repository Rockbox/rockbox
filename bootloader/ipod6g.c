...
{
    printf(" verb: %s", (oflags & 1) ? "On " : "Off");
    printf(" beep: %s", (oflags & 2) ? "On " : "Off");
    line -= 2;
}

static int menu_select_opts(bool *verb, bool *beep)
{
    int fw = FW_NONE;
    int oflags = (*beep << 1) | *verb;
    ...
    /* save default boot options, MEMBYTE0..5 are used by OF */
    pmu_write(PCF5063X_REG_MEMBYTE7, 0xa0 | (oflags << 2) | fw);
...
static int get_saved_opts(bool *verb, bool *beep)
{
...
void main(void)
{
    ...
    usec_timer_init();
    stop_time = USEC_TIMER + 750000;

    opt_fw = get_saved_opts(&opt_verb, &opt_beep);
    ...
    if (opt_fw == FW_NONE) {
        opt_fw = menu_select_opts(&opt_verb, &opt_beep);
        if (opt_fw == FW_APPLE) {
            lcd_shutdown();
            system_reboot();
        }
