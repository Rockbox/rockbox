...
#define FLAG_DEVEL  (1<<2)

#define OPT2MEMBYTE(flags, fw) \
            ((0x5 << 5) | (((flags) & 7) << 2) | ((fw) & 3))

static bool get_saved_opts(int *oflags, int *fw)
{
    uint8_t occshdwn, membyte;

    occshdwn = pmu_rd(PCF5063X_REG_OOCSHDWN);
    membyte = pmu_rd(PCF5063X_REG_MEMBYTE7);
    /* restore OCCSHDWN status, ONB needs it */
    pmu_wr(PCF5063X_REG_OOCSHDWN, occshdwn);

    if ((membyte >> 5) != 0x5)
        return 0; /* no valid data */

    if ((membyte >> 2) & FLAG_DEVEL)
        *oflags = (membyte >> 2) & 7;
    *fw = membyte & 3;
...
{
    uint8_t occshdwn = pmu_rd(PCF5063X_REG_OOCSHDWN);
    /* save boot options, MEMBYTE0..5 are used by OF */
    pmu_wr(PCF5063X_REG_MEMBYTE7, OPT2MEMBYTE(oflags, fw));
    /* restore OCCSHDWN status, ONB needs it */
    pmu_wr(PCF5063X_REG_OOCSHDWN, occshdwn);
}

/* full IM3 size aligned to NOR sector size */
...
static int load_onb(void)
{
    struct Im3Info *hinfo;
    void *fw_addr;
    int nor_offset;

    /* BOOTROM loads IM3 header at IRAM1_ORIG and data at IRAM0_ORIG,
     * actually IRAM1_ORIG contains current RB bootloader IM3 header,
     * it will be replaced by OF IM3 header.
     */
    hinfo = (struct Im3Info*)IRAM1_ORIG;
    fw_addr = (void*)IRAM0_ORIG;
    nor_offset = NORBOOT_OFF + GET_NOR_SZ(hinfo); /* ONB offset */
...
/*
 * next functions needs RB kernel already launched
 */
...
static void print_flags(int oflags)
{
...
static int menu_select_opts(int *oflags)
{
    ...
    {
        lcd_set_foreground(LCD_REDORANGE);
        lcd_puts(0, line+3, button_hold() ? "Hold switch on!"
                                          : "               ");
        lcd_update();
    ...
    *oflags = bak_flags | (*oflags & 3);
    return fw;
}

static void update_boot_opts(int oflags, int fw)
{
    pmu_write(PCF5063X_REG_MEMBYTE7, OPT2MEMBYTE(oflags, fw));
}
        ...
        if (get_power_input_status() != POWER_INPUT_NONE) {
            ...
            while (1) {
                ...
                if (!tmo--) {
                    ...
                    if (vbat < 3200)
                        pmu_wr(PCF5063X_REG_SVMCTL, 0xA /*3200mV*/);
                    power_off();
                }
                sleep(HZ*1);
                if (get_power_input_status() != POWER_INPUT_NONE)
...
/* define which buttons are used to load OF, if not defined then the
   hold switch is used */
// #define LAUNCH_OF_BUTTONS  (BUTTON_SELECT|BUTTON_RIGHT)

/*
 *  Boot sequence, executed when the iPod is powered on. The previous
 *  state is powered off, reset or "hibernated". OF hibernates the
 *  device after an inactive period of ~30 minutes (FW 1.1.2), on this
 *  state SDRAM is in self-refresh mode.
 *
 *  t0 = 0
 *     S5L8702 BOOTROM loads an IM3 image located at NOR offset 32Kb:
 ...
 *     on ~250ms.
 *     Once executed, RB booloader moves itself from IRAM0_ORIG to
 *     IRAM1_ORIG+0x800, preserving current IM3 header that contains
 *     the NOR offset where the ONB (original NORBOOT, see dualboot.c)
 *     is located.
 *
 *  t1 = ~250 ms.
 *     Decrypted ONB (size 128Kb) is loaded from NOR to IRAM0_ORIG
 *     (body) and IRAM1_ORIG (header). ONB will be needed if the iPod
 *     was 'hibernated' by OF, or when the user chooses to use the OF.
 *
 *  t2 = ~600 ms.
 *     Wait for user button selection until t2+800ms is reached. The
 *     iPod might be on 'hibernated' state, SDRAM cannot be modified
 *     until we know that OF is not going to be launched, so we must
 *     wait for the user selection with no kernel, no LCD, no HDD...
 *
 *  t3 = ~1050 ms.
 *     Check user button selection. If diagmode, diskmode or OF is
 *     selected then execute the ONB.
 *     Initialize SDRAM, BSS, launch kernel, init HW including LCD.
 *
 *  t4 = ~1400 ms.
#ifndef LAUNCH_OF_BUTTONS
 *     Check hold switch status, if locked then select OF and silently
 *     reboot the iPod.
#endif
 *     Backlight on, print bootloader version.
 *     Wait for HDD spin-up.
 *
 *  t5 = ~3300 ms.
 *     HDD ready, load rockbox.ipod binary.
 *
 *  t6 = ~3500 ms.
...
void main(void)
{
    ...
    /* default options for first time installation or
       when the PMU data was lost (PMU NoPower state) */
    int fw = FW_ROCKBOX;
    int oflags = FLAG_VERB;

    usec_timer_init();
    stop_time = USEC_TIMER + 800000;
    ...
    if (btn == (BUTTON_SELECT|BUTTON_MENU)) {
        manual_reset = 1;
        while (read_all_buttons(5) == (BUTTON_SELECT|BUTTON_MENU))
            udelay(100000);
        stop_time = USEC_TIMER + 1000000;
        while ((int32_t)USEC_TIMER - stop_time < 0);
        btn = read_all_buttons(5);
    }

    if (((fw == FW_APPLE) && manual_reset && (btn == BUTTON_NONE))
#ifdef LAUNCH_OF_BUTTONS
            || ((fw != FW_APPLE) && (btn == LAUNCH_OF_BUTTONS))
#endif
    ) {
        if (!(oflags & FLAG_DEVEL)) {
            fw ^= 1;
            save_opts(oflags, fw);
        }
        else
            fw = FW_NONE; /* force boot options menu */
    }

    /* enter original diagmode and diskmode using usual buttons */
    if ((rc >= 0) && ((fw == FW_APPLE)
    ...
    /* clever decision! restore exception vector table */
    ...
    if (button_hold()) {
        if (!(oflags & FLAG_DEVEL)) {
            update_boot_opts(oflags, FW_APPLE);
            system_reboot();
        }
        fw = FW_NONE; /* force boot options menu */
    }
    ...
    if (file_exists(EMCORE_BIN)) {
        if (!(oflags & FLAG_DEVEL))
            update_boot_opts(oflags | FLAG_DEVEL, fw);
    }
    else {
        if (oflags & FLAG_DEVEL)
            update_boot_opts(oflags & ~FLAG_DEVEL, fw & 1);
    }
