...
{
    /* enable external wheel controller (it might been disabled if
       holdswitch is locked) */
    PCONE = (PCONE & ~0x00ffff00) | 0x000f0f00;
    ...
    int flags = disable_interrupt_save(IRQ_FIQ_STATUS);
...
void main(void)
{
    ...
    if (pmu_is_hibernated()) {
        fw = FW_APPLE;
        spi_clkdiv(SPI_PORT, 1); /* SPI clock = 27/2 = 13.5 MHz. */
    ...
    i2c_init();
    power_init();
    spi_clkdiv(SPI_PORT, 3); /* SPI clock = 54/4 = 13.5 MHz. */
