...
static void save_opts(int oflags, int fw)
{
    ...
    pmu_wr(PCF5063X_REG_MEMBYTE7, OPTTOMEM(oflags, fw));
    /* restore OCCSHDWN status, ONB needs it */
    pmu_wr(PCF5063X_REG_OOCSHDWN, occshdwn);
}
...
{
    pmu_write(PCF5063X_REG_MEMBYTE7, OPTTOMEM(oflags, fw));
}
...
/*
 *  1) S5L8702 BOOTROM loads and executes an IM3 image located at NOR
 *     offset=32Kb:
 *     - IM3 body (RB bootloader) is loaded at IRAM0_ORIG (0x22000000)
 *     - IM3 header (0x800 bytes) is loaded at IRAM1_ORIG (0x22020000)
 ...
 *     not going to launch OF because sometimes the OF switches to
 *     'hibernate' state, leaving runtime data in SDRAM self-refresh
 ...
 *     rockbox.ipod is loaded from HDD and executed as usual.
 */
void main(void)
{
    int btn;
    ...
    if (btn == (BUTTON_SELECT|BUTTON_RIGHT)) {
        if (!(oflags & FLAG_DEVEL)) {
            /* switch default FW (Rockbox/Apple) */
            fw ^= 1;
            save_opts(oflags, fw);
        }
        else
