...
SPEEXLIB_SRC := $(call preprocess, $(RBCODEC_DIR)/codecs/libspeex/SOURCES)
...
VOICESPEEXLIB_SRC := $(call preprocess, $(RBCODEC_DIR)/codecs/libspeex/SOURCES)
VOICESPEEXLIB_OBJ := $(addsuffix .o,$(basename $(subst $(RBCODEC_DIR)/codecs/libspeex,$(RBCODEC_BLD)/codecs/libspeex-voice,$(VOICESPEEXLIB_SRC))))
...
$(CODECDIR)/libspeex-voice/%.o : $(RBCODEC_DIR)/codecs/libspeex/%.c
	$(SILENT)mkdir -p $(dir $@)
	$(call PRINTS,CC $(subst $(ROOTDIR)/,,$<))$(CC) $(VOICESPEEXFLAGS) -c $< -o $@

$(CODECDIR)/libspeex-voice/%.o : $(RBCODEC_DIR)/codecs/libspeex/%.S
	$(SILENT)mkdir -p $(dir $@)
	$(call PRINTS,CC $(subst $(ROOTDIR)/,,$<))$(CC) $(VOICESPEEXFLAGS) $(ASMFLAGS) -c $< -o $@

$(CODECDIR)/libspeex/%.o: $(RBCODEC_DIR)/codecs/libspeex/%.c
	$(SILENT)mkdir -p $(dir $@)
	$(call PRINTS,CC $(subst $(ROOTDIR)/,,$<))$(CC) $(SPEEXFLAGS) -c $< -o $@

$(CODECDIR)/libspeex/%.o: $(RBCODEC_DIR)/codecs/libspeex/%.S
	$(SILENT)mkdir -p $(dir $@)
