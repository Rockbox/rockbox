    ...
    while (!last_metadata) 
    {
        ...
        else if (type == 6) /* 6 is the PICTURE block */
        {
            if(!id3->has_embedded_albumart) /* only use the first PICTURE */
            {
                ...
                mime_length = get_long_be(&picframe_buf[picframe_pos]);
                ...
                description_length  = get_long_be(&picframe_buf[picframe_pos]);
                   ...
                   && (picframe_pos + 4) - picframe_buf_size > 0)
                {
                    ...
                    id3->albumart.size = get_long_be(&picframe_buf[picframe_pos]);
