...
static void surround_buffer_alloc(void)
{
    ...
    char *str[4] = {"dsp_surround_b0","dsp_surround_b2","dsp_surround_bb","dsp_surround_hh"};
    int len[4] = {B0_DLY,B2_DLY,BB_DLY,HH_DLY};
    for (int i = 0; i < 4; i++)
    {
...
{
    b0 = core_get_data(hid[0]);
    b2 = core_get_data(hid[1]);
    bb = core_get_data(hid[2]);
    hh = core_get_data(hid[3]);
}
