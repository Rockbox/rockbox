...
/* Wipe the filter's history (for discontinuities) */
static void tone_flush(struct dsp_proc_entry *this)
{
    struct dsp_filter *data = (void *)this->data;
...
void tone_set_prescale(int prescale)
{
    ...
    struct dsp_filter tone_filter; /* Temp to hold new coeffs */
    ...
    {
        struct dsp_filter *data = &tone_filters[i];

        data->shift = FILTER_BISHELF_SHIFT;

        memcpy(data->coefs, tone_filter.coefs, sizeof (data->coefs));
                         ...
                         struct dsp_buffer **buf_p)
{
    ...
    filter_process(buf->p32, (void *)this->data, buf->remcount,
                   buf->format.num_channels);
}

static void tone_configure(struct dsp_proc_entry *this,
                           struct dsp_config *dsp,
...
const struct dsp_proc_db_entry tone_proc_db_entry =
{
    .id = DSP_PROC_TONE_CONTROLS,
    .configure = tone_configure
};
