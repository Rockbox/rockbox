...
static void pbe_buffer_alloc(void)
{
    ...
    char *str[6] = {"dsp_pbe_b0_0","dsp_pbe_b0_1","dsp_pbe_b2_0",
                    "dsp_pbe_b2_1","dsp_pbe_b3_0","dsp_pbe_b3_1"};
    int len[6] = {B0_SIZE,B0_SIZE,B2_SIZE,B2_SIZE,B3_SIZE,B3_SIZE};
    for (int i = 0; i < 6; i++)
    {
        hid[i] = core_alloc(str[i],sizeof(int32_t) * len[i]);
    }
...
{
    b0[0] = core_get_data(hid[0]);
    b0[1] = core_get_data(hid[1]);
    b2[0] = core_get_data(hid[2]);
    b2[1] = core_get_data(hid[3]);
    b3[0] = core_get_data(hid[4]);
    b3[1] = core_get_data(hid[5]);
}
