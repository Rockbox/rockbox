...
static int32_t dequeue(int32_t* buffer, int *head, int limit)
{
...
static void enqueue(int32_t var, int32_t* buffer, int *head, int limit)
{
    buffer[*head] = var;
    if (buffer[*head+1] >= limit)
        *head = 0;
...
static bool queuefull(int rptr, int wptr, int limit)
{
    if (wptr < limit -1)
        return (rptr == wptr+1);
    else
                        ...
                        struct dsp_buffer **buf_p)
{
    ...
    for(int ch = 0; ch < num_channels; ch++)
    {
        for (int i = 0; i < count; i++)
        {
            ...
            /* below 160hz with delays */
            var = buf->p32[ch][i] -
                  FRACMUL(buf->p32[ch][i], tcoef1);

            temp_buffer += (count < b0_level)?
                           dequeue(b0[ch], &b0_rptr[ch], b0_level): var;

            if (!queuefull(b0_rptr[ch], b0_wptr[ch],b0_level))
                enqueue(var, b0[ch], &b0_wptr[ch],b0_level );

            /* 500-1150hz with delays */
            var = FRACMUL(buf->p32[ch][i], tcoef2) -
                  FRACMUL(buf->p32[ch][i], tcoef3);

            temp_buffer += (count < b2_level)?
                           dequeue(b2[ch], &b2_rptr[ch], b2_level): var;

            if (!queuefull(b2_rptr[ch], b2_wptr[ch],b2_level))
                enqueue(var, b2[ch], &b2_wptr[ch],b2_level );

            /* above 1150 with delays */
            var = FRACMUL(buf->p32[ch][i], tcoef3);

            temp_buffer += (count < B3_DLY)?
                           dequeue(b3[ch], &b3_rptr[ch], B3_DLY): var;

            if (!queuefull(b3_rptr[ch], b3_wptr[ch],B3_DLY))
                enqueue(var, b2[ch], &b3_wptr[ch],B3_DLY );
