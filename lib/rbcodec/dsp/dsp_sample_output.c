                            ...
                            struct dsp_buffer *src, struct dsp_buffer *dst)
{
    ...
    for (unsigned int ch = 0; ch < channels; ch++)
    {
        ...
        void *d = pcm_dma_t_ptr(dst->pout, ch);
        ...
        for (unsigned long i = 0; i < count; i++)
        {
            ...
            /* Clip and store */
            output = pcm_dma_t_clip(output);
            pcm_dma_t_write_postincr(&d, output, 2);
        }
    }
    ...
    pcm_dma_t_advance((const void **)&d, 1, 1);

    do
    {
        int32_t s = pcm_dma_t_read_postincr((const void **)&d, 0);
        pcm_dma_t_write_postincr(&d, s, 2);
    }
    while (--count);
}
