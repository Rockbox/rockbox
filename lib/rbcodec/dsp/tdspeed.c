...
#define out_size        TDSPEED_OUTBUFCOUNT
...
void tdspeed_init(void)
{
    ...
    } bufdefs[4] =
    {
        ...
        { "tdspeed out L", TDSPEED_OUTBUFCOUNT * sizeof(int32_t) },
        { "tdspeed out R", TDSPEED_OUTBUFCOUNT * sizeof(int32_t) },
    };
...
{
    /* Return worst-case */
    return TDSPEED_OUTBUFCOUNT;
}

void tdspeed_doit(struct dsp_data *data, struct dsp_buffer **buf)
{
    int count = dsp_outbuf.remcount;
    struct dsp_buffer *src = *buf;
    struct dsp_buffer *dst = &dsp_outbuf;

    *buf = dst; /* switch to our buffer */

    if (count > 0)
        return; /* there are still lingering samples to process first */

    dst->bufcount = 0;
    dst->p32[0] = outbuf[0];
    dst->p32[1] = outbuf[data->num_channels - 1];

    count = tdspeed_apply(dst->p32, src->p32, src->remcount, 0,
                          &dst->bufcount);
    ...
    if (count > 0)
    {
        /* have output to pass along */
        dst->bufcount = TDSPEED_OUTBUFCOUNT;
        dst->remcount = count;
        dst->dsp_info = 0;
    }
}
