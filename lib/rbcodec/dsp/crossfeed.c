...
{
    /* 1 / (F.Rforward.C) */
    state->coef1 = fp_muldiv(0x7fffffff, 2128, fout);
    /* 1 / (F.Rcross.C) */
    state->coef2 = fp_muldiv(0x7fffffff, 1000, fout);
}
                                           ...
                                           int fout)
{
    ...
    cutoff = fp_muldiv(0xffffffff, cutoff, fout);
    hf_gain -= lf_gain;
                              ...
                              struct dsp_config *dsp)
{
    ...
    int fout = dsp_get_output_frequency();
    ...
    {
        unsigned len = DELAY_LEN(fout);
        state->index_max = state->delay + MIN(len, ARRAYLEN(state->delay));
