...
/**
 *hermite interpolation resampling that introduces a one sample delay because
...
static struct resample_data
{
    ...
    int32_t  last_sample[6]; /* 08h: Last samples for interpolation (L+R) */
                             /* 10h */
...
int herm_resample_resample(struct resample_data *data, struct dsp_buffer *src,
                          struct dsp_buffer *dst);

static void herm_resample_flush_data(struct resample_data *data)
{
    data->phase = 0;
...
static void herm_resample_flush(struct dsp_proc_entry *this)
{
    struct resample_data *data = (void *)this->data;
    data->resample_buf.remcount = 0;
    herm_resample_flush_data(data);
}

static bool herm_resample_new_delta(struct resample_data *data,
    ...
    do
    {
        ...
        if (pos < count)
        {
            while (1)
            {
                ...
                int i = pos;
                if (i < 3) {
                    x3 = (i < 3 ? data->last_sample[i+0] : s[i-3]) ;
                    x2 = (i < 2 ? data->last_sample[i+1] : s[i-2]) ;
                    x1 = (i < 1 ? data->last_sample[i+2] : s[i-1]) ;
                } else {
                    x3 = s[i-3] ;
                    x2 = s[i-2] ;
                    x1 = s[i-1] ;
...
*/				
				...
				//c0 = x2;
				int32_t c1 = (x1-x3)>>1; //c1 = 1/2.0*(x1-x3);
				int32_t c2 = x3 + x1<<1 - (x0+5*x2)>>1; //c2 = x3 - 5/2.0*x2 + 2*x1 - 1/2.0*x0;
				int32_t c3 = ((x0-x3) + 3*(x2-x1))>>1; //c3 = 1/2.0*(x0-x3) + 3/2.0*(x2-x1);
				//acc0 = FRACMUL(FRACMUL(c3*frac+c2),frac+c1),frac);
                ...
                *d++ = acc0;
                ...
                pos = MIN(pos, count);
                data->last_sample[ch*3+0] = (pos < 3 ? data->last_sample[pos+0] : s[pos-3]);
                data->last_sample[ch*3+1] = (pos < 2 ? data->last_sample[pos+1] : s[pos-2]);
                data->last_sample[ch*3+2] = (pos < 1 ? data->last_sample[pos+2] : s[pos-1]); 
 ...
 * the resampled data. */
static void herm_resample_process(struct dsp_proc_entry *this,
                                 struct dsp_buffer **buf_p)
{
    ...
    *buf_p = dst;
    ...
    if (src->remcount > 0)
    {
        ...
        int consumed = herm_resample_resample(data, src, dst);
...
static void herm_resample_new_format(struct dsp_proc_entry *this,
                                    struct dsp_buffer **buf_p)
{
    struct resample_data *data = (void *)this->data;
    struct dsp_buffer *src = *buf_p;
    ...
    if (src->format.frequency != frequency)
    {
        ...
        active = herm_resample_new_delta(data, src);
...
static void herm_resample_init(struct dsp_config *dsp,
                              enum dsp_ids dsp_id)
{
...
/* DSP message hook */
static intptr_t herm_resample_configure(struct dsp_proc_entry *this,
                                       ...
                                       intptr_t value)
{
    ...
    {
    case DSP_INIT:
        herm_resample_init(dsp, (enum dsp_ids)value);
        break;

    case DSP_FLUSH:
        herm_resample_flush(this);
        break;

    case DSP_PROC_INIT:
        this->data = (intptr_t)&resample_data[dsp_get_id(dsp)];
        this->ip_mask = 0; /* Not in-place */
        this->process[0] = herm_resample_process;
        this->process[1] = herm_resample_new_format;
