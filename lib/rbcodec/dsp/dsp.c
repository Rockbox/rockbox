...
#ifdef HAVE_SOFTWARE_TONE_CONTROLS
struct tone_control_state
{
    int prescale;                                   /* A/V */
    int bass;                                       /* A/V */
    int treble;                                     /* A/V */
    struct eqfilter filter;                         /* A/V */
};
...
{
    eq_process(buf, &dsp->tone_filter, count, data->num_channels);
}
...
static void set_tone_controls(void)
{
    ...
    AUDIO_DSP.tone_process = VOICE_DSP.tone_process =
        (bass != 0 || treble != 0) ? tone_process : NULL;
}
#endif /* HAVE_SW_TONE_CONTROLS */
