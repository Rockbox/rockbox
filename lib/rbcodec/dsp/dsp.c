...
struct dsp_config
{
    ...
    uint32_t enable_mask;        /* Mask of enabled stages */
    uint32_t active_mask;        /* Mask of active stages */
    struct dsp_proc_slot *proc_slots[2]; /* Pointer to first in list of stages,
                                            [0] = first active,
                                            [1] = first enabled */
...
{
    const uint32_t proc_mask = BIT_N(id);

    if ((dsp->enable_mask & proc_mask) == 0)
    ...
    while (s != NULL)
    {
        if (BIT_N(s->db_entry->id) == proc_mask)
            break;
...
{
    const uint32_t proc_mask = BIT_N(id);
    bool enabled = dsp->enable_mask & proc_mask;
    ...
    if (enable)
    {
        ...
        if (!enabled)
        {
            /* Enabled list is built in the same order as the database array
               but only includes enabled entries of course */
            ...
            for (unsigned int i = 0; i < DSP_NUM_PROC_STAGES; i++)
            {
                ...
                uint32_t mask = BIT_N(db_entry->id);

                if (mask == proc_mask)
                {
                    struct dsp_proc_slot *prev = NULL;

                    s = &dsp->proc_slot_arr[slot];

                    if (prev_id != -1)
                        prev = find_proc_slot(dsp, prev_id);
                    ...
                    /* Setup some defaults */
                    s->proc_entry.ip_mask = proc_mask;
                    ...
                    dsp->enable_mask |= proc_mask;
                    break;
                }

                if (dsp->enable_mask & mask)
                    prev_id = db_entry->id;
            }
        }
        else
        ...
        /* Call the INIT function each time, indicating if it was already
           enabled */

        if (s != NULL)
        {
            /* Call to construct the new stage */
    ...
    {
        /* Pull from enabled list and from active list if also active */
        struct dsp_proc_slot *s = dsp->proc_slots[1];
        struct dsp_proc_slot *prev = NULL;
        struct dsp_proc_slot *prev_active = NULL;

        while (s != NULL)
        {
            uint32_t mask = BIT_N(s->db_entry->id);
            bool active = dsp->active_mask & mask;
            ...
            if (mask == proc_mask)
            {
                ...
                {
                    if (prev_active)
                        prev_active->next[0] = s->next[0];
                    else
                        dsp->proc_slots[0] = s->next[0];
                }
                ...
                int slot = s - dsp->proc_slot_arr;
                dsp->slot_free_mask |= BIT_N(slot);
                dsp->enable_mask &= ~proc_mask;
                dsp->active_mask &= ~proc_mask;
                break;
            }
                       ...
                       bool activate)
{
    ...
    bool active = dsp->active_mask & proc_mask;
    ...
    if (activate)
    {
        /* Must traverse the enabled list, remembering the last item
           encountered that is active. The next pointer is let be so
           that if a stage deactivates itself, the processing loop
           may still efficiently find the next enabled item. */
        struct dsp_proc_slot *s = dsp->proc_slots[1];
        struct dsp_proc_slot *prev_active = NULL;
        ...
        while (s != NULL)
        {
            ...
            if (mask == proc_mask)
            {
                if (prev_active)
                {
                    s->next[0] = prev_active->next[0];
                    prev_active->next[0] = s;
                }
                else
                {
    ...
    {
        /* Only need to traverse the active list and remove */
        struct dsp_proc_slot *s = dsp->proc_slots[0];
        struct dsp_proc_slot *prev = NULL;
        ...
        while (s != NULL)
        {
            ...
            {
                if (prev)
                    prev->next[0] = s->next[0];
                else
                    dsp->proc_slots[0] = s->next[0]; /* first */
...
{
    return (dsp->active_mask & BIT_N(id)) != 0;
}
                               ...
                               struct dsp_buffer **buf_p)
{
    ...
    for (int i = 0; (dsp = dsp_get_config(i)); i++)
    {
        ...
        {
            /* Call only if the stage is currently active */
