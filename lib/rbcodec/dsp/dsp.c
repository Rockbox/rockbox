...
static const struct dsp_stage
{
    ...
    {
        uint32_t mask; /* Id bit for stage (for in-place tracking) */
        union
        {
            dsp_proc_fn_type fn;   /* Function for stage */
            void (*init_cb)(struct dsp_proc_entry *new);
        };
    } proc_entry;
    uint32_t proc_mask;        /* Id bit flag in dsp->proc_mask */
} dsp_stages[] =
{
    { .proc_entry = { .mask = DSP_PROC_GAIN,
                      { .fn = dsp_apply_gain } },
    ...
    { .proc_entry = { .mask = DSP_PROC_CROSSFEED,
                      { .fn = apply_crossfeed } },
      .proc_mask  = DSP_PROC_CROSSFEED },
    { .proc_entry = { .mask = DSP_PROC_EQUALIZER,
                      { .fn = eq_process } },
      .proc_mask  = DSP_PROC_EQUALIZER },
#ifdef HAVE_SW_TONE_CONTROLS
    { .proc_entry = { .mask = DSP_PROC_TONE_CONTROLS,
                      { .fn = tone_process } },
      .proc_mask  = DSP_PROC_TONE_CONTROLS },
#endif
    /* Determined later */
    { .proc_entry = { .mask = DSP_PROC_MASK_USE_INIT_CB,
                      { .init_cb = channels_process_init_cb } },
      .proc_mask  = DSP_PROC_CHANNELS },
    { .proc_entry = { .mask = DSP_PROC_COMPRESSOR,
...
static void dsp_update_proc_entries(struct dsp_config *dsp)
{
    ...
    for (unsigned i = 0; i < ARRAYLEN(dsp_stages); i++)
    {
        ...
        {
            if (stage->proc_entry.mask == DSP_PROC_MASK_USE_INIT_CB)
                 ...
                 struct dsp_buffer *dst)
{
    ...
    while (src->remcount > 0 && dst->bufcount > 0)
    {
        ...
        if (count != 0)
        {
            ...
            do
            {
                if ((e->mask & proc_mask) == 0)
                {
                    buf->proc_mask |= e->mask;
                    e->fn(&dsp->data, &buf);

                    if (buf->remcount <= 0)
                        goto dsp_process_complete; /* Stage output nothing */
                }
            }
            while (e++, --count > 0);
        }
