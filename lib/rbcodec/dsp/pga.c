...
void pga_set_gain(enum pga_gain_ids id, int32_t value)
{
    ...
    if (BIT_N(id) & pga_data.enabled)
        pga_update(); /* Gain is enabled; update */
}
...
void pga_enable_gain(enum pga_gain_ids id, bool enable)
{
    ...
    if (!(pga_data.enabled & bit) == enable)
    {
...
static void pga_configure(struct dsp_proc_entry *this,
...
const struct dsp_proc_db_entry pga_proc_db_entry =
{
    .id = DSP_PROC_PGA,
    .configure = pga_configure
};
