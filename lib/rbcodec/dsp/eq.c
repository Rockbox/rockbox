...
/* Wipe the filters' histories (for discontinuities) */
static void eq_flush_band(int band)
{
    memset(eq_data.filters[band].history, 0,
           sizeof(eq_data.filters[band].history));
}
...
void dsp_set_eq_coefs(int band, const struct eq_band_setting *setting)
{
    ...
    if (mask & BIT_N(band))
        eq_flush_band(band); /* Coming online, flush band history */
...
void dsp_eq_enable(bool enable)
{
    ...
    pga_enable_gain(PGA_EQ_PRECUT, enable);
