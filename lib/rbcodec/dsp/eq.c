...
static void update_band_filter(int band, int fout)
{
    ...
    coef_gen(fp_muldiv(0xffffffff, setting->cutoff, fout), setting->q ?: 1,
             setting->gain, filter);
}

/* Resync all bands to a new DSP output frequency */
static void update_samplerate(int fout)
{
    if (eq_data.enabled == 0)
        return; /* Not initialized yet/no bands on */
...
void dsp_set_eq_coefs(int band, const struct eq_band_setting *setting)
{
    ...
    {
        mask |= BIT_N(band);
        update_band_filter(band, dsp_get_output_frequency());
    }
                             ...
                             intptr_t value)
{
    ...
    case DSP_PROC_INIT:
        ...
        update_samplerate(dsp_get_output_frequency());
        /* Fall-through */
    case DSP_PROC_CLOSE:
