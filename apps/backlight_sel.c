...
static int activity_context_to_action_context(int activity_context);
...

{
    while(!quit_thread)
    {
            ...
            {
                backlight_on_force();
                //sleep(HZ/2); /*min BL timeout of HZ we can afford to sleep */
            }
...
{
    static long sel_bl_stack[0x1000/sizeof(long)];
    backlight_on();
    //backlight_off();
    if(start)    
    {    
        ...
        create_thread(selective_backlight_thread, sel_bl_stack, 
                            sizeof(sel_bl_stack), 0, "sel backlight thread" IF_PRIO(, PRIORITY_SYSTEM ) IF_COP(, CPU));
                             ...
                             const struct button_mapping* (*get_context_map)(int) )
{
    ...
    int context=activity_context_to_action_context(get_current_activity());
...
static int activity_context_to_action_context(int activity_context)
{
    //21 activity contexts
    static int action_contexts[21]={-1};
    if(action_contexts[0] == -1)
    {    
        /*action_contexts[]=;*/
        action_contexts[ACTIVITY_UNKNOWN]=CONTEXT_STD;
        action_contexts[ACTIVITY_MAINMENU]=CONTEXT_MAINMENU;
        action_contexts[ACTIVITY_WPS]=CONTEXT_WPS;
        action_contexts[ACTIVITY_RECORDING]=CONTEXT_RECORD;
        action_contexts[ACTIVITY_FM]=CONTEXT_FM;
        action_contexts[ACTIVITY_PLAYLISTVIEWER]=CONTEXT_LIST;
        action_contexts[ACTIVITY_SETTINGS]=CONTEXT_SETTINGS;
        action_contexts[ACTIVITY_FILEBROWSER]=CONTEXT_TREE;
        action_contexts[ACTIVITY_DATABASEBROWSER]=CONTEXT_TREE;
        action_contexts[ACTIVITY_PLUGINBROWSER]=CONTEXT_TREE;
        action_contexts[ACTIVITY_QUICKSCREEN]=CONTEXT_QUICKSCREEN;
        action_contexts[ACTIVITY_PITCHSCREEN]=CONTEXT_PITCHSCREEN;
        action_contexts[ACTIVITY_OPTIONSELECT]=CONTEXT_STD;
        action_contexts[ACTIVITY_PLAYLISTBROWSER]=CONTEXT_LIST;
        action_contexts[ACTIVITY_PLUGIN]=CONTEXT_STD;
        action_contexts[ACTIVITY_CONTEXTMENU]=CONTEXT_STD;
        action_contexts[ACTIVITY_SYSTEMSCREEN]=CONTEXT_STD;
        action_contexts[ACTIVITY_TIMEDATESCREEN]=CONTEXT_SETTINGS_TIME;
        action_contexts[ACTIVITY_BOOKMARKSLIST]=CONTEXT_BOOKMARKSCREEN;
        action_contexts[ACTIVITY_SHORTCUTSMENU]=CONTEXT_LIST;
        action_contexts[ACTIVITY_ID3SCREEN]=CONTEXT_ID3DB;
        action_contexts[ACTIVITY_USBSCREEN]=CONTEXT_USB_HID;
           
    }
return action_contexts[activity_context];
}
