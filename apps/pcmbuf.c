...
#define PCMBUF_FRAME_SIZE   PCM_DMA_T_FRAME_SIZE
...
#define PCMBUF_CHUNK_SIZE    (2048*PCMBUF_FRAME_SIZE)
...
#define BYTERATE            (pcmbuf_sampr * PCMBUF_FRAME_SIZE)
...
static unsigned int pcmbuf_sampr = PLAY_SAMPR_DEFAULT;
#else
...
{
    return pcm_dma_t_amplify(s, faderp->factor, MIXFADE_UNITY_BITS);
}
                               ...
                               unsigned long elapsed, off_t offset)
{
    ...
    while (size)
    {
        ...
        if (alloced)
        {
            ...
            {
                int32_t left = pcm_dma_t_read_postincr(&input_buf, 1);
                int32_t right = pcm_dma_t_read_postincr(&input_buf, 1);

                left = mixfade_sample(faderp, left);
                right = mixfade_sample(faderp, right);

                pcm_dma_t_write_postincr(&outbuf, left, 1);
                pcm_dma_t_write_postincr(&outbuf, right, 1);
        ...
        else if (input_buf)
        {
            ...
            for (size_t s = amount; s != 0; s -= PCMBUF_FRAME_SIZE)
            {
                ...
                int32_t left  = pcm_dma_t_read_postincr(&input_buf, 1);
                int32_t right = pcm_dma_t_read_postincr(&input_buf, 1);
                int32_t leftout = pcm_dma_t_read(outbuf, 0);
                int32_t rightout = pcm_dma_t_read(outbuf, 1);

                left = pcm_dma_t_mix_clip(leftout, left, 0);
                left = mixfade_sample(faderp, left);

                right = pcm_dma_t_mix_clip(rightout, right, 0);
                right = mixfade_sample(faderp, right);
        ...
        else
        {
            ...
            for (size_t s = amount; s != 0; s -= PCMBUF_FRAME_SIZE)
            {
                int32_t left = pcm_dma_t_read(outbuf, 0);
                int32_t right = pcm_dma_t_read(outbuf, 0);
                left = mixfade_sample(faderp, left);
                right = mixfade_sample(faderp, right);
                pcm_dma_t_write_postincr(&outbuf, left, 1);
                pcm_dma_t_write_postincr(&outbuf, right, 1);
