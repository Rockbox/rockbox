...

{
    ...
    unsigned long type[NUM_VOLUMES];
};
...
static void refresh_data(struct info_data *info)
{
    int i;
    for (i = 0 ; CHECK_VOL(i) ; i++) {
	volume_size(IF_MV(i,) &info->size[i], &info->free[i]);
	info->type[i] = LANG_DISK_NAME_INTERNAL;
#ifdef HAVE_MULTIDRIVE
	if (volume_drive(i) > 0) /* -1 == Not existing, 0 == internal */
		info->type[i] = LANG_DISK_NAME_MMC;
#endif
    }
    info->new_data = false;
}
                                ...
                                char *buffer, size_t buffer_len)
{
    ...
    switch (selected_item)
    {
		...
		snprintf(buffer, buffer_len, "%s %s/%s", str(info->type[i]),
			 s1, s2);
#ifdef HAVE_MULTIVOLUME
	    } else {
		snprintf(buffer, buffer_len, "%s %s", str(info->type[i]),
...
static int info_speak_item(int selected_item, void * data)
{
    ...
    switch (selected_item)
    {
	    ...
	    i = selected_item - INFO_DISK1;
	    if (info->size[i]) {
		talk_ids(false, info->type[i], LANG_DISK_FREE_INFO);
...
static int show_info(void)
{
    ...
    int count = INFO_COUNT + NUM_VOLUMES - 1; // XXX be smarter, but how to distinguish between "empty slot" vs "unused volume entry"?
