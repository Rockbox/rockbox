...
/* BT Flags */
#define BT_PIN_REQUIRED 0x01
#define BT_PIN_SUPPLIED 0x02
#define BT_AUTO_CONNECT 0x04
// #define BT_ERRORS?
...
static int simplelist_action_callback(int action, struct gui_synclist *lists)
{
            ...
            (action == ACTION_STD_OK && item != BT_CMENU_BACK))
    {
        ...
        (action == ACTION_STD_CANCEL || item == BT_CMENU_BACK))
    {
        ...
        return ACTION_NONE;
            ...
            (action == ACTION_STD_OK && item != BT_PMENU_BACK))
    {
        ...
        else /*error*/
        {
            ...
            (action == ACTION_STD_CANCEL || item == BT_PMENU_BACK))
    {
...
static char *bt_menu_item_get_name(int selected_item, void *data, char *buffer, size_t len)
{
    ...
    {
        struct bluetooth_device *device = &bt_device_buf[last_device];
        snprintf(buffer, len, " %s [%s]", device->name, device->address);
        ...
        switch (item)
        {
            ...
            case BT_PMENU_AUTO_FMT:
                ...
                str((device->flags & BT_AUTO_CONNECT) ? LANG_SET_BOOL_YES: LANG_SET_BOOL_NO));
                return buffer;
            case BT_PMENU_BLANK:
                return "";
        }
