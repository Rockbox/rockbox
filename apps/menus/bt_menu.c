...
enum bt_menus {BT_MENU_SCAN = 0, BT_MENU_TRUSTED, BT_MENU_PROFILE, BT_MENU_CONNECT};
...
static int simplelist_action_callback(int action, struct gui_synclist *lists)
{
    ...
    else if(menu_param == BT_MENU_CONNECT && action == ACTION_STD_OK)
    {
    ...
    else if(menu_param == BT_MENU_CONNECT && action == ACTION_STD_CANCEL)
    {
        int items = BT_DEVICE_SCAN_MAX_DEVICES;
        bluetooth_scan(bt_device_buf, &items);
        bt_menu_change(lists, str(LANG_BLUETOOTH), 1, 0, BT_MENU_SCAN, items);
    ...
    else if(menu_param == BT_MENU_PROFILE && action == ACTION_STD_OK)
    {
        ...
        {
            bt_menu_change(lists, str(LANG_BLUETOOTH_CONNECT), 1,
                           BT_CMENU_PROFILE, BT_MENU_CONNECT, BT_MENU_CONNECT_ITEMS);
            return ACTION_NONE;
        }
    }
    else if(menu_param == BT_MENU_PROFILE && action == ACTION_STD_CANCEL)
    {
        bt_menu_change(lists, str(LANG_BLUETOOTH_CONNECT), 1,
                       BT_CMENU_PROFILE, BT_MENU_CONNECT, BT_MENU_CONNECT_ITEMS);
        return ACTION_NONE;
...
static char *bt_menu_item_get_name(int selected_item, void *data, char *buffer, size_t len)
{
    ...
    {
        return bt_menu_connect[selected_item].desc;
    }
    else if(menu_param == BT_MENU_PROFILE)
    {
        switch (item)
        {
            ...
            case BT_PMENU_ADDRESS_FMT:
                snprintf(buffer, len, " %s", "00:00:00:00:00:00");
                return buffer;
            case BT_PMENU_PIN_FMT:
                snprintf(buffer, len, " %s", "____");
                return buffer;
            case BT_PMENU_AUTO_FMT:
                snprintf(buffer, len, " %s", "NO");
                return buffer;
            case BT_PMENU_BLANK:
                return "";
        }
        ...
        return bt_menu_profile[selected_item].desc;
    }
    ...
    while (true)
    {
        ...
        switch (info.selection)
        {
            ...
            case 1:
                break;
            case 2:
                menu_param = BT_MENU_CONNECT;
                info.selection = 0;
                info.count = BT_MENU_CONNECT_ITEMS;
                continue;
            case 3:
                break;
        }
#endif
        //bt_menu_item_get_name(info.selection, &menu_param, title, MAX_PATH);
...
MAKE_MENU(bluetooth_audio_menu, ID2P(LANG_BLUETOOTH), NULL, Icon_Audio,
        &bt_enable, &bt_browse, &bt_scanned, &bt_save);
#else
MAKE_MENU(bluetooth_audio_menu, ID2P(LANG_BLUETOOTH), NULL, Icon_Audio,
        &bt_enable, &bt_scan);
#endif
