...
static int play_frequency_func(void)
{
    ...
    int freq = ARRAYLEN(names) - 1 - global_settings.play_frequency;
    bool ret = set_option(str(LANG_PLAYBACK_FREQUENCY),
                          &freq, INT, names, ARRAYLEN(names), NULL );

    if (!ret)
    {
        freq = ARRAYLEN(names) - 1 - freq;
        if (freq != global_settings.play_frequency)
        {
            global_settings.play_frequency = freq;
            settings_apply_play_freq(freq);
        }
    }
