    ...
    for (band = 0; band < EQ_NUM_BANDS; band ++) {
        ...
        if (mode == GAIN)
            draw_slider_bar(screen, eq_data->band.x, eq_data->band.y[band] + eq_data->slider.y,
                           ...
                           min_item, max_item);
        else /* for Q/CUTOFF draw a blank bar. Better use dec/inc buttons than slider */
            screen->drawrect(eq_data->band.x, eq_data->band.y[band] + eq_data->slider.y,
                           eq_data->band.width, eq_data->slider.height);
    }
    return;
}
                           ...
                           struct eq_screen_data  *eq_data)
{
    ...
    eq_data->band.x = btn_bar->cancel.width + BUTTON_MARGIN;
    eq_data->band.width = vp->width - btn_bar->cancel.width * 2 - BUTTON_MARGIN * 2;
