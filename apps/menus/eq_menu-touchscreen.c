...
#define RIGHT_EDGE 2
    ...
    screen->set_foreground(LCD_RGBPACK(255, 205, 54));
    screen->fillrect(inner_x, inner_y + inner_ht /3, start - inner_x, inner_ht/3);

    screen->set_foreground(LCD_RGBPACK(136, 136, 136));
    screen->fillrect(start+size, inner_y + inner_ht /3, inner_wd - start + inner_x - size, inner_ht/3);

    screen->set_foreground(LCD_RGBPACK(45, 45, 45));
    screen->fillrect(inner_x, inner_y + inner_ht / 3 - 1, inner_wd, 1);
    screen->fillrect(inner_x, inner_y + inner_ht / 3 * 2, inner_wd, 1);
    screen->fillrect(inner_x, inner_y + inner_ht / 3, 1, inner_ht / 3);
    screen->fillrect(inner_x + inner_wd, inner_y + inner_ht / 3, 1, inner_ht / 3);
    /* draw slider */
    screen->set_foreground(LCD_RGBPACK(45, 45, 45));
    screen->fillrect(start, inner_y, size, inner_ht);
    screen->set_foreground(fg);
    screen->fillrect(start + 2, inner_y + 2, size - 4, inner_ht - 4);
    screen->set_foreground(fg);
                         ...
                         int x, int y, int width, int height)
{
    ...
    screen->set_foreground(fg);
    screen->drawrect(x+1, y+1, width-2, height-2);
    /* draw button text */
                            ...
                            struct button_bar *btn_bar)
{
    ...
    draw_button(screen, buf, btn_bar->inc.x, btn_bar->height + BUTTON_MARGIN,
                        btn_bar->inc.width, vp->height - btn_bar->height - BUTTON_MARGIN * 2);
    /*dec button*/
    snprintf(buf, sizeof(buf), " - ");
    draw_button(screen, buf, btn_bar->dec.x, btn_bar->height + BUTTON_MARGIN,
                        btn_bar->dec.width, vp->height - btn_bar->height - BUTTON_MARGIN * 2);
    return;
}
                           ...
                           struct eq_screen_data  *eq_data)
{
    ...
    screen->getstringsize(buf, &w, &h);
    btn_bar->dec.x = 0;
    btn_bar->dec.width = w2;

    /*inc button*/
    snprintf(buf, sizeof(buf), " + ");
    screen->getstringsize(buf, &w, &h);
    btn_bar->inc.width = w2;
    ...
    eq_data->band.x = w2 + BUTTON_MARGIN;
    eq_data->band.width = vp->width - w2 * 2 - BUTTON_MARGIN * 2;
...
bool eq_menu_graphical(void)
{
    ...
    vp[SCREEN_MAIN].width -= EQ_OUTER_MARGIN;
            ...
            {   /* convert touch into VP coordinates */
                        ...
                        do {  /* find out which item and band has been touched */
                                    ...
                                    eq_data.slider.y + eq_data.slider.height)))
                            {
                                ...
                                if (touch_y <= eq_data.band.y[selected_band] +
                                                        eq_data.slider.y)
                                {
                                    no_slider_change = true;
                                    if  (button_touchscreen & BUTTON_REL)
                                    {
                                        if (touch_x <= vp[SCREEN_MAIN].width / 3)
                                            mode = GAIN;
                                        else if (touch_x <= vp[SCREEN_MAIN].width * 2 / 3)
                                            mode = CUTOFF;
                                        else
                                            mode = Q;
                                    }
                                }
                                else if (mode == CUTOFF || mode == Q)  /* disable slider */
                                    no_slider_change = true;
                            }
                            else
                            {
                ...
                if (slider_touched || slider_change)
                {
                    if (!(slider_change))
                    {
                        ...
                        else if (slider_touched && !no_slider_change) {
                            slider_change = true;
                        }
                    }
