                                ...
                                int val, const char *unit)
{
  ...
  case SLEEPTIMER_STOP:
    return str(LANG_STOP);
  case SLEEPTIMER_CONTINUE: {
    if (get_sleep_timer()) {
      snprintf(buffer, buffer_size, "%s %d %s\n", str(LANG_CONTINUE), seconds_to_min(get_sleep_timer()), unit);
    }
    else {
  ...
  case SLEEPTIMER_START: 
    {
      if (get_sleep_timer()) {
        snprintf(buffer, buffer_size, "%s %d %s\n", str(LANG_RESTART), get_sleeptimer_duration(), unit);
        }
      else {
        snprintf(buffer, buffer_size, "%s %d %s\n", str(LANG_START), get_sleeptimer_duration(), unit); break;
      }
      break;
    }
  }
