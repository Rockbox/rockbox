...
static void load_initial_clips(int fd)
{
    ...
    for(index = id = 0; index < num_clips; index++)
    {
        int handle;
...
static void load_voicefile_data(int fd)
{
...
void talk_init(void)
{
    ...
    has_voicefile = audio_buffer_available() >= voicefile_size;

    if (!create_clip_buffer(voicefile_size))
        goto out;
