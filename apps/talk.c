...
#define MAX_CLIP_BUFFER_SIZE (1<<30) /* 1GB should be enough for everybody */
#endif
...
/* most, if not all, clips should be well below 8k (but even if not, clips
 * can be split for a tiny performance penalty) */
static unsigned char commit_buffer[8<<10];
...
static void load_initial_clips(int fd)
{
    ...
    for(index = id = 0; index < max_clips; index++)
    {
        int handle;
        ...
        add_cache_entry(handle, index, index2id(index));
    }
#endif
}
...
static int get_clip(long id, struct queue_entry *q)
{
    ...
    {   /* clip is in memory already; find where it was loaded */
        ...
        for (i = 0; cc[i].voice_id != id || !cc[i].handle; i++)
            printf("Foo %d %d %d %d\n", i, id, cc[i].voice_id, cc[i].handle);
...
static bool load_voicefile_data(int fd, size_t max_size)
{
    ...
    if (talk_handle < 0)
        goto load_err_free10;

    buflib_init(&clip_ctx, core_get_data(talk_handle), voicefile_size);
...
load_err_free20:
    metadata_table_handle = core_free(metadata_table_handle);
load_err_free10:
    index_handle = core_free(index_handle);
    return false;
}
...
static inline void commit_transfer(struct queue_entry *qe)
{
    void *buf = NULL; /* shut up gcc */
    int offset = qe->offset;
#if CONFIG_CODEC != SWCODEC
    sent = MIN(qe->length, 0xFFFF);
#else
    sent = qe->length;
#endif
...
static void mp3_callback(const void** start, size_t* size)
{
    ...
    {   /* feed the next 64K-1 chunk */
        commit_transfer(&queue[queue_read]);
        *start = commit_buffer;
        *size = sent;
        return;
    }
...
re_check:
    ...
    else if (silence.offset > 0               /* silence clip available */
             && last_clip->offset != silence.offset
             && last_clip->type == silence.type   /* previous clip wasn't silence */
             && last_clip->type != THUMB_OFFSET)  /*.or thumbnail */
...
static void queue_clip(struct queue_entry *clip, bool enqueue)
{
    ...
    {   /* queue was empty, we have to do the initial start */
        last_clip = qe;
        commit_transfer(qe);
        mp3_play_data(commit_buffer, sent, mp3_callback);
        curr_hd[0] = commit_buffer[1];
        curr_hd[1] = commit_buffer[2];
        curr_hd[2] = commit_buffer[3];
...
void talk_init(void)
{
    ...
    /* after the load_voicefile_index() call the file postion is at the start
     * of the audio data */
    total_size = lseek(filehandle, 0, SEEK_END);
    /* to not let empty clips turn down the average */
    avg_size = total_size / (num_clips - empty_clips);

    max_clips = MIN((int)(MAX_CLIP_BUFFER_SIZE/avg_size) + 1, num_clips);
    ...
    voicefile_size = total_size + sizeof(struct clip_cache_metadata) * max_clips;
    if (voicefile_size > MAX_CLIP_BUFFER_SIZE)
    ...
    load_voicefile_data(filehandle, voicefile_size);
                      ...
                      const long *prefix_ids, bool enqueue)
{
    ...
    if (size > 0)    /* Don't play missing clips */
    {
        ...
        talk_queue_unlock();
        clip = (struct queue_entry){ .offset = thumb_used, .length = size, .type = THUMB_OFFSET };
...
bool talk_get_debug_data(struct talk_debug_data *data)
{
    ...
    data->memory_usage = sizeof(commit_buffer) + sizeof(voicefile)
