...
static bool load_voicefile(int fd, bool index_only)
{
     ...
     * voicefile_size has been set before after a call to this with
     * index_only == true */
    talk_handle = core_alloc_ex("voice data", voicefile_size, &talk_ops);
    if (talk_handle < 0)
        goto load_err_free;
#else
    ...
    load_size  = voicefile_size - voicefile.table - clips_size;
...
#endif
        *start = core_get_data(talk_handle) + queue[queue_read].offset;
        *size = sent;
        return;
    }
    talk_queue_lock();
...
void talk_init(void)
{
     ...
     * at once */
    if (!load_voicefile(filehandle, true))
    {
        voicefile_size = 0;
        goto out;
    }
    ...
    if (!load_voicefile(filehandle, false))
    {
        voicefile_size = 0;
        goto out;
    }
...
int talk_id(int32_t id, bool enqueue)
{
    ...
    if (has_voicefile && (talk_handle <= 0 || index_handle <= 0)) /* reload needed? */
    {
        ...
        if (fd < 0 || !load_voicefile(fd, false))
            return -1;
    }
