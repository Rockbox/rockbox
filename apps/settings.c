...
void settings_applied_on_powerup (void ) {
  settings_apply(true);
  if ( global_settings.sleeptimer_on_powerup == true ) {
    /* if sleeptimer is SLEEPTIMER_ON, it has been started in settings_apply by now 
       but continue failed since that value is not saved (yet)? */
    if ( global_settings.sleeptimer == SLEEPTIMER_START )
      sleep_timer_start(SLEEPTIMER_START); 
  }
  /* if on_powerup is false or  sleeptimer is OFF give it preference */
  else sleep_timer_start(SLEEPTIMER_STOP); 
