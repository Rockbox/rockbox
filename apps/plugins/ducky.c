...
bool validVariable(char c)
{
    if(!isDigit(c) && !getop(c) && !isSpace(c))
    {
        vid_writef("%c is a valid var %d", c, variables[(int)c]);
        return true;
    }
    vid_writef("%c isn't valid", c);
    return false;
}
        ...
        do {
            ...
            else if(TOKEN_IS("IF"))
            {
                ...
                if(isValidNumber(tok))
                    ...
                    left_value = variables[(int)tok[0]];
                else
                    error("invalid left-hand operand for IF");
                ...
                if(isValidNumber(tok))
                    ...
                    right_value = variables[(int)tok[0]];
                else
            ...
            else if(TOKEN_IS("OUT"))
            {
                ...
                if(tok && rb->strlen(tok) == 1)
                {
                    ...
                    if(!validVariable(var))
                        error("invalid variable %c", var);
                    char str[32];
                    rb->snprintf(str, sizeof(str), "%d", variables[(int)var]);
                    send_string(str);
                }
                else
            ...
            else if(TOKEN_IS("SET"))
            {
                ...
                if(varname && rb->strlen(varname) == 1 && validVariable(varname[0]))
                {
                    char var = varname[0];
            ...
            {
                tok = rb->strtok_r(NULL, " \t", &save);
                if(tok && rb->strlen(tok) == 1 && validVariable(tok[0]))
                {
