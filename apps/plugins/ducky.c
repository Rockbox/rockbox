 ...
 *
 * "SET X <EXPR>" - loads the value of <EXPR> into variable X
 *                  <EXPR> should have it's tokens separated by whitespace, and
 *                  use a _ character to denote unary minus (negation). Greedy.
 * "SWP X Y" - X <-> Y
 *
 ...
 * "LOGVAR X" - outputs variable X in decimal to the device's screen
 ******************************************************************************/
...
void vid_writef(const char*, ...) __attribute__((format(printf,1,2)));
...
const struct op_s {
    ...
    {"(", 1, ASSOC_NONE, 0, NULL},
    {")", 1, ASSOC_NONE, 0, NULL},
    {"==",0, ASSOC_LEFT, 0, eval_eq},
    {"!=",0, ASSOC_LEFT, 0, eval_neq},
    {"<=",0, ASSOC_LEFT, 0, eval_leq},
    {">=",0, ASSOC_LEFT, 0, eval_geq},
    {"<", 0, ASSOC_LEFT, 0, eval_lt},
    {">", 0, ASSOC_LEFT, 0, eval_gt},
    {"!", 0, ASSOC_LEFT, 1, eval_logical_neg},
};

const struct op_s *getop(const char *ch, int *len)
{
    unsigned int i;
    vid_writef("getop %s: %c", ch, ch[0]);
    for(i=0; i<ARRAYLEN(ops); ++i) {
        if(rb->strncmp(ch, ops[i].op, rb->strlen(ops[i].op)) == 0)
        {
            vid_writef("found %s", ops[i].op);
            if(len)
                *len = rb->strlen(ops[i].op);
            return ops + i;
        }
    }
    vid_writef("not found");
    return NULL;
}
...
{
    vid_writef("pushing op %s on stack", op->op);
...
{
    vid_writef("popping op from stack");
    if(!nopstack) {
...
{
    vid_writef("pushing %d on stack", num);
...
{
    vid_writef("popping from numstack");
    if(!nnumstack) {
...
bool isValidVariable(const char *c)
{
    ...
    {
        vid_writef("%c is a valid var", c[0]);
        return true;
    }
    vid_writef("%c is not a valid var", c[0]);
    return false;
}
...
int getValue(const char *str)
{
    ...
    vid_writef("returing atoi %d", rb->atoi(str));
    return rb->atoi(str);
}
...
bool isValidNumberOrVariable(const char *c)
{
    ...
    {
        vid_writef("%s is a valid number or var", c);
        return true;
    }
    vid_writef("%s is not a valid number or var", c);
    return false;
}

void shunt_op(const struct op_s *op)
{
    vid_writef("shunting op %s", op->op);
    ...
    if(op->assoc==ASSOC_RIGHT) {
        ...
        vid_writef("op %s has assoc of left", op->op);
        while(nopstack && op->prec<=opstack[nopstack-1]->prec) {
            vid_writef("popping from opstack...");
            pop=pop_opstack();
            n1=pop_numstack();
            if(pop->unary) push_numstack(pop->eval(n1, 0));
            else {
    ...
    vid_writef("about to push %s", op->op);
    push_opstack(op);
}
            ...
            if((op=getop(expr, &len))){
                if(lastop && (lastop==&startop || rb->strcmp(lastop->op, ")") == 0)) {
                    if(rb->strcmp(op->op, "-") == 0) op=getop("_", &len);
                        ...
                        error("illegal use of binary operator (%c)", op->op);
                    }
                }
                shunt_op(op);
                lastop=op;
    ...
    while(1)
    {
        ...
        do {
            ...
            else if(TOKEN_IS("OUT"))
            {
                ...
                if(tok && rb->strlen(tok) == 1)
                {
                    if(!isValidVariable(tok))
                        error("invalid variable %c", tok[0]);
                    char str[32];
                    rb->snprintf(str, sizeof(str), "%d", variables[(int)tok[0]]);
                    send_string(str);
                }
                else
                    error("expected valid variable name");
            }
            else if(TOKEN_IS("SWP"))
            {
                char a, b;
                tok = rb->strtok_r(NULL, " \t", &save);
                if(tok && rb->strlen(tok) == 1)
            ...
            else if(TOKEN_IS("SET"))
            {
                ...
                {
                    char var = varname[0];
                    char *expr = rb->strtok_r(NULL, "", &save);
                    variables[(int)var] = eval_expr(expr);
                }
                else
                {
            ...
            {
                tok = rb->strtok_r(NULL, " \t", &save);
                if(tok && rb->strlen(tok) == 1 && isValidVariable(tok))
                {
                    vid_writef("%d", variables[(int)tok[0]]);
                }
                else
