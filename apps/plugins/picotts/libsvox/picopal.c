...
/* needed for PICO_PLATFORM definition */
#include "picopltf.h"
#if (PICO_PLATFORM == PICO_ROCKBOX)
/* !!!! WARNING !!!!
 * plugin.h includes <stdio.h>
 * since we redefine some stuff in stdio_compat.h
 * it must be included AFTER regular <stdio.h>
 * so plugin.h MUST be included at the very top
 */
#include "plugin.h"
#define IMPLEMENT_TIMER 0
#define atoi(s) rb->atoi(s)
#define strcmp(s1,s2) rb->strcmp(s1,s2)
#define strncmp(s1,s2,l) rb->strncmp(s1,s2,l)
#define strlen(s) rb->strlen(s)
#define strchr(s,c) rb->strchr(s,c)
#define strcat(s1,s2) rb->strcat(s1,s2)
#define strcpy(d,s) rb->strcpy(d,s)
#define vsnprintf(b,s,f,ap) rb->vsnprintf(b,s,f,ap)
#define memmove(d,s,l) rb->memmove(d,s,l)
#define memset(d,b,l) rb->memset(d,b,l)
#endif
...
picopal_int16 picopal_snprintf(picopal_char * dst, picopal_objsize_t siz, const picopal_char *fmt, ...)
{
    picopal_int16 i;
    va_list args;

    va_start(args, (char *)fmt);
    i = (picopal_int16)vsnprintf((char *) dst, siz, (const char *)fmt, args);
    va_end(args);
    return i;
}
        ...
        if (*f == '%') {
            switch (*(++f)) {
                case 'i':
                    ...
                    picopal_snprintf(buf,sizeof(buf),(picopal_char *)"%i",ival);
                    b = buf;
                    break;
                case 'c':
                    f++;
                    cval = va_arg(args,int);
                    picopal_snprintf(buf,sizeof(buf),(picopal_char *)"%c",cval);
                    b = buf;
                    break;
                case 's':
                    f++;
...
picopal_uint32 picopal_flength (picopal_File stream)
{
    ...
    picopal_fseek(stream,0,PICOPAL_SEEK_END);
