...
static volatile uint32_t curr_txtchunk = 0;
...
static uint32_t *pcmbuf_guard;

ssize_t pcm_limit_wm(void)
{
    return PCMOUT_BUFSIZE - PCMOUT_HIGH_WM;
}

static void dbg_check_guard(void)
{
    if (*pcmbuf_guard != 0xdeadbeef)
        DEBUGF("!!!!!!!!! GUARD OVERWRITTEN !!!!!!!!!!!\n");
}
...
/* Return physical space free - backbuffer */
...
static void get_more(const void **start, size_t *size)
{
    ...
    if (sz > pcmbuf_threshold)
    {
        ...
        if (head->txtchunk != curr_txtchunk)
        {
            DEBUGF("Speak txtchunk: %d\n", head->txtchunk);
            curr_txtchunk = head->txtchunk;
        }
...
/* Commit the buffer returned by pcm_ouput_get_buffer */
...
bool pcm_output_init(void *mem_ptr, size_t *size)
{
    ...
    size_t align_size = (*size/PCMOUT_CHUNK)*PCMOUT_CHUNK;
    if (align_size + 4 < *size)
        align_size -= PCMOUT_CHUNK;

    pcmbuf_end  = SKIPBYTES(pcm_buffer, align_size - PCMOUT_CHUNK);

    // DEBUG GUARD
    pcmbuf_guard = (uint32_t *)SKIPBYTES(pcmbuf_end, PCMOUT_CHUNK);
    *pcmbuf_guard = 0xdeadbeef;
