...
static void write_setting(const struct settings_list *setting, int fd, unsigned int group)
{
    ...
    {
        case F_T_CUSTOM:
            strcpy(text, "No information available. Check the manual for valid values");
            break;
        case F_T_INT:
        case F_T_UINT:
            if (setting->flags&F_RGB)
                strcpy(text, "RGB colour value in the form RRGGBB (in hexadecimal)");
            else if (setting->flags&F_T_SOUND)
                snprintf(text, sizeof(text), "Min: %d, Max %d",
                         sound_min(setting->sound_setting->setting),
                         sound_max(setting->sound_setting->setting)); 
            else if (setting->flags&F_TABLE_SETTING)
            {
                char temp[8];
                int i;
                snprintf(text, sizeof(text), "Setting allows the following values%s: ",
            ...
            else if (setting->flags&F_INT_SETTING)
            {
                ...
                snprintf(text, sizeof(text), "Min: %d, Max: %d, Step size: %d",
                         min, max, step);
            }
            ...
            {
                snprintf(text, sizeof(text), "%s", setting->cfg_vals);
            }
            break; /* F_T_*INT */
        case F_T_BOOL:
            snprintf(text, sizeof(text), "on, off");
            break;
        case F_T_CHARPTR:
        case F_T_UCHARPTR:
            snprintf(text, sizeof(text), "Text.. Up to %d characters. ",
                         setting->filename_setting->max_len-1);
            if (setting->filename_setting->prefix &&
                setting->filename_setting->suffix)
            {
                strcat(text, "Should start with:\"");
                strcat(text, setting->filename_setting->prefix);
                strcat(text, "\" and end with:\"");
                strcat(text, setting->filename_setting->suffix);
                strcat(text, "\"");
            }
            break;
    }
                                ...
                                const void* parameter)
{
    ...
    if (fd < 0)
        return PLUGIN_ERROR;
    list = get_settings_list(&setting_count);
    fdprintf(fd, "# .cfg file created by rockbox %s - "
            "http://www.rockbox.org\r\n\r\n", rbversion);
    
    fdprintf(fd, "# -- Sound settings -- #\r\n");
