...
static bool file_properties(char* selected_file)
{
    ...
    strlcpy(tstr, selected_file, dirlen + 1);
    ...
    if (dir)
    {
        while(0 != (entry = readdir(dir)))
        {
            ...
            if(!strcmp(entry->d_name, selected_file+dirlen))
            {
                unsigned log;
                snprintf(str_dirname, sizeof str_dirname, "%s", tstr);
                snprintf(str_filename, sizeof str_filename, "%s",
                             selected_file+dirlen);
                log = human_size_log((unsigned long)info.size);
                snprintf(str_size, sizeof str_size, "%lu %cB",
                ...
                snprintf(str_time, sizeof str_time, "%02d:%02d",
                ...
                int fd = open(selected_file, O_RDONLY);
                if (fd >= 0 &&
                    get_metadata(&id3, fd, selected_file))
#else
                if (!mp3info(&id3, selected_file))
#endif
                {
                    long dur = id3.length / 1000;           /* seconds */
                    snprintf(str_artist, sizeof str_artist,
                                 "%s", id3.artist ? id3.artist : "");
                    snprintf(str_title, sizeof str_title,
                                 "%s", id3.title ? id3.title : "");
                    snprintf(str_album, sizeof str_album,
                    ...
                    {
                        if (dur < 3600)
                            snprintf(str_duration, sizeof str_duration,
                                         "%d:%02d", (int)(dur / 60),
                                         (int)(dur % 60));
                        else
                            snprintf(str_duration, sizeof str_duration,
...
static bool _dir_properties(DPS* dps)
{
    ...
    result = true;
    dirlen = strlen(dps->dirname);
    dir = opendir(dps->dirname);
    if (!dir)
    ...
    while(result && (0 != (entry = readdir(dir))))
    {
        ...
        snprintf(dps->dirname+dirlen, dps->len-dirlen, "/%s",
                     entry->d_name);
        ...
        if (info.attribute & ATTR_DIRECTORY)
        {
            ...
            if (current_tick - lasttick > (HZ/8))
            {
                lasttick = current_tick;
                lcd_clear_display();
                lcd_puts(0,0,"SCANNING...");
                lcd_puts(0,1,dps->dirname);
                lcd_puts(0,2,entry->d_name);
                lcd_putsf(0,3,"Directories: %d", dps->dc);
                lcd_putsf(0,4,"Files: %d", dps->fc);
                log = human_size_log(dps->bc);
                lcd_putsf(0,5,"Size: %lu %cB", (unsigned long)(dps->bc >> (10*log)),
...
static bool dir_properties(char* selected_file)
{
    ...
    strlcpy(dps.dirname, selected_file, MAX_PATH);
    ...
    strlcpy(str_dirname, selected_file, MAX_PATH);
    snprintf(str_dircount, sizeof str_dircount, "%d", dps.dc);
    snprintf(str_filecount, sizeof str_filecount, "%d", dps.fc);
    log = human_size_log(dps.bc);
    snprintf(str_size, sizeof str_size, "%ld %cB",
                              ...
                              char *buffer, size_t buffer_len)
{
    ...
    if(its_a_dir)
    {
        ...
        {
            strlcpy(buffer, props_dir[selected_item], buffer_len);
        }
    }
    else
    {
        ...
        {
            strlcpy(buffer, props_file[selected_item], buffer_len);
        }
    }
    return buffer;
}
...
enum plugin_status plugin_start(const void* parameter)
{
    ...
    strcpy(file, (const char *) parameter);
    ...
    int dirlen = (ptr - file);
    strlcpy(str_dirname, file, dirlen + 1);

    dir = opendir(str_dirname);
    if (dir)
    {
        while(0 != (entry = readdir(dir)))
        {
            if(!strcmp(entry->d_name, file+dirlen))
            {
                struct dirinfo info = dir_get_info(dir, entry);
    ...
    gui_synclist_init(&properties_lists, &get_props, file, false, 2, NULL);
    gui_synclist_set_title(&properties_lists, its_a_dir ?
                                                  "Directory properties" :
                                                  "File properties", NOICON);
    gui_synclist_set_icon_callback(&properties_lists, NULL);
    gui_synclist_set_nb_items(&properties_lists, num_properties * 2);
    gui_synclist_limit_scroll(&properties_lists, true);
    gui_synclist_select_item(&properties_lists, 0);
    ...
    while(!quit)
    {
        ...
        if (gui_synclist_do_button(&properties_lists,&button,LIST_WRAP_ON))
            continue;
        switch(button)
        {
