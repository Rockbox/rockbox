...
****************************************************************************
    ...
    A Visual String Theory Physics Simulator of the Elements
                    A Work in Progress**
...
The Q-bit: a 2D Brane of quantum information by casting over twos
  ...
  V=32
    ...
    **Classic Four Quantum Numbers**
    ...
    QNspin              =  ...+2,+1,0,-1-2...
                   ...
                   Quantum spin can be Derived geometricly from the 2D brane
                   (casting over twos) XOR half W over the other half of W

                   "spin 180 degrees through X/2 and cancel touching pairs"

                   A col caught in the middle cancels out for obvious reason, see n=3
                   A bisecting seperation along Y/2 assign charge -bottom and +top
...
The Truth Table Memory Requirements
    ...
    seg_counter=2
        ...
        {{1, 0, 0, 1}}
        {{0, 1, 1, 0}}____//Half way point;end  rule1;end rule2

        {{0, 0, 0, 0}}    //rule1 OR and rotate to complete the pattern
        {{0, 0, 0, 0}}
        {{0, 0, 0, 0}}
...
QUANTUM SPIN
  ...
  n=0
    ...
    {{0}}  XOR  0 SUM 0
    {{0}}  XOR  0 SUM 0  //avoids use of null for QNprime
...
When casting over twos, ee only need half the pattern
to get the last half with OR
...
signed int c = 0;  //-128 to +128  we only need 118 for the elements but require a signed int for compare
...
void *alloc_bytes(size_t n)
{
    ...
    for (i = 0; i <= n; i++)
    {
        ...
        for (i = 0; i <= n; i++)
        {
            signed counter = counter * n;

            if(bytes_left - counter >= 0)
            {
                ret = ptr;
                ptr = (char*) ptr + counter;
                bytes_left -= counter;
//                rb->splashf(HZ/60, "2 to the %d", n);
            }
...
{
    {-DIST * MAGNIFY,  DIST * 2 * MAGNIFY, 0},
    { DIST * MAGNIFY,  DIST * 2 * MAGNIFY, 0},
    {-DIST * MAGNIFY,  DIST * MAGNIFY   , 0},
    {        0,  DIST * MAGNIFY   , 0},
    {-DIST * MAGNIFY,            0, 0},
    {        0,            0, 0},
    { DIST * MAGNIFY,            0, 0},
...
//ToDo more easier color assignment like 2 maybe
...
//            mode_switch = false;
        ...
        if (!paused)
        {
            ...
            alloc_bytes(c);  //counter1
//            c++;

            // One Calcium atom has (2^20) * 20 bits
            // 2 Megabytes of string bits in the ground state
            if (c >= 118)
            {
                c=118;
            }
            // One Hydrogen atom has (2^1) * 1 bits
            // One bit of string bits
            if (c <= 0)
            {
                c=0;
            }
