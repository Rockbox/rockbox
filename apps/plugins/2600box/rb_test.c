...
/* time left (in frames) to display alerts */
unsigned tst_alert_timer;
uint8_t tst_alert[NUM_ALERTS];

char *al_token[NUM_ALERTS] = TST_ALERT_TOKENS;
...
char *tst_get_alert_tokens(void)
{
    ...
    for (int i=0; i<NUM_ALERTS; ++i) {
        if (tst_alert[i]) {
            rb->snprintf(tok, sizeof(tok), "%s*%s", tok, al_token[i]);
            --tst_alert[i];
        }
    }
    return tok;
...
jmp_buf tst_assert_jmpbuf;
char tst_assert_msg[40];

void tst_assert_print(void)
{
    // TODO: put text on screen and wait for button press
    rb->splash(4*HZ, tst_assert_msg);
}
...
unsigned long prof_tia_w_direct, prof_tia_w_ind;
...
//  #define FRM_CNT(num)     tst_format_number(num, txt, ARRAY_LEN(txt))
    ...
    DEBUGF("*** TIA WRITE ***\n");
    FRM_CNT(prof_tia_w_direct);
    DEBUGF("Direct calls: %s", txt);
    FRM_CNT(prof_tia_w_ind);
    DEBUGF(" / indirect: %s (per frame)\n", txt);
    i = 0;
    do {
