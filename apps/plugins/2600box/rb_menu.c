...
int menu_sound(void)
{
    ...
    static const struct opt_items stereo_setting[] = {
        { "separated channels", -1 },
        { "Stereo", -1 },
        { "Mono", -1 },
    };
    enum {
        MENU_STEREO_SEPARATE = 0,
        MENU_STEREO_NORMAL,
        MENU_STEREO_MONO,
    };
    ...
    while(!exit_menu) {
        result = rb->do_menu(&menu, &selected, NULL, false);
        switch (result) {
            case MENU_SOUND_ENABLE:
                rb->set_bool("Enable Sound", &pcm.audio_on);
                break;
            case MENU_SET_VOLUME:
                ...
                break;
            case MENU_SET_STEREO:
                new_setting = pcm.stereo_setting;
                rb->set_option("Stereo Setting", &new_setting, INT, stereo_setting,
                    ARRAY_LEN(stereo_setting), NULL );
                pcm.stereo_setting = new_setting;
                break;
            default:
                exit_menu=true;
                break;
