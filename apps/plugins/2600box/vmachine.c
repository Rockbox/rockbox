...
new_frame:
        ...
        if (tst_time_usec_start > 0) {
            tmp_usec = USEC_TIMER - tmp_usec;
            tst_usec[TST_USEC_SOUND] += tmp_usec;
        }
#endif
    /* wait for audio sync */
    struct queue_event ev;
    if (screen.lockfps) {   /* when framerate is "as fast as possible" skip wait */
        rb->queue_wait(&audiosync, &ev);
    }
    if (!rb->queue_empty(&audiosync)) {
        DEBUGF("!!!! Audio queue overrun !!!!\n");
    }
