...
read_stick (void)
{
  ...
  int buttons = rb->button_status();
      if (buttons & input.joy1_left)
          v[0] &= 0x0B;
      if (buttons & input.joy1_right)
          v[0] &= 0x07;
      if (buttons & input.joy1_up)
          v[0] &= 0x0E;
      if (buttons & input.joy1_down)
          v[0] &= 0x0D;
  ...
  /* Swap if necessary */
  if (base_opts.swap)
    riot.read[SWCHA] = (v[1] << 4) | v[0];
  else
    riot.read[SWCHA] = (v[0] << 4) | v[1];
}
...
read_trigger (void)
{
  int kr = 0x80, kl = 0x80;
    ...
    int buttons = rb->button_status();
    if (buttons & input.joy1_trigger) {
        kl = 0x00; // ?? atari asteroids need this!
        kr = 0x00; // ?? batari basic soccer game needs this!
    }
...
read_console (void)
{
  riot.read[SWCHB] |= 0x03;
    int buttons = rb->button_status();
    if (buttons & input.console_select)
        riot.read[SWCHB] &= 0xFD;
    if (buttons & input.console_reset)
        riot.read[SWCHB] &= 0xFE;

  if (keybdrv_pressed(kmapF2))
    riot.read[SWCHB] &= 0xFE; /* Reset */
  if (keybdrv_pressed(kmapF3))
    riot.read[SWCHB] &= 0xFD; /* Select */

  if (keybdrv_pressed(kmapF4))
    {
      if (riot.read[SWCHB] & 0x08)
        /* BW */
        riot.read[SWCHB] &= 0xF7;
      else
        /* Color */
  ...
  if (keybdrv_pressed(kmapF5))
    riot.read[SWCHB] &= 0xBF;    /* P0 amateur */
  else if (keybdrv_pressed(kmapF6))
    riot.read[SWCHB] |= 0x40;    /* P0 pro */
  if (keybdrv_pressed(kmapF7))
    riot.read[SWCHB] &= 0x7f;    /* P1 amateur */
  else if (keybdrv_pressed(kmapF8))
    riot.read[SWCHB] |= 0x80;    /* P1 pro */
}
