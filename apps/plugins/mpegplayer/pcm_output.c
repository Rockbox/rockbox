    ...
    if (sz > pcmbuf_threshold)
    {
        ...
        while (1)
        {
            ...
            else if (offset < 100*CLOCK_RATE/1000)
            {
                ...
                unsigned long frcount = pcm_dma_t_size_frames(frcount);
...
static inline bool pcm_output_is_playing(void)
{
    return rb->pcm_get_state(pcm_channel) == PCM_STATE_RUNNING;
}

static inline bool pcm_output_is_stopped(void)
{
    int rc = rb->pcm_get_state(pcm_channel);
    return rc >= 0 && rc == PCM_STATE_STOPPED;
}

/* Start the pcm playback, native output format */
void pcm_output_play(void)
{
    rb->pcm_set_amplitude(pcm_channel, MIX_AMP_UNITY);
    rb->pcm_play_data(pcm_channel, pcm_callback, NULL, 0, NULL);
}
...
{
    rb->pcm_lock_callback(pcm_channel);

    unsigned int state = rb->pcm_get_play_state(pcm_channel);

    /* Stop PCM to clear current buffer */
    if (state != PCM_STATE_STOPPED)
        rb->pcm_play_stop(pcm_channel);

    rb->pcm_unlock_callback(pcm_channel);
...
uint32_t pcm_output_get_clock(void)
{
    ...
    {
        time = clock_time;
        rem = rb->pcm_get_frames_waiting(pcm_channel);

        if (time != clock_time)
            continue;

        if (rem == 0 && pcm_output_is_playing())
            continue;
...
void pcm_output_play_pause(bool play)
{
    ...
    {
        rb->pcm_play_pause(pcm_channel, play);
        rb->pcm_unlock_callback(pcm_channel);
    }
    else
    {
...
{
    if (rb->pcm_open(&pcm_channel, PCM_STREAM_PLAYBACK))
        return false;
    ...
    old_sampr = rb->pcm_get_frequency();
    rb->pcm_set_frequency(CLOCK_RATE);
    return true;
}
