                                     ...
                                     void *buf, size_t *bufsize)
{
    ...
    osd->lcd_bitmap_data = _grey_info.curbuffer;
    osd->back_bitmap_stride = w;
    osd->back_bitmap_data = buf;
...
static void _osd_lcd_viewport_set_buffer(void *buffer)
{
    if (buffer)
    {
        ...
        if (buffer == native_osd.back_bitmap_data)
            native_osd.framebuf.stride = _OSD_WIDTH2BYTES(native_osd.back_bitmap_stride);
        else
            native_osd.framebuf.stride = _OSD_WIDTH2BYTES(native_osd.lcd_bitmap_stride);
