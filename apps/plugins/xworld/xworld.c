...
/*
    ...
    delete e;
*/
    size_t plugin_buffer_sz;
    struct Engine *e=rb->plugin_get_buffer(&plugin_buffer_sz);
    if(plugin_buffer_sz<sizeof(struct Engine))
    {
        static struct Engine engine_data;
        e=&engine_data;
    }
    struct System sys;
    debug(DBG_SYS, "plugin_start Point 0");

    /* engine_create causes garbage at the bottom of the screen */
    /* but not the crash */
    engine_create(e, &sys, dataPath, savePath);

    debug(DBG_SYS, "plugin_start Point 1");
    /* engine_init causes more garbage at the bottem of the screen */
    /* crash bug is in engine_init */
    engine_init(e);


    debug(DBG_SYS, "plugin_start Point 2");
    engine_run(e);
    debug(DBG_SYS, "plugin_start Point 3");
    engine_finish(e);

    debug(DBG_SYS, "End plugin_start");

    /* sleep for a long time and wait for crash */
    rb->sleep(60000);
    return PLUGIN_OK;
}
