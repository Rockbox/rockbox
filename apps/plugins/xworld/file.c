...
bool file_open(struct File* f, const char *filename, const char *directory, const char *mode) {
    if(f->magic!=0x600DF00D)
        warning("open() called on unconstruced file %s!", filename);
    debug(DBG_FILE, "File ID is %d", f->uniq_id);
    debug(DBG_FILE, "file_open(%s, %s, %s)", filename, directory, mode);
    char buf[512];
    ...
    f->fd=-1;
    f->fd = rb->open(buf, O_RDWR, 0666);
    if (f->fd < 0) { // let's try uppercase
        string_upper(p);
        debug(DBG_FILE, "trying uppercase: %s", buf);
        debug(DBG_FILE, "file_exists(): %s", rb->file_exists(buf)==true?"true":"false");
        f->fd = rb->open(buf, O_RDWR, 0666);
    }
    debug(DBG_FILE, "FD is %d", f->fd);
    debug(DBG_FILE, "errno is %d", errno);
    if(f->fd > 0)
        return true;
    else
        return false;
