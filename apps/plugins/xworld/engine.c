...
void engine_init(struct Engine* e) {
    ...
    rb->splashf(HZ, "Start engine_init");
    //Init system
    sys_init(e->sys, "Out Of This World");
    debug(DBG_SYS, "engine_init Point 0");
    rb->splashf(HZ, "engine_init Point 0");

    video_init(&e->video);
    debug(DBG_SYS, "engine_init Point 1");
    rb->splashf(HZ, "engine_init Point 1");

    res_allocMemBlock(&e->res);
    debug(DBG_SYS, "engine_init Point 2");
    rb->splashf(HZ, "engine_init Point 2");

    res_readEntries(&e->res);
    debug(DBG_SYS, "engine_init Point 3");
    rb->splashf(HZ, "engine_init Point 3");

    vm_init(&e->vm);
    debug(DBG_SYS, "engine_init Point 4");
    rb->splashf(HZ, "engine_init Point 4");

    mixer_init(&e->mixer);
    debug(DBG_SYS, "engine_init Point 5");
    rb->splashf(HZ, "engine_init Point 5");

    player_init(&e->player);
    debug(DBG_SYS, "engine_init Point 6");
    rb->splashf(HZ, "engine_init Point 6");
