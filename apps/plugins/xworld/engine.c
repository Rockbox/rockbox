 ...
 * entry format:
   struct file_entry_t
   {
     ...
     char* str;    // INCLUDES NULL TERMINATOR
   }
*/
void engine_loadStringTable(struct Engine* e)
{
    ...
    char *title_buf = sys_get_buffer(e->sys, title_length);
    ...
    for(unsigned int i=0;i<num_entries && i<ARRAYLEN(video_stringsTableEng);++i)
    {
        ...
        video_stringsTableEng[i].str=sys_get_buffer(e->sys, len);
        file_read(&f, video_stringsTableEng[i].str, len);
        debug(DBG_ENG, "entry %d has length %d (with null) and value %s", video_stringsTableEng[i].id,
              len,
              video_stringsTableEng[i].str);
    }
