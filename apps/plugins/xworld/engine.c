...
void engine_loadStringTable(struct Engine* e)
{
    ...
    for(int i=0;i<ARRAYLEN(video_stringsTableEng);++i)
    {
        video_stringsTableEng[i].id=file_readUint16BE(&f);
        ret=file_read(&f, &video_stringsTableEng[i].str, sizeof(video_stringsTableEng[i].str));
        if(ret!=sizeof(video_stringsTableEng[i].str))
        {
            warning("Invalid string table entry, falling back to alternate strings");
            goto fail;
        }
    }
    fail:
