...
void sfxplay_create(struct SfxPlayer* sfx, Mixer *mix, Resource *res, System *stub)
{
    sfx->mixer=mix;
    sfx->res=res;
    sfx->sys=stub;
...
void sfxplay_setEventsDelay(struct SfxPlayer* sfx, uint16_t delay) {
    debug(DBG_SND, "sfxplay_setEventsDelay(%d)", delay);
    MutexStack_t ms;
    MutexStack(&ms, sys, sfx->_mutex);
    sfx->_delay = delay * 60 / 7050;
    MutexStack_destroy(&ms);
}

void sfxplay_loadSfxModule(struct SfxPlayer* sfx, uint16_t resNum, uint16_t delay, uint8_t pos) {

    debug(DBG_SND, "sfxplay_loadSfxModule(0x%X, %d, %d)", resNum, delay, pos);
    MutexStack(sys, sfx->_mutex);
    ...
    if (me->state == MEMENTRY_STATE_LOADED && me->type == Resource::RT_MUSIC) {
        ...
        warning("sfxplay_loadSfxModule() ec=0x%X", 0xF8);
    }
}

void sfxplay_prepareInstruments(struct SfxPlayer* sfx, const uint8_t *p) {
...
void sfxplay_start(struct SfxPlayer* sfx) {
    debug(DBG_SND, "sfxplay_start()");
    MutexStack(sys, sfx->_mutex);
    sfx->_sfxMod.curPos = 0;
    _timerId = sys->addTimer(sfx->_delay, eventsCallback, this);
}

void sfxplay_stop(struct SfxPlayer* sfx) {
    debug(DBG_SND, "sfxplay_stop()");
    MutexStack(sys, sfx->_mutex);
    if (sfx->_resNum != 0) {
        sfx->_resNum = 0;
...
void sfxplay_handleEvents(struct SfxPlayer* sfx) {
    MutexStack(sys, sfx->_mutex);
...
void sfxplay_handlePattern(struct SfxPlayer* sfx, uint8_t channel, const uint8_t *data) {
    SfxPattern pat;
    ...
    if (pat.note_1 != 0xFFFD) {
        ...
        if (sample != 0) {
            ...
            if (ptr != 0) {
                debug(DBG_SND, "sfxplay_handlePattern() preparing sample %d", sample);
    ...
    if (pat.note_1 == 0xFFFD) {
        ...
        *_markVar = pat.note_2;
        ...
        if (pat.note_1 == 0xFFFE) {
            ...
            debug(DBG_SND, "sfxplay_handlePattern() adding sample freq = 0x%X", freq);
...
void sfxplay_saveOrLoad(struct SfxPlayer* sfx, Serializer &ser) {
    sys->lockMutex(sfx->_mutex);
    Serializer::Entry entries[] = {
        SE_INT(&sfx->_delay, Serializer::SES_INT8, VER(2)),
        SE_INT(&sfx->_resNum, Serializer::SES_INT16, VER(2)),
        SE_INT(&sfx->_sfxMod.curPos, Serializer::SES_INT16, VER(2)),
        SE_INT(&sfx->_sfxMod.curOrder, Serializer::SES_INT8, VER(2)),
        SE_END()
    };
    ser.saveOrLoadEntries(entries);
    sys->unlockMutex(sfx->_mutex);
    if (ser._mode == Serializer::SM_LOAD && sfx->_resNum != 0) {
        uint16_t delay = sfx->_delay;
        loadSfxModule(sfx->_resNum, 0, sfx->_sfxMod.curOrder);
        sfx->_delay = delay;
        _timerId = sys->addTimer(sfx->_delay, eventsCallback, this);
    }
}
