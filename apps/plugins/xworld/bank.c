...
void bank_init(struct Bank* b, const char *dataDir)
{
    b->_dataDir=dataDir;
}
...
bool bank_read(struct Bank* b, const MemEntry *me, uint8_t *buf) {
	...
	if (!f.open(bankName, _dataDir))
		error("bank_read() unable to open '%s'", bankName);
...
bool bank_unpack(struct Bank* b) {
	b->_unpCtx.size = 0;
	b->_unpCtx.datasize = READ_BE_UINT32(b->_iBuf); b->_iBuf -= 4;
	b->_oBuf = b->_startBuf + b->_unpCtx.datasize - 1;
	b->_unpCtx.crc = READ_BE_UINT32(b->_iBuf); b->_iBuf -= 4;
	b->_unpCtx.chk = READ_BE_UINT32(b->_iBuf); b->_iBuf -= 4;
