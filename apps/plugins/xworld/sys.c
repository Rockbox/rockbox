...
void sys_setPalette(struct System* sys, uint8_t start, uint8_t n, const uint8_t *buf)
{
    for(int i=start;i<start+n;++i)
    {
        ...
        sys->palette[i]=(fb_data) { c[2], c[1], c[0] };
...
void sys_copyRect(struct System* sys, uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *buf, uint32_t pitch)
{
    ...
    else
    {
        ...
        if(sys->settings.zoom)
        {
            ...
            /* clockwise */
            case 1:
                rb->lcd_bitmap_part(framebuffer, CODE_Y-LCD_WIDTH, CODE_X, STRIDE(SCREEN_MAIN, 200, 320), 0, 0, 200-CODE_Y+LCD_WIDTH, 320-CODE_X+LCD_HEIGHT);
                break;
            /* counterclockwise */
            case 2:
                rb->lcd_bitmap_part(framebuffer, CODE_Y, CODE_X-LCD_HEIGHT, STRIDE(SCREEN_MAIN, 200, 320), 0, 0, 200-CODE_Y, 320-CODE_X);
                break;
            default:
                error("fall-through");
                break;
...
void *sys_get_buffer(struct System* sys, size_t sz)
{
    ...
    else
    {
        ...
        /* not really needed except to silence a warning */
        return NULL;
    }
}
