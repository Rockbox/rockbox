...
#define BTN_LEFT       BUTTON_LEFT
#define BTN_RIGHT      BUTTON_RIGHT
5#define BTN_FIRE       BUTTON_SELECT
...
void sys_init(struct System* sys, const char* title)
{
    ...
    rb->splashf(HZ, "sys_init Point 0");
    debug(DBG_SYS, "timer 0");
    rb->splashf(HZ, "timer 0");
    rb->sleep(HZ);
    debug(DBG_SYS, "timer 1");
    rb->splashf(HZ, "timer 1");
    rb->sleep(HZ);
    debug(DBG_SYS, "timer 2");
    rb->splashf(HZ, "timer 2");
    rb->sleep(HZ);
    debug(DBG_SYS, "timer 3");
    rb->splashf(HZ, "timer 3");
    rb->sleep(HZ);
    debug(DBG_SYS, "timer 4");
    rb->splashf(HZ, "timer 4");
    rb->sleep(HZ);
    debug(DBG_SYS, "timer 5");
    rb->splashf(HZ, "timer 5");
    rb->sleep(HZ);

    /*do_title(); */
    debug(DBG_SYS, "sys_init Point 1");
    rb->splashf(HZ, "sys_init Point 1");
    memset(&sys->input, 0, sizeof(sys->input));
    debug(DBG_SYS, "sys_init Point 2");
    rb->splashf(HZ, "sys_init Point 2");
    sys->mutex_bitfield=~0;
    debug(DBG_SYS, "End sys_init");
    rb->splashf(HZ, "End sys_init");
}
