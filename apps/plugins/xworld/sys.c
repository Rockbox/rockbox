...
void sys_copyRect(struct System* sys, uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *buf, uint32_t pitch)
{
    ...
    if(sys->settings.scaling_quality)
    {
        ...
        out_bmp.data = (unsigned char*) rb->_viewport_get_framebuffer(NULL, NULL);
...
#ifdef HAVE_LCD_COLOR
                int r, g, b;
                fb_data pix = rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x];
#if (LCD_DEPTH > 24)
                r = 0xff - pix.r;
                g = 0xff - pix.g;
                b = 0xff - pix.b;
                rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x] = (fb_data) { b, g, r, 255 };
#elif (LCD_DEPTH == 24)
                r = 0xff - pix.r;
                g = 0xff - pix.g;
                b = 0xff - pix.b;
                rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x] = (fb_data) { b, g, r };
#else
                r = RGB_UNPACK_RED  (pix);
                g = RGB_UNPACK_GREEN(pix);
                b = RGB_UNPACK_BLUE (pix);
                rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x] = LCD_RGBPACK(0xff - r, 0xff - g, 0xff - b);
#endif
#else
                rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x] = LCD_BRIGHTNESS(0xff - rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x]);
#endif
            }
        }
    }
    ...
    if(sys->settings.blur)
    {
        ...
        if(prev_frames && orig_fb)
        {
            ...
            rb->memcpy(orig_fb, rb->_viewport_get_framebuffer(NULL, NULL), sizeof(fb_data) * LCD_WIDTH * LCD_HEIGHT);
            ...
            for(int y = 0; y < LCD_HEIGHT; ++y)
            {
                for(int x = 0; x < LCD_WIDTH; ++x)
                {
                    ...
                    rb->_viewport_get_framebuffer(NULL, NULL)[y * LCD_WIDTH + x] = LCD_RGBPACK(r, g, b);
                }
            }
