...
void sys_copyRect(struct System* sys, uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *buf, uint32_t pitch)
{
    ...
    while (height--) {
        ...
        for (int i = 0; i < width / 2; ++i) {
            ...
            rb->lcd_drawpixel(x+i*2, y+height);
            rb->lcd_set_foreground(sys->pallete[pix2]);
            rb->lcd_drawpixel(x+i*2+1, y+height);
        }
        buf += pitch;
    }
    rb->lcd_update();
...
{
    int mutex_number=(mutex-sys->mutex_memory)/sizeof(struct mutex); /* pointer arithmetic! check for bugs! */
