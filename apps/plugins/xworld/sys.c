...
{
    fb_data* framebuffer=(fb_data*) sys->e->res._memPtrStart + MEM_BLOCK_SIZE + sizeof(struct VirtualMachine);
    fb_data* dest=framebuffer;
    fb_data* temp_fb=(fb_data*) sys->e->res._memPtrStart + MEM_BLOCK_SIZE + sizeof(struct VirtualMachine) + sizeof(fb_data) * 320 * 200;
    buf += y * pitch + x;
    int next=0;
    while (h--) {
    ...
    {
        rb->memcpy(temp_fb, framebuffer, 320 * 200 * sizeof(fb_data));
        dest=framebuffer;
