...
static bool sys_save_settings(struct System* sys)
{
    File f;
    file_create(&f, false);
    if(!file_open(&f, "settings.cfg", sys->e->_saveDir, "wb"))
    {
        return false;
    }
...
static bool sys_load_settings(struct System* sys)
{
    File f;
    file_create(&f, false);
    if(!file_open(&f, "settings.cfg", sys->e->_saveDir, "rb"))
    {
        return false;
    }
...
void sys_copyRect(struct System* sys, uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *buf, uint32_t pitch)
{
    ...
    if(sys->settings.rotation_option)
    {
        ...
        else
        {
            while (h--) {
                /* one byte gives two pixels */
                /* Currently broken appears to be the wrong stride, I don't fully understand this function */
                for (int i = 0; i < w / 2; ++i) {
                    uint8_t pix1=*(buf+i)>>4;
                    uint8_t pix2=*(buf+i)&0xF;
...
void sys_processEvents(struct System* sys)
{
    ...
    /* 1 << 31 is a speical mask on iPods; it causes errant button presses and should be ignored */
