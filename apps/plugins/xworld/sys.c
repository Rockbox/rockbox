...
#define NBSAMPLES 1024

static int16_t rb_soundbuf[NBSAMPLES];
static int8_t temp_soundbuf[NBSAMPLES];
static AudioCallback audio_callback;
static void* audio_param;
static void get_more(const void** start, size_t* size)
{
    audio_callback(audio_param, temp_soundbuf, 128);
    debug(DBG_SYS, "pcm data:");
    /* convert xworld format to rockbox format */
