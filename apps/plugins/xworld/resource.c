...
/*res_Resource(Video *vid, const char *dataDir)
: video(vid), _dataDir(dataDir), currentPartId(0),requestedNextPart(0) {
}*/

void res_create(struct Resource* res, struct Video* vid, const char* dataDir)
{
    res->video=vid;
...
void res_readEntries(struct Resource* res) {
    ...
    if (!file_open(&f, "memlist.bin", res->_dataDir, "rb")) {
        error("res_readEntries() unable to open 'memlist.bin' file\n");
    ...
    while (1) {
        assert(res->_numMemList < ARRAYSIZE(res->_memList));
        memEntry->state = file_readByte(&f);
        memEntry->type = file_readByte(&f);
        memEntry->bufPtr = 0; file_readUint16BE(&f);
...
void res_allocMemBlock(struct Resource* res) {
    ...
    */
    ...
    if(sz < MEM_BLOCK_SIZE + (4 * VID_PAGE_SIZE) + sizeof(struct VirtualMachine))
    {
