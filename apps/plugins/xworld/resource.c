...
void resource_readBank(const MemEntry *me, uint8_t *dstBuf) {
    uint16_t n = me - _memList;
    debug(DBG_BANK, "resource_readBank(%d)", n);

    Bank bk(_dataDir);
    if (!bk.read(me, dstBuf)) {
        error("resource_readBank() unable to unpack entry %d\n", n);
    }
...
void resource_readEntries() {
    ...
    if (!f.open("memlist.bin", _dataDir)) {
        error("resource_readEntries() unable to open 'memlist.bin' file\n");
    ...
    while (1) {
        ...
        if (me->type == RT_POLY_ANIM) {
            ...
            if (me->size > _vidBakPtr - _scriptCurPtr) {
                warning("resource_load() not enough memory");
        ...
        if (me->bankId == 0) {
            warning("resource_load() ec=0x%X (me->bankId == 0)", 0xF00);
            me->state = MEMENTRY_STATE_NOT_NEEDED;
        } else {
...
/* This method serves two purpose:
   ...
   be a part id. */
void resource_loadPartsOrMemoryEntry(uint16_t resourceId) {
...
void resource_setupPart(uint16_t partId) {
    ...
    if (partId < GAME_PART_FIRST || partId > GAME_PART_LAST)
        error("resource_setupPart() ec=0x%X invalid partId", partId);
