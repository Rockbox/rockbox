...
void video_init(struct Video* v) {
    ...
    /*
      for (int i = 0; i < 4; ++i) {
      v->_pagePtrs[i] = allocPage();
      }
    */
    for (int i = 0; i < 4; ++i) {
        v->_pagePtrs[i] = tmp + i * VID_PAGE_SIZE;
    }
...
/*
  This
*/
void video_video_setDataBuffer(struct Video* v, uint8_t *dataBuf, uint16_t offset) {
...
void video_readAndDrawPolygon(struct Video* v, uint8_t color, uint16_t zoom, const Point &pt) {
    ...
    if (i >= 0xC0) {    // 0xc0 = 192
        ...
        video_video_fillPolygon(v, v, color, zoom, pt);
        ...
        if (i == 1) {
            ...
            video_video_readAndDrawPolygonHierarchy(v, v, zoom, pt);
...
*/
void video_video_readAndDrawPolygonHierarchy(struct Video* v, uint16_t zoom, const struct Point *pgc) {
...
void video_video_drawString(struct Video* v, uint8_t color, uint16_t x, uint16_t y, uint16_t stringId) {
...
void video_drawLineBlend(struct Video* v, int16_t x1, int16_t x2, uint8_t color) {
    ...
    int16_t xmax = AWMAX(x1, x2);
    int16_t xmin = AWMIN(x1, x2);
...
void video_saveOrLoad(struct Video* v, Serializer &ser) {
    ...
    Serializer::Entry entries[] = {
        ...
        SE_INT(&v->paletteIdRequested, Serializer::SES_INT8, VER(1)),
        SE_INT(&mask, Serializer::SES_INT8, VER(1)),
        SE_ARRAY(v->_pagePtrs[0], video_VID_PAGE_SIZE, Serializer::SES_INT8, VER(1)),
        SE_ARRAY(v->_pagePtrs[1], video_VID_PAGE_SIZE, Serializer::SES_INT8, VER(1)),
        SE_ARRAY(v->_pagePtrs[2], video_VID_PAGE_SIZE, Serializer::SES_INT8, VER(1)),
        SE_ARRAY(v->_pagePtrs[3], video_VID_PAGE_SIZE, Serializer::SES_INT8, VER(1)),
        SE_END()
    };
