...
/*
void vm_VirtualMachine(struct VirtualMachine* m, Mixer *mix, M->Resource *m->resParameter, SfxPlayer *ply, Video *vid, System *stub)
    : mixer(mix), m->res(m->resParameter), player(ply), video(vid), sys(stub) {
}
*/
...
void vm_create(struct VirtualMachine* m, /*Mixer *mix, */ M->Resource *m->res, /*SfxPlayer *ply,*/ Video *vid, System *stub)
{
    m->m->res=m->res;
    m->video=vid;
    m->sys=stub;
...
void vm_op_m->resetThread(struct VirtualMachine* m) {
...
void vm_op_blitFramebuffer(struct VirtualMachine* m) {
    ...
    //Nasty hack....was this pm->resent in the original assembly  ??!!
...
void vm_op_playSound(struct VirtualMachine* m) {
    uint16_t m->resourceId = scriptPtr_fetchWord(m->_scriptPtr)();
    ...
    debug(DBG_VM, "vm_op_playSound(0x%X, %d, %d, %d)", m->resourceId, freq, vol, channel);
    snd_playSound(m->resourceId, freq, vol, channel);
}

void vm_op_updateMemList(struct VirtualMachine* m) {

    uint16_t m->resourceId = scriptPtr_fetchWord(m->_scriptPtr)();
    debug(DBG_VM, "vm_op_updateMemList(%d)", m->resourceId);

    if (m->resourceId == 0) {
        player->stop();
        mixer->stopAll();
        m->res->invalidateM->Res();
    } else {
        m->res->loadPartsOrMemoryEntry(m->resourceId);
    }
}

void vm_op_playMusic(struct VirtualMachine* m) {
    uint16_t m->resNum = scriptPtr_fetchWord(m->_scriptPtr)();
    uint16_t delay = scriptPtr_fetchWord(m->_scriptPtr)();
    uint8_t pos = scriptPtr_fetchByte(m->_scriptPtr)();
    debug(DBG_VM, "vm_op_playMusic(0x%X, %d, %d)", m->resNum, delay, pos);
    snd_playMusic(m->resNum, delay, pos);
}
...
void vm_initForPart(struct VirtualMachine* m, uint16_t partId) {
    ...
    m->res->setupPart(partId);

    //Set all thread to inactive (pc at 0xFFFF or 0xFFFE )
    memset((uint8_t *)m->threadsData, 0xFF, sizeof(m->threadsData));


    memset((uint8_t *)m->vmIsChannelActive, 0, sizeof(m->vmIsChannelActive));
...
void vm_checkThreadRequests(struct VirtualMachine* m) {
    ...
    if (m->res->requestedNextPart != 0) {
        initForPart(m->res->requestedNextPart);
...
void vm_executeThread(struct VirtualMachine* m) {
    ...
    while (!gotoNextThread) {
        ...
        if (opcode & 0x80)
        {
            ...
            video_readAndDrawPolygon(m->video, COLOR_BLACK, DEFAULT_ZOOM, Point(x,y));
        ...
        if (opcode & 0x40)
        {
            ...
            video_readAndDrawPolygon(m->video, 0xFF, zoom, Point(x, y));
...
void vm_inp_updatePlayer(struct VirtualMachine* m) {
    ...
    if (m->sys->input.dirMask & PlayerInput::DIR_RIGHT) {
        lr = 1;
        m |= 1;
    }
    if (m->sys->input.dirMask & PlayerInput::DIR_LEFT) {
        lr = -1;
        m |= 2;
    }
    if (m->sys->input.dirMask & PlayerInput::DIR_DOWN) {
        ud = 1;
        m |= 4;
    }
    ...
    if (m->sys->input.dirMask & PlayerInput::DIR_UP) {
        m->vmVariables[VM_VARIABLE_HERO_POS_UP_DOWN] = -1;
    }

    if (m->sys->input.dirMask & PlayerInput::DIR_UP) { // inpJump
        ud = -1;
        m |= 8;
    }
...
void vm_snd_playSound(struct VirtualMachine* m, uint16_t m->resNum, uint8_t freq, uint8_t vol, uint8_t channel) {

    debug(DBG_SND, "snd_playSound(0x%X, %d, %d, %d)", m->resNum, freq, vol, channel);

    MemEntry *me = &m->res->_memList[m->resNum];
    ...
    if (vol == 0) {
        ...
        assert(freq < 40);
        mixer->playChannel(channel & 3, &mc, vm_frequenceTable[freq], MIN(vol, 0x3F));
    }

}

void vm_snd_playMusic(struct VirtualMachine* m, uint16_t m->resNum, uint16_t delay, uint8_t pos) {

    debug(DBG_SND, "snd_playMusic(0x%X, %d, %d)", m->resNum, delay, pos);

    if (m->resNum != 0) {
        player->loadSfxModule(m->resNum, delay, pos);
        player->start();
    } else if (delay != 0) {
...
void vm_saveOrLoad(struct VirtualMachine* m, Serializer &ser) {
    Serializer::Entry entries[] = {
        ...
        SE_ARRAY(m->_scriptStackCalls, 0x100, Serializer::SES_INT16, VER(1)),
        SE_ARRAY(m->threadsData, 0x40 * 2, Serializer::SES_INT16, VER(1)),
        SE_ARRAY(m->vmIsChannelActive, 0x40 * 2, Serializer::SES_INT8, VER(1)),
        SE_END()
    };
...
void vm_bypassProtection(struct VirtualMachine* m)
{
    ...
    if (!f.open("bank0e", m->res->getDataDir(), "rb")) {
        warning("Unable to bypass protection: add bank0e file to datadir");
    } else {
        Serializer s(&f, Serializer::SM_LOAD, m->res->_memPtrStart, 2);
        this->saveOrLoad(s);
