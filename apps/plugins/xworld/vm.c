...
void virtualmachine_VirtualMachine(struct VirtualMachine* m, Mixer *mix, Resource *resParameter, SfxPlayer *ply, Video *vid, System *stub)
	: mixer(mix), res(resParameter), plastruct VirtualMachine* myer(ply), video(vid), sys(stub) {
}

void virtualmachine_init(struct VirtualMachine* m) {
...
void virtualmachine_op_movConst(struct VirtualMachine* m) {
	uint8_t variableId = _scriptPtr.fetchByte();
	int16_t value = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_movConst(0x%02X, %d)", variableId, value);
	vmVariables[variableId] = value;
}

void virtualmachine_op_mov(struct VirtualMachine* m) {
	uint8_t dstVariableId = _scriptPtr.fetchByte();
	uint8_t srcVariableId = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_mov(0x%02X, 0x%02X)", dstVariableId, srcVariableId);
	vmVariables[dstVariableId] = vmVariables[srcVariableId];
}

void virtualmachine_op_add(struct VirtualMachine* m) {
	uint8_t dstVariableId = _scriptPtr.fetchByte();
	uint8_t srcVariableId = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_add(0x%02X, 0x%02X)", dstVariableId, srcVariableId);
	vmVariables[dstVariableId] += vmVariables[srcVariableId];
}

void virtualmachine_op_addConst(struct VirtualMachine* m) {
	if (res->currentPartId == 0x3E86 && _scriptPtr.pc == res->segBytecode + 0x6D48) {
		warning("virtualmachine_op_addConst() hack for non-stop looping gun sound bug");
	...
	uint8_t variableId = _scriptPtr.fetchByte();
	int16_t value = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_addConst(0x%02X, %d)", variableId, value);
	vmVariables[variableId] += value;
}

void virtualmachine_op_call(struct VirtualMachine* m) {

	uint16_t offset = _scriptPtr.fetchWord();
	uint8_t sp = _stackPtr;

	debug(DBG_VM, "virtualmachine_op_call(0x%X)", offset);
	_scriptStackCalls[sp] = _scriptPtr.pc - res->segBytecode;
	if (_stackPtr == 0xFF) {
		error("virtualmachine_op_call() ec=0x%X stack overflow", 0x8F);
	}
	++_stackPtr;
	_scriptPtr.pc = res->segBytecode + offset ;
}

void virtualmachine_op_ret(struct VirtualMachine* m) {
	debug(DBG_VM, "virtualmachine_op_ret()");
	if (_stackPtr == 0) {
		error("virtualmachine_op_ret() ec=0x%X stack underflow", 0x8F);
	}
	--_stackPtr;
	uint8_t sp = _stackPtr;
...
void virtualmachine_op_pauseThread(struct VirtualMachine* m) {
	debug(DBG_VM, "virtualmachine_op_pauseThread()");
	gotoNextThread = true;
}

void virtualmachine_op_jmp(struct VirtualMachine* m) {
	uint16_t pcOffset = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_jmp(0x%02X)", pcOffset);
	_scriptPtr.pc = res->segBytecode + pcOffset;
}

void virtualmachine_op_setSetVect(struct VirtualMachine* m) {
	uint8_t threadId = _scriptPtr.fetchByte();
	uint16_t pcOffsetRequested = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_setSetVect(0x%X, 0x%X)", threadId,pcOffsetRequested);
	threadsData[REQUESTED_PC_OFFSET][threadId] = pcOffsetRequested;
}

void virtualmachine_op_jnz(struct VirtualMachine* m) {
	uint8_t i = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_jnz(0x%02X)", i);
	--vmVariables[i];
	if (vmVariables[i] != 0) {
		op_jmp();
	} else {
...
void virtualmachine_op_condJmp(struct VirtualMachine* m) {
...
//FCS Whoever wrote this is patching the bytecode on the fly. This is ballzy !!
	...
	if (res->currentPartId == GAME_PART_FIRST && _scriptPtr.pc == res->segBytecode + 0xCB9) {
		...
		*(_scriptPtr.pc + 0x9A) = 0x5A;
		printf("virtualmachine_op_condJmp() bypassing protection");
	...
	debug(DBG_VM, "virtualmachine_op_condJmp(%d, 0x%02X, 0x%02X)", opcode, b, a);
	...
	case 5: // jle
		...
		break;
	default:
		warning("virtualmachine_op_condJmp() invalid condition %d", (opcode & 7));
		break;
	}
...
void virtualmachine_op_setPalette(struct VirtualMachine* m) {
	uint16_t paletteId = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_changePalette(%d)", paletteId);
	video->paletteIdRequested = paletteId >> 8;
}

void virtualmachine_op_resetThread(struct VirtualMachine* m) {
	...
	if (n < 0) {
		warning("virtualmachine_op_resetThread() ec=0x%X (n < 0)", 0x880);
		return;
	}
	++n;
	uint8_t a = _scriptPtr.fetchByte();

	debug(DBG_VM, "virtualmachine_op_resetThread(%d, %d, %d)", threadId, i, a);
...
void virtualmachine_op_selectVideoPage(struct VirtualMachine* m) {
	uint8_t frameBufferId = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_selectVideoPage(%d)", frameBufferId);
	video->changePagePtr1(frameBufferId);
}

void virtualmachine_op_fillVideoPage(struct VirtualMachine* m) {
	uint8_t pageId = _scriptPtr.fetchByte();
	uint8_t color = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_fillVideoPage(%d, %d)", pageId, color);
	video->fillPage(pageId, color);
}

void virtualmachine_op_copyVideoPage(struct VirtualMachine* m) {
	uint8_t srcPageId = _scriptPtr.fetchByte();
	uint8_t dstPageId = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_copyVideoPage(%d, %d)", srcPageId, dstPageId);
	video->copyPage(srcPageId, dstPageId, vmVariables[VM_VARIABLE_SCROLL_Y]);
}


uint32_t lastTimeStamp = 0;
void virtualmachine_op_blitFramebuffer(struct VirtualMachine* m) {

	uint8_t pageId = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_blitFramebuffer(%d)", pageId);
...
void virtualmachine_op_killThread(struct VirtualMachine* m) {
	debug(DBG_VM, "virtualmachine_op_killThread()");
	_scriptPtr.pc = res->segBytecode + 0xFFFF;
	gotoNextThread = true;
}

void virtualmachine_op_drawString(struct VirtualMachine* m) {
	...
	debug(DBG_VM, "virtualmachine_op_drawString(0x%03X, %d, %d, %d)", stringId, x, y, color);

	video->drawString(color, x, y, stringId);
}

void virtualmachine_op_sub(struct VirtualMachine* m) {
	uint8_t i = _scriptPtr.fetchByte();
	uint8_t j = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_sub(0x%02X, 0x%02X)", i, j);
	vmVariables[i] -= vmVariables[j];
}

void virtualmachine_op_and(struct VirtualMachine* m) {
	uint8_t variableId = _scriptPtr.fetchByte();
	uint16_t n = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_and(0x%02X, %d)", variableId, n);
	vmVariables[variableId] = (uint16_t)vmVariables[variableId] & n;
}

void virtualmachine_op_or(struct VirtualMachine* m) {
	uint8_t variableId = _scriptPtr.fetchByte();
	uint16_t value = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_or(0x%02X, %d)", variableId, value);
	vmVariables[variableId] = (uint16_t)vmVariables[variableId] | value;
}

void virtualmachine_op_shl(struct VirtualMachine* m) {
	uint8_t variableId = _scriptPtr.fetchByte();
	uint16_t leftShiftValue = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_shl(0x%02X, %d)", variableId, leftShiftValue);
	vmVariables[variableId] = (uint16_t)vmVariables[variableId] << leftShiftValue;
}

void virtualmachine_op_shr(struct VirtualMachine* m) {
	uint8_t variableId = _scriptPtr.fetchByte();
	uint16_t rightShiftValue = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_shr(0x%02X, %d)", variableId, rightShiftValue);
	vmVariables[variableId] = (uint16_t)vmVariables[variableId] >> rightShiftValue;
}

void virtualmachine_op_playSound(struct VirtualMachine* m) {
	...
	debug(DBG_VM, "virtualmachine_op_playSound(0x%X, %d, %d, %d)", resourceId, freq, vol, channel);
	snd_playSound(resourceId, freq, vol, channel);
}

void virtualmachine_op_updateMemList(struct VirtualMachine* m) {

	uint16_t resourceId = _scriptPtr.fetchWord();
	debug(DBG_VM, "virtualmachine_op_updateMemList(%d)", resourceId);
...
void virtualmachine_op_playMusic(struct VirtualMachine* m) {
	uint16_t resNum = _scriptPtr.fetchWord();
	uint16_t delay = _scriptPtr.fetchWord();
	uint8_t pos = _scriptPtr.fetchByte();
	debug(DBG_VM, "virtualmachine_op_playMusic(0x%X, %d, %d)", resNum, delay, pos);
	snd_playMusic(resNum, delay, pos);
}

void virtualmachine_initForPart(struct VirtualMachine* m, uint16_t partId) {
...
*/
void virtualmachine_checkThreadRequests(struct VirtualMachine* m) {
...
void virtualmachine_hostFrame(struct VirtualMachine* m) {
	...
	for (int threadId = 0; threadId < VM_NUM_THREADS; threadId++) {
		...
		if (n != VM_INACTIVE_THREAD) {
			...
			debug(DBG_VM, "virtualmachine_hostFrame() i=0x%02X n=0x%02X *p=0x%02X", threadId, n, *_scriptPtr.pc);
			executeThread();
			...
			debug(DBG_VM, "virtualmachine_hostFrame() i=0x%02X pos=0x%X", threadId, threadsData[PC_OFFSET][threadId]);
			if (sys->input.quit) {
				break;
			}
		}
...
void virtualmachine_executeThread(struct VirtualMachine* m) {
	...
	while (!gotoNextThread) {
		...
		if (opcode > 0x1A)
		{
			error("virtualmachine_executeThread() ec=0x%X invalid opcode=0x%X", 0xFFF, opcode);
		}
		else
		{
...
void virtualmachine_inp_updatePlayer(struct VirtualMachine* m) {
...
void virtualmachine_inp_handleSpecialKeys(struct VirtualMachine* m) {
	...
	if (vmVariables[0xC9] == 1) {
		warning("virtualmachine_inp_handleSpecialKeys() unhandled case (vmVariables[0xC9] == 1)");
	}

}

void virtualmachine_snd_playSound(struct VirtualMachine* m, uint16_t resNum, uint8_t freq, uint8_t vol, uint8_t channel) {
...
void virtualmachine_snd_playMusic(struct VirtualMachine* m, uint16_t resNum, uint16_t delay, uint8_t pos) {
...
void virtualmachine_saveOrLoad(struct VirtualMachine* m, Serializer &ser) {
...
void virtualmachine_bypassProtection(struct VirtualMachine* m)
{
	File f(true);
