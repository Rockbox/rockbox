...
#if !defined(_MSC_VER) && !defined(ROCKBOX) // unistd.h does not exist in the Windows SDK.
#include <unistd.h>
#endif
#include <fcntl.h>
...
void __pascal far hof_write() {
	...
	// no O_TRUNC
	handle = rb->open(hof_path, O_WRONLY | O_CREAT | O_BINARY, 0600);
	if (handle < 0 ||
	    rb->write(handle, &hof_count, 2) != 2 ||
	    rb->write(handle, &hof, sizeof(hof)) != sizeof(hof) ||
	    rb->close(handle))
		perror(hof_path);
	if (handle >= 0)
		rb->close(handle);
...
void __pascal far hof_read() {
	...
	char custom_hof_path[POP_MAX_PATH];
	const char* hof_path = get_hof_path(custom_hof_path, sizeof(custom_hof_path));
	handle = rb->open(hof_path, O_RDONLY | O_BINARY);
	if (handle < 0)
		return;
	if (rb->read(handle, &hof_count, 2) != 2 ||
	    rb->read(handle, &hof, sizeof(hof)) != sizeof(hof)) {
		perror(hof_path);
		hof_count = 0;
	}
	rb->close(handle);
