...
void __pascal far save_game() {
	...
	// no O_TRUNC
	handle = rb->open(save_path, O_WRONLY | O_CREAT | O_BINARY, 0600);
	if (handle == -1) goto loc_1DB8;
	if (rb->write(handle, &rem_min, 2) == 2) goto loc_1DC9;
	loc_1D9B:
	rb->close(handle);
	if (!success) {
		unlink(save_path);
	...
	goto loc_1E2E;
	loc_1DC9:
	if (rb->write(handle, &rem_tick, 2) != 2) goto loc_1D9B;
	if (rb->write(handle, &current_level, 2) != 2) goto loc_1D9B;
	if (rb->write(handle, &hitp_beg_lev, 2) != 2) goto loc_1D9B;
	success = 1;
	goto loc_1D9B;
	loc_1E18:
...
short __pascal far load_game() {
	...
	char custom_save_path[POP_MAX_PATH];
	const char* save_path = get_save_path(custom_save_path, sizeof(custom_save_path));
	handle = rb->open(save_path, O_RDONLY | O_BINARY);
	if (handle == -1) goto loc_1E99;
	if (rb->read(handle, &rem_min, 2) == 2) goto loc_1E9E;
	loc_1E8E:
	rb->close(handle);
	loc_1E99:
	return success;
	loc_1E9E:
	if (rb->read(handle, &rem_tick, 2) != 2) goto loc_1E8E;
	if (rb->read(handle, &start_level, 2) != 2) goto loc_1E8E;
	if (rb->read(handle, &hitp_beg_lev, 2) != 2) goto loc_1E8E;
#ifdef USE_COPYPROT
