...
const char* locate_file_(const char* filename, char* path_buffer, int buffer_size) {
    printf("locating file %s...", filename);
	if(file_exists(filename)) {
            printf(" -> exists as %s", filename);
		return filename;
	} else {
...
#else
        snprintf_check(path_buffer, buffer_size, "/%s", filename);
#endif
        printf(" -> doesn't exist, trying %s instead", path_buffer);
...
chtab_type* __pascal load_sprites_from_file(int resource,int palette_bits, int quit_on_error) {
    printf("load_sprites_from_file(%d)", resource);
	int i;
	int n_images = 0;
	...
	for (i = 1; i <= n_images; i++) {
		...
		if (image == NULL) printf("load image failed");
		if (image != NULL) {
...
void load_from_opendats_metadata(int resource_id, const char* extension, FILE** out_fp, data_location* result, byte* checksum, int* size, dat_type** out_pointer) {
    printf("load_from_opendats_metadata(%d)", resource_id);
		...
		if (pointer->handle != NULL) {
			...
			if (i < dat_table->res_count) {
				...
				*size = dat_table->entries[i].size;
                                printf("found resource %d in file", resource_id);
			...
			if (fp != NULL) {
				...
				if (fstat(fileno(fp), &buf) == 0) {
                                    printf("found resource in directory %s, size=%d of file %s", pointer->filename, buf.st_size, locate_file(image_filename));
					...
					*size = buf.st_size;
					...
					fp = NULL;
                            ...
                            printf("failed to open file: %s", image_filename);
                        }
		}
	}
	*out_fp = fp;
...
void far *__pascal load_from_opendats_alloc(int resource, const char* extension, data_location* out_result, int* out_size) {
	// stub
    printf("load_from_opendats_alloc() id = %d\n",resource);
	dat_type* pointer;
	data_location result;
	byte checksum;
	int size;
	...
	if (result == data_none) return NULL;
        ...
        printf("found resource, fp=%p, size=%d", fp, size);
