                    ...
                    uint32_t rate, const char *start, uint32_t length )
{
    ...
    for (i = 0; i < length; i++) {
        ...
        out = fp_div(fp_mul(s, MV_LeftVolume << FRACBITS, FRACBITS), MV_MaxVolume << FRACBITS, FRACBITS);
        //out = (double)s * (double)MV_LeftVolume / (double)MV_MaxVolume;
        out >>= 15; /* divide by 0x8000 */
                      ...
                      uint32_t rate, const char *start, uint32_t length )
{
    ...
    for (i = 0; i < length; i++) {
        ...
        left = fp_div(fp_mul(MV_LeftVolume << FRACBITS, s, FRACBITS), MV_MaxVolume << FRACBITS,
                      FRACBITS);
        //left = (double)MV_LeftVolume * (double)s / (double)MV_MaxVolume;
        left >>= 15; /* divide by 0x8000 */
        //left = left / ((double)0x8000);
        right = fp_div(fp_mul(MV_RightVolume << FRACBITS, s, FRACBITS), MV_MaxVolume << FRACBITS,
                       FRACBITS);
                       ...
                       uint32_t rate, const char *start, uint32_t length )
{
    ...
    for (i = 0; i < length; i++) {
        ...
        /* left and right are fixed-point with FRACBITS */
        left = fp_div(fp_mul(MV_LeftVolume << FRACBITS, s, FRACBITS), MV_MaxVolume << FRACBITS, FRACBITS);
        //left = (double)MV_LeftVolume * (double)s / (double)MV_MaxVolume;
        left >>= 15; /* divide by 0x8000 */
        //left = left / ((double)0x8000);
        right = fp_div(fp_mul(MV_RightVolume << FRACBITS, s, FRACBITS), MV_MaxVolume << FRACBITS, FRACBITS);
        //right = (double)(MV_RightVolume * s) / MV_MaxVolume;
        right >>= 15;
        //right = right / ((double)0x8000);
        dest[0] += left;
