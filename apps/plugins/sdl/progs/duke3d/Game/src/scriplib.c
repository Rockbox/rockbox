...
static void SCRIPT_parseline (char  *curline, scriptnode_t *node)
{
	...
	if (!strncmp (token, "0x", 2)) {
		...
		node->data.string[0] = SCRIPT_copystring (workbuf + 1); /* discard " */
                node->data.string[0][strlen(workbuf) - 2] = '\0'; /* other " */
		...
		if (token[0] == '"') {
			...
			node->data.string[1] = SCRIPT_copystring (workbuf + 1);
                        node->data.string[1][strlen(workbuf) - 2] = '\0'; /* other " */
		}
	} else {
		/* Error state! */
		SCRIPT_recursivefree (node);
...
void SCRIPT_Save (int32 scripthandle, char*  filename)
{
	...
	fp = fopen (filename, "w");
	if (fp == NULL) { rb->splashf(HZ, "Failed to write %s", filename); return; }
