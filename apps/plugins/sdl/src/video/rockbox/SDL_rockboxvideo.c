...
/*      printf("Setting mode %dx%d\n", width, height); */
    ...
    current->pixels = this->hidden->direct ? rb->_viewport_get_framebuffer(NULL, NULL) : this->hidden->buffer;
...
static void flip_pixels(int x, int y, int w, int h)
{
    for(int y_0 = y; y_0 < y + h; ++y_0)
    {
        for(int x_0 = x; x_0 < x + w; ++x_0)
        {
            ...
            rb->_viewport_get_framebuffer(NULL, NULL)[y_0 * LCD_WIDTH + x_0] = swap16(rb->_viewport_get_framebuffer(NULL, NULL)[y_0 * LCD_WIDTH + x_0]);
        }
    }
}
#endif
...
static void blit_rotated(fb_data *src, int x, int y, int w, int h)
{
    for(int y_0 = y; y_0 < y + h; ++y_0)
        for(int x_0 = x; x_0 < x + w; ++x_0)
            rb->_viewport_get_framebuffer(NULL, NULL)[x_0 * LCD_WIDTH + y_0] = src[(LCD_WIDTH - y_0) * LCD_HEIGHT + x_0];
}
        ...
        for(int i = 0; i < numrects; ++i)
        {
            ...
            else
            {
                   ...
                   rects[i].x == 0 && rects[i].y == 0)
                {
                    ...
                    out_bmp.data = (char*)rb->_viewport_get_framebuffer(NULL, NULL);
