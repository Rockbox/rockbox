...
saveGame()
{
  ...
  if(!(fp = fopen(path, "wb"))) {
      printf("Can't open file when saving game\n");
      fflush(stderr);
      return;
  }
...
loadGame()
{
  ...
  if(!loadMapPath(path, 0)) {
    // if can't find saved map load static map
      printf("Can't find current saved map. Will try to use static map.\n");
    if(!loadMap(0)) {
  ...
  while(version > 0) {
    ...
    printf("Trying to load saved game: %s\n", path);
    fflush(stderr);
    fp = fopen(path, "rb");
    if(fp) {
      printf("Saved game found. %s\n", path);
      fflush(stderr);
      break;
    }
    printf("Saved game not found. %s\n", path);
    fflush(stderr);
    version--;
  }
  if(version == 0) {
    printf("Can't find or open saved game.\n");
    fflush(stderr);
    return 0;
  }
  rwop = SDL_RWFromFP(fp, 1);
