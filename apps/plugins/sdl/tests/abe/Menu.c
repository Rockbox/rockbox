...
getSelectionCharIndex(int n)
{
  ...
  for(t = 0; t < entries[n].choice_count; t++) {
    ...
    if(entries[n].selected == t)
      return sum;
    sum += rb->strlen(entries[n].choices[t]);
  }
  return -1;
}
...
{
  return rb->strlen(entries[n].choices[entries[n].selected]);
}
  ...
  for(i = 0; i < MAX_ENTRIES; i++) {
    ...
    if(entries[i].choice_count > 0) {
      ...
      for(t = 0; t < entries[i].choice_count; t++) {
        if(t > 0)
            rb->strlcat(s, " ", 999);
        rb->strlcat(s, entries[i].choices[t], 999);
      }
