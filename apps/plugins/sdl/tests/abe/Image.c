    ...
    if(!mode) {
      ...
      if(!found) {
        ...
        *(name + strlen(name) - 4) = 0;
        ...
        printf("Found: >%s< size=>%ld< blocks=>%d<\n", name,
        ...
        if(!(tmp = fopen(tmp_path, "wb"))) {
          printf("Cannot open temp file for writing: %s\n",
                  tmp_path);
          fflush(stderr);
          exit(0);
        }
