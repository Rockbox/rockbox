...
int32_t initgroupfile(const char  *filename)
{
    ...
    else
    {
        ...
        archive->cached = archive->data != NULL;
    }
    if(archive->cached)
        printf("Caching GRP in RAM (%d KB)\n", total / 1024);
    else
...
#define CRC_BUFLEN 1024*128
    ...
    /* we need a temporary buffer to calculate the CRC */
    if(!archive->cached)
...
int32_t filelength(int32_t fd)
{
    LOGF("=== filelength fd=%d", fd);
    off_t cur = lseek(fd, 0, SEEK_CUR);
    off_t len = lseek(fd, 0, SEEK_END);
    lseek(fd, cur, SEEK_SET);
    LOGF("cur %d len %d", cur, len);
    return len;
}
