...
static uint16_t readLE16(void *addr)
{
    uint8_t *ptr = addr;
    return (*(ptr+1) << 8) | *ptr;
}

static uint32_t readLE32(void *addr)
{
    uint8_t *ptr = addr;
...
playbackstatus MV_GetNextVOCBlock
   ...
   while( !done )
      ...
      blocktype = ( int )*ptr;
      blocklength = ( readLE32( ptr + 1 ) ) & 0x00ffffff;
      ptr += 4;
      ...
      switch( blocktype )
         ...
         case 9 :
            ...
            samplespeed = readLE32(ptr);
            BitsPerSample = readLE32( ptr + 4 );
            Channels = readLE32( ptr + 5 );
            Format = ( unsigned )readLE16( ptr + 6 );
...
int MV_PlayLoopedVOC
   ...
   voice->NextBlock   = ptr + readLE16( ptr + 0x14 );
   voice->DemandFeed  = NULL;
   voice->LoopStart   = NULL;
   voice->LoopCount   = 0;
   voice->BlockLength = 0;
