...
/* road parameters */
#define ROAD_WIDTH (LCD_WIDTH/2) /* half the road width for easier math */
#define MAX_ROAD_LENGTH 4096
#define SEGMENT_LENGTH (ROAD_WIDTH * 12)
...
inline bool project_segment(struct road_segment *base, struct road_segment *seg, int x, int dx, struct point_3d *camera, struct point_2d *p1, struct point_2d *p2, int *maxy)
{
    seg->looped = (seg->idx < base->idx);

    /* future optimization: do one projection per segment */
    ...
    int camera_offset = (seg->looped ? camera->z + SEGMENT_LENGTH : 0);
...
void add_hill(int s, int slope, int len)
{
    int enter=len/4;
    int hold=len/2;
    int leave=len/4;
...
/* this following function calculates the camera depth given its FOV by evaluating */
/* LCD_WIDTH / tan(fov) */
