...
inline void render_segment(struct point_2d *p1, struct point_2d *p2, unsigned color, unsigned border_color, int lanes)
{
    LOGF("render_segment");
    ...
    int x1 = p1->x,
        ...
        r1 = rumble_1,
        r2 = rumble_2;
    fill_poly(x1-w1-r1, y1, x1-w1, y1, x2-w2, y2, x2-w2-r2, y2, border_color);
    fill_poly(x1+w1+r1, y1, x1+w1, y1, x2+w2, y2, x2+w2+r2, y2, border_color);
...
void render(struct point_3d *camera)
{
    ...
    for(int i=0;i<DRAW_DIST;++i)
    {
        ...
        render_segment(&p1, &p2, seg->color, seg->border_color, LANES);
    }
    rb->lcd_update();
}
