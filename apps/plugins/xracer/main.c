...
void render(struct point_3d *camera)
{
    ...
    /* interpolate so as to prevent jumpy movements on hills */
    ...
    for(int i = 0; i < DRAW_DIST;++i)
    {
        ...
        /* some nasty code to loop the track */
        ...
        /* store the relative camera z of the 2nd point to test if it's behind the camera */
        int p2_cz = project(camera, CAMERA_DEPTH, &p2_3d, &p2);

        if(p2_cz <= CAMERA_DEPTH || /* behind camera */
           p2.y >= p1.y          || /* backface cull */
           p2.y >= maxy) /* clipped */
        {
            continue;
        }
...
void generate_random_road(void)
{
    ...
    add_road(0, DRAW_DIST, 0, 0);
    int len;
    for(unsigned int i=DRAW_DIST;i<road_length-DRAW_DIST;i+=len)
    {
        /* make sure it's even */
        len=rb->rand()%40*2;
    ...
    add_road(road_length-DRAW_DIST, DRAW_DIST, 0, 0);
}
