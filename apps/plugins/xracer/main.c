...
void fill_gradient(int top, unsigned top_color, int bottom, unsigned bottom_color)
{
    ...
    {
        rb->lcd_set_foreground(LCD_RGBPACK((int)r,(int)g,(int)b));
...
void render(struct point_3d *camera)
{
    ...
    int x  = 0;
    int maxy=LCD_HEIGHT;
    camera->y=base->p1_y+CAMERA_HEIGHT;
    for(int i=0;i<DRAW_DIST;++i)
    {
        ...
        struct point_3d p1_3d={-x,    seg->p1_y, seg->p1_z},
                        p2_3d={-x-dx, seg->p2_y, seg->p2_z};

        int old_z=camera->z;
        if(p2_3d.z-camera->z<0)
           camera->z=0;
        project(camera, CAMERA_DEPTH, &p1_3d, &p1);
        int p1_cz = project(camera, CAMERA_DEPTH, &p2_3d, &p2);

        if(p1_cz <= CAMERA_DEPTH || /* behind camera */
           p2.y >= maxy) /* clipped */
        {
            continue;
        }
        maxy=p2.y;
        render_segment(&p1, &p2, seg->color, seg->border_color, LANES, seg->lane_color, seg->grass_color);
        x+=dx;
        dx+=seg->curve;
        camera->z=old_z;
    }
...
void generate_random_road(void)
{
    ...
    int len;
    for(unsigned int i=0;i<ARRAYLEN(road);i+=len)
    {
        /* make sure it's even */
        len=rb->rand()%10*2;
...
void generate_straight_road(void)
{
    add_road(0, ROAD_LENGTH, 0, 0);
}

inline float ease_in(int a, int b, float percent)
{
    return a + (b-a) * (percent * percent);
}

void generate_hilly_road(void)
{
}
...
enum plugin_status plugin_start(const void *param)
{
    ...
    while(1)
    {
        ...
        camera.z+=512;
        if(camera.z>=ROAD_LENGTH*SEGMENT_LENGTH)
            camera.z%=ROAD_LENGTH;
        render(&camera);
        rb->lcd_update();
        rb->yield();
    }
