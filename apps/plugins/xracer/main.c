...
void render(struct point_3d *camera)
{
    ...
    for(int i=0;i<DRAW_DIST;++i)
    {
        ...
        LOGF("i: %d idx: %d", i, (base->idx+i)%ROAD_LENGTH);
        struct point_2d p1, p2; /* projected points */
        struct point_3d p1_3d={-x,    seg->p1_y, seg->p1_z, 0},
                        p2_3d={-x-dx, seg->p2_y, seg->p2_z, 0};
...
void add_straightaway(int s, int len)
{
    int last_y=0;
