...
static inline int project(int cam_x, int cam_y, int cam_z, int camera_depth, struct point_3d *pt_3d, struct point_2d *pt_2d)
{
                                   ...
                                   int road_length)
{
    ...
    int p2_cz = project(camera->pos.x, camera->pos.y, camera->pos.z - camera_offset, camera->depth, &p2_3d, p2);
    ...
    *maxy = p2->y;
    ...
    project(camera->pos.x, camera->pos.y, camera->pos.z - camera_offset, camera->depth, &p1_3d, p1);
...
void render(struct camera_t *camera, struct road_segment *road, unsigned int road_length)
{
    ...
    float base_percent = (camera->pos.z % SEGMENT_LENGTH) / SEGMENT_LENGTH;
    ...
    for(int i = 0; i < DRAW_DIST; ++i)
    {
        ...
        if(project_segment(base, seg, x, dx, camera, &p1, &p2, &maxy, road_length))
