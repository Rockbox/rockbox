...
static inline int interpolate(long a, long b, long percent)
{
    int result = a + fp_mul(b-a, percent, FRACBITS);
...
void render(struct camera_t *camera, struct road_segment *road, unsigned int road_length, int camera_height)
{
    ...
    camera->pos.y = interpolate(base->p1_y << FRACBITS, base->p2_y << FRACBITS, base_percent) + camera_height;
    //camera->pos.y = camera_height;
