...
{
    return pf_idx.album_names + pf_idx.album_index[slide_index].name_idx;
}
...
static char* get_album_artist(const int slide_index)
{
    if (slide_index < pf_idx.album_ct && slide_index >= 0){
        ...
        if (artist_pos < pf_idx.artist_ct && artist_pos >= 0){
            return pf_idx.artist_names + pf_idx.artist_index[artist_pos].name_idx;
        }
    }
    return NULL;
}
...
static void draw_progressbar(int step)
{
    ...
    rb->lcd_fillrect(x+1, y+1, step * w / pf_idx.album_ct, bar_height-2);
#if LCD_DEPTH > 1
    rb->lcd_set_foreground(N_BRIGHT(255));
#endif
    rb->lcd_update();
