...
static int get_wps_current_index(void)
{
    ...
    if(id3)
    {
        ...
        for (i = 0; i < pf_idx.album_ct; i++ )
        {
            int album_idx = pf_idx.album_index[i].name_idx;
...
retry:
        ...
        if (len > avail)
        {
            while (len > avail)
            {
                ...
                new_tracks = (struct track_data *)(out + (uintptr_t)tracks);
...
static bool incremental_albumart_cache(bool verbose)
{
    ...
    idx = aa_cache.idx;
    if (idx >= pf_idx.album_ct || idx < 0) { idx = 0; }


    aa_cache.idx++;
    aa_cache.inspected++;
    if (aa_cache.idx >= pf_idx.album_ct) { aa_cache.idx = 0; }
...
aa_success:
    if(verbose)/* direct interaction with user */
        return true;
    else /*background thread back off every 3 albums */
        return ((aa_cache.inspected % 3) != 0);
}
...
static void thread(void)
{
    ...
    while (1) {
        ...
        if(ev.id != SYS_TIMEOUT) {
          ...
          while (incremental_albumart_cache(false)) {
            rb->yield();
            switch (ev.id) {
                case EV_EXIT:
                    return;
...
static void select_prev_track(void)
{
    if (selected_track > 0 ) {
        ...
        start_index_track_list = track_count - track_list_visible_entries;
    }
}
...
static int pictureflow_main(void)
{
    ...
    size_t aa_bufsz = ALIGN_DOWN(pf_idx.buf_sz / 2, 0x4);
