...
#define THREAD_STACK_SIZE DEFAULT_STACK_SIZE + 0x400
...
static int save_album_index(void){
    int fd;
    fd = rb->creat("/.rockbox/rocks/demos/album_ndx.tmp",0666);
    if(fd >= 0)
    {        
        rb->write(fd, artist_names, ((char *)buf - (char *)artist_names));

        unsigned int artist_pos = (char *)artist - (char *)artist_names;
        rb->write(fd, &artist_pos, sizeof(unsigned int));
        
        unsigned int album_names_pos = (char *)album_names - (char *)artist_names;
        rb->write(fd, &album_names_pos, sizeof(unsigned int));
        
        unsigned int album_pos = (char *)album - (char *)artist_names;
        rb->write(fd, &album_pos, sizeof(unsigned int));

        rb->write(fd, &artist_count, sizeof(int));
        rb->write(fd, &album_count, sizeof(int));
...
static int load_album_index(void){
    ...
    int fr = rb->open("/.rockbox/rocks/demos/album_ndx.tmp", O_RDONLY);
    if (fr >= 0){
        ...
        rb->read(fr,&pos ,sizeof(unsigned int));
        ...
        rb->read(fr,&artist_count ,sizeof(int));
        rb->read(fr,&album_count ,sizeof(int));
...
{
    return artist_names + artist[album[slide_index].artist_idx].name_idx;
}
...
static void draw_album_text(void)
{
    ...
    mylcd_putsxy(albumtxt_x, albumtxt_y, albumtxt);    set_scroll_line(artisttxt, PF_SCROLL_ARTIST);
