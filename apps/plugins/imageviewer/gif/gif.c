...
#ifdef USEGSLIB
    return (p_decoder->width/ds) * (p_decoder->height/ds);
#else
    return (p_decoder->width/ds) * 
           (p_decoder->height/ds) * 
           FB_DATA_SZ;
#endif
}
                      ...
                      unsigned char *buf, ssize_t *buf_size)
{
    ...
    disp_buf = p_decoder->mem + p_decoder->native_img_size;
