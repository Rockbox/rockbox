...
static void set_canvas_background(GifPixelType *Line, void *out, GifFileType *GifFile)
{
    int i;

    /* Reading Gif spec it seems one should always use background color in canvas
     * but most real files omit this and sets background color to 0 (which IS
     * valid index). We can choose to either conform to standard (and wrongly
     * display most of gifs with transparency) or stick to common practise and
     * treat background color 0 as transparent. I preffer the second.
     */
    ...
    {
#ifdef HAVE_LCD_COLOR
        memset(out, 0, GifFile->SWidth*GifFile->SHeight*sizeof(struct uint8_rgb));    
#else
        memset(out, 0xff, GifFile->SWidth*GifFile->SHeight);
#endif
    }
                ...
                void (*pf_progress)(int current, int total))
{
    ...
    int buf_idx = 0;
#ifdef HAVE_LCD_COLOR
    struct uint8_rgb *pixels_buffer[2];
    pixels_buffer[0] = (struct uint8_rgb *)malloc(GifFile->SWidth * GifFile->SHeight * sizeof(struct uint8_rgb));
    pixels_buffer[1] = NULL;
#else
    unsigned char *pixels_buffer[2];
    pixels_buffer[0] = (unsigned char *)malloc(GifFile->SWidth * GifFile->SHeight);
    pixels_buffer[1] = NULL;
#endif
    ...
    set_canvas_background(Line, (void *)pixels_buffer[0], GifFile);
    ...
    do
    {
        ...
        switch (RecordType)
        {
            case IMAGE_DESC_RECORD_TYPE:
                ...
                if (GifFile->Image.GCB && GifFile->Image.GCB->DisposalMode == DISPOSE_PREVIOUS)
                {
                    /* We need to take a snapshot before rendering the image in order
                     * to restore canvas to previous state after rendering
                     */
                     buf_idx ^= 1;
                     ...
                     {
#ifdef HAVE_LCD_COLOR
                         pixels_buffer[buf_idx] = (struct uint8_rgb *)malloc(GifFile->SWidth * GifFile->SHeight * sizeof(struct uint8_rgb));
#else
                         pixels_buffer[buf_idx] = (unsigned char *)malloc(GifFile->SWidth * GifFile->SHeight);
#endif
                     }
                }
                ...
                if (GifFile->Image.Interlace)
                {
                    ...
                    for (i = 0; i < 4; i++)
                    {
                        for (j = Row + InterlacedOffset[i]; j < Row + Height; j += InterlacedJumps[i])
                        {
                            ...
                            gif2pixels(Line, pixels_buffer[buf_idx], Row + j, Col, Width);
                        }
                ...
                else
                {
                    for (i = 0; i < Height; i++)
                    {
                        ...
                        gif2pixels(Line, pixels_buffer[buf_idx], Row + i, Col, Width);
                ...
                if (GifFile->Image.GCB && GifFile->Image.GCB->DelayTime != 0)
                {
                    ...
                    switch (GifFile->Image.GCB->DisposalMode)
                    {
                        case DISPOSE_BACKGROUND:
                            set_canvas_background(Line, pixels_buffer[buf_idx], GifFile);
                            break;
                        ...
                        default:
                            /* DISPOSAL_UNSPECIFIED and DISPOSE_DO_NOT */
                            break;
                    }
    ...
    if (!recalc_dimension(&dim_dst, &dim_src))
    {
         ...
         * free memory
         */
        if (d->native_img_size*d->frames_count + c_native_img_size <= d->mem_size)
        {
            img_dst.width = dim_dst.width;
            img_dst.height = dim_dst.height;
            img_dst.data = (unsigned char *)d->mem + d->native_img_size*d->frames_count;
            ...
            for (i = 0; i < d->frames_count; i++)
            {
                ...
                memmove(d->mem + i*c_native_img_size, img_dst.data, c_native_img_size);
            }
