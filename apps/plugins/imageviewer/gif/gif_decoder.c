                       ...
                       int Col)
{
    ...
    output_row_8(Row, (void *)(buf + Col), ctx);
...
void gif_decoder_init(struct gif_decoder *d, void *mem, size_t size)
{
    ...
    /* mem allocator init */
    init_memory_pool(d->mem, d->mem_size);
}
            ...
            case IMAGE_DESC_RECORD_TYPE:
                    ...
                    for (i = 0; i < 4; i++)
                    {
                        for (j = Row + InterlacedOffset[i]; j < Row + Height; j += InterlacedJumps[i])
                        {
                            ...
                            if (!skip || (GCB && GCB->DelayTime == 0))
                                gif2native(&ctx, ColorMap, GCB, &Line[Col], j, Col);
                        }
                ...
                else
                {
                    for (i = 0; i < Height; i++)
                    {
                        ...
                        if (!skip || (GCB && GCB->DelayTime == 0))
                            gif2native(&ctx, ColorMap, GCB, &Line[Col], i, Col);
