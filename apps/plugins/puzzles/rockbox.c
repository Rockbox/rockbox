...
static char *init_for_game(const game *gm, int load_fd)
{
...
static bool load_game(void)
{
    ...
    else
    {
        ...
        for(int i = 0; i < gamecount; ++i)
        {
            ...
            {
                sfree(ret);
                ret = init_for_game(gamelist[i], fd);
                if(ret)
                {
                    rb->splash(HZ, ret);
                    sfree(ret);
...
#else
    giant_buffer = rb->plugin_get_audio_buffer(&giant_buffer_len);
#endif

#ifndef COMBINED
    rb->snprintf(save_file_path, sizeof(save_file_path), "%s/%s.sav", PLUGIN_GAMES_DATA_DIR, thegame.htmlhelp_topic);
#endif
