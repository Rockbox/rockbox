...
#endif
            draw_antialiased_line(rb->_viewport_get_framebuffer(NULL, NULL), LCD_WIDTH, LCD_HEIGHT, x1, y1, x2, y2);
    }
    else
    {
...
#error no vertical stride
    ...
    if(bl && bl->bmp.data)
    {
        ...
        fb_data *fb = zoom_enabled ? zoom_fb : rb->_viewport_get_framebuffer(NULL, NULL);
...
static void timer_cb(void)
{
    ...
    if(debug_settings.timerflash)
    {
        ...
        what = !what;
        if(what)
            rb->_viewport_get_framebuffer(NULL, NULL)[0] = LCD_BLACK;
        else
            rb->_viewport_get_framebuffer(NULL, NULL)[0] = LCD_WHITE;
        rb->lcd_update();
    }
#endif
...
static void bench_aa(void)
{
    ...
    {
        draw_antialiased_line(rb->_viewport_get_framebuffer(NULL, NULL), LCD_WIDTH, LCD_HEIGHT, 0, 0, 20, 31);
        ++i;
    }
