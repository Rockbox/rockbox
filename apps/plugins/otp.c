    ...
    if(!rb->memcmp(magic, buf, 4))
    {
        ...
        if(encrypted)
        {
            ...
            for(int i = 0; i < 3; ++i)
            {
                ...
                hmac_sha1(key, sizeof(key), accounts, next_slot * sizeof(struct account_t), mac_calculated);
...
static void save_accts(void)
{
    ...
    if(encrypted)
    {
        ...
        hmac_sha1_init(&hmac_ctx, key, sizeof(key));
