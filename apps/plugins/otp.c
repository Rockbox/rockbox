        ...
        do {
            ...
            do {
                ...
                else if(!rb->strcmp(tok, "period"))
                {
                    if(!accounts[next_slot].is_totp)
                    {
                        rb->splash(HZ * 2, "WARNING: period parameter specified for HOTP!?");
                        goto fail;
                    }
                    tok = rb->strtok_r(NULL, "&", &save);
...
static void show_code(int acct)
{
    ...
    {
        rb->splashf(0, "%0*d (%ld second(s) left)", accounts[acct].digits, TOTP(accounts[acct].secret,
                                                                             accounts[acct].sec_len,
                                                                             accounts[acct].totp_period,
                                                                             accounts[acct].digits),
                    accounts[acct].totp_period - rb->mktime(rb->get_time()) % accounts[acct].totp_period);
    }
#else
    else
    {
...
static void edit_menu(int acct)
{
    ...
    MENUITEM_STRINGLIST(menu, "Edit Account", NULL,
                        "Rename",
                        "Delete",
                        "Change HOTP Counter",
                        "Back");
    ...
    while(!quit)
    {
        ...
        case 0:
            ...
            rb->strlcpy(buf, accounts[acct].name, sizeof(buf));
            ...
            rb->strlcpy(accounts[acct].name, buf, sizeof(accounts[acct].name));
            save_accts();
            break;
        case 1:
            if(danger_confirm())
        ...
        case 2:
            if(accounts[acct].is_totp)
            {
                rb->splash(HZ * 2, "Not an HOTP account!");
                break;
            }

            /* no 64-bit printf support */
            rb->snprintf(buf, sizeof(buf), "%d", (int)accounts[acct].hotp_counter);
            if(rb->kbd_input(buf, sizeof(buf)) < 0)
                break;

            accounts[acct].hotp_counter = rb->atoi(buf);
            save_accts();
...
static void edit_accts(void)
{
    ...
    if(next_slot > 0)
    {
        rb->lcd_putsf(0, 0, "Edit Accounts");
        rb->lcd_putsf(0, 1, "%s", accounts[0].name);
        rb->lcd_update();
    }
    else
