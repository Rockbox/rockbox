...
local function start_round(level, goal, nrBalls, total)
    ...
    if level < 1 then
        level = 1
        test_time = true
        cursor = { x = LCD_W, y = LCD_H, color = 0x0}
        table.insert(Balls, Ball:new({
                            x = 1, y = 1, xi = 1, yi = 1,
                            color = 0x0, step_delay = 1,
    ...
    if test_time and test_time > 0 then
        test_time = (rb.current_tick() - test_time - 100)
        if  test_time < 25 then
            MAX_BALL_SPEED = MAX_BALL_SPEED * 2
...
local idx, highscore = 1, 0
    ...
    start_round(0, 0, 1, 0) -- test speed of target
