...
function Ball:new(o, level, color)
    local Ball = self or {} --so we can use Ball. instead of self.
   ...
   if o == nil then
       ...
       o = {
               ...
               draw_fn = self.draw, -- reference to current drawing function
    ...
    function Ball.draw_exploded(self)
    ...
    function Ball.step_exploded(self, tick)
    ...
    function Ball.checkHit(self, other)
...
function start_round(level, goal, nrBalls, total)
       ...
       for _, Ball in ipairs(Balls) do
            if tick > Ball.next_tick then
                ...
                if Ball:step_fn(tick) == B_DEAD then
                    ...
                    checkhit_fn(Ball) -- changes dynamically at runtime
                end
            end
       end
       return tick
   ...
   if level < 1 then
       ...
       speedtest = nil -- discard speedtest function
