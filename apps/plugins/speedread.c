...
int word_num;
off_t line_offs, begin_offs;

int line_len = -1, custom_font = FONT_UI;

const char *get_next_word(void)
{
    if(fd >= 0)
    {
...
static bool load_bookmark(const char *fname)
{
    int bookmark_fd = rb->open(BOOKMARK_FILE, O_RDONLY);
    if(bookmark_fd >= 0)
    {
...
static void load_font(void)
{
    ...
    if(font_fd < 0)
        return;
    char buf[MAX_PATH + 1];
    int len = rb->read(font_fd, buf, MAX_PATH);
    buf[len] = '\0';
    rb->close(font_fd);
    new_font(buf);
}
        ...
        case BACK:
            if(fd >= 0)
            {
                ...
                do {
                    ...
                    rb->splashf(0, "%ld/%ld bytes", offs + base_offs, file_size);
                    rb->yield();
                } while(get_useraction() == BACK && offs + base_offs < file_size && offs + base_offs >= 0);
