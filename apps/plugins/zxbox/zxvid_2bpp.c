...
void update_screen(void)
{
    ...
    {
        frameb = rb->_viewport_get_framebuffer(NULL, NULL) + (y) * FB_WIDTH;
    ...
    for(y = 0; y < LCD_HEIGHT; y++)
    {
        frameb = rb->_viewport_get_framebuffer(NULL, NULL) + (y/4) * LCD_WIDTH;
        srcx = 0;           /* reset our x counter before each row... */
        shift = ((y & 3 ) * 2 );
        mask = ~pixmask[y & 3];
        for(x = 0; x < LCD_WIDTH; x++)
    ...
    for(y = 0; y < LCD_HEIGHT; y++)
    {
        frameb = rb->_viewport_get_framebuffer(NULL, NULL) + (y/8) * LCD_WIDTH;
        srcx = 0;           /* reset our x counter before each row... */
        shift = (y & 7);
        mask = ~pixmask[y & 7];
        for(x = 0; x < LCD_WIDTH; x++)
