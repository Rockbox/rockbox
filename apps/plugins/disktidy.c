...
static enum plugin_status tidy_clean(char *path, int *path_length, bool rm_all)
{
    ...
    while ((entry = rb->readdir(dir)))
    {
        ...
        bool remove = tidy_remove_item(entry->d_name, info.attribute) || rm_all;
        ...
        if (info.attribute & ATTR_DIRECTORY) {
            ...
            if (!remove) {
                ...
                /* If this is a directory that doesn't need to be deleted then
                   check it for files/directores that may need to deleted */
                disktidy_status = tidy_clean(path, path_length, false);
        ...
        if (info.attribute & ATTR_DIRECTORY)
        {
            /* Remove this directory and all files/directores it contains */
            disktidy_status = tidy_clean(path, path_length, true);
    ...
    if (rm_all && disktidy_status == PLUGIN_OK) {
        rb->rmdir(path);
        run_stats.dirs_removed++;
    }
    ...
    while (!exit && disktidy_status == PLUGIN_OK) {
        switch(rb->do_menu(&menu, &selection, NULL, false)) {
            ...
            case 1:
                ...
                break;
            case 2:
                if (rb->file_exists(LAST_RUN_STATS_FILE)) {
                    disktidy_status = display_run_stats();
                } else {
                    rb->splash(HZ * 2, "Run disktidy to generate stats");
                }
