...
#define OP_RUN_ITEM     LAST_ACTION_PLACEHOLDER + 0x1
#define OP_ADD_ITEM     LAST_ACTION_PLACEHOLDER + 0x2
#define OP_REMOVE_ITEM  LAST_ACTION_PLACEHOLDER + 0x3

#define OP_PLUGIN_RESTART (PLUGIN_GOTO_PLUGIN | 0x200)
...
const off_t op_name_sz = OPEN_PLUGIN_NAMESZ +
                         (op_entry.name - (char*)&op_entry);
...
static bool plugin_entry_read(int fd, int selected_item, bool include_paths)
{
    rb->memset(&op_entry, 0, op_entry_sz);
    op_entry.lang_id = -1;
    off_t data_sz = (include_paths ? op_entry_sz : op_name_sz);
    return ((selected_item >= 0) &&
        (rb->lseek(fd, selected_item  * op_entry_sz, SEEK_SET) >= 0) &&
        (rb->read(fd, &op_entry, data_sz) == data_sz));
}

static int plugin_entry_get_path(void)
{
    int ret = 0;
...
static void plugin_entry_get_param(void)
{
    char tmp_buf[OPEN_PLUGIN_BUFSZ+1];
...
static int pte_exclude_hash(struct open_plugin_entry_t *op_entry, int item, void *data)
{
    (void)item;
    if (data)
    {
...
static int pte_exclude_empty(struct open_plugin_entry_t *op_entry, int item, void *data)
{
    (void)item;
    (void)data;
...
static int plugin_entry_transfer(int fd_tmp,
                            int(*compfn)(struct open_plugin_entry_t*, int, void*),
                            void *data)
{
    int entries = -1;
    if (fd_tmp && fd_dat && rb->lseek(fd_dat, 0, SEEK_SET) == 0)
    {
        entries = 0;
...
uint32_t plugin_entry_add_path(const char *key, const char *plugin, const char *parameter)
{
    int len;
    uint32_t hash;
    char *pos;
    ...
    if (plugin)
    {
        ...
        len = rb->strlcpy(op_entry.name, pos, OPEN_PLUGIN_NAMESZ);

        if(len > 5 && rb->strcasecmp(&(pos[len-5]), ".rock") == 0)
        {
            fd = rb->open(OPEN_PLUGIN_DAT ".tmp", O_WRONLY | O_CREAT | O_TRUNC, 0666);
            if (fd < 0)
                return 0;
            ...
            {
                if (parameter[0] == '\0' &&
                    _yesno_pop(ID2P(LANG_OPEN_PLUGIN_ADD_PARAMETER)))
                {
                    plugin_entry_get_param();
                }
                else
                    rb->strlcpy(op_entry.param, parameter, OPEN_PLUGIN_BUFSZ);
            }

            rb->write(fd, &op_entry, op_entry_sz);
        }
        else
        {
    ...
    if (plugin_entry_transfer(fd, pte_exclude_hash, &hash) == 0)
        hash = 0;

    return hash;
}

void plugin_entry_browse_add(int selection)
{
    char* key;
    plugin_entry_read(fd_dat, selection, true);
    if (plugin_entry_get_path() > 0)
    {
        ...
        plugin_entry_add_path(key, op_entry.path, NULL);
    }
}

static void plugin_entry_remove(int selection)
{
...
static void plugin_entry_remove_empty(void)
{
    bool resave = false;
    ...
    if (resave)
    {
        int fd = rb->open(OPEN_PLUGIN_DAT ".tmp", O_WRONLY | O_CREAT | O_TRUNC, 0666);
        if (fd < 0)
            return;

        plugin_entry_transfer(fd, &pte_exclude_empty, NULL);
    }

}

static int plugin_entry_run(int selection)
{
    int ret = PLUGIN_ERROR;
    char* path;
    char* param;
    if (plugin_entry_read(fd_dat, selection, true))
    {
        rb->splash(0, ID2P(LANG_OPEN_PLUGIN));
        path = op_entry.path;
                                    ...
                                    char* buf, size_t buf_len)
{
    ...
    {
        if (plugin_entry_read(fd_dat, selected_item, false))
        {
            if (op_entry.lang_id >= 0)
                            ...
                            struct gui_synclist *this_list)
{
    ...
    else if ((action == ACTION_STD_OK))
    {
        ...
        /*Run, Edit, Remove, Blank, Add, Back*/
        switch(selection)
        {
            case 0:
            case 1:
...
void edit_menu(int selection)
{
    ...
    synclist_set(MENU_ID_EDIT, 0, 6, 2);
    ...
    if (crc != rb->crc_32(&op_entry, op_entry_sz, 0xffffffff) &&
        ...
        rb->lseek(fd_dat, selection  * op_entry_sz, SEEK_SET) >= 0)
    {
        rb->write(fd_dat, &op_entry, op_entry_sz);
    }
}
...
static int context_menu(int selection)
{
    ...
    if (plugin_entry_read(fd_dat, selection, true))
    {
        MENUITEM_STRINGLIST(menu, op_entry.name, context_menu_cb,
            "Run", "Edit", ID2P(LANG_REMOVE), ID2P(VOICE_BLANK),"Add", "Back");
        ...
        switch (selected_item)
        {
            ...
            case 1: /*edit*/
                ...
                break;
            case 2: /*remove*/
                plugin_entry_remove(selection);
                break;
            case 3: /*blank*/
                break;
            case 4:
                plugin_entry_browse_add(-1);
...
enum plugin_status plugin_start(const void* parameter)
{
    ...
    if (parameter)
    {
        ...
        while (read(fd_dat, &op_entry, op_entry_sz) == op_entry_sz)
        {
            item++;
            if (op_entry.hash == hash)
            {
        ...
        if (selection >= 0)
        {
            if (plugin_entry_run(selection) == PLUGIN_GOTO_PLUGIN)
                return PLUGIN_GOTO_PLUGIN;
        }
        else
        {
            plugin_entry_read(fd_dat, selection, true);
            plugin_entry_add_path(parameter, parameter, "\0");
            selection = 0;
            items++;
            fd_dat = rb->open(OPEN_PLUGIN_DAT, creat_flags, 0666);
            if (!fd_dat)
    ...
    while (!exit)
        {
            ...
            switch (action)
            {
                ...
                case ACTION_STD_OK:
                    ret = plugin_entry_run(selection);
                    exit = true;
                    break;
                case ACTION_STD_CANCEL:
                {
