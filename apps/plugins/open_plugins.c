...
uint32_t op_entry_add_path(const char *key, const char *plugin, const char *parameter);
...
static int op_entry_transfer(int fd_tmp,
...
uint32_t op_entry_add_path(const char *key, const char *plugin, const char *parameter)
{
    ...
    if (op_entry_transfer(fd, op_et_exclude_hash, &hash) > 0)
    {
        rb->close(fd);
        rb->close(fd_dat);
...
static void op_entry_remove_empty(void)
{
    ...
    if (resave)
    {
        ...
        if ((op_entry_transfer(fd, &op_et_exclude_builtin, NULL)
            + op_entry_transfer(fd, &op_et_exclude_user, NULL)) > 0)
        {
            rb->close(fd);
            rb->close(fd_dat);
            rb->remove(OPEN_PLUGIN_DAT);
...
static int op_entry_run(int selection)
{
...
static int context_menu(int selection)
{
    ...
    if (op_entry_read(fd_dat, selection, op_entry_sz))
    {
        MENUITEM_STRINGLIST(menu, op_entry.name, context_menu_cb,
            ID2P(LANG_RUN), ID2P(LANG_EDIT), ID2P(LANG_REMOVE),
...
enum plugin_status plugin_start(const void* parameter)
{
    ...
    if (parameter)
    {
        ...
        if (selection >= 0)
        {
            if (op_entry_run(selection) == PLUGIN_GOTO_PLUGIN)
                return PLUGIN_GOTO_PLUGIN;
        }
        else
        {
