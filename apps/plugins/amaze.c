    ...
    if(fd >= 0)
    {
        ...
        for(y=0; y < maxy; y++)
        {
            for (x=0; x < maxx; x++)
            {
                ...
                // TODO: folgendes nur in einer Map machen, sonst überschreiben
                // Markierungen den Player/den Start oder umgekehrt!
                if(y == py && x == px)
...
int pause_menu(void)
{
    ...
    MENUITEM_STRINGLIST(menu,"Amaze Menu", NULL, "Resume Game", "Start New Game",
                        "View Map", "Mark Ground", "Clear Mark", "View Compass",
                        "Show Solution",
