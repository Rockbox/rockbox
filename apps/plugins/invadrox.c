...
{
    return rb->_viewport_get_framebuffer(NULL, NULL)[x*LCD_HEIGHT+y];
}
#else
static inline fb_data get_pixel(int x, int y)
{
    return rb->_viewport_get_framebuffer(NULL, NULL)[ytab[y] + x];
}
#endif
...
static inline fb_data get_pixel(int x, int y)
{
    return (rb->_viewport_get_framebuffer(NULL, NULL)[ytab[y] + (x >> 2)] >> shifts[x & 3]) & 3;
}
#else
/* Vertical packing */
static const unsigned char shifts[4] = {
...
static inline fb_data get_pixel(int x, int y)
{
    return (rb->_viewport_get_framebuffer(NULL, NULL)[ytab[y] + x] >> shifts[y & 3]) & 3;
}
