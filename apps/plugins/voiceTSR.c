...
static uint16_t g_bin_added = 0;
static int g_target_info = 0;
static int g_info_reset_timeout = 0;
...
static enum themable_icons icon_callback(int selected_item, void * data)
{
    ...
    {
        uint16_t bin = 1 << (selected_item);
        if ((g_bin_added & bin) == bin)
            return Icon_Submenu;
    }
    ...
    if ((action == ACTION_STD_OK))
    {
        ...
        switch(selection) {
            ...
            case 8: /*Clear All*/
                ...
                break;
            case 9: /* inspect it */
                rb->kbd_input(gAnnounce.wps_fmt, MAX_ANNOUNCE_WPS);
...
static int announce_menu(void)
{
    ...
    MENUITEM_STRINGLIST(announce_menu, "Announcements", announce_menu_cb,
                        ...
                        "",
                        "Clear All",
                        ID2P(LANG_WPS_ANNOUNCEMENT_FORMAT),
                        ID2P(LANG_VOICE),
                        "Cancel",
                        ID2P(LANG_SAVE_EXIT));
...
static int settings_menu(void)
{
    ...
    MENUITEM_STRINGLIST(settings_menu, "VoiceTSR Settings", NULL,
                        ID2P(LANG_ID3_FREQUENCY),
                        "Announce On: ",
                        "Grouping",
                        ID2P(LANG_WPS_ANNOUNCEMENT_FORMAT),
                        "",
    ...
    do {
        ...
        switch(selection) {
            ...
            case 0:
                rb->set_int(rb->str(LANG_ID3_FREQUENCY), "s", UNIT_SEC,
                            &gAnnounce.interval, NULL, 1, 1, 360, NULL );
                break;
            case 1:
                rb->set_option("Announce On: ",
                      &gAnnounce.announce_on, INT, announce_options, 3, NULL);
                break;
            case 2:
                rb->set_int("Grouping", "", 1,
            ...
            case 8:
                old_val = boolval;
                rb->set_bool(rb->str(LANG), &boolval);
                if (old_val == boolval) /* changed? */
                    break;
                /* fallthrough if changed */
...
void thread_create(void)
{
    ...
    gThread.stacksize = gThread.buf_size / 2;
...
int plugin_main(const void* parameter)
{
    ...
    gAnnounce.grouping = 1;

    rb->splash(HZ / 2, "Voice TSR");
    rb->splash(0, "Hold for settings");
...
static int voice_general_info(void)
{
    ...
    #if CONFIG_RTC
            /* announce the time */
            voice_info_group("D1Dt ");
    #else
            /* announce elapsed play for this track */
            voice_info_group("T1Te ");
    #endif
        return -1;
    }

    if (TIME_AFTER(current_tick, g_info_reset_timeout))
    {
        g_target_info = 0;
    }

    g_info_reset_timeout = current_tick + ANNOUNCEMENT_TIMEOUT * HZ;
    ...
    */
    ...
    while (*infotemplate != 0 && test_info != g_target_info)
    {
        if (*infotemplate == '.')
        {
            test_info++;
    ...
    while (*current_token != 0 && *current_token != '.')
    {
        ...
        else if (current_char == 'R')
        {
            ...
            */
            ...
            if (current_char == 'S')
            {
                sleep_remaining = rb->get_sleep_timer();
            ...
            {
                rb->talk_id(LANG_REMAINING, true);
            }
        ...
        else if (current_char == 'P')
        {
            ...
            */
            ...
            {
                rb->talk_id(LANG_TRACKONLY, true);
            }
            else if (current_char == '2')
            {
                rb->talk_id(LANG_OF, true);
