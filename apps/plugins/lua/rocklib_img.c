...
RLI_LUA rli_copy(lua_State *L)
{
    ...
    {
        rbp()->memcpy(dst->data, src->data, dst->elems * sizeof(fb_data));
        return 0;
    }
...
#define RB_SCREEN_STRUCT(luastate, narg) \
        rbp()->screens[get_screen(luastate, narg)]
#define RB_SCREENS(luastate, narg, func, ...) \
        rbp()->screens[get_screen(luastate, narg)]->func(__VA_ARGS__)
...
#define RB_SCREEN_STRUCT(luastate, narg) \
        rbp()->screens[SCREEN_MAIN]
#define RB_SCREENS(luastate, narg, func, ...) \
        rbp()->screens[SCREEN_MAIN]->func(__VA_ARGS__)
#endif
...
RB_WRAP(font_getstringsize)
{
    ...
    if (fontnumber == FONT_UI)
        fontnumber = rbp()->global_status->font_id[SCREEN_MAIN];
    else
    ...
    if lua_isnil(L, 2)
        ...
        result = rbp()->font_getstringsize(str, &w, &h, fontnumber);
...
{
    rli_wrap(L, rbp()->lcd_framebuffer, LCD_WIDTH, LCD_HEIGHT);
    return 1;
}
...
RB_WRAP(gui_scrollbar_draw)
{
    ...
    rbp()->gui_scrollbar_draw(RB_SCREEN_STRUCT(L, 9), v[x], v[y], v[w], v[h],
...
{
    fb_data* backdrop = rbp()->lcd_get_backdrop();
    if(backdrop == NULL)
        lua_pushnil(L);
    else
...
RB_WRAP(lcd_blit_grey_phase)
{
    ...
    rbp()->lcd_blit_grey_phase(values, phases, v[bx], v[by], v[bw], v[bh], v[stride]);
    return 0;
}
...
RB_WRAP(lcd_blit_mono)
{
    ...
    rbp()->lcd_blit_mono(data, v[x], v[by], v[w], v[bh], v[stride]);
    return 0;
}
...
RB_WRAP(read_bmp_file)
{
    ...
    int result = rbp()->read_bmp_file(filename, &bm, 0, format | FORMAT_RETURN_SIZE, NULL);

    if(result > 0)
    {
        bm.data = (unsigned char*) rli_alloc(L, bm.width, bm.height);
        if(rbp()->read_bmp_file(filename, &bm, result, format, NULL) < 0)
        {
