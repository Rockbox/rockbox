...
static enum themable_icons menu_item_get_icon_cb(int selected_item, void *data)
{
    (void) selected_item;
    struct menu_cb *menu_cb = data;
    return menu_cb->icon_id < Icon_NOICON ? Icon_NOICON : menu_cb->icon_id;
}
...
static int menu_action_cb(int action, struct gui_synclist *lists)
{
    ...
    else if (action == ACTION_STD_MENU) /* emulate behavior of rb->do_menu */
    {
...
RB_WRAP(do_menu)
{
    ...
    menu_cb.idx_action_cb = (lua_type(L, 4) ==  LUA_TFUNCTION) ? 4 : 0;

    if (menu_cb.name_argtype == LUA_TTABLE)
        count = luaL_optinteger(L, 5, lua_objlen(L, 2));
    else
        count = luaL_checkint(L, 5);

    rb->simplelist_info_init(&info, (char*) title, count, &menu_cb);
#ifdef HAVE_LCD_BITMAP
    icon = luaL_optinteger(L, 6, icon);
    if (icon >= Icon_NOICON)
    {
