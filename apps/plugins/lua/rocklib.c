...
#if 0
RB_WRAP(memread_char_ptr)
{
    int ret = 0;
    const uintptr_t address = (uintptr_t) luaL_checkint(L, 1);
    const size_t    maxsize = MAX_PATH;
    lua_settop(L, 0);

    ret = mem_read_write(L, address, maxsize);
...
RB_WRAP(audio_next_track)
{
    ...
    intptr_t offset = (intptr_t) lua_tointeger(L, 1);
    lua_settop(L, 2); /* no writes alllowed */
    ...
    if(offset < 0)
    {
        ...
        lua_replace(L, 1);
        lua_pushinteger(L, luaL_optint(L, 2, maxsize + offset));
        lua_replace(L, 2);
    }
