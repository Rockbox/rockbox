...
RB_WRAP(playlist)
{
    ...
    int result = 0, index, random_seed;
    int pos, queue, recurse, sync, crc;
    ...
    switch(option)
    {
        ...
        case PLAYL_INSERTTRACK:
            ...
            pos = luaL_optint(L, 2, PLAYLIST_INSERT);
        ...
        case PLAYL_INSERTDIRECTORY:
            dir = luaL_checkstring(L, 1); /* only required parameter */
            pos = luaL_optint(L, 2, PLAYLIST_INSERT);
            queue = lua_toboolean(L, 3); /* default to false */
