...
/* conversion between packed native formats and individual pixel addressing */
    ...
    *pixmask = pixmask_v1;
    ...
    static const fb_data pixmask_v2[4] = {0x03, 0x0C, 0x30, 0xC0};
    *y_shift = 2U;
    *pixmask = pixmask_v2;
    ...
    *pixmask = pixmask_vi2;
    ...
    /* MSB on left */
    static const fb_data pixmask_h2[4] = {0x03, 0x0C, 0x30, 0xC0};
    *x_shift = 2U;
    *pixmask = pixmask_h2;
...
static inline fb_data set_masked_pix(fb_data old,
                                     fb_data new,
                                     fb_data mask,
                                     int bit_n)
{
...
static void dim_rlimage(struct rocklua_image *a,
                        int width, int height,
                        int w_native, int h_native)
{
    a->stride = w_native;
...
static fb_data* rli_alloc(lua_State *L, int width, int height)
{
    ...
    a->data = &a->dummy[0][0]; /* ref to beginning of alloc'd fb data */
...
static inline fb_data* get_rli_element(struct rocklua_image* img, int x, int y)
{
    return data_element(img->data, img->elems, x, y, img->stride);
}
...
static inline void swap_int(int *v1, int *v2)
{
    int swap = *v1;
    *v1 = *v2;
    *v2 = swap;
                          ...
                          bool swx, bool swy)
{
    ...
    d->dx  = dx;
    d->dy  = dy;
    d->elem = get_rli_element(img, d->x, d->y);
                  ...
                  bool clip)
{
    ...
    {
        element = get_rli_element(img, x1, y1);
        if(clip || element)
...
static int d_ellipse_rect(struct rocklua_image *img,
                          int x1, int y1,
                          int x2, int y2,
                          fb_data *clr,
                          fb_data *fillclr,
                          bool clip)
{
    ...
    do {
        element1 = get_rli_element(img, x2, y1);
        element2 = get_rli_element(img, x1, y1);
        element3 = get_rli_element(img, x1, y2);
        element4 = get_rli_element(img, x2, y2);
    ...
    {
        element1 = get_rli_element(img, x2 + 1, y1);
        element2 = get_rli_element(img, x1 - 1, y1);
        element3 = get_rli_element(img, x1 - 1, y2);
        element4 = get_rli_element(img, x2 + 1, y2);
        if (clip || (element1 && element2 && element3 && element4))
        {
...
static int custom_op(lua_State *L,
...
static int blit_op(lua_State *L,
                   struct rli_iter_d *ds,
                   struct rli_iter_d *ss,
                   int op,
                   fb_data *c)
...
static int copy_op(lua_State *L,
                   struct rli_iter_d *ds,
                   struct rli_iter_d *ss,
                   int op,
                   fb_data *c)
...
RLI_LUA rli_setget(lua_State *L)
{
    ...
    bool is_get = lua_type(L, 4) == LUA_TBOOLEAN;

    if(idx_top < 4 || is_get) /* get element */
        clip = luaL_optboolean(L, 4, false);
    else /* set element */
    {
        clr = FB_SCALARPACK((unsigned)luaL_optnumber(L, 4, LCD_BLACK));
        clip = luaL_optboolean(L, 5, false);
    }
    ...
    fb_data *element = get_rli_element(a, x, y);
...
RLI_LUA rli_tostring(lua_State *L)
{
    ...
    switch(luaL_optint(L, 2, 0))
    {
        ...
        case RLI_INFO_BYTES:   { lua_pushfstring(L, "%d", a->bytes  ); break; }
...
RLI_LUA rli_ellipse_rect(lua_State *L)
{
...
RLI_LUA rli_invert(lua_State *L) /* also marshal */
{
    ...
    /* enough room on stack for custom function?? */
    luaL_argcheck(L, lua_checkstack (L, 4) || !c_funct, 9, ERR_DATA_OVF);
...
RLI_LUA rli_copy(lua_State *L)
{
    ...
    trans transform = copy_op; /* is default transformation */
    ...
    {
        luaL_argcheck(L, lua_isfunction(L, 11), 11, "function expected");
        transform = custom_op;
                             ...
                             bool is_unsigned)
{
    ...
    if(is_unsigned)
        *(unsigned*)res = luaL_optint(L, -1, 0);
    else
        *(int*)res = luaL_optint(L, -1, 0);
