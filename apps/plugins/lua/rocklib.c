...
static inline fb_data* get_img_element(struct rocklua_image* img, int x, int y)
{
                          ...
                          bool swx, bool swy)
{
    ...
    d->dx  = dx;
    d->dy  = dy;
    d->elem = get_img_element(img, d->x, d->y);
                  ...
                  bool clip)
{
    ...
    {
        element = get_img_element(img, x1, y1);
        if(clip || element)
                          ...
                          bool clip)
{
    ...
    do {
        element1 = get_img_element(img, x2, y1);
        element2 = get_img_element(img, x1, y1);
        element3 = get_img_element(img, x1, y2);
        element4 = get_img_element(img, x2, y2);
    ...
    {
        element1 = get_img_element(img, x2 + 1, y1);
        element2 = get_img_element(img, x1 - 1, y1);
        element3 = get_img_element(img, x1 - 1, y2);
        element4 = get_img_element(img, x2 + 1, y2);
        if (clip || (element1 && element2 && element3 && element4))
        {
...
RLI_LUA rli_setget(lua_State *L)
{
    ...
    fb_data *element = get_img_element(a, x, y);
...
static struct viewport* opt_viewport(lua_State *L,
                                     int narg,
                                     struct viewport* vp,
                                     struct viewport* alt)
{
