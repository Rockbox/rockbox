...
static void bounds_check_xy(lua_State *L, rocklua_image *img,
...
static bool init_rli_iter(rli_iter_d *d, rocklua_image *img,
                                                int x1, int y1,
                                                int x2, int y2,
                                                int dx, int dy,
                                            bool swx, bool swy)
...
static bool next_rli_iter(rli_iter_d *d)
{
...
static rocklua_image * alloc_rlimage_data(lua_State *L, int sz_bytes)
{
    rocklua_image *img = (rocklua_image *)lua_newuserdata(L, sz_bytes);
    luaL_getmetatable(L, ROCKLUA_IMAGE);
    lua_setmetatable(L, -2);
    return img;
}

static void dim_rlimage(rocklua_image *a, int w, int h, int w_native, int h_native)
{
    a->stride = w_native;
...
static void rli_wrap(lua_State *L, fb_data *src, int width, int height)
{
    ...
    size_t sz_header = sizeof(rocklua_image);

    rocklua_image *a = alloc_rlimage_data(L, sz_header);
...
static fb_data* rli_alloc(lua_State *L, int width, int height)
{
    ...
    rocklua_image *a = alloc_rlimage_data(L, nbytes);
...
{
    rocklua_image *a = rli_checktype(L, 1);
    rocklua_image *b = rli_checktype(L, 2);
...
static int d_line(rocklua_image *img, int x1, int y1, int x2, int y2,
...
static int d_ellipse_rect(rocklua_image *img, int x1, int y1, int x2, int y2,
...
static int custom_op(lua_State *L, rli_iter_d *ds, rli_iter_d *ss, int op, fb_data *c)
{
    (void) c;
    int ret;
...
static int blit_op(lua_State *L, rli_iter_d *ds, rli_iter_d *ss, int op, fb_data *c)
{
    (void) L;
    fb_data clr = *c;
    fb_data dst = data_get(ds->elem, ds->x, ds->y);
...
static int copy_op(lua_State *L, rli_iter_d *ds, rli_iter_d *ss, int op, fb_data *c)
{
    (void) L;
    (void) c;
    (void) op;
...
static int rli_copy(lua_State *L)
{
    ...
    typedef int (*trans)(lua_State *, rli_iter_d *, rli_iter_d *, int, fb_data *);
    ...
    rocklua_image *d = rli_checktype(L, 1); /*dst*/
    rocklua_image *s = rli_checktype(L, 2); /*src*/
    ...
    {
        luaL_argcheck(L, op != 0xFF, 11, "function expected");
...
static int rli_iterator(lua_State *L) {
    ...
    rli_iter_d *ds = (rli_iter_d *)lua_touserdata(L, lua_upvalueindex(1));
...
static int rli_iterator_factory(lua_State *L) {
    ...
    rocklua_image *a = rli_checktype(L, 1); /*image we wish to iterate*/
    int x1  = luaL_optint(L, 2, 1);
    int y1  = luaL_optint(L, 3, 1);
    ...
    ds = (rli_iter_d *) lua_newuserdata(L, sizeof(rli_iter_d));
