...
#define pix_to_native(x, y, xn, yn) {*xn = x; *yn = y;}
#define init_pixmask(x, y, m, p) do { } while (0)
#define pix_to_fb(x, y, o, n) do { } while (0)
...
static inline fb_data get_masked_pix(fb_data val, fb_data mask, int bit_n)
{
    val = val & mask;
    val = val >> bit_n;
    return val;
...
/* conversion between packed native formats and individual pixel addressing */
    ...
    *oldv = get_masked_pix(*oldv, mask, bit_n);
                                            ...
                                            int width, int height)
{
    ...
    img->elems = n_elems;
    img->bytes = n_elems * sizeof(fb_data);
...
static inline fb_data data_setget(fb_data *elem, int x, int y, fb_data *new_val)
{
#if LCD_DEPTH > 2
    (void) x;
    (void) y;
...
static inline fb_data data_set(fb_data *elem, int x, int y, fb_data *new_val)
{
    ...
    fb_data old_val;
    old_val = data_setget(elem, x, y, new_val);
...
static inline fb_data data_get(fb_data *elem, int x, int y)
{
    /* get and set share the same underlying function */
    fb_data old_val;
    old_val = data_setget(elem, x, y, NULL);
...
static int rli_setget(lua_State *L, bool is_get)
{
    ...
    fb_data clr = 0; /* 4 is color if set element */
    fb_data *p_clr = &clr;
...
static bool next_rli_iter(struct rli_iter_d *d)
{
    ...
    */
    ...
    /* rather than checking correct direction just check for overflow */
    /*if((d->dx > 0 && d->x < d->x2) || (d->dx < 0 && d->x > d->x2))*/
    /*else if((d->dy > 0 && d->y < d->y2) || (d->dy < 0 && d->y > d->y2))*/
    if(d->x >= 0 && d->x != d->x2)
        d->x += d->dx;
    else if(d->y >= 0 && d->y != d->y2)
    {
        d->x = d->x1; /* Reset x*/
        d->y += d->dy;
    }
    ...
    for(;numpixels > 0; numpixels--)
    {
        ...
        if(d_err >= 0) /* 0 is our target midpoint(exact point on the line) */
        {
            *b1 += s_b; /* whichever axis is in 'b' stepped(-1 or 1) */
            ...
            d_err += r_b; /* 'a' - 'b' allows to only add 'b' when d_err < 0 */
