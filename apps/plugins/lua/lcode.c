...
unsigned char *growLineInfo(FuncState *fs) {
  int start = (fs->packedlineinfoSize > 0 ? fs->packedlineinfoSize - 1 : 0);
  Proto *f = fs->f;
  unsigned char *p;

  lua_assert(f->packedlineinfo==NULL || f->packedlineinfo[start] == '\0');
  ...
  memset(p, INFO_FILL_BYTE, fs->packedlineinfoSize - start);
  f->packedlineinfo[fs->packedlineinfoSize - 1] = '\0';
  f->sizelineinfo = fs->packedlineinfoSize;
  return p;
}

static void generateInfoDeltaLine(FuncState *fs, int line) {
  /* Handle first time through when lineinfo points is NULL */
  unsigned char mask = INFO_DELTA_MASK;
...
#define addDLbyte(v) if (*p=='\0') p = growLineInfo(fs); *p++ = (v);
  ...
  if (delta) {
    ...
    delta -= 1;
    mask |= (delta & INFO_DELTA_6BITS);
    delta >>= 6;
    addDLbyte(mask);

    while (delta) {
      mask = INFO_DELTA_MASK | (delta & INFO_DELTA_7BITS);
      delta >>= 7;
      addDLbyte(mask);
    }
  }
