...
img:_data(element) -- returns raw pixel data
...
local _img = {} do
    ...
    local function resize(dst, src)
        ...
        -- pretty rough results I highly recommend building one more suited..
        local dst_width, dst_height = dst:width(), dst:height()
        local src_height, src_width = src:height(), src:width()

        local xstep = (bit.lshift(src:width(),8) / (dst_width)) + 1
        local ystep = (bit.lshift(src:height(),8) / (dst_height))
        local xpos, ypos = 0, 0
        local src_x, src_y
        local data = 0

        for dst_y=1, dst_height / 2 + 1 do
            src_y = bit.rshift(ypos,8) + 1
            xpos = xstep - bit.rshift(xstep,4) + 1
            for dst_x=1, dst_width / 2 + 1 do
                src_x = bit.rshift(xpos,8) + 1
                data = src:get(src_x, src_y)
                dst:set(dst_x, dst_y, data)

                data = src:get(src_x, src_height - src_y)
                dst:set(dst_x, dst_height - dst_y + 1, data)

                data = src:get(src_width - src_x +1, src_y)
                dst:set(dst_width - dst_x + 1, dst_y, data)

                data = src:get(src_width - src_x + 1, src_height - src_y)
                dst:set(dst_width - dst_x + 1, dst_height - dst_y + 1, data)
    ...
    local function save(img, name)
        ...
        local depth  = tonumber(img:__tostring(6))
        local format = tonumber(img:__tostring(7))

        local bpp, bpl -- bits per pixel, bytes per line
        -- bpl, pad rows to a multiple of 4 bytes
        -- return (width * bitcount + 31)/32;
        if depth <= 4 then
            bpp = 4 -- 16 color image
            bpl = (w / 2 + 3)
        elseif depth <= 16 then
            ...
            bpp = 24
            --bpl = 4 * ((width * bpp + 31)/32);
            bpl = (w * 3 + 3)
        end

        local linebytes = bit.band(bpl, bit.bnot(3))
        ...
        local h_ppm, v_ppm = 0x00000EC4, 0x00000EC4
        ...
        local bmpheader = fbuffer
              ...
              bmpheader[05] = s_bytesLE(32, 40) -- BITMAPINFOHEADER

			  bmpheader[06] = s_bytesLE(32, w)
              bmpheader[07] = s_bytesLE(32, h)
			  bmpheader[08] = "\1\0" -- color planes ALWAYS 1
        ...
        local bytesleft = linebytes - (bytesperpixel * w) --TODO
        local t_data = {}
        local fs_bytes_E = s_bytesLE -- default save in Little Endian
--[[
        local function save_data4_bit(data, x, y) -- 4bits / pixel
            t_data[#t_data + 1] = data or 0
            if #t_data >= 2 then
                local data8 = bit.bor(bit.lshift(t_data[1], 4), t_data[2])
                imgdata[#imgdata + 1] = fs_bytes_E(8, data8)
                t_data[1], t_data[2] = _NIL, _NIL
                dump_fbuffer(31) -- buffered write, increase # for performance
            end
            return data or 0
        end

        local function save_data16_bit(data, x, y) --16 or 24 bits / pixel
            imgdata[#imgdata + 1] = fs_bytes_E(bpp, data or 0)
            dump_fbuffer(31) -- buffered write, increase # for performance
            return data or 0
        end
]]
        if depth >= 16 then
            if format == 3553 then -- RGB565SWAPPED
                fs_bytes_E = s_bytesBE
            end
            -- Bitmap lines start at bottom unless biHeight is negative
            --img:marshal(1, h, w + bytesleft, 1, _NIL, _NIL, true, save_data16_bit)
            for point in img:points(1, h, w + bytesleft, 1) do
                imgdata[#imgdata + 1] = fs_bytes_E(bpp, point or 0)
                dump_fbuffer(31) -- buffered write, increase # for performance
            end
        else
            --w = w + w % 2 -- need even number of pixels
            --img:marshal(1, h, w + bytesleft, 1, _NIL, _NIL, true, save_data4_bit)
            for point in img:points(1, h, w + (w % 2) + bytesleft, 1) do
                t_data[#t_data + 1] = point or 0
                if #t_data >= 2 then
                    local data8 = bit.bor(bit.lshift(t_data[1], 4), t_data[2])
                    imgdata[#imgdata + 1] = fs_bytes_E(8, data8)
                    t_data[1], t_data[2] = _NIL, _NIL
