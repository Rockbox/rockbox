...
--[[ bClip allows drawing out of bounds without raising an error it is slower than
     having a correctly bounded figure but can be helpful in some cases..
]]
...
local _draw = {} do
    ...
    local function circle(img, cx, cy, radius, color, bClip)
        local r = radius
        img:draw_ellipse_rect(cx - r, cy - r, cx + r, cy + r, color, bClip)
    end
    ...
    local function circle_filled(img, cx, cy, radius, color, fillcolor, bClip)
        ...
        local r = radius
        img:draw_ellipse_rect(cx - r, cy - r, cx + r, cy + r, color, fillcolor, bClip)
    end

    -- ellipse that fits into defined rect
    local function ellipse_rect(img, x1, y1, x2, y2, color, bClip)
        img:draw_ellipse_rect(x1, y1, x2, y2, color, _NIL, bClip)
    end
    ...
    local function ellipse_rect_filled(img, x1, y1, x2, y2, color, fillcolor, bClip)
        ...
        img:draw_ellipse_rect(x1, y1, x2, y2, color, fillcolor, bClip)
    end

    -- ellipse cx, cy define center point; a, b the major/minor axis
    local function ellipse(img, cx, cy, a, b, color, bClip)
        img:draw_ellipse_rect(cx - a, cy - b, cx + a, cy + b, color, _NIL, bClip)
    end
    ...
    local function ellipse_filled(img, cx, cy, a, b, color, fillcolor, bClip)
        if not fillcolor then fillcolor = color end
        img:draw_ellipse_rect(cx - a, cy - b, cx + a, cy + b, color, fillcolor, bClip)
    end
    ...
    local function rounded_rect_filled(img, x, y, w, h, radius, color, fillcolor)
        ...
        c_img = rb.new_image(r * 2 + 1, r * 2 + 1)
        c_img:clear()
        circle_filled(c_img, r + 1, r + 1, r, color, fillcolor)

        -- copy 4 pieces of circle to their respective corners
        img:copy(c_img, x, y, _NIL, _NIL, r + 1, r + 1) --TL
        img:copy(c_img, x + w - r - 2, y, r, _NIL, r + 1, r + 1) --TR
        img:copy(c_img, x, y + h - r - 2, _NIL, r, r + 1, _NIL) --BL
        img:copy(c_img, x + w - r - 2, y + h - r - 2, r, r, r + 1, r + 1)--BR
        c_img = _NIL

        -- finish filling areas circles didn't cover
        img:clear(fillcolor, x + r, y, x + w - r, y + h - 1)
        img:clear(fillcolor, x, y + r, x + r, y + h - r)
        img:clear(fillcolor, x + w - r, y + r, x + w - 1, y + h - r - 1)

        if fillcolor ~= color then
            hline(img, x + r, y, w - r - r, color)
            vline(img, x, y + r, h -r - r, color)
            vline(img, x + w, y + r, h -r - r, color)
            hline(img, x + r, y + h, w - r - r, color)
        end

    end

    -- draws an image at xy coord in dest image
    local function image(dst, src, x, y)
        if not src then --make sure an image was passed, otherwise bail
            rb.splash(rb.HZ, "No Image!")
            return _NIL
        end
        dst:copy(src,x,y,1,1)
    end
    ...
    function flood_fill(img, x, y, targetclr, fillclr)
        ...
        local function seed_pt(x, y)
            ...
            for qhead = 2, max_w * max_h * max_w * max_h, 2 do
                ...
                if targetclr == img:get(x, y)then
                    iter_n = img:points(x, y - 1, 1, y - 1)
                    iter_s = img:points(x, y + 1, 1, y + 1)
                    img:marshal(x, y, 1, y, _NIL, _NIL, false, check_ns)
    ...
    local function polyline(img, x, y, t_points, color, bClosed)

        if #t_points < 2 then error("not enough points", 3) end

        local pt_first_last = #t_points
        if bClosed then pt_first_last = t_points[1] end
        ...
        for i = 1, #t_points, 1 do
            ...
            img:draw_line(pt1[1] + x, pt1[2] + y, pt2[1]+x, pt2[2]+y, color)
        end

    end

    -- draws a closed figure based on points in t_points
    local function polygon(img, x, y, t_points, color, fillcolor)
        ...
        if #t_points < 2 then error("not enough points", 3) end
            ...
            if fillcolor then
                ...
                for i = 1, #t_points, 1 do
                    ...
                    fill_img:draw_line(pt1[1] - x_min, pt1[2] - y_min,
                                      pt2[1]- x_min, pt2[2] - y_min, 0)

                end
            flood_fill(fill_img, fill_img:width(), fill_img:height() , 1, 0)
            img:copy(fill_img, x - 1, y - 1, _NIL, _NIL, _NIL, _NIL, false, BSAND, fillcolor)
        end

        polyline(img, x, y, t_points, color, true)
    end
    ...
    local function text(img, x, y, width, height, font, color, text)
        ...
        if w > LCD_W then -- text is too long for the screen do it in chunks
            ...
            while lenr > 1 do
                ...
                img:copy(_LCD, x + width, y + height, 1, 1, _NIL, _NIL, false, BSAND, color)
                x = x + wp
                rb.lcd_clear_display()
                lenr = text:len()
            end
            ...
            -- using the mask we made blit color into img
            img:copy(_LCD, x + width, y + height, _NIL, _NIL, _NIL, _NIL, false, BSAND, color)
        end
