...
local function get_core_settings()
    ...
    local list_settings = "cursor_style|show_icons|statusbar|scrollbar|scrollbar_widthlist_separator_height|backdrop_file|"
...
function print_menu(menu_t, func_t, selected, settings, copy_screen)
    ...
    if not settings.linedesc or rb.core_list_settings_table then
        ...
        local linedesc = {
                                separator_height = t_l.separator_height or 0,
                                show_cursor = (t_l.cursor_style or 0) == 0,
                                style = t_l.cursor_style or 0,
        ...
        for k, v in pairs(linedesc) do
            if not settings.linedesc[k] then --dont overwrite supplied settinmgs
                settings.linedesc[k] = v
            end
        end
    end
