...
local _print = {} do
    ...
    local _p_opts = _NIL
    local udvp = nil --holds reference to alloc'd userdata for set_viewport
...
--------------------------------------------------------------------------------
   -- clamps value to >= min and <= max
    ...
    local function set_viewport(vp)
        ...
        if udvp then vp["vp"] = udvp end -- reuse userdata alloc for vp struct
    ...
    --replaces select escape characters with their equivalent function
...
--------------------------------------------------------------------------------
    ...
    local function set_defaults()
        _p_opts = { x = 1, y = 1,
                    width = LCD_W - 1, height = LCD_H - 1,
                    font = rb.FONT_UI,
                    drawmode = DRMODE_SOLID,
                    fg_pattern = WHITE,
                    bg_pattern = BLACK,
                    sel_pattern = WHITE,
                    line = 1, max_line = _NIL,
                    col = 0, ovfl = "auto",
                    justify = "left", autoupdate = true,
                  }
