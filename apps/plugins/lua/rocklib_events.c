 ...
 * 
 * Usage:
 * local evt = rockevents.timer_register(cb_function, timeout_ticks)
 *    cb_function() ... end
 * rockevents.timer_set_period(timeout_ticks)
 * 
 * local evb = rockevents.button_register(cb_function)
 *    cb_function(button) ... end
 * 
 * local eva = rockevents.action_register(cb_function)
 *    cb_function(action) ... end
 * 
 * local evp = rockevents.playbk_register(cb_function, [flags])
 *    cb_function(id, data) ... end
 *
 * rockevents.event_unregister(evX)
 * Use event_release(evX) to remove an event
...
static void event_thread(void)
{
    while(!ev_data.thread_quit && lua_status(ev_data.L) == 0)
    {
        ...
        {
            if (!lua_event_callback(ev_data.NEWL, ev_data.td))
                break;
        ...
        {
            if (!lua_event_callback(ev_data.NEWL, ev_data.pd))
                break;
        ...
        if ((ev_data.thread_state & THREAD_ACTEVENT) == THREAD_ACTEVENT)
        {
            ...
            /* don't send an event for ACTION_NONE more than once*/
            if(ev_data.ad->id != action || action != ACTION_NONE)
            {
                ev_data.ad->id = action;
                if (!lua_event_callback(ev_data.NEWL, ev_data.ad))
                    break;
            }
        }
        ...
        if ((ev_data.thread_state & THREAD_BUTEVENT) == THREAD_BUTEVENT)
        {
            ...
            if(ev_data.bd->id)
            {
                if (!lua_event_callback(ev_data.NEWL, ev_data.bd))
                    break;
            }
        }
...
static void init_event_thread(bool init, struct event_data *ev_data)
{
    if (ev_data->event_stack_ref != LUA_NOREF) /* make sure we don't double free */
    {
        if (!init && ev_data->thread_id != UINT_MAX)
        {
            ...
            /* remove lua thread ref so it can be garbage collected */
            luaL_unref (ev_data->L, LUA_REGISTRYINDEX, ev_data->NEWL_ref);
            ev_data->NEWL_ref = LUA_NOREF;
            ev_data->NEWL = NULL;
        }
        return;
    ...
    /* create and store new lua thread ref to make it persist */
    ev_data->NEWL = lua_newthread(ev_data->L);
    ev_data->NEWL_ref = luaL_ref(ev_data->L, LUA_REGISTRYINDEX );
...
static void create_event_userdata(lua_State *L, struct cb_data ** cbdata)
{
    ...
    else if ((*cbdata) != NULL) /* function is already registered , unregister it */
        luaL_unref (L, LUA_REGISTRYINDEX, (*cbdata)->cb_ref);
