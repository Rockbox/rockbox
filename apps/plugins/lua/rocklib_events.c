...
static const unsigned char thread_states[EVENT_CT] =
{
...
static inline void rev_init_mtx(void)
{
...
static void event_thread(void)
{
    ...
    while(ev_data.thread_state != THREAD_QUIT && lua_status(ev_data.L) == LUA_SUCCESS)
    {
        ...
        {
            if ((ev_data.thread_state & thread_states[event]) != thread_states[event])
                continue; /* check next event */

            ev_data.thread_state &= ~thread_states[event]; /* event handled */
            ...
            switch (event)
            {
                ...
                case BUTEVENT:
                    ...
                    break;
                case CUSTOMEVENT:
                    ev_data.thread_state |= thread_states[CUSTOMEVENT]; // don't reset */
                    break;
                case PLAYBKEVENT:
                    break;
                case TIMEREVENT:
        ...
        do
        {
            ...
            ev_data.next_event = EV_TICKS;
            rb->yield();
        } while(ev_data.thread_state == THREAD_YIELD);
...
static void rev_timer_isr(void)
{
    ...
    {
        if (ev_data.cb[ACTEVENT] != NULL)
            ev_data.thread_state |= thread_states[ACTEVENT];
        if (ev_data.cb[BUTEVENT] != NULL)
            ev_data.thread_state |= thread_states[BUTEVENT];
        ev_data.next_input = EV_INPUT;
    }

    if (ev_data.cb[TIMEREVENT] != NULL)
    {
        if (TIME_AFTER(*rb->current_tick, ev_data.cb[TIMEREVENT]->id))
        {
            ev_data.thread_state |= thread_states[TIMEREVENT];
...
static void playback_event_callback(unsigned short id, void *data)
{
    ...
    ev_data.thread_state |= thread_states[PLAYBKEVENT];
                           ...
                           void (*handler)(unsigned short id, void *data))
{
    ...
    {
        for(; i < ARRAYLEN(playback_events); i++)
        {
...
static void destroy_event_userdata(lua_State *L, int event)
{
    ...
    {
        ev_data.thread_state &= ~thread_states[event];
...
static int rock_event_trigger(lua_State *L)
{
...
static int rock_event_register(lua_State *L)
{
    int event = luaL_checkoption(L, 1, NULL, ev_map);
    int playbk_events;
...
static const struct luaL_reg evlib[] = {
    {"register",  rock_event_register},
    {"trigger",    rock_event_trigger},
    {"unregister", rock_event_unregister},
    {NULL, NULL}
};

static void reg_integer_const(lua_State *L, int index, const char *k, int value)
{
    lua_pushinteger(L, value);
    lua_setfield(L, index, k);
}

#define RB_NAMEDCONSTANT(name, x)        reg_integer_const(L, -2, name, x)
int luaopen_rockevents (lua_State *L) {
    ...
    RB_NAMEDCONSTANT("START_PLAYBACK",         PLAYBACK_EVENT_START_PLAYBACK);
    RB_NAMEDCONSTANT("TRACK_BUFFER",           PLAYBACK_EVENT_TRACK_BUFFER);
    RB_NAMEDCONSTANT("CUR_TRACK_READY",        PLAYBACK_EVENT_CUR_TRACK_READY);
    RB_NAMEDCONSTANT("TRACK_FINISH",           PLAYBACK_EVENT_TRACK_FINISH);
    RB_NAMEDCONSTANT("TRACK_CHANGE",           PLAYBACK_EVENT_TRACK_CHANGE);
    RB_NAMEDCONSTANT("TRACK_SKIP",             PLAYBACK_EVENT_TRACK_SKIP);
    RB_NAMEDCONSTANT("NEXTTRACKID3_AVAILABLE", PLAYBACK_EVENT_NEXTTRACKID3_AVAILABLE);
    lua_setfield (L, -2, ev_map[PLAYBKEVENT]);
    return 1;
}
