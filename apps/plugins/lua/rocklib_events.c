        ...
        for( event = 0; event < EVENT_CT; event++)
        {
            ...
            switch (event)
            {
                case ACTEVENT:
                    ...
                    /* don't send an event for ACTION_NONE, ACTION_UNKNOWN */
                ...
                case BUTEVENT:
                    ...
                    break;
                case CUSTOMEVENT:
                    /* ev_data.thread_state &= ~thread_states[event]; // don't reset */
                    break;
                case TIMEREVENT:
...
static void create_event_userdata(lua_State *L, int event, int index)
{
    ...
    ev_data.cb[event]->cb_ref = ref_lua; /* store reference for later release */
