...
#define lcd_framebuffer (*rb->lcd_framebuffer_p) /* Make FBADDR macros work in
                                                    plugin */
...
#endif
    osd_lcd_bitmap.data = (void *)lcd_framebuffer;
...
{
    /* x: 1 .. 100, z: 15
     * [9900 * e ^ ((100 - x) / z) - 100*(x - 1)] / e ^ (99 / z) + 100 */
    int z = 15;
    int f = fp16_exp(fp_div(100 - value, z, 16));
    int g = fp16_exp(fp_div(99, z, 16));
    int h = (value - 1) << 16;

    osc_delay = (9900ll*f - 100*h) / g + 100;
    osc_delay_error = 0;
}
