...
static unsigned long pcm_sampr = PLAY_SAMPR_DEFAULT;
...
{
    size_t size = pcm_dma_t_frames_size(frames);
    ...
    int rc = rb->audio_add_remove_pcm_hook(waveform_pcm_hook, false);

    waveform_buffer_threshold = threshold;

    if (rc == 0)
        rb->audio_add_remove_pcm_hook(waveform_pcm_hook, true);
}
...
static long anim_waveform_horizontal(void)
{
    ...
    if (rb->audio_get_pcm_state() != PCM_STATE_RUNNING)
    {
        osd_lcd_update_prepare();
...
static void anim_waveform_exit(void)
{
    ...
    rb->audio_add_remove_pcm_hook(waveform_pcm_hook, false);
...
#ifdef OSCILLOSCOPE_GRAPHMODE
    pcm_sampr = rb->pcm_get_frequency();
#endif
