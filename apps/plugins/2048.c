...
/* there appears to be no exit() in the Rockbox API */
extern void exit(int);
/* TODO
 * Draw the background for the grid
 * Draw tiles better (fill the whole space)
...
{
    rb->lcd_set_background(LCD_RGBPACK(0xfa, 0xf8, 0xef));
    ...
    rb->lcd_set_background(LCD_RGBPACK(0x77, 0x6e, 0x65));
    ...
    rb->lcd_set_foreground(LCD_RGBPACK(0x77, 0x6e, 0x65));
    rb->lcd_set_background(LCD_RGBPACK(0xfa, 0xf8, 0xef));
...
/* place a 2 or 4 at a random empty tile */
...
/* copy old_grid to ctx->grid, used for legal move checking */
    ...
    for(int y=0;y<GRID_SIZE;++y)
    {
        for(int x=0;x<GRID_SIZE;++x)
        {
            ...
            if(ctx->grid[x][y]==WINNING_TILE && !ctx->already_won)
            {
                ...
                const struct text_message yes={(const char*[]){"Yes"}, 1};
                const struct text_message no={(const char*[]){"No"}, 1};
                enum yesno_res keepgoing=rb->gui_syncyesno_run(&prompt, &yes, &no);
