...
{
  int hs_idx=highscore_update(ctx->score,0,"",highscores,NUM_SCORES);
  if(!noshow)
    {
      if(hs_idx>=0)
...
static int mainmenu_shouldshow(int action, const struct menu_item_ex *this_item)
{
  ...
  if(action==ACTION_REQUEST_MENUITEM && !loaded && (idx==0 || idx==4))
...
static enum plugin_status do_2048_menu(void)
{
  int sel=0;
  loaded=load_game();
  MENUITEM_STRINGLIST(menu,"2048 Menu", mainmenu_shouldshow, "Resume Game", "Start New Game","High Scores","Playback Control", "Quit without Saving", "Quit");
  bool quit=false;
  while(!quit)
    {
      int item;
        ...
        case 1:
          {
            ...
            switch(ret)
              ...
              case PLUGIN_USB_CONNECTED:
                highscore_update(ctx->score,0,"",highscores,NUM_SCORES);
                highscore_save(HISCORES_FILE,highscores,NUM_SCORES);
