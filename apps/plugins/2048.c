...
#define TITLE_X 0
#define TITLE_Y 0
#define BASE_Y (BMPHEIGHT__2048_tiles-MIN_SPACE)
...
static void draw(void)
{
    ...
    const struct int_pair image_table[] = {
        ...
        { 121072, BMPWIDTH__2048_tiles-BMPHEIGHT__2048_tiles*18 }
    };
    ...
    for(int y=0;y<GRID_SIZE;++y)
        {
            for(int x=0;x<GRID_SIZE;++x)
                {
                    for(int i=0;i<sizeof(image_table)/sizeof(struct int_pair);++i)
                        {
...
{
    (void)param;
    rb->srand(*(rb->current_tick)*LCD_WIDTH+LCD_HEIGHT);
    load_hs();
    rb->lcd_setfont(WHAT_FONT);
