  ...
  for(int y=starty;y!=stopy;y+=dy)
    {
      for(int x=startx;x!=stopx;x+=dx)
        {
          if(grid[x+lx][y+ly]==grid[x][y] && grid[x+lx][y+ly] && !merged_grid[x+lx][y+ly] && !merged_grid[x][y]) /* Slide into */
            {
  ...
  for(int y=3;y>=0;--y)
    {
      for(int x=0;x<4;++x)
        {
          if(grid[x][y])
            rb->lcd_putsf(5*x,y+3,"%d",grid[x][y]);
        }
    }
  rb->lcd_putsf(0,0,"Score: %d", score);
  rb->lcd_update();
}
static void place_random(void)
{
  /* We can spare 32 bytes! */
  char posx[16];
  char posy[16];
  int back=0;
  for(int y=0;y<4;++y)
  ...
  if(back==0) /* No empty spots */
    return; 
  grid[(int)posx[rand_range(0,back-1)]][(int)posy[rand_range(0,back-1)]]=rand_2_or_4();
}
...
static enum plugin_status do_game(void)
{
  ...
  for(int i=0;i<MAX_STARTING_TILES;++i)
    {
      grid[rand_range(0,3)][rand_range(0,3)]=2;
    }
  rb->lcd_clear_display();
  draw();
  while(1)
    {
      ...
      int made_move=0;
      memset(&merged_grid,0,16*sizeof(char));
      memcpy(&old_grid, &grid, sizeof(int)*16);
      switch(button)
        {
          /* The slide into behavior is actually wrong, in the real game, tiles only slide into one other tile per move... */
        case BUTTON_UP:
          for(int i=0;i<4;++i)
            up();
          made_move=1;
