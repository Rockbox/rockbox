                    ...
                    for(unsigned int i=0;i<sizeof(image_table)/sizeof(struct int_pair);++i)
                        {
                            if(image_table[i].a==ctx->grid[x][y])
                                {
#if LCD_DEPTH < 4 /* grAyscale */
                                    grey_gray_bitmap_part(_2048_tiles,
                                                          image_table[i].b, 0,
                                                          STRIDE(SCREEN_MAIN, BMPWIDTH__2048_tiles, BMPHEIGHT__2048_tiles),
...
{
    (void)param;
    rb->srand(*(rb->current_tick)+*(rb->cpu_frequency));
    load_hs();
    rb->lcd_setfont(WHAT_FONT);
