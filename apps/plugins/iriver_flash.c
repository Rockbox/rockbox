...
#define BOOT_SECTOR_SIZE (SEC_SIZE - BOOT_VECTOR_SIZE)
...
static bool flash_erase_sector(uint32_t offset)
{
    ...
    FB[0x5555] = 0xAA; /* enter command mode */
    FB[0x2AAA] = 0x55;
    FB[offset / WORD_SIZE] = 0x30; /* erase the sector */
...
static bool flash_program_word(uint32_t offset, uint16_t data)
{
    ...
    FB[0x2AAA] = 0x55;
    FB[0x5555] = 0xA0; /* byte program command */
    FB[offset / WORD_SIZE] = data; /* write the word data */
...
#if 0
static bool flash_verify(uint32_t offset, const void* src, uint32_t len)
{
    const uint32_t word_offset = offset / WORD_SIZE;
    const uint16_t* word_src = src;
    const uint32_t word_len = len / WORD_SIZE;
    for (uint32_t i = 0; i < word_len; i++)
        if (FB[word_offset + i] != word_src[i])
            return false;
    return true;
}
#endif
