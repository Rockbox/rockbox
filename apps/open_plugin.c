...
uint32_t open_plugin_add_path(const char *key, const char *plugin, const char *parameter)
{
    ...
    if (plugin)
    {
        ...
        if(len > 5 && strcasecmp(&(open_plugin_entry.name[len-5]), ".rock") == 0)
        {
            ...
            {
                if (parameter [0] == '\0' && 
                    yesno_pop(ID2P(LANG_OPEN_PLUGIN_ADD_PARAMETER)))
                {
    ...
    if (fd1)
    {
        int rd = read(fd1, &open_plugin_entry, op_entry_sz);
        while (rd == op_entry_sz)
        {
            if (open_plugin_entry.hash != hash)
                write(fd, &open_plugin_entry, op_entry_sz);

            rd = read(fd1, &open_plugin_entry, op_entry_sz);
        }
        close(fd1);
    }
    close(fd);
