...
static int remove_dir(struct dirrecurse_params *parm)
{
    ...
    while (ret == 0) {
        ...
        parm->append = append + path_append(&parm->path[append], NULL,
                                            entry->d_name,
...
static bool create_dir(void)
{
    ...
    size_t pathlen = path_append(dirname, getcwd(NULL, 0), PATH_SEPSTR,
                                 sizeof (dirname));
                                    ...
                                    unsigned int *flagsp)
{
    ...
    while (ret == RET_SUCCESS) {
        ...
        src->append = srcap +
            path_append(&src->path[srcap], NULL, entry->d_name,
                        sizeof(src->path) - srcap);

        target->append = targetap +
            path_append(&target->path[targetap], NULL, entry->d_name,
