        ...
        case ACTION_REQUEST_MENUITEM:
            if ((audio_status() & AUDIO_STATUS_PLAY) &&
                (this_item != &queue_menu ||
                 global_settings.queue_options == SUBMENU_QUEUE))
                    return action;
            else
                return ACTION_EXIT_MENUITEM;
            break;
                                 ...
                                 struct gui_synclist *this_list)
{
    ...
    switch (action)
    {
        case ACTION_REQUEST_MENUITEM:
            ...
            else if (global_settings.queue_options == SHOW_QUEUE &&
            ...
            {
                if ((global_settings.shuffled_adding == SHOW_SHUFFLED_ADDING) &&
                    (global_status.resume_index != -1) &&
                    ((audio_status() & AUDIO_STATUS_PLAY) ||
                     (selected_file_attr & ATTR_DIRECTORY) ||
                     ((selected_file_attr & FILE_ATTR_MASK) == FILE_ATTR_M3U)))
            ...
            else if ((this_item == &q_shuf_pl_item &&
                      global_settings.queue_options == SHOW_QUEUE) ||
                     this_item == &q_shuf_pl_submenu_item)
            {
                if ((global_settings.shuffled_adding == SHOW_SHUFFLED_ADDING) &&
                    (audio_status() & AUDIO_STATUS_PLAY))
                {
