...
static int copy_segments(int fd, struct flat_hdr * header, void * dram, ssize_t dram_size, void * iram, ssize_t iram_size)
{
    ...
    ssize_t icode_size = header->icode_start - header->data_start;
...
/* hack valid for S5L8701 with 176KB of IRAM
 * the rest of iram is taken by core and codecs?
 */
#define __IRAMORIG__ (0x00000000 + (48*1024))
#define __IRAMSIZE__ (128*1024)
int plugin_load(const char* plugin, const void* parameter)
{
    ...
    current_plugin_handle = bflt_open(plugin, dram, dram_size, (unsigned char *)__IRAMORIG__, (size_t)__IRAMSIZE__);
