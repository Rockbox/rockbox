...
static int copy_segments(int fd, struct flat_hdr * header, void * dram, ssize_t dram_size, void * iram, ssize_t iram_size)
{
    ...
    /* we do not have adress space split so we can copy
     * everything in one run
     */
    ssize_t copy_size = text_size + icode_size + data_size + idata_size;

    if (read(fd, dram, copy_size) != copy_size)
    {
        DEBUGF("Error reading segments");
        return 2;
    }
#else
...
#else
        if (reloc < (header->icode_start - header->entry))
        {
            /* .text */
            fixme = (uint32_t *)((uintptr_t)dram_base + reloc);
        }
        else if (reloc < (header->data_start - header->entry))
        {
            /* .icode */
            fixme = (uint32_t *)((uintptr_t)iram_base + reloc - text_size);
        }
        else if (reloc < (header->data_end - header->entry))
        {
            /* .data */
...
static void * bflt_open(const char *filename, unsigned char *dram, size_t dram_size, unsigned char *iram, size_t iram_size)
{
    ...
    /* patch rock header */
    struct lc_header *lc_header_ptr = (struct lc_header *)dram;
    lc_header_ptr->load_addr = (unsigned char *)dram;
    lc_header_ptr->end_addr = (unsigned char *)((uintptr_t)dram + 
                                                (header.icode_start - header.entry) +
