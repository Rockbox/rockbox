...
static int check_subdir_for_music(char *dir, const char *subdir, bool recurse)
{
    ...
    if (path_append(dir + dirlen, NULL, subdir, MAX_PATH - dirlen) >=
            MAX_PATH - dirlen)
    {
        return 0;
    }
                                 ...
                                 const char *dir)
{
    ...
    /* prepend directory only if path is relative */
    len = path_append(dest, path_is_absolute(src) ? "" : dir, src, buf_length);
                             ...
                             int position, bool queue)
{
    ...
    dir = strlendupa(filename,
                path_strip_basename(filename, (const char **)&dir));
                       ...
                       void* temp_buffer, size_t temp_buffer_size)
{
    ...
    if (result >= 0)
    {
        *(char *)mempcpy(tmp_buf, path, pathlen) = 0; /* remove "_temp" */
