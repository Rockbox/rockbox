...
static int disk_callback(int btn, struct gui_synclist *lists)
{
    ...
    static const unsigned char * const nsec_units[] = { "ns", "袖s", "ms" };
#if (CONFIG_STORAGE & STORAGE_MMC)
...
static int disk_callback(int btn, struct gui_synclist *lists)
{
    ...
    snprintf(buf, sizeof buf, "%ld MB",
              (unsigned long)identify_info[61] << 5 |
              (unsigned long)identify_info[60] >> 11 );
#ifdef HAVE_LBA48
    if (identify_info[83] & (1 << 10)) {
        snprintf(buf, sizeof buf, "%lld MB",
                  (unsigned long long)identify_info[103] << 37 |
                  (unsigned long long)identify_info[102] << 21 |
                  (unsigned long long)identify_info[101] << 5  |
                  (unsigned long long)identify_info[100] >> 11 );
    }
#endif /* HAVE_LBA48 */
    simplelist_addline(
             "Size: %s", buf);
...
static const char * ata_smart_get_attr_name(unsigned char id)
{
    switch (id)
    {
        ...
        case 5:     return "Retired Block Count";
        ...
        case 173:   return "SSD Wear Leaving Count";
        ...
        case 195:   return "On-the-Fly ECC Uncorrectable Error Count";
        ...
        case 201:   return "Uncorrectable Soft Read Error Rate";
        case 204:   return "Soft ECC Correction Rate";
        ...
        case 230:   return "Life Curve Status";
        case 231:   return "SSD Life Left";
        case 240:   return "Transfer Error Rate";   /* Fujitsu */
        /*case 240:   return "Head Flying Hours";*/
        case 241:   return "Lifetime Writes to Host";
        case 242:   return "Lifetime Reads to Host";
...
{
    struct simplelist_info info;
    simplelist_info_init(&info, "S.M.A.R.T. Data [CONTEXT: dump]", 1, NULL);
...
static int disk_callback(int btn, struct gui_synclist *lists)
{
    ...
    simplelist_addline(
             ...
             */
             "Size: %lu MB", info.num_sectors*(info.sector_size/512)/2024);
    unsigned long free;
    volume_size( IF_MV(0,) NULL, &free );
    simplelist_addline(
             /*
