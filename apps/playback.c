...
{
    struct track_buf_info *tbi = track_buf_info_get(infop->self_hid);
    if (!tbi)
        return false;
...
{
    int sgn = SGN(offset);
    struct track_buf_info *tbi;

    while (1)
    {
        if (!(tbi = track_buf_info_get(hid)))
            break;

        if (!offset)
            break;

        if ((hid = tbi->link[(unsigned)(sgn + 1) / 2]) <= 0)
        {
            tbi = NULL;
            break;
        }
...
static bool track_list_commit_buf_info(struct track_buf_info *tbi,
                                       const struct track_info *src_infop)
{
    /* Leaves the list unmodified if anything fails */
    if (tbi->link[1] != ERR_HANDLE_NOT_FOUND)
        return false;

    int hid = tbi->info.self_hid;
    int last_hid = track_list.last_hid;
    struct track_buf_info *last_tbi = NULL;

    if (last_hid > 0 && !(last_tbi = track_buf_info_get(last_hid)))
        return false;
...
static void track_list_free_buf_info(struct track_buf_info *tbi)
{
    int hid = tbi->info.self_hid;
    int next_hid = tbi->link[1];

    if (next_hid != ERR_HANDLE_NOT_FOUND)
    {
        int prev_hid = tbi->link[0];
        struct track_buf_info *prev_tbi = NULL;
        struct track_buf_info *next_tbi = NULL;

        if ((prev_hid > 0 && !(prev_tbi = track_buf_info_get(prev_hid))) ||
            (next_hid > 0 && !(next_tbi = track_buf_info_get(next_hid))))
        {
            return;
        }

        if (prev_tbi)
            prev_tbi->link[1] = next_hid;
        else
            track_list.first_hid = track_list.current_hid = next_hid;

        if (next_tbi)
            next_tbi->link[0] = prev_hid;
        else
...
static bool track_list_alloc_info(struct track_info *infop)
{
    ...
    struct track_buf_info *tbi = track_buf_info_get(hid);
    if (!tbi)
    {
        infop->self_hid = ERR_HANDLE_NOT_FOUND;
        bufclose(hid);
    ...
    tbi->link[0] = 0;
    tbi->link[1] = ERR_HANDLE_NOT_FOUND; /* "Not in list" */
    tbi->info.self_hid = hid;
    track_info_wipe(&tbi->info);
...
static bool track_list_commit_info(const struct track_info *infop)
{
    ...
    return track_list_commit_buf_info(tbi, infop);
}
