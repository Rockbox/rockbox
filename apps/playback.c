...
struct track_info
{
    /* In predecessor, self, successor order */
    union
    {
        struct
        {
            int prev_hid;                /* handle for previous item */
            int self_hid;                /* handle for this structure */
            int next_hid;                /* handle for next item */
        };
        int link[3];                     /* array for SGN function */
    };
    ...
    union
    {
        ...
        int handle[TRACK_INFO_HANDLES];  /* array for efficient wipe/close */
    };
};
...
static void track_info_close(struct track_info *infop)
{
    /* Close them in the order they are allocated on the buffer to speed up
       the handle searching */
    track_info_close_handle(&infop->self_hid);

    FOR_EACH_TRACK_INFO_HANDLE(i)
        track_info_close_handle(&infop->handle[i]);
}
...
static inline int track_info_successor(const struct track_info *infop)
{
    return infop->next_hid;
}
...
static struct track_info * track_info_get(int hid)
{
    void *p;
    ssize_t size = bufgetdata(hid, sizeof (struct track_info), &p);
    return size == (ssize_t)sizeof (struct track_info) ? p : NULL;
}

/* Synchronize the buffer object with the cached track info */
static bool track_info_sync(const struct track_info *infop)
{
    struct track_info *ti = track_info_get(infop->self_hid);
    if (!ti)
        return false;

    memcpy(ti, infop, sizeof (*ti));
    return true;
}

/* Return track info a given offset from the info referenced by hid and
 * place a copy into *infop, if provided */
static struct track_info * track_list_get_info_from(int hid, int offset,
                                                    struct track_info *infop)
{
    int sgn = SGN(offset);
    ...
    while (1)
    {
        if (!(ti = track_info_get(hid)))
            break;

        if (!offset)
            break;

        if ((hid = ti->link[sgn + 1]) <= 0)
        {
            ti = NULL;
            break;
        }
...
{
    struct track_info *newinfop =
        track_list_get_info_from(track_list.current_hid, offset, infop);

    if (!newinfop)
        return false;

    track_list.current_hid = newinfop->self_hid;
    return true;
}
...
static bool track_list_alloc_info(struct track_info *infop)
{
    ...
    infop->prev_hid = 0;
    infop->self_hid = bufalloc(NULL, sizeof (*infop), TYPE_RAW_ATOMIC);
    infop->next_hid = ERR_HANDLE_NOT_FOUND; /* "Not in list" */

    return infop->self_hid > 0;
}

/* Actually commit the track info to the track list */
static bool track_list_commit_info(struct track_info *infop)
{
    struct track_info *last_infop = NULL;

    /* Leaves the list unmodified if anything fails */
    if (track_list.first_hid > 0)
    {
        last_infop = track_info_get(track_list.last_hid);
        if (!last_infop)
            return false;

        infop->prev_hid = last_infop->self_hid;
    }

    infop->next_hid = 0; /* "In list" */

    if (!track_info_sync(infop))
    {
        infop->next_hid = ERR_HANDLE_NOT_FOUND; /* Revert */
        return false;
    }

    if (last_infop)
    {
        last_infop->next_hid = infop->self_hid;
    }
    else
    {
        track_list.first_hid   = infop->self_hid;
        track_list.current_hid = infop->self_hid;
    }
    
    track_list.last_hid = infop->self_hid;
    track_list.count++;
    return true;
}
...
{
    int next_hid = infop->next_hid;

    track_info_close(infop);

    if (next_hid == ERR_HANDLE_NOT_FOUND)
        return; /* Never added to list */

    int prev_hid = infop->prev_hid;
    struct track_info *ti_prev = NULL;
    struct track_info *ti_next = NULL;

    if ((prev_hid > 0 && !(ti_prev = track_info_get(prev_hid))) ||
        (next_hid > 0 && !(ti_next = track_info_get(next_hid))))
    {
        return;
    }

    if (ti_prev)
        ti_prev->next_hid = next_hid;
    else
        track_list.first_hid = track_list.current_hid = next_hid;

    if (ti_next)
        ti_next->prev_hid = prev_hid;
    else
        track_list.last_hid = track_list.current_hid = prev_hid;
...
static void track_list_clear(unsigned int action)
{
    ...
    {
        struct track_info cur;
        if (!track_list_current(0, &cur) || cur.id3_hid < 0)
    ...
    while (hid != last_hid)
    {
        struct track_info *infop = track_info_get(hid);
        int next_hid = track_info_successor(infop);
...
static int audio_fill_file_buffer(void)
{
    ...
    if (trackstat >= LOAD_TRACK_OK)
    {
        ...
        if (filling == STATE_FULL && !track_info_successor(&user_cur))
        {
