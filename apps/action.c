...
static int do_selective_backlight(int action, int context, bool act_pending);
...
static int do_selective_softlock(int action, int context, bool act_pending);
...
static int do_selective_softlock(int action, int context, bool act_pending)
{
    ...
    if (screen_has_lock && is_context_filtered(context))
    {
        ...
        else if (keys_locked)
        {
            ...
            {
                if ((selective_softlock_mask & SEL_ACTION_NONOTIFY) == 0)
                    {
                        notify_user = true;
