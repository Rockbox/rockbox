...
static inline bool get_filtered_action(int action, int actmask, int context)
{
    ...
    switch (action)
    {
        case ACTION_NONE:
            break;
        case ACTION_UNKNOWN:
            match = mask_has_flag(actmask, SEL_ACTION_NOUNKNOWN);
            break;
        case ACTION_WPS_PLAY:
        case ACTION_FM_PLAY:
            match = mask_has_flag(actmask, SEL_ACTION_PLAY);
            break;
        ...
        case ACTION_STD_NEXTREPEAT:
            match = mask_has_flag(actmask, SEL_ACTION_SEEK);
            break;
        ...
        case ACTION_STD_NEXT:
            match = mask_has_flag(actmask, SEL_ACTION_SKIP);
            break;
        case ACTION_WPS_VOLUP:
        case ACTION_WPS_VOLDOWN:
            match = mask_has_flag(actmask, SEL_ACTION_VOL);
            break;
        case ACTION_SETTINGS_INC:
        case ACTION_SETTINGS_INCREPEAT:
        case ACTION_SETTINGS_DEC:
        case ACTION_SETTINGS_DECREPEAT:
            match = (context == CONTEXT_FM) && 
                                        mask_has_flag(actmask, SEL_ACTION_VOL);
            break;
        default:
...
static bool is_unfiltered_action(int action, int context, int actmask, 
        bool is_pre_button, bool *filtered_p, bool *is_last_filter_p, int *tick)
{
    bool ret = false;
    *filtered_p = get_filtered_action(action, actmask, context);
        /*directly after a match a key release event may trigger another*/
        if (*filtered_p && action != ACTION_UNKNOWN)
            *tick = current_tick;
        ...
        if (is_action_completed() || !is_pre_button)
        {
            ...
            if (!*filtered_p && 
             (!*is_last_filter_p || *tick + LAST_FILTER_TICKS < current_tick))
            {
                *is_last_filter_p = true;
                ret = true;
            }
         }/*is_action_completed() || !is_pre_button*/
        else
            *is_last_filter_p = *filtered_p;
...
!(mask_has_flag(sel_backlight_mask, SEL_ACTION_NOEXT) && power_input_present()))
#else
...
static int do_selective_softlock(int action, int context, bool is_pre_btn)
{
    ...
    {
#ifdef HAVE_BACKLIGHT
    if (mask_has_flag(sel_softlock_mask, SEL_ACTION_AUTOLOCK))
    {
        if(mask_has_flag(sel_softlock_mask, SEL_ACTION_ALOCK_OK) && 
            !keys_locked && !is_backlight_on(false))
        {
            keys_locked = true;
            last_button = BUTTON_NONE;
#if defined(HAVE_TOUCHPAD) || defined(HAVE_TOUCHSCREEN)
        /* disable touch device on keylock if user selected disable touch */
            if (mask_has_flag(sel_softlock_mask, SEL_ACTION_NOTOUCH))
               button_enable_touch(!keys_locked);
#endif
        } 
        else if (action == ACTION_STD_KEYLOCK && !keys_locked)
        {
            sel_softlock_mask ^= SEL_ACTION_ALOCK_OK;
            if (mask_has_flag(sel_softlock_mask, SEL_ACTION_ALOCK_OK))
            {
        ...
        else if (keys_locked && action != ACTION_NONE && action != ACTION_REDRAW)
        {
            ...
            if (sl_activate)
            {
                if (!mask_has_flag(sel_softlock_mask, SEL_ACTION_NONOTIFY))
                    {
                        notify_user = true;
                        action = ACTION_REDRAW;
                    }
