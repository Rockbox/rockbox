...
static bool get_filtered_action(int action, int actmask, int context);
...
*/
static bool is_unfiltered_action(bool action, bool *last_action_ptr, int tick)
{
    bool ret = false;
        /* has button been rel/rep or is this the only action it could be */
        if(is_action_completed() || !action_pending)
        {
            if (!action && !*last_action_ptr)
            {
                *last_action_ptr = true;
                ret              = true;
            }
            ...
            {
                *last_action_ptr = true;
                if (!action && tick + LAST_ACTION_TICKS < current_tick)
                    ret = true;
            }
        }/*is_action_completed() || !action_pending*/
        else
            *last_action_ptr = action;
...
#endif
    {
        filtered = get_filtered_action(action, selective_act_mask, context);
        /*directly after a match a key release event may trigger another*/
        if (filtered)
            last_filtered_action_tick = current_tick;

        backlight_activate = is_unfiltered_action(filtered, &last_filtered,
...
static int do_selective_softlock(int action)
{
    ...
    if (screen_has_lock && is_context_filtered(context))
    {
        ...
        {
            filtered = get_filtered_action(action, selective_softlock_mask,
                                                                        context);
            if (filtered)
                last_filtered_action_tick = current_tick;

            softlock_activate = is_unfiltered_action(filtered, &last_filtered,
                                                    last_filtered_action_tick);
