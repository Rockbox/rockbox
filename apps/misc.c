...
/* format_time_auto - return an auto ranged time string,
   ...
   01:00:00.00 would display as 1:00:00.00
                                  ...
                                  unsigned int (*idx_pos)[2])
{
    ...
    unsigned int       offsets[UNIT_IDX_TIME_COUNT] = 
                       {
                            ...
                            [UNIT_IDX_SEC] = 4,
                            [UNIT_IDX_MS]  = 0,
                       }; /* {10,7,4,0}; RTL offsets */

    unsigned int       fwidth[UNIT_IDX_TIME_COUNT] = 
                       {
                            [UNIT_IDX_HR]  = 0, /* hr width is variable length */
                            [UNIT_IDX_MIN] = 2,
                            [UNIT_IDX_SEC] = 2,
                            [UNIT_IDX_MS]  = 3,
    ...
    if (!is_rtl)
    {
        len = snprintf(timebuf, sizeof(timebuf), "%02d:%02d:%02d.%03d",
         ...
         * and then add our other offsets to it
        */
    ...
    {
        /*RTL Languages*/
        len = snprintf(timebuf, sizeof(timebuf), "%03d.%02d:%02d:%02d",
                 units_in[UNIT_IDX_MS],
                 units_in[UNIT_IDX_SEC],
                 units_in[UNIT_IDX_MIN],
                 units_in[UNIT_IDX_HR]);

        /* 999.59:59:0 we pre calculated the offsets since they don't change 
         * 0  .4 :7 :10 */
        /* we can use the default width (1) of hour to figure out
         * how long the HOUR field ended up being since it is variable length */
        fwidth[UNIT_IDX_HR] = len - offsets[UNIT_IDX_HR];
        ...
        if (value < 0) /* add negative sign if needed*/
            strlcat(buffer, &"-"[value < 0 ? 0 : 1] , buf_len);
    }
