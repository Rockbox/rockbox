...
static union enc_chunk_hdr *
encbuf_read_ptr_incr(size_t p, size_t *pout)
{
...
static union enc_chunk_hdr *
encbuf_get_write_ptr(size_t p, size_t v, size_t *pout)
{
    union enc_chunk_hdr *hdr = encbuf_ptr(p);
...
static void mark_stream(const char *path, enum mark_stream_action action)
{
    ...
    if (action & MARK_STREAM_START)
    {
        ...
        if (action & MARK_STREAM_PRE)
        {
            ...
            file = (void *)encbuf_get_write_ptr(enc_ridx, count, &widx);
        }
        else
        {
            /* The usual: START marker goes first or after existing data */
            file = (void *)encbuf_get_write_ptr(enc_widx, count, &widx);
...
static void tally_prerecord_data(void)
{
    ...
    for (size_t idx = enc_ridx; idx != enc_widx;)
    {
        struct enc_chunk_data *data =
            (void *)encbuf_read_ptr_incr(idx, &idx);
    ...
    {
        struct enc_chunk_data *data =
            (void *)encbuf_read_ptr_incr(enc_ridx, &enc_ridx);
...
static struct enc_chunk_data * enc_encbuf_get_buffer(size_t need)
{
    ...
    while (avail <= need + ENCBUF_MIN_SPLIT_MARGIN ||
           (enc_widx + need > enc_buflen && enc_ridx <= need))
    {
        if (UNLIKELY(state == REC_STATE_IDLE))
        {
            ...
            size_t ridx;
            struct enc_chunk_data *data =
                (void *)encbuf_read_ptr_incr(enc_ridx, &ridx);
        ...
        {
            /* Empty: Request larger than any possible space */
    ...
    struct enc_chunk_data *data =
        (void *)encbuf_get_write_ptr(enc_widx, need, &enc_widx);
