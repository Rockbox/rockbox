 ...
 *               the search tree does a successive approximation on
 *               the input value. This avoids big lookup-tables.
 *.
...
#else /* Unrolled (no point; it only runs 3 times unless isample < 24) */
    int i;
    i = 2   + (isample >= keys[0]);
    i = 2*i + (isample >= keys[i]);
    i = 2*i + (isample >= keys[i]);
