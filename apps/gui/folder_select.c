...
static char* get_full_path(struct folder *start)
{
    ...
    /* dont build up recursively for simple paths
     * (get_full_path_r() doesnt handle them) */
    if (!start->name[0] || !strcmp(start->name, "/"))
        return start->name;
...
static void save_folders_r(struct folder *root, char* dst, size_t maxlen)
{
    ...
    while (i < root->children_count)
    {
        ...
        {
            char temp[MAX_PATH]; /* avoid allocation in recursive call */
            const char* rpath = get_full_path(root);
            if (rpath[0] && strcmp(rpath, "/"))
                snprintf(temp, MAX_PATH, "%s/%s:", rpath, this->name);
            else
                snprintf(temp, MAX_PATH, "%s%s:", rpath, this->name);
            strlcat(dst, temp, maxlen);
        }
