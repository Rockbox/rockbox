...
#endif
    [WPS] = { NULL, NULL, wps_default_skin, true, 0},
#if CONFIG_TUNER
...
struct gui_skin *skin_find_free(enum screen_type screen, int priority)
{
...
{
    struct gui_skin *g_skin = skin_find_free(screen, skin_helpers[skin].priority);
    bool loaded = false;
...
struct gui_wps *skin_get_gwps(enum skinnable_screens skin, enum screen_type screen)
{
    ...
    if (!g_skin || g_skin->data.wps_loaded == false)
    {
        char filename[MAX_PATH];
