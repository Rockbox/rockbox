...
*/
int mask_select(int actmask, char* headermsg, 
                             struct s_mask_items *mask_items, size_t items)
{
    ...
    struct child action_child[items];
            for (unsigned i = 0; i < items; i++)
            {
                ...
                action_child[i].state     = mask_items[i].bit_value & actmask ?
                                                SELECTED : DESELECTED;
            }
   ...
   struct category root = {
    ...
    simplelist_info_init(&info, headermsg, count_items(&root), &root);
    ...
    //info.callback_data   = (void*)&actmask_orig;//&mask_items;
