...
/* option_get_timestring */
#define IDX_HR         0U
#define IDX_MIN        1U
#define IDX_SEC        2U
#define IDX_MS         3U
#define IDX_TIME_COUNT 4U
#define THIS_TIME_UNIT 1U
...
{
    unsigned int  offsets[IDX_TIME_COUNT] = {0}, units_in[IDX_TIME_COUNT] = {0};
    ...
    if (abs_val == 0)
    {
        ...
        fmt_idx = lang_is_rtl()? base_idx + IDX_TIME_COUNT : base_idx;
    }
    else if (abs_val < units_in[IDX_SEC])
    {
        unit = unit_strings[UNIT_MS];
        fmt_idx = lang_is_rtl()? base_idx + IDX_TIME_COUNT : IDX_MS;
    }
    else if (abs_val < units_in[IDX_MIN])
    {
        unit = unit_strings[UNIT_SEC];
        fmt_idx = lang_is_rtl()? base_idx + IDX_TIME_COUNT : IDX_SEC;
    }
    else if (abs_val < units_in[IDX_HR])
    {
        unit = unit_strings[UNIT_MIN];
        fmt_idx = lang_is_rtl()? base_idx + IDX_TIME_COUNT : IDX_MIN;
    }
    else
    {
        unit = unit_strings[UNIT_HOUR];
        fmt_idx = lang_is_rtl()? base_idx + IDX_TIME_COUNT : IDX_HR;
    }

    /* snprintf is supplied the format buffer, strings are supplied pre-rotated
        depending on unneeded units with a terminator placed at the minimum or
        maximum unit dependent on lang direction; All values are contained in
        each string due to restrictions of snprintf and the byte postions are
        copied into the offset array which will later be used to update the
        terminator position depending on the actual value, units will later be
        appended to the end in non-RTL languages and a negative sign will be
        added to the beginning if needed.
    */
