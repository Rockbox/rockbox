...
static int skintouch_to_wps(struct wps_data *data)
{
    ...
    switch (button)
    {
        ...
        case ACTION_TOUCH_SCROLLBAR:;
            long seekto = skin_get_global_state()->id3->length*offset/100;
            skin_get_global_state()->id3->elapsed = seekto;
#if CONFIG_CODEC == SWCODEC
            audio_seek(seekto, AUDIO_SEEK_SET_IMMEDIATE);
#else /* !SWCODEC */
    ...
    while (!exit)
    {
        switch ( button )
        {
            ...
            case ACTION_WPS_STOPSEEK:;
                long newpos = skin_get_global_state()->id3->elapsed+ff_rewind_count;
                skin_get_global_state()->id3->elapsed = newpos;
#if (CONFIG_CODEC == SWCODEC)
                audio_seek(newpos, AUDIO_SEEK_SET_IMMEDIATE);
#else
...
#if (CONFIG_CODEC == SWCODEC)
        audio_seek(0, AUDIO_SEEK_SET_IMMEDIATE);
#else
        if (!state->paused)
            audio_pause();
...
#if (CONFIG_CODEC == SWCODEC)
    audio_seek(elapsed, AUDIO_SEEK_SET_IMMEDIATE);
#else
