...
void BootloaderInstall6g::installStageMkdfu(void)
{
    ...
    emit logItem(tr("Experimental - gerrit patch set 4\n"
...
void BootloaderInstall6g::installStageWaitForProcs(void)
{
    ...
    if (iTunesHelperPid) {
        if (!Utils::pauseProcess(iTunesHelperPid)) {
            emit logItem(tr("Could not pause iTunesHelper."), LOGERROR);
            emit done(true);
            return;
        }
    }
...
void BootloaderInstall6g::installStageSendDfu(void)
{
    ...
    if (!System::listUsbIds().contains(0x05ac1223)) {
        ...
        emit logItem(tr("Device is not in DFU mode. It seems that\n"
                        "the previous required action failed, click\n"
                        "'Abort' and try again."), LOGERROR);
        emit done(true);
        return;
    }
...
#if !defined(Q_OS_LINUX)
    if (iTunesHelperPid) {
        if (!Utils::resumeProcess(iTunesHelperPid))
