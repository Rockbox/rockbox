...
/** Maximum progress expressed in number of increments */
#define YPR_PROGRESS_MAX    (8)
...
bool BootloaderInstallYpr::install(void)
{
    ...
    QCoreApplication::processEvents();    /* do not freeze the GUI */
    ...
    if (tempdir.isValid())
    {
        ...
        if (decrypt_res != 0)
        {
            emit logItem(tr("Firmware unpacking failed"), LOGERROR);
            emit logItem(patchError(decrypt_res), LOGERROR);
        }
        else
        {
            emit logProgress(progress++, YPR_PROGRESS_MAX);
        ...
        {
            emit logItem(tr("The patch data stream is invalid"), LOGINFO);
            emit done(false);
            return false;
        }
        else
        {
            /*emit logItem(tr("Patch has been uncompressed, now applying"), LOGINFO);*/
            emit logProgress(progress++, YPR_PROGRESS_MAX);
            QCoreApplication::processEvents();    /* do not freeze the GUI */
            ...
            if (cramfs_input != NULL)
            {
                ...
                if (cramfs_in_data != NULL)
                {
                    ...
                    if (fread_check != oldsize)
                    {
                        ...
                        emit logItem(tr("Error reading original cramfs"), LOGERROR);
                    }
                    /* close the file */
                    fclose(cramfs_input);
                    emit logItem(tr("Original firmware read OK"), LOGINFO);
                    emit logProgress(progress++, YPR_PROGRESS_MAX);
                    QCoreApplication::processEvents();    /* do not freeze the GUI */
                }
                else
                {
                    /* allocation failed */
                    emit logItem(tr("Memory allocation failed"), LOGERROR);
                }
            }
            ...
            if (cramfs_in_data != NULL)
            {
                ...
                if (cramfs_new_data != NULL)
                {
                    ...
                    {
                        /* Patching has failed */
                        emit logItem(tr("Patch operation has failed"), LOGERROR);
                    }
                    else
                    {
                         emit logProgress(progress++, YPR_PROGRESS_MAX);
                        ...
                        emit logItem(tr("Saving patched cramfs to disk"), LOGINFO);
                        ...
                        if (cramfs_output != NULL)
                        {
                            ...
                            if (encrypt_res != 0)
                            {
                                emit logItem(tr("Firmware packing failed"), LOGERROR);
                                emit logItem(patchError(encrypt_res), LOGERROR);
                            }
...
QString BootloaderInstallYpr::patchError(int err)
{
    QString error;
    switch(err) {
        case SAMSUNG_SUCCESS: error = tr("No error"); break;
        case SAMSUNG_READ_ERROR: error = tr("Can't open input file or read error"); break;
        case SAMSUNG_ARG_ERROR: error = tr("Argument error"); break;
        case SAMSUNG_FORMAT_ERROR: error = tr("invalid file"); break;
        case SAMSUNG_MD5_ERROR: error = tr("invalid md5sum"); break;
        case SAMSUNG_WRITE_ERROR: error = tr("Can't open output file or write error"); break;
        default: error = tr("unknown error: ") + err; break;
    }
    return error;
}
