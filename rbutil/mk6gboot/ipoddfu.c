...
#ifdef __linux
        /* needs libusb >= 1.0.16 */
        /* on Windows this function does nothing, it is disabled to
           allow to use libusb < 1.0.16 if needed */
        libusb_set_auto_detach_kernel_driver(dfuh->devh, 1);
#endif
                            ...
                            char* errstr, int errstrsize)
{
    ...
    if (memcmp(data, IM3_IDENT, 4)) {
        dfu_set_errstr(dfuh, "Bad DFU imagen data");
        goto error;
    }
