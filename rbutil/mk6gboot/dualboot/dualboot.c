...
uint16_t fatal[] = { 3000,500,500, 3000,500,500, 3000,500,0, 0 };
#define sad2 (&fatal[3])
#define sad  (&fatal[6])
...
unsigned char of_sha[][SIGN_SZ] = {
    ...
    "\x60\x80\x7D\x33\xA8\xDE\xF8\x49\xBB\xBE\x01\x45\xFF\x62\x40\x19"  /* v2.0.5 */    /* XXX: not tested */
};
...
bye:
    ...
    while ((int32_t)USEC_TIMER - end_time < 0);
    piezo_seq(status);
